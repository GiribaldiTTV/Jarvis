...
def build_active_overlay_recording_target_snapshot(
    *,
    ...
    return {
        ...
        "recordingExecutionState": "ready" if target_state in {"ready", "high-volume-membership"} else "blocked",
        "fileWritingState": "ready" if target_state in {"ready", "high-volume-membership"} else "blocked",
...
def normalize_monitoring_hud_overlay_profiles(payload=None, monitor_ids=None) -> dict[str, object]:
    ...
    return {
        ...
        "activeOverlayRecordingTargetProof": {
            ...
            "noHiddenRecordingTargetState": True,
            "recordingExecutionState": recording_target_snapshot["recordingExecutionState"],
            "fileWritingState": recording_target_snapshot["fileWritingState"],
            "recordingProfileBoundary": "recording-profile-state-absent-future-gated",
        },
    }
