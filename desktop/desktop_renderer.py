...
class AIControlCenterDeck(QFrame):
    def paintEvent(self, event):
        ...
        try:
            ...
            painter.fillPath(path, QColor(3, 27, 34, 72))

            grid_rect = self.rect().adjusted(1, 1, -2, -2)
            painter.setPen(QColor(117, 228, 255, 12))
            step = 20
            for x in range(grid_rect.left(), grid_rect.right() + 1, step):
    ...
    def __init__(self, screen, event_logger=None):
        ...
        self.setStyleSheet(
            ...
            QFrame#fam007AiControlCenterShell {
                border: 1px solid rgba(125, 235, 255, 0.30);
                border-radius: 28px;
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 rgba(4, 17, 32, 0.96), stop:0.56 rgba(2, 15, 25, 0.95), stop:1 rgba(1, 7, 16, 0.94));
            }
            ...
            QFrame[role="surfaceRole"] {
                ...
                border-radius: 18px;
                background: rgba(8, 39, 53, 0.42);
            }
            QLabel[role="surfaceRoleLabel"] {
                color: rgba(146, 199, 213, 0.92);
                font-size: 11px;
                font-weight: 760;
                text-transform: uppercase;
            }
            ...
            QFrame[role="factRow"] {
                ...
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 rgba(117, 228, 255, 0.06), stop:0.45 rgba(117, 228, 255, 0.035), stop:1 transparent);
            }
            ...
            QPushButton[buttonRole="primary"] {
                border-color: rgba(122, 232, 255, 0.32);
                background: qlineargradient(x1:0, y1:0, x2:1, y2:1, stop:0 rgba(7, 42, 62, 0.91), stop:1 rgba(3, 18, 32, 0.89));
            }
            QPushButton[buttonRole="secondary"] {
                color: rgba(211, 234, 238, 0.94);
        ...
        self.shell = QFrame(self)
        self.shell.setObjectName("fam007AiControlCenterShell")
        ...
        self.chrome_bar = DialogChromeBar(
            "AI Control Center",
            self,
            object_prefix="fam007AiControlCenter",
            parent=self.shell,
            show_title=False,
        )
        ...
        try:
            ...
            pass
        self.chrome_bar.close_button.clicked.connect(self.close)

        self.header_text = QWidget(self.chrome_bar)
        ...
        self._title.setProperty("role", "title")
        self._summary = QLabel("FAM-007 owned; no provider/model execution is active.", self.header_text)
        self._summary.setWordWrap(True)
        ...
        role_surface.setMinimumHeight(72)
        role_layout = QGridLayout(role_surface)
        role_layout.setContentsMargins(14, 10, 14, 10)
        role_layout.setHorizontalSpacing(10)
        role_layout.setVerticalSpacing(7)
        role_items = (
            ("FAM-007", "AI CONTROL CENTER"),
            ("BOUNDARY", "NO PROVIDER / MODEL ACTIVE"),
        ...
        quick_label = QLabel("QUICK ACCESS", quick_row)
        quick_label.setProperty("role", "quickLabel")
        quick_action = QPushButton("NO-PROVIDER CHECK", quick_row)
        quick_action.setAccessibleName("Quick access no-provider check")
        quick_action.setToolTip("Run no-provider check")
        quick_action.setProperty("buttonRole", "primary")
        quick_action.setFixedWidth(250)
        quick_action.clicked.connect(self.run_local_assist_check)
        quick_state = QLabel("PROVIDER DATA: NONE", quick_row)
        quick_state.setProperty("role", "sectionPill")
        quick_layout.addWidget(quick_label, 0, Qt.AlignVCenter)
        quick_layout.addWidget(quick_action, 0, Qt.AlignVCenter)
        quick_layout.addStretch(1)
        quick_layout.addWidget(quick_state, 0, Qt.AlignVCenter)
        shell_layout.addWidget(quick_row)
        ...
        self.content.setProperty("visualParity", "monitoring-hud__control-hub")
        ...
        facts_panel.setAttribute(Qt.WA_StyledBackground, True)
        facts = QVBoxLayout(facts_panel)
        facts.setContentsMargins(14, 0, 14, 2)
        facts.setSpacing(0)
        facts_header = QFrame(self)
        facts_header.setProperty("role", "sectionHeader")
        facts_header.setAttribute(Qt.WA_StyledBackground, True)
        facts_header_layout = QHBoxLayout(facts_header)
        facts_header_layout.setContentsMargins(0, 14, 0, 12)
        ...
        result_panel.setProperty("role", "resultPanel")
        result_panel.setAttribute(Qt.WA_StyledBackground, True)
        result_layout = QVBoxLayout(result_panel)
        result_layout.setContentsMargins(14, 0, 14, 12)
        result_layout.setSpacing(7)
        result_header = QFrame(self)
        result_header.setProperty("role", "sectionHeader")
        result_header.setAttribute(Qt.WA_StyledBackground, True)
        result_header_layout = QHBoxLayout(result_header)
        result_header_layout.setContentsMargins(0, 12, 0, 10)
        ...
        self._local_assist.setFixedWidth(250)
...
class DesktopRuntimeWindow(QWidget):
    ...
    def _publish_ai_provider_state_to_page(self):
        ...
        self._emit_runtime_signal(
            ...
            runtime_category=payload.get("runtimeStateCategory", ""),
            runtime_reason=payload.get("runtimeReasonCode", ""),
            runtime_provenance=payload.get("runtimeProvenance", ""),
            ...
            durable_consent_record=payload.get("durableConsentRecordState", ""),
            durable_setup_consent=payload.get("durableSetupConsentState", ""),
            durable_execution_consent=payload.get("durableExecutionConsentState", ""),
            durable_consent_status_proof=payload.get("durableConsentStatusProofState", ""),
            ...
            gpu_capability=payload.get("gpuCapabilityState", ""),
            cpu_fallback=payload.get("cpuFallbackState", ""),
            hardware_detection_level=payload.get("hardwareDetectionLevel", ""),
            ram_readiness=payload.get("ramReadinessState", ""),
            disk_readiness=payload.get("diskReadinessState", ""),
            model_workload=payload.get("modelWorkloadState", ""),
            model_workload_metadata=payload.get("modelWorkloadMetadataState", ""),
            ...
            memory_indexing=payload.get("memoryIndexingState", ""),
            network_egress=payload.get("networkEgressState", ""),
            windows_resilience=payload.get("windowsResilienceState", ""),
            voice_runtime=payload.get("voiceRuntimeState", ""),
            persona_voice_boundary=payload.get("personaCoreVoiceState", ""),
            validation_gates=payload.get("validationProofGateState", ""),
            release_proof=payload.get("releaseProofGateState", ""),
            copy_contract=payload.get("coreDesktopRuntimeStateContract", ""),
            contract_ready=payload.get("contractReadyMarker", ""),
