...
- Family-package Workstream entry or continuation is blocked while `Product Vision Input Missing`, `USER Vision Questions Unanswered`, `Branch Reach Unproven`, `Feature Element Breakdown Missing`, `Acceptance Criteria Missing`, `User-Facing Proof Standard Missing`, `Current Branch vs Future Package Boundary Missing`, or `Branch Readiness Planning Incomplete` remains active unless explicit USER waiver text is recorded; these are planning blockers, not implementation blockers.
