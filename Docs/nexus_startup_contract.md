...
- canonical backlog identity model is `FAM` broad product family -> `Package` bulk branch/release package -> `Slice` traceable deliverable area -> `Seam` execution/validation checkpoint; SLC is a branch-planning alias or historical ID for Slice-level line items, not a separate identity; PR numbers are evidence only.
