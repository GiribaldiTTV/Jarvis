...
- Runtime-focused implementation branches require `## Runtime Branch Engineering Contract` before Workstream begins or resumes, with `USER Engineering Planning Review:`, `Runtime Implementation Approval:`, `Current Runtime Baseline:`, `Planned Runtime Delta:`, `User-Facing Runtime Delta:`, `State / Config / Schema Delta:`, `Validator / Helper Delta:`, `Expected Changed Files / Surfaces:`, `Approval-Boundary Audit:`, `Future-Gated Items:`, `Workstream Seam Map:`, `Proof Expectations:`, `Risk Forecast:`, `Recommendations And Alternatives:`, `Plan Version / Revision Status:`, and `Plan-To-Implementation Traceability:` so ChatGPT/Codex prompts preserve engineering intent from Branch Readiness through Release Readiness instead of letting Workstream invent shallow deltas. New or re-entering runtime branches also use a Branch Runtime Engineering Plan under `Docs/branch_plans/<branch_slug>.md`; backlog and roadmap remain compact pointer/status surfaces while branch-specific per-seam details live in that plan until PR Readiness fold-down. Workstream Entry prompts must require a Desktop `USER Review Desktop Bundle` with copied branch vision, branch plan, branch authority, relevant Nexus/family vision, matrix, UFD/change-intent, and source-truth files before asking USER to green-light implementation. Prompts must also require the `USER Branch Plan Review Gate` packet and preserve `USER Branch Plan Review Missing` when the readable branch goal, planned user-facing outcome, implementation breakdown, Hardening plan, Live Validation / UTS plan, USER design review questions, recommendations, implementation options, USER Review Response, Codex Response Digest, alternatives/tradeoffs, scope dispositions, or exact USER decision needed is missing. Prompts must require `USER Review Packet Finding:` and preserve `USER Review Packet Stale` / `USER Review Packet Not Digested` when `START_HERE.md`, `USER_BRANCH_PLAN_REVIEW.md`, and the exported zip are not loaded, digested, freshness-checked, when USER response has not been attached/digested, or when the requirement is not explicitly waived. When multiple slices or seams are admitted, prompts must also require Workstream Entry whole-package analysis before first-seam implementation and preserve `Workstream Entry Whole-Package Analysis Missing` as a blocker.
- USER Branch Plan Review product/design reinforcement: prompts must require `USER_BRANCH_PLAN_REVIEW.md` as the USER Branch Plan Contract with `Contract Status`, `Contract Version / Revision`, `What Will I Actually See, And Where Will I See It?`, `End-State Vision`, `Visual / Functional Walkthrough`, `Surface Map`, implementation options, recommended direction, Nexus-fit rationale, current branch scope, future-gated scope, Implementation Staging Notes, USER decisions, USER response, Codex response digest, implementation constraints created by USER response, rejected/deferred ideas, source-truth impact, contract change log, completion checklist, and Workstream Entry result so USER can inspect what the feature will look and feel like before implementation approval. Prompts must treat Draft, Pending USER Response, Pending Codex Digest, and Pending USER Confirmation Contract Status values as implementation blockers; only Complete or Waived by USER can unlock implementation approval. If USER feedback changes direction, UI behavior, workflow, scope, boundaries, or seam order, prompts must require source-truth updates, Desktop packet/ZIP refresh, and USER confirmation of the revised contract or explicit waiver. Prompts must not turn the review into a per-slice decision packet; SLC/slice/seam details are only implementation staging after the end-state is clear.
