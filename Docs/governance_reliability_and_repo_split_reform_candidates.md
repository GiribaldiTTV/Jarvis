...
When final hardening reaches a blocker such as `Golden Template / Reference Promotion Blocked`, Codex must stop and wait rather than request PR Readiness. This hold does not approve template creation, golden-reference promotion, design-token implementation, shared UI primitive implementation, helper/validator code, FAM worktree mutation, external-state mutation, PR creation, merge, release, or cleanup. Those remain separate USER decisions under current source truth. The template/reference plan-completion portion is completed by the 2026-06-16 planning completion below once USER accepts that disposition; actual promotion, shared primitives, helper/validator enforcement, fixtures, and FAM adoption remain future-gated.
...
Disposition: `Template / Reference Plan Completed As Planning Evidence; Promotion Deferred`. FAM-007 prerequisite evidence exists and the template/reference plan is now complete as planning evidence, but actual promotion remains deferred. PR Readiness may be considered only after USER accepts this planning-completion disposition or separately approves a limited PR path. This repair does not create, promote, or imply a template, golden reference, design token, shared primitive, reusable component library, fixture, helper, validator, or product implementation. Branches must continue to use current FAM-002 presentation grammar, phase gates, visual proof, and USER manual validation where required until a later USER-approved template/reference promotion path exists.
...
USER may either accept this planning completion and approve PR Readiness Stage 1 for the Governance branch without template/reference promotion, or approve a separate future template/reference owner-admission or promotion cycle. Neither path authorizes release, PR creation, merge, FAM worktree mutation, external-state mutation, issue mutation, helper/validator code, fixture mutation, design-token/shared-primitive implementation, or runtime/provider/private/cache/memory work without separate approval.
...
- `Template / Reference Plan Completed As Planning Evidence`: FAM-007 prerequisite proof exists and the template/reference plan classifies candidates, future owners, schema, and promotion criteria, but no reference/template/promotion/implementation authority exists until a later USER decision.
...
| FAM-002 component anatomy | `Docs/family_visions/FAM-002_desktop_interface.md`; `Docs/phase_governance.md` | Binding component anatomy and element-group acceptance language exists | Binding Now as reusable presentation law | Planning-completion portion closed after USER acceptance; promotion remains future-gated | Future Template/Golden Reference Work; Future Helper/Validator Enforcement | FAM-007 prerequisite evidence exists; template/reference plan completed as evidence; actual promotion remains USER-gated | Use current grammar now; do not infer promoted visual references |
...
| FAM-007 lessons digest | FAM-007 branch closeout / USER-reviewed evidence; Governance consumes only after merge/rebaseline | FAM-007 prerequisite evidence exists through merged PR #262, Governance PRs #263 through #268 lesson digestion, and merged PR #269 AI Control Center H4/template-first candidate evidence; no active FAM mutation here | Template / Reference Plan Completed As Planning Evidence | Planning-completion portion closed after USER acceptance | USER Decision Required for any later template/reference promotion | Actual template/reference promotion still requires USER approval | Do not infer active FAM packet state or golden-reference authority; evaluate PR #269 evidence only as candidate template/reference input |
| Golden templates/reference surfaces | Future USER-approved FAM-002 / template-reference carrier; `Docs/family_visions/FAM-002_desktop_interface.md` owns presentation law | Ownership exists and the planning packet recommends future owner/location/schema; actual golden references are not promoted | Future Template/Golden Reference Work | Promotion remains USER-gated; not created by this plan completion | USER Decision Required; future helper/validator evidence | Actual template/reference promotion remains unapproved | Do not create/promote now; use this plan only as future promotion input |
...
| One integrated PR plan | This planning file plus `Docs/branch_records/feature_release_readiness_source_truth_intake.md` | USER preference recorded; template/reference prerequisite evidence exists and the plan is completed as planning evidence | Binding Now for current Governance branch posture | PR Readiness may be considered only after USER accepts this completion disposition or approves a limited PR path | USER Decision Required for PR Readiness and any later template/reference promotion | PR creation/merge/release remain separate USER decisions | Stop before PR Readiness until USER accepts the completion disposition and grants PR Readiness Stage 1 approval |
...
- `Limited PR Path USER Approval Missing`: if USER wants PR Readiness without accepting the planning-completion disposition, USER must explicitly approve that limited PR path or reclassify the blocker.
