...
| Tray/menu doorway references | Preserve doorway grammar with FAM-003 behavior split and FAM-002 presentation split | Implement resident access, quick actions, AI status, or setup education in owning FAMs | F3-FF01 and future tray proof are candidate evidence | Separate grouped package unless USER admits now | Target FAMs adopt when tray/runtime work is admitted |
| Design tokens and shared UI rules | Decide durable ownership and promotion prerequisites after references are stable | Consume tokens/primitives only after they are approved | CSS or component reuse remains evidence, not a standard | Future/deferred authority work unless USER admits implementation | Target FAMs migrate only after approved token/primitive carrier merges |
| Negative examples / bad fixtures | Define bad-example classes and future enforcement expectations | Avoid known-bad patterns and provide candidate defect evidence | PR review/LV failures may become negative fixture candidates | Future helper/validator/fixture work unless USER approves mutation | Target FAMs consume once fixtures/validators exist |
| Helper and validator expectations | Record future machine-check guidance without overclaiming current enforcement | Treat validators as evidence and still perform manual proof | Failed packets and review comments feed future fixtures | Future code mutation unless separately approved | Target FAMs run updated checks after merge |
...
| Lane | Current Disposition | Current-Branch PR-Blocking? | Reason | Required Proof / Work | Legal Carrier | Recommended Package | USER Approval Needed |
| --- | --- | --- | --- | --- | --- | --- | --- |
| HUD/FAM-006 surface reference | `REVISE` | Yes, as candidate authority work if kept in current PR scope | Useful candidate evidence, but not promotion-ready | USER visual approval or revise/defer/reject disposition | Governance/FAM-002 promotion path | Top-level window evidence comparison | USER visual promotion or explicit reclassification |
| PR #269 AI Control Center surface reference | `REVISE` | Yes, as candidate authority work if kept in current PR scope | Strong candidate, but still FAM-007 evidence and not a global standard | Compare against Project UI Vision, FAM-002, F2-FF01, HUD, and proof hierarchy | Governance/FAM-002 promotion path | Top-level window evidence comparison | USER visual promotion or explicit reclassification |
| Golden window reference | `REVISE` | Yes | No promoted top-level window reference exists | Promotion packet with applicability, limitations, proof artifacts, and final disposition | Governance/FAM-002 plus UI catalog | Top-level window reference | USER promotion approval |
| Golden control-cluster reference | `REVISE` | Yes | The compact cluster is candidate-only | Applicability matrix for close/minimize/maximize/restore/blocked controls by window class | Governance/FAM-002 plus UI catalog | Window control cluster | USER promotion approval |
| Close/minimize/maximize cluster reference | `REVISE` | Yes | Related to control cluster and should not fragment into tiny standards | Element-group state proof, accessibility/hitbox proof, and top-level/child-window distinction | Governance/FAM-002 plus UI catalog | Window control cluster | USER promotion approval |
| Full button set | `DEFERRED` | Yes only if USER keeps it inside current PR scope | F2-FF01 preserves it but proof is not promotion-ready | State matrix and positive/negative examples | Governance/FAM-002 future package | Broader controls | USER admission/promotion approval |
| Dropdown/menu/list/filter reference | `DEFERRED` | Yes only if USER keeps it inside current PR scope | Needs real surface proof and state coverage | Open/closed/hover/focus/selected/disabled/empty/overflow proof | Governance/FAM-002 future package | Broader controls | USER admission/promotion approval |
| Modal/dialog template | `DEFERRED` | Yes only if USER keeps it inside current PR scope | Needs modal/child/top-level/platform exception taxonomy | Dialog proof and exception disposition | Governance/FAM-002 future package | Surface classes | USER admission/promotion approval |
| Status/failure/recovery panel template | `DEFERRED` | Yes only if USER keeps it inside current PR scope | Needs FAM-001 meaning plus FAM-002 presentation proof | Failure/recovery state proof and truth mapping | Governance/FAM-002/FAM-001 routed package | Surface classes | USER admission/promotion approval |
| Tray/menu doorway template | `DEFERRED` | Separate-carrier leaning | Depends on FAM-003 resident-access behavior and Windows tray constraints | Tray doorway proof and quick-access limits | Future FAM-003/FAM-002 routed carrier | Tray doorway | USER admission/promotion approval |
| Design tokens | `DEFERRED` | Future/deferred authority work | Tokens should follow stable references, not precede them by inference | Token ownership, naming, code-to-visual trace | Future implementation/template carrier | Implementation substrate | USER implementation approval |
| Shared primitives | `DEFERRED` | Future/deferred authority work | Code/runtime implementation is not approved here | Component API, reuse boundaries, parity proof | Future implementation carrier | Implementation substrate | USER implementation approval |
| Negative fixtures / bad examples | `DEFERRED` | Future/deferred enforcement work | Fixture mutation is outside this approval | Bad examples for default OS chrome, unproven references, weak proof, mismatched states | Future helper/validator/fixture carrier | Enforcement | USER fixture/code approval |
| Helper/validator enforcement | `DEFERRED` | Future/deferred enforcement work | Helper/validator mutation is outside this approval | Machine checks after false-positive review and fixture coverage | Future helper/validator carrier | Enforcement | USER code approval |
| Active FAM-006/FAM-007 adoption mutation | Future target-FAM adoption | No for this Governance PR | Target FAMs consume after merge; Governance must not mutate sibling worktrees | Rebaseline adoption packet and branch-local repair in each FAM | FAM-006/FAM-007 worktrees at next legal gate | Adoption after merge | USER approval in each target carrier |

Grouped Packages:

| Package | Included Lanes | Why Grouped | Can Start Next? | Blocks PR Readiness? | Exclusions | Recommended Next Action |
| --- | --- | --- | --- | --- | --- | --- |
| Top-Level Window Reference Package | HUD/FAM-006 surface, AI Control Center surface, golden window reference | These decide the global window baseline and compare strongest candidate evidence together | Yes, after USER approves promotion-planning/implementation packet | Yes if current PR scope keeps template authority work inside this branch | No FAM worktree mutation, no code template implementation unless separately approved | Prepare promotion-ready comparison and USER decision packet |
| Window Control Cluster Package | Golden control-cluster and close/minimize/maximize cluster | Same element group and same window-class applicability rules | Yes, preferably with or immediately after top-level window package | Yes if current PR scope keeps template authority work inside this branch | No sibling-FAM adoption mutation | Build applicability matrix and promotion packet |
| Broader Controls Package | Full button set, dropdown/menu/list/filter | Shared state coverage and bad-example needs overlap | Later | Blocking only if USER keeps all deferred lanes in this PR scope | No helper/validator code unless approved | Admit after top-level/window-control references stabilize |
| Surface Classes Package | Modal/dialog, status/failure/recovery, tray/menu doorway | These require class-specific exception and meaning rules | Later; tray may need separate FAM-003 carrier | Blocking only if USER keeps all deferred lanes in this PR scope | No runtime/tray implementation | Split tray if FAM-003 behavior proof is required |
| Implementation Substrate Package | Design tokens and shared primitives | Code-level reuse should follow accepted visual references | Later | Blocking only if USER keeps it in current PR scope | No code/runtime mutation without exact approval | Defer until references are promoted or USER admits implementation |
| Enforcement Package | Negative fixtures, bad examples, helper/validator enforcement | Machine checks need approved references or precise rules to avoid false positives | Later | Blocking only if USER keeps it in current PR scope | No helper/validator/fixture mutation without exact approval | Add after reference standards are stable |
| Target-FAM Adoption Package | FAM-006/FAM-007/future FAM reconciliation | Adoption belongs to each target worktree after merge/rebaseline | No, not in Governance branch | Not blocking this Governance PR unless USER reclassifies adoption as required before PR | No FAM worktree mutation from Governance | Defer to each target FAM's next legal gate |
...
| Top-level window and control-cluster promotion path | Pending | FAM-002/F2-FF01/catalog | USER-approved promotion packet or explicit reclassification | `Current Branch Template Work Incomplete` | Yes, but must be explicit |
| Broader deferred template/reference lanes | Pending/deferred | F2-FF01 and future approved carriers | Admit and complete, or explicitly reclassify out of current PR scope | `Current Branch Template Work Incomplete` if kept in scope | Yes |
| Design-token/shared-primitive implementation | Future/deferred | Future implementation/template carrier | USER-approved implementation carrier | `Shared Primitive Promotion Blocked` if claimed now | Yes |
| Helper/validator/fixture enforcement | Future/deferred | Validation registry/future code carrier | USER-approved code/fixture mutation | Future enforcement gap | Yes |
...
Current PR Posture: `Current Branch Template Work Incomplete` remains active. PR Readiness remains blocked until the current-branch Governance/template authority work is either completed and hardened or USER explicitly reclassifies remaining lanes out of current PR scope. The recommended next grouped package is the `Top-Level Window Reference Package`, paired with the `Window Control Cluster Package` if USER wants the highest-leverage path to reduce FAM-006/FAM-007 UI drift.
