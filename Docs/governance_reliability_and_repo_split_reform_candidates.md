...
When final hardening reaches a blocker such as `Golden Template / Reference Promotion Blocked`, Codex must stop and wait rather than request PR Readiness. This hold does not approve template creation, golden-reference promotion, design-token implementation, shared UI primitive implementation, helper/validator code, FAM worktree mutation, external-state mutation, PR creation, merge, release, or cleanup. Those remain separate USER decisions under current source truth. The template/reference dependency portion of this hold is superseded by the 2026-06-16 closure below; PR Readiness itself still requires a later USER approval.

## Template / Reference Dependency Closure - 2026-06-16

Document Status: Non-Binding Planning Closure. Binding authority remains with `Docs/phase_governance.md` for the dependency gate, `Docs/family_visions/FAM-002_desktop_interface.md` for current presentation law, and the future USER-approved template/reference carrier for any actual template, golden-reference, design-token, or shared UI primitive promotion.

Closure Evidence: FAM-007 PR #262 merged, later Governance PRs #263 through #268 digested and hardened Family Feature Vision, UI, proof, and release-handoff lessons, and USER approved bounded Governance continuation on 2026-06-16 to complete the remaining Template/Reference dependency blocker and reconcile stale Governance external operational state records.

Disposition: `Template Dependency Resolved / Promotion Deferred`. This closes the template/reference PR-hold dependency for the current Governance reform path. It does not create, promote, or imply a template, golden reference, design token, shared primitive, reusable component library, fixture, helper, validator, or product implementation. Branches must continue to use current FAM-002 presentation grammar, phase gates, visual proof, and USER manual validation where required until a later USER-approved template/reference promotion path exists.
...
- `Template Dependency Resolved / Promotion Deferred`: FAM-007 prerequisite proof exists for current Governance PR-hold closure, but actual template/reference promotion remains future USER-approved work.
...
| FAM-002 component anatomy | `Docs/family_visions/FAM-002_desktop_interface.md`; `Docs/phase_governance.md` | Binding component anatomy and element-group acceptance language exists | Binding Now as reusable presentation law | Not PR-Blocking | Future Template/Golden Reference Work; Future Helper/Validator Enforcement | FAM-007 dependency resolved for PR-hold closure; actual template/reference promotion remains USER-gated | Use current grammar now; create USER-approved references later |
...
| FAM-007 lessons digest | FAM-007 branch closeout / USER-reviewed evidence; Governance consumes only after merge/rebaseline | Resolved for PR-hold closure by merged PR #262 plus later Governance PRs #263 through #268 lesson digestion; no active FAM mutation here | Template Dependency Resolved / Promotion Deferred | Not PR-Blocking after dependency closure | USER Decision Required for any later template/reference promotion | Actual template/reference promotion still requires USER approval | Do not infer active FAM packet state; use merged/hardened evidence only |
| Golden templates/reference surfaces | Future USER-approved FAM-002 / template-reference carrier; `Docs/family_visions/FAM-002_desktop_interface.md` owns presentation law | Ownership exists; actual golden references are not promoted | Future Template/Golden Reference Work | Not PR-Blocking after dependency closure | USER Decision Required; future helper/validator evidence | Actual template/reference promotion remains unapproved | Do not create/promote now; run dedicated template/reference review only after USER approves that future path |
...
| One integrated PR plan | This planning file plus `Docs/branch_records/feature_release_readiness_source_truth_intake.md` | USER preference recorded; template/reference dependency closure recorded as resolved/promotion deferred | Binding Now for current Governance branch posture | Not PR-Blocking after current blocker closure; PR Readiness still requires separate USER approval | USER Decision Required for PR Readiness and any later template/reference promotion | PR creation/merge/release remain separate USER decisions | Complete bounded validation and return PR Readiness Stage 1 as the next legal phase only after USER approves it |

Closed PR-Hold Blockers:

- `Golden Template / Reference Promotion Blocked`: closed as a PR-hold dependency only by `Template Dependency Resolved / Promotion Deferred`; actual template/reference promotion remains future USER-approved work.
- `Integrated PR Path Held`: closed for the current template/reference dependency after USER approved bounded Governance continuation on 2026-06-16; PR Readiness, PR creation, merge, and release remain separate USER decisions.
- `FAM-007 Lessons Digest Missing For Template Promotion`: closed for current dependency routing by merged FAM-007 PR #262 plus Governance PRs #263 through #268 lesson digestion; active FAM packets and chat remain non-authoritative.

Current PR-Hold Blocker List:

- None from the template/reference dependency after 2026-06-16 closure.
...
The FAM-007 template dependency is closed for the current Governance PR-hold path as `Template Dependency Resolved / Promotion Deferred`. This closure does not make future helper/validator implementation, active FAM worktree mutation, design-token implementation, shared UI primitive implementation, or template/golden-reference promotion legal in this docs-only classification cycle.

One Integrated PR Plan Result:

The default path remains one integrated Governance PR after all admitted blockers are resolved. After the 2026-06-16 template/reference dependency closure, Codex may recommend PR Readiness Stage 1 as the next legal phase only when validation is green and USER separately approves PR Readiness. Codex must not create a PR, merge, release, promote templates/references, mutate FAM worktrees, or create/mutate GitHub issues without the next exact USER approval.
