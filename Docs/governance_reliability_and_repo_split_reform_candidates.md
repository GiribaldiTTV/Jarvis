...
| FAM-002 component anatomy | `Docs/family_visions/FAM-002_desktop_interface.md`; `Docs/phase_governance.md` | Binding component anatomy and element-group acceptance language exists | Binding Now as reusable presentation law | Not PR-Blocking | Future Template/Golden Reference Work; Future Helper/Validator Enforcement | Template/reference promotion blocked until FAM-007 dependency clears | Use current grammar now; create USER-approved references later |
...
| FAM-007 adoption at next legal gate | `Docs/phase_governance.md`; active FAM-007 branch/worktree when resumed | Governance records next-gate adoption requirement; no FAM-007 mutation here | Binding At Next Legal Gate | Not PR-Blocking for Governance PR, except as template dependency below | Future Helper/Validator Enforcement | FAM-007 mutation excluded | FAM-007 must evaluate merged standards after rebaseline and before continuing branch gates |
| Rebaseline current-branch implementation and UI-UX conformance scan | `Docs/phase_governance.md`; `Docs/branch_plans/README.md`; active external branch plan or USER packet for active rows | Binding adoption review fields now require current branch implementation inventory, implemented/touched UI-UX surfaces, merged standard comparison, and current violation findings | Binding At Next Legal Gate | Not PR-Blocking for Governance PR | Future Helper/Validator Enforcement | Active FAM worktree mutation and live-state tracking excluded | Rebaselining FAM branches must identify what they already implemented and whether current UI/UX output violates merged standards before advancement |
| FAM-007 lessons digest | FAM-007 branch closeout / USER-reviewed evidence; Governance consumes only after merge/rebaseline | Not available in this Governance branch yet | Blocked By FAM-007 PR/Merge/Rebaseline/Lessons Dependency | PR-Hold Blocker only for template/reference promotion path | USER Decision Required after FAM-007 merge | FAM-007 PR push/merge, rebaseline/reconciliation, durable lessons digestion | Wait; do not infer FAM-007 lessons from chat or active branch packets |
| Golden templates/reference surfaces | Future USER-approved FAM-002 / template-reference carrier; `Docs/family_visions/FAM-002_desktop_interface.md` owns presentation law | Ownership exists; actual golden references are not promoted | Future Template/Golden Reference Work | PR-Hold Blocker under current USER no-PR direction until dependency clears or USER records limited PR path | USER Decision Required; future helper/validator evidence | Blocked by FAM-007 PR/merge/rebaseline/lessons dependency | Do not create/promote now; after dependency clears, run dedicated template/reference review and USER approval |
...
| One integrated PR plan | This planning file plus `Docs/branch_records/feature_release_readiness_source_truth_intake.md` | USER preference recorded; PR held until admitted blockers resolve or limited PR path approved | Binding Now for current Governance branch posture | PR-Hold Blocker controller | USER Decision Required for limited PR path or later PR Readiness | Current no-PR hold and FAM-007 template dependency | Continue bounded integrated plan; do not request PR Readiness until blockers clear or USER changes path |

PR-Hold Blocker List:

- `Golden Template / Reference Promotion Blocked`: hard hold for template/reference promotion until FAM-007 pushes its PR, the PR merges, post-merge rebaseline/reconciliation completes, durable FAM-007 lessons are digested, and USER approves template/reference promotion.
- `Integrated PR Path Held`: current USER direction keeps this broader Governance branch out of PR Readiness until admitted blockers are complete or USER records an explicit limited PR path.
- `FAM-007 Lessons Digest Missing For Template Promotion`: template/reference promotion must not infer lessons from chat, active branch packets, or stale review evidence.
...
The FAM-007 template dependency creates a hard no-PR hold only for the template/golden-reference promotion path and the current USER-selected integrated PR path. It does not make future helper/validator implementation, active FAM worktree mutation, design-token implementation, or shared UI primitive implementation legal in this docs-only classification cycle.

One Integrated PR Plan Result:

The default path remains one integrated Governance PR after all admitted blockers are resolved. Codex must not split the work into multiple PRs by inertia. Codex also must not request PR Readiness while the no-PR hold is active unless USER records an explicit limited PR path that names what remains deferred and why it is not blocking that limited PR.
