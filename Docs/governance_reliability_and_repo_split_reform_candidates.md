...
- `Issue Candidate Approval Missing`: revalidation may produce issue candidates for already-implemented or previous branch output, but GitHub issue creation, labeling, commenting, closing, reopening, or auto-close wording remains blocked until USER approves the exact issue action.
