...
Observed failure class: after rebaseline and merged UIREF / vision / proof standards, FAM worktrees could repair one visible defect or report helper/validator green while broader owned-surface drift, previous-branch issue candidates, accepted-reference comparison, code-to-visual trace, backend/state ownership trace, screenshot/contact-sheet evidence, and USER packet proof remained incomplete. A second failure variant is circular proof: the helper, validator, marker, plan, or attractive screenshot that claimed a surface green became the only proof that the surface matched the accepted UIREF/reference.
...
- Require owned-surface inventory, affected-file inventory, affected-surface inventory, code-to-visual trace, accepted reference / template comparator, UI element inventory, backend/state ownership trace, screenshot/video/contact-sheet evidence, visual element-group ledger, repair-versus-issue boundary, issue-candidate table, RAR USER decision table, validation summary, and exact USER packet path/ZIP path when USER decisions are needed.
...
RAR hardening disposition added by the 2026-06-20 follow-up: RAR rows should classify affected surfaces and element groups as `CONFORMING`, `NONCONFORMING`, `UNPROVEN`, `PARTIAL`, `EXCEPTION NEEDED`, `ISSUE CANDIDATE`, `SOURCE-TRUTH GAP`, `REFERENCE GAP`, or `OUT OF CURRENT SCOPE`. Normal phase progression remains blocked while unresolved owned surfaces are `NONCONFORMING`, `UNPROVEN`, `PARTIAL`, `EXCEPTION NEEDED`, `ISSUE CANDIDATE`, `SOURCE-TRUTH GAP`, or `REFERENCE GAP` unless repaired, explicitly waived, routed to an approved issue candidate, or dispositioned by the correct owner. RAR proof must be non-circular: requirement text, validator green, helper output, Codex summary, attractive screenshots, UIREF records, or templates cannot prove themselves without comparison to source truth, rendered output, code-to-visual trace, backend/state truth, and USER review/waiver where required.

Future hardening recommendation: add helper/validator/fixture support after the docs contract settles. Priority checks are `Affected Surface Inventory Missing`, `Owned Surface Nonconformance Ledger Missing`, `Accepted Reference Comparator Missing`, `Code-To-Visual Trace Missing`, `Backend State Ownership Trace Missing`, `Screenshot Contact-Sheet Proof Missing`, `RAR USER Packet Missing`, `Owned Surface Issue Candidate Missing`, `Circular Validation Evidence`, `Source-Truth Gap Unrouted`, `Reference Gap Unrouted`, `Unproven Owned Surface`, `RAR Packet Missing For USER Judgment`, and `Partial Repair Reported Complete`.
...
| Folder/ZIP/hash/file-count verification | Prevent folder/ZIP drift and stale upload artifacts | USER packet rule requires timestamped ZIP and folder/list validation; helper now validates existing local packets for ZIP-beside-folder placement, duplicate ZIP entry rejection, stale same-label ZIPs, stable ZIP rejection, one-primary review file, file-class layout, and folder/ZIP file-list plus content-hash parity | Optional package-specific hash summary remains future-only and must keep hash proof out of USER-facing files | Existing `dev/orin_user_review_bundle.py` validation mode plus fixture coverage; future visual/hash helper only for image-proof lanes | Yes for packet reviewability when generated |
