...
- AI Dashboard should keep top-level content compact, orienting, trust-critical, immediately useful, or navigational. Detailed reports, diagnostics, setup flows, logs, selectors, provider internals, capability details, memory/cache/private setup, and long workflows should route through categories, child/domain windows, Global Settings, or future-gated placeholders instead of becoming permanent stacked top-level cards unless source truth and USER approval create an explicit exception.
- AI Dashboard category cards are doorways, not workspaces. Each top-level category card should contain compact status/summary plus launcher buttons that open the relevant external-unique or exclusive child/domain window for this branch's approved Dashboard repair. Local AI Check, readiness report generation/results, diagnostics details, provider internals, capability details, setup, settings internals, proof data, and maintenance lifecycle details must run or display inside the opened window rather than inline in the AI Dashboard hub.
- AI Dashboard child/domain windows created by this branch must use Nexus-native product chrome, not generic Windows OS title bars. The approved branch-local child-window model is custom frameless NDAI chrome with compact product window controls, real launcher click paths, non-overlapping default placement near the Dashboard, and full-desktop plus focused proof that the opened windows are not helper-created substitutes.
...
- AI Control Center category direction should start from USER jobs, including AI Control Center, AI Readiness / Diagnostics, Capabilities & Maintenance, Developer / Owner, Activity / Recovery, and AI Settings handoff when an owning surface is approved. The top level may show compact summaries and doorway actions; child/domain windows own deeper reports, setup, selectors, settings, logs, and long diagnostic content.
...
- This vision does not admit tray AI-status implementation or a second AI tray icon by itself. The active FAM-007 AI Dashboard repair branch may implement only the USER-approved branch-local child/domain windows needed for Dashboard category launchers; that approval does not admit provider/model execution, downloads, runtime cache, memory, private setup, FAM-003 settings implementation, FAM-008 packaging/update execution, or sibling worktree mutation.
