...
- The visible public product surface name is `AI Control Center` unless a later USER-approved source-truth change renames it. Internally, the accepted durable product model is `AI Home / Control Foyer`: the top-level AI orientation and control-entry surface that tells the USER what AI exists, what state it is in, what is safe or blocked, and where to go for each AI subsystem.
- AI Control Center should keep top-level content compact, orienting, trust-critical, or navigational. Detailed reports, diagnostics, setup flows, logs, selectors, provider internals, capability details, memory/cache/private setup, and long workflows should route through categories, focused surfaces, child/drill-down surfaces, or future-gated placeholders instead of becoming permanent stacked top-level cards unless source truth and USER approval create an explicit exception.
- AI Control Center placement decisions must classify AI-related items by role: top-level summary, top-level action, category doorway, focused surface, child/drill-down detail, diagnostic/status/trust-boundary, concrete USER-facing feature, future-gated placeholder, cross-FAM dependency support, or out-of-scope. The concrete USER-facing feature classification gate remains active: runtime-visible status, blocked-state explanation, provider-visible-data `none`, and truthful unavailable-capability copy do not count as concrete features by themselves.
