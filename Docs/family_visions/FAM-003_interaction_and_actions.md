...
- Own the minimal Nexus Tray & Quick Access settings interaction needed by the resident doorway, including the Global Settings entry/shell foundation for quick-slot configuration. The visible owned section should be framed as Nexus Tray & Quick Access or Quick Access, not Resident Access by itself. Other FAM dependencies may appear as clearly owner-bounded connected-surface cards, route links, or future-gated status explanations, but not as fake editable settings categories unless the owning FAM and USER approval admit proportional dependency-bounded carry-in.
- Quick Access settings must make edit state explicit: slot add/remove/reorder/dropdown changes are pending until saved, Revert restores the last saved model, Reset Quick Access stages documented defaults instead of silently saving, and closing or chrome-close behavior must not silently lose pending changes.
