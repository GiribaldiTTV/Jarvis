...
- Own the primary tray icon hover tooltip mechanism and compact resident status copy, while consuming FAM-007 and AI Runtime And Trust source truth for deeper AI/provider/privacy state. The tray tooltip must not become the only privacy-critical state surface because Windows may hide third-party tray icons.
- Treat the FAM-003 resident tray tooltip/status implementation as current dependency evidence for future FAM-007 BR1/BR2/BP rather than a frozen AI/privacy design. FAM-007 owns exact AI/provider/privacy status taxonomy, permission-state truth, provider-visible-data truth source, Trust Journal or AI Command Center detail routing, and real Privacy Lockdown enforcement; if that analysis proves the resident scaffold, route, copy, indicator, or infrastructure needs to change, FAM-007 may propose the change through its own legal gates.
- Keep the resident tray menu compact, with core Nexus actions, safety/status doorways, state-dependent optional feature doorways, and a small USER-configurable Quick Access section rather than a command wall. The Windows taskbar tray icon right-click menu must not carry long status/header text; resident status belongs in the tray-icon hover tooltip, the styled fallback/status surface, or the owning full surface. The compact native tray menu should start with Global Settings, place Quick Access immediately below Global Settings, use categorized submenus such as Quick Access and AI when grouping reduces top-level clutter, keep Exit Nexus easy to find, and defer Developer/Owner menu categories until FAM-007 admits the relevant Dev/Owner model semantics.
...
- Adopt the Nexus UI Exposure Contract locally for FAM-003 resident, tray, menu, and minimal Global Settings surfaces: visible product UI may expose only real, enabled/available, or meaningfully shown owner-backed items. Future categories, deferred controls, unowned internals, not-installed surfaces, unsupported surfaces, disabled-by-USER features, and unknown state must stay hidden unless they are useful owner-bounded status, setup/configuration, unavailable-with-reason, or future-gated context.
...
- Own the minimal Nexus Tray & Quick Access settings interaction needed by the resident doorway, including the Global Settings entry/shell foundation for quick-slot configuration. The visible owned section should be framed as Nexus Tray & Quick Access or Quick Access, not Resident Access by itself. Dependency ownership, route eligibility, and future-gated surface contracts belong in source truth, USER packets, and owning surfaces; they must not appear in Global Settings as non-actionable connected-surface/status categories. Other FAM routes may appear in Global Settings only when they are real USER-facing settings, admitted route controls, or USER-approved proportional dependency carry-in.
- Quick Access settings must make edit state explicit: slot add/remove/reorder/dropdown changes are pending until saved, Revert restores the last saved model, Use Defaults or equivalent reset-default control stages documented defaults instead of silently saving, and closing or chrome-close behavior must not silently lose pending changes.
...
- Own branch-local resident/tray/runtime-window icon proof needed for the current resident doorway and Live Validation path while consuming FAM-002 visual identity standards. FAM-008 remains owner of packaged shortcut, installer, startup, update, and durable taskbar/system identity behavior.
...
- This vision does not admit full app-wide Global Settings implementation, other-FAM settings internals, or a settings takeover by itself. FAM-003 settings work is limited to the Nexus Tray & Quick Access foundation unless Branch Readiness and USER approval admit a broader owner-bounded dependency.
