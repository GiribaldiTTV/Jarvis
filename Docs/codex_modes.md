...
- the exported or refreshed local USER hub `UTS - <worktree-label>.txt` copy when the slice is a relevant desktop user-facing path, or an explicit explanation of why that export was skipped
...
- the local USER hub `C:\Nexus USER\UTS - <worktree-label>.txt` file is the required user-facing exported copy when relevant, while `C:\Nexus USER\User Test Summary.txt` is a template/layout example only and not the default canonical repo record
