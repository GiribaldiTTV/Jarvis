...
- confirmation that `PR Watcher Provisioning Unproven` is clear whenever the branch expects watcher-based PR monitoring, including explicit watcher target, runtime path, run-proof method, fallback, teardown, replacement provisioning for the live PR, and the same working thread watcher contract; accepted same-thread proof may come from native Codex heartbeat run evidence or a bounded local same-thread watcher that posts through the official Codex thread-resume path, and manual rollout-file or transcript-file injection does not count as same-thread proof
- confirmation that the watcher contract uses minute cadence and reports only when a watched PR status changes
