...
- confirmation that `PR Watcher Provisioning Unproven` is clear whenever the branch expects watcher-based PR monitoring, including explicit watcher target, runtime path, run-proof method, fallback, teardown, replacement provisioning for the live PR, and the same working thread watcher contract
