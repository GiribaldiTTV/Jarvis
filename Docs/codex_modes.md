...
- USER Branch Plan Review product/design reinforcement: Workstream Entry must treat `USER_BRANCH_PLAN_REVIEW.md` as the USER Branch Plan Contract, a user-facing planning document and decision ballot, not as a normal Codex digest. The contract must be branch-vision-first and lead with `Family Vision Context`, `Feature Vision`, `Branch Goal`, `End-State Vision`, `What Will I Actually See, And Where Will I See It?`, `How It Will Function`, `User Experience Flow`, `Visual / Functional Walkthrough`, `Surface Map`, `Implementation Options / Product Shapes`, `Recommended Direction`, and `Why This Fits The Nexus Vision` before SLC/slice/seam route detail. The review must also include `Contract Status`, `Contract Version / Revision`, `USER Design Direction Decision`, `Current Branch Scope`, `Future-Gated Scope`, `How Codex Would Build This After USER Accepts The Direction`, `Implementation Staging Notes`, USER decisions, USER response, Codex response digest, implementation constraints created by USER response, rejected/deferred ideas, source-truth impact, contract change log, completion checklist, and Workstream Entry result so USER can decide whether the planned feature end-state and possibility space should proceed. `Contract Status` must be `Complete` or `Waived by USER` before implementation approval is legal; `Draft`, `Pending USER Response`, `Pending Codex Digest`, and `Pending USER Confirmation` block implementation. Workstream Entry must return REPAIR/BLOCKED when the contract is SLC-centered, first-seam-centered, stale, unclear, missing user-facing flow/options/constraints, or not accepted/waived. If USER feedback changes direction, UI behavior, workflow, scope, boundaries, or seam order, Codex must update source truth and the Desktop packet/ZIP, set Contract Status to Pending USER Confirmation, and wait for confirmation or explicit waiver. SLC/slice/seam details are implementation staging only and must not become the primary USER decision surface.
