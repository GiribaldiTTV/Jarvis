...
If Live Validation exports or refreshes the desktop `User Test Summary.txt` convenience copy and that file is still a handoff copy, draft copy, or `NOT RETURNED RESULTS`, Codex must stop at UTS handoff. The source-truth packet must report `Automated validators and live helper evidence: GREEN.`, `User Test Summary Results: PENDING.`, and `Final phase advancement is BLOCKED until the filled User Test Summary is submitted and digested.` It must not name PR Readiness as the next legal phase until UTS is `PASS` or `WAIVED` with a waiver reason in the canonical repo `## User Test Summary` section.
