...
Main is the first repo loader and routing index. After Main, follow the owner chain to execution posture docs, phase governance, Nexus Vision, family vision, active branch plans, branch records, workstream records, and helper/validator owners as the task requires.
...
- AI Operational Cache Governance is not memory and is not a new backlog family by default. Route cache behavior first through existing FAM-007 AI/runtime/capability-pack owners, FAM-008 setup/install/cache-root UX owners, FAM-009 workspace/data/root/evidence owners, FAM-010 privacy/safety/local-only/provider-visible-data owners, and the active branch plan for implementation-specific cache behavior; create a new architecture/policy source-truth owner only after `Source-Truth Placement Preflight` proves `No Existing Owner Fits` or USER approves a companion file.
...
- confirmation that the PR Watcher Mode Contract in `Docs/pr_watcher_mode_contract.md` is followed: watcher mode is exactly `Silent Monitor`, `Verify Once`, `Repair Mode`, or `Blocked Mode`, and Stage 2 final handoff includes `Watcher Health Proof:` with `Watcher Mode:`, `Configured CWD:`, `Worktree / Branch:`, `PR:`, `Head SHA:`, `Mergeability:`, `Unresolved Review Threads:`, `Latest Bot Review:`, `Repair Authority:`, `Delivery Route Proof:`, `Runtime Proof:`, and `Next Watcher Posture:`
- confirmation that the watcher contract uses minute cadence, reports only when a watched PR status changes, treats a thumbs-up reaction as PR-entry green, posts exactly one PR conversation nudge for the current head SHA if no Codex bot comment or thumbs-up/approval signal appears after at least two minutes, and treats one or more actionable bot comments as a bounded same-branch repair-worker trigger that verifies identity, evaluates source truth, fixes only approved PR scope, reruns validation, commits, pushes, replies/resolves when required by the review-thread contract, and then records `Comment addressed` for the current head; out-of-scope bot requests must be reported as blockers instead of repaired
...
- runtime-focused implementation branches must also carry `## Runtime Branch Engineering Contract` before Workstream begins or resumes, with `USER Engineering Planning Review:`, `Runtime Implementation Approval:`, `Current Runtime Baseline:`, `Planned Runtime Delta:`, `User-Facing Runtime Delta:`, `State / Config / Schema Delta:`, `Validator / Helper Delta:`, `Expected Changed Files / Surfaces:`, `Approval-Boundary Audit:`, `Future-Gated Items:`, `Workstream Seam Map:`, `Proof Expectations:`, `Risk Forecast:`, `Recommendations And Alternatives:`, `Plan Version / Revision Status:`, and `Plan-To-Implementation Traceability:`; new or re-entering runtime branches must also carry a Branch Runtime Engineering Plan at `Docs/branch_plans/<branch_slug>.md` or a pending Stage 2 requirement, linked by `Branch Runtime Engineering Plan Path:`, with `Engineering Plan Status:` and `PR Fold-Down Packet:` markers; Workstream Entry must create or refresh the active worktree's Desktop `USER Review Desktop Bundle` under `Nexus USER Review\<worktree-label>` with flat copied branch vision, branch plan, branch authority, relevant Nexus/family vision, matrix, UFD/change-intent, and source-truth files before asking USER to green-light implementation; Workstream Entry must also return the `USER Branch Plan Review Gate` packet and stop on `USER Branch Plan Review Missing`, `USER Review Packet Stale`, or `USER Review Packet Not Digested` when the plain-language goal, planned user-facing outcome, implementation breakdown, Hardening plan, Live Validation / UTS plan, USER design review questions, recommendations, implementation options, alternatives/tradeoffs, USER Review Response, Codex Response Digest, scope dispositions, exact USER decision, or `USER Review Packet Finding:` is absent or stale; use the contract and plan to compare seam start packets, seam closeouts, Workstream Green, Hardening proof, Live Validation proof, PR Readiness scope integrity, and Release Readiness public scope against admitted engineering intent
...
- USER Branch Plan Review product/design reinforcement: Workstream Entry must treat `USER_BRANCH_PLAN_REVIEW.md` as the USER Branch Plan Contract, a user-facing planning document and decision ballot, not as a normal Codex digest. The review must include `Contract Status`, `Contract Version / Revision`, `What Will I Actually See, And Where Will I See It?`, `End-State Vision`, `Visual / Functional Walkthrough`, `Surface Map`, `Implementation Options`, `Recommended Direction`, `Why This Fits The Nexus Vision`, `USER Design Direction Decision`, `Current Branch Scope`, `Future-Gated Scope`, `Implementation Staging Notes`, USER decisions, USER response, Codex response digest, implementation constraints created by USER response, rejected/deferred ideas, source-truth impact, contract change log, completion checklist, and Workstream Entry result so USER can decide whether the planned feature end-state and possibility space should proceed. `Contract Status` must be `Complete` or `Waived by USER` before implementation approval is legal; `Draft`, `Pending USER Response`, `Pending Codex Digest`, and `Pending USER Confirmation` block implementation. If USER feedback changes direction, UI behavior, workflow, scope, boundaries, or seam order, Codex must update source truth and the Desktop packet/ZIP, set Contract Status to Pending USER Confirmation, and wait for confirmation or explicit waiver. SLC/slice/seam details are implementation staging only and must not become the primary USER decision surface.
...
- Workstream Entry must provide the USER Branch Plan Review Gate before implementation so USER can accept, revise, defer with waiver, reject, or request more analysis on the readable branch plan
