...
- confirmation that `Bot Review Signal Pending` is clear for the live PR through a Codex Connector bot thumbs-up reaction or green approval comment on the current head; when a bot comment appeared after the last approval, a later thumbs-up/approval signal is required after same-PR repair and comment-resolution closeout
