...
| `dev/orin_monitoring_hud_live_validation.ps1` | Helper Status: Reusable | monitoring HUD live desktop validator | Workstream/Live Validation helper for FAM-006 Monitoring HUD proof. Reuse for launched-desktop marker proof, visible-client screenshot capture, `-RunInteractionSelfQA` live HUD control exercise, interaction manifest output, no-progress watchdog, and cleanup verification before creating another HUD live-proof helper. |
| `dev/orin_monitoring_hud_internal_sandbox_validation.py` | Helper Status: Reusable | monitoring HUD internal sandbox validator | Use for FAM-006 Workstream and Hardening internal product proof. Validates the HUD shell/module markers, tray/unanchor path, anchored click-through versus unanchored edit mode, draggable/resizable/snapping category-card model, provider-truthful telemetry contracts, 1s polling floor, visual warning posture, no fake GPU/thermal values, and retired-name sterilization without treating User Test Summary results as a Workstream or Hardening gate. |
