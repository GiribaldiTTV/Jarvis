...
| --- | --- | --- | --- | --- | --- |
| `dev/orin_fb038_seam2_validation.ps1` | Helper Status: Workstream-scoped | FB-038 | Created as a deterministic seam runner while Python/PySide6 path availability was failing; it coordinates existing reusable validators rather than replacing them. | Fold runner semantics into documented validator invocation guidance or keep only if FB-038 Hardening proves the runner is needed as a reusable tray validation entrypoint. | Before FB-038 PR Readiness. |
| `dev/orin_fb038_seam2_live_validation.ps1` | Helper Status: Workstream-scoped | FB-038 | Created to prove tray-origin Create Custom Task dialog-open/no-write behavior without overloading the broader FB-036 interactive suite mid-Workstream. | Consolidate into a reusable tray live validation helper or into `dev/orin_saved_action_authoring_interactive_validation.ps1` if Hardening proves the flow belongs there. | Before FB-038 PR Readiness. |
| `dev/orin_fb038_seam3_live_validation.ps1` | Helper Status: Workstream-scoped | FB-038 | Created to prove tray-origin create completion, catalog reload, exact-match re-resolution, and cleanup while keeping Seam 3 separate from Seam 2 no-write proof. | Consolidate with the Seam 2 tray helper into a reusable tray/Create Custom Task live validation helper or fold into the saved-action interactive suite. | Before FB-038 PR Readiness. |
