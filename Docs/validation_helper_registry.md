...
| `dev/orin_monitoring_hud_internal_sandbox_validation.py` | Helper Status: Reusable | monitoring HUD internal sandbox validator | Use for FAM-006 Workstream internal product proof before Hardening. Validates the HUD shell/module markers, tray/unanchor path, anchored click-through versus unanchored edit mode, draggable/resizable/snapping category-card model, provider-truthful telemetry contracts, 1s polling floor, visual warning posture, no fake GPU/thermal values, and retired-name sterilization without treating User Test Summary results as a Workstream gate. |
