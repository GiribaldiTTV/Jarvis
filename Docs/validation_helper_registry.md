...
| Path | Helper Status | Family | Reuse / Consolidation |
| --- | --- | --- | --- |
| `dev/orin_ai_provider_state_validation.py` | Helper Status: Reusable | FAM-007 provider state validator | Reuse for static FAM-007 no-provider/provider-privacy state checks before creating another local-AI/provider-state validator. Extend for provider selection, visible provider/privacy state, local-only/no-provider semantics, no hidden external calls, prompt-acceptance gating, and future consent-boundary checks when those seams are admitted. |
...
| `dev/orin_monitoring_hud_human_client_validation.ps1` | Helper Status: Active / Required before FAM-006 LV1 handoff | monitoring HUD human-client desktop validator | WS48/WS49+ helper for the actual USER-like path. Launches `C:\Users\anden\OneDrive\Desktop\Nexus Desktop Launcher.lnk` by default or a governed `NEXUS_DESKTOP_VALIDATION_SHORTCUT_PATH` equivalent when the real shortcut targets a different worktree, resolves the shortcut target and working directory before launch, records `shortcutResolution`, and requires `shortcut_targets_active_worktree=PASS` so the helper fails fast if the shortcut is not rooted in the active FAM-006 worktree. It uses visible Windows tray/menu actions, exact native tray menu item rectangles by label, broadened tray icon resolution, mouse/cursor click/drag evidence, Dashboard open/close/move/resize checks, real-shortcut first-open screenshot-sequence proof for #123 recurrence with visual-continuity analysis, Dashboard IA-card Settings open/Done close proof, Settings double-click native-maximize guard proof, window-level Dashboard Close hide/reopen proof, pre-click Windows resize-cursor discovery, stricter resize-fluidity geometry samples, NCP Create Custom Task/Create Custom Group/Manage Custom Tasks/Manage Custom Groups clicks while Dashboard is visible/moved, tray Create Custom Task duplicate-dialog guard proof, visible shutdown-confirmation check, timeout/cancel/accept proof, virtual-desktop screenshot-frame artifacts, fallback cleanup for non-UIA dialogs, and a JSON manifest at `dev/logs/fam_006_human_client_validation/latest_manifest.json`. Tray menu actions must resolve inside the native tray popup or exact native menu-coordinate fallback so in-app buttons cannot be over-credited as tray proof. This helper is Workstream/Hardening proof infrastructure only; it must not generate or digest the formal UTS. The Live Validation helper must not export a green UTS unless this human-client manifest is PASS with shortcut/worktree alignment, Settings/window-level Close/first-open visual-continuity/resize-fluidity proof, or USER explicitly waives the missing proof. |
