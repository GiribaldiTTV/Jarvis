...
| --- | --- | --- | --- |
| `dev/orin_branch_governance_validation.py` | Helper Status: Reusable | governance validator | Extend when repo-wide source-of-truth, phase, branch, release, helper registry, prompt-contract, Source-Truth Placement Preflight, Element Validation Ledger placement, Element Validation Ledger drift, PR Readiness post-merge source truth, Release Readiness Health Pass rules, `Origin/Main Freshness Check` rules, `Standing Governance Intake Branch` rules, or `Assigned Worktree Confinement` rules become machine-checkable. It owns `--standing-governance-intake-gate` for `feature/release-readiness-source-truth-intake`, `Release Readiness digest` intake, `RRI-YYYYMMDD-NNN`, `One Active Cycle`, the pre-intake `Sync Rule`, `Bootstrap Exception Limit`, `Waiting For Governance Intake` / `Waiting For Updated Main`, and the required `Return Digest`; it also owns `--worktree-confinement-gate` for `Expected Worktree Root:`, `Actual Worktree Root:`, `No Cross-Worktree Mutation`, `GitHub Desktop-bound worktree`, `Worktree Escape User Waiver Missing`, and `Worktree Escape User Waiver: Granted` validation, and `--pr-readiness-gate` enforces PR Readiness `Branch Creation Base:`, `Current origin/main:`, `Origin/Main Advanced Since Branch Creation:`, `Reconciliation File List:`, `Reconciliation Recommendation:`, and no file fixes during Stage 1. |
...
- `dev/orin_branch_governance_validation.py --release-readiness-health-gate` must prove post-merge source truth through the `Release Readiness Health Pass` before PR creation or merge approval; the owning record must include `Post-Merge Branch Authority Projection:`, `Stale Active Branch Wording Scan:`, `Stale PR Creation / PR Readiness Pending Wording Scan:`, `Merged-Unreleased Scope Posture:`, `Release Execution Gate:`, `Watcher / Live PR State Projection:`, `Branch Cleanup Plan:`, `Branch Cleanup Execution Gate:`, `FAM Overlap Routing:`, and `Projected Post-Merge Validation:` after any Stage 2 or bot-review source-truth repair. `Branch Cleanup Execution Gate:` keeps stale/old branch cleanup, worktree removal, branch switching, and GitHub Desktop-bound worktree cleanup out of Release Readiness; cleanup belongs to `Branch Readiness Stage 2 - Execution Gate` after `Stale Branch Cleanup Plan:`, branch/worktree creation or validation, `git worktree list`, and replacement target proof show no repository will lose its valid branch target.
- `dev/orin_branch_governance_validation.py --standing-governance-intake-gate` must prove the single `Standing Governance Intake Branch` is on the correct branch/worktree contract, has clean tracked state, uses a `Release Readiness digest`, carries at most `One Active Cycle` with `RRI-YYYYMMDD-NNN`, satisfies the pre-intake `Sync Rule`, preserves `Waiting For Governance Intake` / `Waiting For Updated Main`, restricts changed files to governance/source-truth/validator scope, and requires the post-merge `Return Digest`
