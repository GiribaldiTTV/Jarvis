...
| Path | Helper Status | Family | Reuse / Consolidation |
| --- | --- | --- | --- |
| `dev/orin_monitoring_hud_surface_validation.py` | Helper Status: Reusable | monitoring HUD surface validator | Reuse for static Monitoring/HUD visual-surface markers, desktop-mode activation, runtime telemetry-boundary markers, renderer-owned placement markers, controls-visibility markers, fail-safe/no-data/degraded-status behavior markers, slice-boundary copy, no-hardware-polling/no-settings-persistence/no-fake-hardware-values/no-recovery-claims/no-voice boundary checks, no expanded retired legacy user-facing identity, Stage 2-R13 Dashboard-first Workstream handoff source-truth markers, Stage 2-R12 Dashboard-first Interface Release Boundary source-truth markers, WS31 Dashboard-only acceptance baseline markers, WS32 Dashboard standalone movement/clipping/Core-Overlay decoupling markers, WS33 Dashboard settings/control content and monitor-group clarity markers, Dashboard-specific acceptance/proof separation, Stage 2-R11 dashboard-versus-minimal-HUD architecture expectations, WS18/WS30 Core desktop-layer/non-blocking ownership markers, WorkerW coordinate-rebase proof, no Core topmost foreground ownership, WS19-WS33 historical/supporting markers, and Overlay/display deferred/non-gating proof classification before creating another HUD surface validator. |
| `dev/orin_monitoring_hud_live_validation.ps1` | Helper Status: Reusable | monitoring HUD live desktop validator | Workstream/Live Validation helper for FAM-006 Monitoring HUD proof. Reuse for Dashboard-first WS31+ live proof, WS32 dashboard standalone travel/clipping/decoupling proof, WS33 dashboard settings/control content and monitor-management clarity proof, launched-desktop marker proof, dashboard-specific full virtual-desktop screenshot capture, Core desktop-layer/non-interference runtime markers, Dashboard/Overlay/Core independence evidence, raw PNG evidence copies under `C:\Users\anden\OneDrive\Pictures\Screenshots\Nexus Desktop AI\fam_006_monitoring_hud_live_validation\<stamp>\`, `-RunInteractionSelfQA` live HUD control exercise, `-ActiveUserFacingClient` foreground/user-observable validation, interaction manifest output, no-progress watchdog, cleanup verification, Dashboard-first self-QA that does not require Overlay/display unanchor/card-drag/card-resize acceptance, and future Overlay/display proof only when that interface is re-admitted or USER grants explicit interface-bundle approval. Remaining gaps before package-complete claims: Dashboard-focused Workstream/Hardening/Live Validation reruns, and formal returned-UTS acceptance. |
| `dev/orin_monitoring_hud_internal_sandbox_validation.py` | Helper Status: Reusable | monitoring HUD internal sandbox validator | Use for FAM-006 internal product proof. Validates Dashboard-first Workstream handoff posture, WS31 Dashboard-only acceptance baseline posture, WS32 Dashboard standalone movement/clipping/Core-Overlay decoupling posture, WS33 Dashboard settings/control content and monitor-group clarity posture, the standalone HUD shell/module files, ORIN Core render-file isolation, dashboard-specific settings/control behavior, provider-truthful telemetry contracts, 1s polling floor, visual warning posture, no fake GPU/thermal values, retired-name sterilization, and Overlay/display deferred/non-gating classification without treating User Test Summary results as a Workstream or Hardening gate. Historical WS18-WS30 markers remain supporting repair evidence, not current Dashboard acceptance by themselves. Remaining gaps before package-complete claims: Dashboard-focused Workstream/Hardening/Live Validation reruns, and formal returned-UTS evidence. |
