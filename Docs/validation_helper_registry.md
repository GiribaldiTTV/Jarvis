...
| --- | --- | --- | --- |
| `dev/orin_branch_governance_validation.py` | Helper Status: Reusable | governance validator | Extend when repo-wide source-of-truth, phase, branch, release, helper registry, prompt-contract, Source-Truth Placement Preflight, Element Validation Ledger placement, Element Validation Ledger drift, PR Readiness post-merge source truth, Release Readiness Health Pass rules, `Standing Governance Intake Branch` rules, or `Assigned Worktree Confinement` rules become machine-checkable. It owns `--standing-governance-intake-gate` for `feature/release-readiness-source-truth-intake`, `Release Readiness digest` intake, `RRI-YYYYMMDD-NNN`, `One Active Cycle`, the pre-intake `Sync Rule`, `Waiting For Governance Intake` / `Waiting For Updated Main`, and the required `Return Digest`; it also owns `--worktree-confinement-gate` for `Expected Worktree Root:`, `Actual Worktree Root:`, `No Cross-Worktree Mutation`, `GitHub Desktop-bound worktree`, `Worktree Escape User Waiver Missing`, and `Worktree Escape User Waiver: Granted` validation. |
