...
| `dev/orin_monitoring_hud_live_validation.ps1` | Helper Status: Reusable | monitoring HUD live desktop validator | Workstream/Live Validation helper for FAM-006 Monitoring HUD proof. Reuse for launched-desktop marker proof, visible-client screenshot capture, `-RunInteractionSelfQA` live HUD control exercise, `-ActiveUserFacingClient` foreground/user-observable validation, interaction manifest output, no-progress watchdog, and cleanup verification before creating another HUD live-proof helper. |
