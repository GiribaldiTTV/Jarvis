...
| Path | Helper Status | Family | Reuse / Consolidation |
| --- | --- | --- | --- |
| `dev/orin_monitoring_hud_surface_validation.py` | Helper Status: Reusable | monitoring HUD surface validator | Extend for static Monitoring/HUD visual-surface markers, desktop-mode activation, runtime telemetry-boundary markers, renderer-owned placement markers, controls-visibility markers, fail-safe/no-data/degraded-status behavior markers, slice-boundary copy, no-hardware-polling/no-settings-persistence/no-fake-hardware-values/no-recovery-claims/no-voice boundary checks, no expanded retired legacy user-facing identity, and Stage 2-R11 dashboard-versus-minimal-HUD architecture expectations before creating another HUD surface validator. |
| `dev/orin_monitoring_hud_live_validation.ps1` | Helper Status: Reusable | monitoring HUD live desktop validator | Workstream/Live Validation helper for FAM-006 Monitoring HUD proof. Reuse for launched-desktop marker proof, full virtual-desktop screenshot capture, `C:\Users\anden\OneDrive\Pictures\Screenshots\Nexus Desktop AI\fam_006_monitoring_hud_live_validation\<stamp>\` raw PNG evidence copies for USER inspection, `-RunInteractionSelfQA` live HUD control exercise, `-ActiveUserFacingClient` foreground/user-observable validation, interaction manifest output, no-progress watchdog, and cleanup verification before creating another HUD live-proof helper. Stage 2-R11 gap to repair before future green claims: extend proof for dashboard-versus-minimal-HUD separation, OS-level minimal-HUD click-through/non-focus behavior, Core visual transparency/non-interference, dashboard drag smoothness, dashboard content relevance, styled scrollbar/product polish, fresh full-desktop proof root, and UTS evidence-root freshness. |
| `dev/orin_monitoring_hud_internal_sandbox_validation.py` | Helper Status: Reusable | monitoring HUD internal sandbox validator | Use for FAM-006 Workstream and Hardening internal product proof. Validates the standalone HUD shell/module files, verifies the ORIN Core render files remain HUD-free, validates tray/unanchor path, anchored click-through versus unanchored edit mode, draggable/resizable/snapping category-card model, provider-truthful telemetry contracts, 1s polling floor, visual warning posture, no fake GPU/thermal values, and retired-name sterilization without treating User Test Summary results as a Workstream or Hardening gate. Stage 2-R11 gap to repair before future green claims: add sandbox/proof coverage for dashboard/minimal-HUD role separation, independent minimal HUD ownership, Core non-blocking visual behavior, dashboard configuration-content relevance, and product scrollbar styling. |
