...
| Path | Helper Status | Family | Reuse / Consolidation |
| --- | --- | --- | --- |
| `dev/orin_ai_provider_state_validation.py` | Helper Status: Reusable | FAM-007 provider state validator | Reuse for static FAM-007 no-provider/provider-privacy, provider-selection/consent-boundary, disabled Assisted Desktop no-provider interaction, provider-visible-data disclosure, and no-provider fallback state checks before creating another local-AI/provider-state validator. It now covers local-only/no-provider fallback semantics, visible provider/privacy/selection/consent state, disabled prompt/interaction affordance, no hidden external calls, prompt-acceptance gating, no memory persistence, and future consent-boundary extension points when later seams are admitted. |
