...
- no PR-ready before PR creation and PR validation:
  ...
  - if a branch expects watcher-based PR monitoring, `PR Watcher Provisioning Unproven` also keeps the gate blocked until the watcher target, runtime path, run-proof method, fallback, teardown rule, replacement provisioning for the next live PR, and same working thread contract are explicit and proven
