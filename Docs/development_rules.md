...
- no PR-ready without user-facing desktop-shortcut validation:
  ...
  - when the declared launcher is a Windows desktop shortcut and Computer Use is available, the final gate must use Codex's visible desktop-control session to click, double-click, or keyboard-open the exact USER shortcut from a visible selected Desktop folder item, then prove the resulting tray/menu/window path with photo or video evidence; visible Computer Use mouse movement, left-click, right-click, double-click, and keyboard input against a visually identified target are valid user-simulation inputs, but hidden app activation calls, direct handler calls, internal signal dispatch, synthetic tray/menu activation, scripted shortcuts that bypass the visible control, UIAutomation-only helpers, shell-launched shortcuts, code/log/helper proof, marker output, and blind taskbar/tray coordinate probing are support evidence only or invalid proof routes. The required escalation order is Computer Use primary, bounded visible coordinate fallback second for screenshot-visible OS chrome that Computer Use cannot target as a window, and single-path manual USER waiver last.
