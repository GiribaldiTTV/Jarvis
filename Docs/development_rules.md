...
- no PR Readiness Stage 2 without USER review approval:
  ...
  - `Branch Readiness Fallback Required` also blocks Stage 2 when governance/source-of-truth ledger drift spans identity model, FAM taxonomy, package/branch rule, USER approval blockers, real-carrier routing, branch authority lifecycle, watcher/automation proof, release execution boundaries, Element Coverage misuse, ChatGPT loader/source-truth drift, project direction, current workflow, after-release workflow, or absolute guardrails and the fix requires Branch Readiness rather than bounded current-branch PR Stage 1 repair
