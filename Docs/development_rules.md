...
- no PR-ready without user-facing desktop-shortcut validation:
  ...
  - when the declared launcher is a Windows desktop shortcut, the final gate must use visible user-level desktop control to click, double-click, or keyboard-open the exact USER shortcut from a visible selected Desktop folder item, then prove the resulting tray/menu/window path with photo or video evidence; the governed human-client helper, old-style real-cursor movement, or another visible mouse/keyboard path may be the primary proof route when it preserves before/during/after evidence and exercises the actual USER-facing control. Computer Use is prohibited for Nexus validation: Codex and validation tooling must not invoke it for inspection, capture, target discovery, control assistance, launcher activation, interaction, evidence generation, or cleanup. Any Computer Use participation invalidates the current validation run, requires its gating artifacts to be excluded, records `Computer Use Validation Path Prohibited`, and restarts from a clean preflight through the governed human-client path. Hidden app activation calls, direct handler calls, internal signal dispatch, synthetic tray/menu activation, scripted shortcuts that bypass the visible control, UIAutomation-only helpers, shell-launched shortcuts, code/log/helper proof, marker output, Computer Use activation, and blind taskbar/tray coordinate probing are invalid proof routes. Codex owns operation of the validation step through reliable visible user-level desktop control. If safe control-surface acquisition remains unavailable after the deterministic acquisition loop, record `Live Validation Control-Surface Acquisition Blocked`; do not switch to Computer Use and do not transfer the interaction step to the USER.
  - visible shortcut discovery must preserve the user's current desktop workspace. Live Validation helpers must not use desktop-wide `MinimizeAll`, show-desktop, or broad window-state disruption merely to find the shortcut; they must prefer an already-visible desktop icon, an already-visible Desktop folder item, or a bounded File Explorer select/open fallback with before/after screenshot proof.
  - Live Validation must exercise the actual USER-facing control surfaces the USER would use. Codex must not create, add, or depend on a new user-visible runtime control surface, dev-only control window, shortcut side channel, or validation-only product window merely to make existing tray/menu/window controls targetable. If the actual tray/menu/window path cannot be operated safely through visible user-equivalent input, stop on `Live Validation Control-Surface Acquisition Blocked` and route the exact Codex-owned acquisition or tooling repair need; do not ask the USER to perform the interaction as substitute proof.
  - activation-path proof is non-transferable: a visible mouse click proving a button path does not prove the matching keybind path, a keybind proof does not prove the click path, a tray/menu proof does not prove a dashboard/control-window path, and injected/internal activation never proves any USER-facing path. Each admitted activation path must be validated separately with photo/video or ordered frame evidence for both the action and the user-visible state/result it changes. If exhaustive Codex-owned visible attempts cannot safely prove exactly one activation path, USER may grant a named single-path waiver as an acceptance decision without operating the control; that waiver remains supporting evidence for the rest of Live Validation and cannot be generalized to other activation paths or product surfaces.
...
The shortcut blocker lifts only after `User-Facing Shortcut Validation: PASS` is recorded with governed visible desktop-control evidence from the declared shortcut path, or `User-Facing Shortcut Validation: WAIVED` is recorded with `User-Facing Shortcut Waiver Reason:` proving the branch is not desktop/user-facing or the shortcut path is explicitly unavailable. Computer Use must not be invoked or consumed as validation evidence; any run containing it is invalid and must be restarted through the governed human-client path.
