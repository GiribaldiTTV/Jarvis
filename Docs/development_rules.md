...
- no PR-ready with unresolved post-merge planning:
  - if post-merge truth needs `No Active Branch` handling, release-debt handling, next-workstream planning, next-workstream canon sync, minimal scope, branch-authority cleanup, selected-next defer/waiver truth, or branch-creation deferral, that handling must already be complete inside PR Readiness Stage 1 when Stage 1 finds it; Stage 2 may verify and execute the PR, but it must not be the first phase that repairs those source-truth projections
- no PR Readiness Stage 2 without USER review approval:
  ...
  - Stage 1 specifically owns merge-target `No Active Branch` projection, merged-unreleased release-debt owner contracts, selected-next defer/waiver truth, and active-branch-authority cleanup when those items are found; leaving them as a Stage 2 sync plan keeps `PR Readiness Stage 1 Repair Pending` active
