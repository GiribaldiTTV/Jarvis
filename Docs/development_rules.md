...
- no PR-ready without user-facing desktop-shortcut validation:
  ...
  - when the declared launcher is a Windows desktop shortcut, the final gate must use visible user-level desktop control to click, double-click, or keyboard-open the exact USER shortcut from a visible selected Desktop folder item, then prove the resulting tray/menu/window path with photo or video evidence; the governed human-client helper, old-style real-cursor movement, or another visible mouse/keyboard path may be the primary proof route when it preserves before/during/after evidence and exercises the actual USER-facing control. Computer Use is not authorized as validation proof: it may support inspection, capture, target discovery, or a non-gating control attempt, but it cannot record `User-Facing Shortcut Validation: PASS`, cannot satisfy `Exact USER Desktop Launcher Validation: PASS`, and cannot replace the required visible desktop-control, returned USER manual validation, or explicit USER waiver evidence. Hidden app activation calls, direct handler calls, internal signal dispatch, synthetic tray/menu activation, scripted shortcuts that bypass the visible control, UIAutomation-only helpers, shell-launched shortcuts, code/log/helper proof, marker output, Computer Use-only activation, and blind taskbar/tray coordinate probing are support evidence only or invalid proof routes. The required escalation order is reliable visible user-level desktop control first, governed manual USER validation/waiver when the visible path cannot be safely proven, and no Computer Use-based validation green.
