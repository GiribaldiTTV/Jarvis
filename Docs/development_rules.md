...
- no PR Readiness Stage 2 without USER review approval:
  ...
  - the user-facing `## Next Workstream` block must include `Recommended Next Workstream:`, recommended family/package, candidate slices, `User-Facing Output:`, why it is next, dependencies/blockers, validation needs, release impact, selection-truth status, and branch-creation status
