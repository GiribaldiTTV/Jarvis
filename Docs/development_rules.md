...
The stable local USER hub folder and exported zip must be regenerated from a clean output folder, not incrementally reused. `dev/orin_user_review_bundle.py` owns the durable path: clear the worktree-labeled review folder under `C:\Nexus USER`, write current root `START_HERE.md`, place exactly one primary current-gate decision file under `USER Review`, place generated supporting digests/checklists under `Review Aids`, copy fresh repo-relative source-truth context under `Source Truth Context`, atomically replace the stable `C:\Nexus USER\<worktree-label>.zip`, create a byte-identical timestamped USER upload zip beside it at `C:\Nexus USER\<worktree-label>__YYYYMMDD-HHMMSS.zip`, then validate the zip's file list, applicable `USER_BRANCH_VISION_REVIEW.md` / `USER_BRANCH_PLAN_REVIEW.md`, packet finding, unresolved placeholders, and packet-count consistency before claiming the USER packet is current. Codex must tell USER to upload the timestamped zip so repeated same-name uploads do not collide; the stable folder and stable zip remain the canonical local USER hub pair. Stale folder contents, stale uploaded zip metadata, missing applicable Branch Planning review files, or a mismatch between folder and zip blocks on `USER Review Packet Stale`.
