...
Assigned Worktree Confinement is mandatory for every thread assigned to a specific worktree. Before mutation, Stage 2 execution, commit, push, PR work, release work, runtime validation, shortcut/provider/model action, branch/worktree cleanup, or GitHub Desktop handoff, report `Active Thread Owner:`, `Thread Assignment Status:`, `Worktree Ownership Ledger:`, `Intended Write Set:`, `Same Worktree / Same Branch Collision Check:`, `Dirty Worktree Collision Check:`, `Dirty Worktree Recovery Packet:`, `Off-Worktree Work Routing:`, `Governance Routing Barrier:`, `New Worktree Decision Gate:`, `Expected Worktree Root:`, `Actual Worktree Root:`, `No Cross-Worktree Mutation:`, and the `GitHub Desktop-bound worktree`. If an active authority record also carries `Current Write Set:`, that value must be independently bounded and its concrete repo paths must be a subset of the confinement `Intended Write Set:`; the current value cannot override, broaden, or contradict intended authority. If the active root is not the assigned root, stop on `Worktree Escape User Waiver Missing`; if another active thread owns the same worktree or branch, stop on `Parallel Worktree Coordination Missing`; if the target worktree is dirty and ownership is unclear, stop for a `Dirty Worktree Recovery Packet`. If the requested work belongs outside the assigned worktree, outside the active branch scope, or to another active lane, stop on `Governance Routing Barrier` and route the packet to `C:\Nexus Worktrees\Governance`; Governance decides whether an existing owner, new worktree/thread, or USER waiver is required. The assigned thread must not mutate sibling worktrees, parked clones, or the neutral/main folder by convenience unless the USER grants `Worktree Escape User Waiver: Granted` with expected root, actual root, target root, bounded allowed commands/files, expiration or stop condition, required validation, and return path. Without that complete grant, `Expected Worktree Root:`, `Actual Worktree Root:`, and `GitHub Desktop-bound worktree:` must all equal the active Git root; with a grant, the declared actual and Desktop roots must equal the active Git root and the confinement expected root must remain bound to the assigned identity root.
