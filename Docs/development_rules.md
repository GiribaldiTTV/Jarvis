...
- no PR-ready without user-facing desktop-shortcut validation:
  ...
  - when the declared launcher is a Windows desktop shortcut and Computer Use is available, the final gate must use Codex's visible desktop-control session to click or double-click the exact USER shortcut, then prove the resulting tray/menu/window path with photo or video evidence; scripted clicks, UIAutomation-only helpers, shell-launched shortcuts, code/log/helper proof, marker output, and blind taskbar/tray coordinate probing are support evidence only or invalid proof routes
