...
The stable Desktop review folder and exported zip must be regenerated from a clean output folder, not incrementally reused. `dev/orin_user_review_bundle.py` owns the durable helper path: clear the worktree-labeled review folder, copy fresh repo-relative source truth, write `START_HERE.md` as a plain-language index, atomically replace the stable `Nexus USER Review\<worktree-label>.zip`, then validate the zip's branch, `HEAD`, `origin/main`, file list, `USER_BRANCH_PLAN_REVIEW.md`, stale-guard proof, and packet finding internally before claiming the USER packet is current. Technical proof prints in helper output; stale folder contents, stale uploaded zip metadata, missing `USER_BRANCH_PLAN_REVIEW.md`, or mismatched helper proof blocks on `USER Review Packet Stale`.
