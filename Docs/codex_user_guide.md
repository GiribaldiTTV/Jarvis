...
The Stage 1 `## Next Workstream` block must include `Recommended Next Workstream:`, recommended family/package, candidate slices, `User-Facing Output:`, why it is next, dependencies/blockers, validation needs, release impact, selection-truth status, and branch-creation status. If no legal next workstream candidate is found, Stage 1 must stop on `Next Workstream Candidate Not Found` and report the still-not-closed FAM list plus every not-complete package and slice.
