...
The review file should read like a user-facing product/design planning document and decision ballot. It must show `End-State Vision`, `Visual / Functional Walkthrough`, surface map, implementation options with risks, Codex's recommended direction, current scope, future-gated scope, SLC package plan, USER decisions, response area, Codex digest area, and Workstream Entry result area, so USER can answer whether they actually like what Codex is about to build.
