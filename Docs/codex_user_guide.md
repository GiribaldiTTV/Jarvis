...
- relevant desktop UI Live Validation must use the exact normal USER desktop runtime launcher declared for the UTS path, or record an explicit USER waiver; sandbox/offscreen/direct-runtime/WebView/helper launches are supporting evidence only and cannot replace that launcher gate
...
PR Readiness does not require selected-next truth or a waiver by default. It may encode or validate selected-next truth only when USER explicitly approves PR-time selected-next sync or selected-next truth already exists and would merge as durable repo truth.
When USER-approved PR-time selected-next truth exists, selection must be priority-led: use open backlog `Priority` and deferred-context readiness, not `Target Version`, to choose the next candidate.
...
If USER approval for a new or successor backlog identity, package admission, branch creation, backlog split, promotion, or single-slice package waiver is absent, `Backlog Addition User Approval Missing` remains active only for the attempted mutation; it does not force PR Readiness to invent selected-next truth.
If no USER-approved selected-next truth exists, PR Readiness may project post-merge `No Active Branch` and route normal successor selection to Branch Readiness Stage 1.
If approval exists but no real runtime candidate can be selected, `Next Runtime Candidate Selection Pending` applies only to that approved PR-time selected-next path.
...
Stage 1 must return `## PR Readiness Stage 1 Analysis Packet` with the planned PR title/base/head/summary, required post-merge path, release-debt impact, release-debt handling status, selected-next validation status when selected-next truth exists or PR-time selection is explicitly approved, required current-branch source-truth sync, planned merge-target canon updates, planned direct PR verification, `Planned Watcher Provisioning: Denied by default` unless a USER-approved watcher exception exists, `Pre-PR Live State:`, `PR Creation Approval:`, `Stage 2 PR Creation:`, `Post-Merge No Active Branch Projection:`, planned validations, expected file changes, Stage 2 sync plan, drift findings, blockers, waivers, release-window audit posture, rollback path, `Next Legal Phase:` digest field, and the exact Stage 2 green-light decision needed. Branch Readiness Stage 1 owns the normal next runtime implementation pipeline selection; PR Readiness does not require selected-next truth or a waiver by default.
When Stage 1 has not yet created a PR, it may record `Pre-PR Live State: No live PR`, `PR Creation Approval: Pending USER approval`, and `Stage 2 PR Creation: Pending USER approval`; this is normal Stage 1 posture, not PR-ready green. For source-only, docs-only, governance, validator, or repo-wide support branches, Stage 1 may merge with post-merge `No Active Branch` when no USER-approved selected-next truth exists; that is not permission to create, select, or admit a runtime successor.
...
The Stage 1 `## Next Workstream` block is optional and appears only when USER asks PR Readiness for successor-selection analysis or selected-next truth already exists. When included, it must include `Recommended Next Workstream:`, recommended family/package, candidate slices, `Candidate Work To Be Done:`, `User-Facing Output:`, why it is next, dependencies/blockers, validation needs, release impact, selection-truth status, and branch-creation status.
Missing next-workstream recommendation, missing selected-next truth, missing `Next Workstream User Waiver:`, or missing `## Next Branch Pre-Plan` does not block Stage 2 by default. If selected-next truth already exists or USER explicitly approves PR-time successor selection, Stage 1 must validate that truth and stop on the applicable selected-next blocker when it is inconsistent.
Stage 1 may include a no-work `## Next Branch Pre-Plan` section only when USER asks PR Readiness for a next-branch recommendation or selected-next truth already exists. Normal next-branch package-shape proof belongs to Branch Readiness Stage 1.
If USER-approved PR-time selected-next truth exists and the next branch cannot be shown as a broad FAM/package with multiple concrete candidate slices, `Next Branch Package Shape Unproven` blocks Stage 1 continuation; if the pre-plan looks like single-seam or single-slice drift, `Single-Slice Branch Drift Risk Unresolved` blocks; if the pre-plan drifts away from FAM -> Package -> Slice -> Seam or revives old live `FB-###` identity behavior, `Family Organization Drift Risk Unresolved` blocks. The same fallback classification applies when the governance/source-of-truth ledger audit finds identity, FAM taxonomy, package/branch, USER approval, real-carrier, branch-authority, watcher/automation, release execution, Element Coverage, ChatGPT loader/source-truth, project direction, current workflow, after-release workflow, or absolute-guardrail drift that cannot be cleared as bounded current-branch PR Stage 1 repair.
...
`PR Readiness Stage 2 - Execution Gate` then performs final PR execution only: final PR package sync, commit/push if needed, PR creation, direct PR verification, bot-review handling, mergeability validation, and direct merge/close verification. Recurring PR watcher automation is denied by default unless USER separately approves a named watcher exception for the exact PR.
Stage 2 owns final PR execution only after the readiness-lock outcome is green.
PR creation is blocked while any Stage 1 blocker, Stage 1 repair item, selected-next validation item when applicable, branch-shape review item when applicable, or Stage 2 sync prerequisite remains unresolved.
Branch Readiness Stage 1 owns the normal next runtime implementation pipeline selection. Branch Readiness fallback is real carrier branch/package analysis; current-branch re-entry keeps the same legal carrier, while new-carrier fallback requires a new real carrier branch.
...
Before Workstream implementation begins or resumes, Branch Planning must return BP1 `USER Branch Vision Review`, BP2 `USER Branch Plan Review`, and BP3 Workstream Entry / Orchestration Validation through the stable local USER hub packet at `C:\Nexus USER\<label>` with matching timestamped `C:\Nexus USER\<label>-YYYYMMDD-HHMMSS.zip`. BP1 uses `USER_BRANCH_VISION_REVIEW.md` for the readable branch vision. BP2 uses `USER_BRANCH_PLAN_REVIEW.md` for the readable engineering plan. BP3 proves the plan implements the accepted vision and can recommend the entry implementation seam for the admitted same-branch Workstream package. `USER Review Packet Finding:` must prove the local USER hub packet was loaded and digested or explicitly waived. Missing or first-seam-only review blocks on `BP1 Branch Vision Review Missing`, `BP2 Branch Plan Review Missing`, or `BP3 Orchestration Validation Missing`; stale packet metadata blocks on `USER Review Packet Stale`; an omitted packet digest or missing USER response/digest/waiver blocks on `USER Review Packet Not Digested`. USER may accept, revise, defer with waiver, reject, or request more analysis before implementation.

The review file should read like a user-facing vision or plan document and decision ballot. `USER_BRANCH_VISION_REVIEW.md` is the BP1 Branch Vision Contract. `USER_BRANCH_PLAN_REVIEW.md` is the BP2 Branch Plan Contract, and the named BP2 checkpoint is the `USER Branch Plan Review Gate`. `Contract Status` must be `Complete` or `Waived by USER` before implementation approval is legal; `Draft`, `Pending USER Response`, `Pending Codex Digest`, and `Pending USER Confirmation` block implementation. When USER feedback changes the plan, Codex updates source truth and the local USER hub packet/ZIP and returns the contract to Pending USER Confirmation until USER confirms or explicitly waives it. The decision ballot is about accepting, changing, or adding to the end-state and recommendation; SLC/slice/seam details are background implementation staging, with SLC resolving to Slice-level line items rather than seams or separate branches.
...
When a `PR Readiness` pass is package-ready, green, or reports `PR READY: YES`, require inclusion-only copy-ready `PR Creation Details` operator blocks. Include a standardized `Next Branch` block only when USER explicitly requested PR-time successor selection, selected-next truth already exists, or Branch Readiness is the next legal phase.
This keeps PR creation instructions from being reinvented or omitted while preventing PR Readiness from inventing successor branches by inertia.
For Codex-created PRs, `PR Readiness GREEN` also requires the live PR to clear `Bot Review Signal Pending` through a thumbs-up reaction or green approval comment from the Codex Connector bot on the current head. A bot comment is not approval; it keeps `PR Validation Pending` active until the branch fixes the comment on the same PR, pushes, replies to and resolves the review thread, requests Codex Connector bot revalidation, and receives a later Codex Connector bot thumbs-up reaction or green approval comment for the repaired live PR head. That approval proof must be bound to the current live PR head by review commit SHA, PR timeline order, or equivalent GitHub live-head evidence, not by local commit time alone. This is the same-PR Codex bot-review repair loop. Stage 2 final handoff cannot be green until the post-repair bot thumbs-up/approval latch is verified.
...
If USER explicitly approves watcher-based PR monitoring for the exact PR, `PR Watcher Provisioning Unproven` keeps watcher-based proof blocked until the watcher target, approved reporting surface, routing proof, runtime path, run-proof method, fallback, teardown rule, replacement provisioning for the live PR, and the live bot-review action contract are explicit and proven. Without that exception, direct PR verification owns the live PR check. Manual rollout-file or transcript-file injection does not count as proof. Watcher configuration is not runtime proof.
...
Standard bounded PR2 operating procedure is direct PR verification in the active Codex turn. Inspect the live PR, comments, reviews, unresolved review threads, reactions where available, checks, mergeability, head SHA, and merge/close state through the GitHub connector, `gh`, GraphQL review-thread reads, or equivalent direct GitHub evidence. Recurring PR watcher automation is denied by default and requires a separate USER-approved watcher exception for the exact PR before watcher proof can support a gate. The Direct PR2 Continuation Rule blocks quiet handoff after revalidation requests or repair pushes while direct verification can continue.
Every Codex Connector review-request or revalidation PR comment must be 3-5 words only; proof detail belongs in the Codex digest/helper/validator/external-state surfaces.
Watcher status-change output, delivery proof, and retry behavior are exception-only requirements for a USER-approved watcher path.
After live PR creation, live PR validation, merge-status green, and bot-review approval are complete, `PR Merge Verification Pending` stays active until direct GitHub/GitHub-connector verification proves that the PR is `merged`. A watcher may supplement that proof only under a USER-approved watcher exception.
...
`Next Runtime Candidate Selection Pending` keeps PR Readiness non-green only after USER-approved PR-time successor selection exists or selected-next truth already exists and is inconsistent. Otherwise normal next runtime candidate selection waits for Branch Readiness Stage 1.
...
- in `PR Readiness`, require Stage 1 to record `PR Readiness Stage 1 Repair Required` for bounded current-branch PR-readiness drift/blockers and keep `PR Readiness Stage 1 Repair Pending` active until those repairs are validated, committed, and pushed under a USER-approved legal current-branch repair seam. Stage 1 then stops on `PR Readiness Execution User Approval Missing` until USER approval to enter Stage 2 exists, and the Stage 1 packet must include required post-merge path, release-debt impact, selected-next validation status when selected-next truth exists or PR-time selection is explicitly approved, Stage 1 repairs made, Stage 1 repair validation, `Governance Ledger Fallback:`, `Branch Readiness Fallback:`, and Stage 2 sync plan before accepting Stage 2 approval; if selected-next, next-branch, no-release-debt, or governance-ledger blockers cannot be cleared without USER approval or Branch Readiness carrier analysis, classify the route as `Current-Branch Branch Readiness Re-entry Required` or `New Carrier Branch Required` instead of Stage 2. Then require hard blocker checks before accepting `PR READY: YES`: `stale-canon`, `post-merge`, `next-workstream-if-selected`, `dirty`, `docs-sync`, `desktop-shortcut`, `uts-results`, `Backlog Addition User Approval Missing`, `Backlog Exhaustion User Decision Pending`, `Single-Slice Package User Approval Missing`, `Package Completion Unproven`, `Next Runtime Candidate Selection Pending` only when selected-next truth is being validated, `PR Readiness Scope Missed`, `Release Window Audit Incomplete`, `Between-Branch Canon Repair Attempt`, `Next Branch Created Too Early`, `Merge-Target Authority Projection Unproven` until active branch authority is merge-stable, `PR Merge Status Unproven` until the live PR reports a green merge status, `PR Watcher Provisioning Unproven` until the watcher contract is explicit and proven, `PR Watcher Routing Unverified` until the configured watcher route and delivery proof are proven to match the recorded reporting surface, `PR Merge Verification Pending` until that watcher verifies the PR is merged, and `Bot Review Signal Pending` for the live PR until a Codex Connector bot thumbs-up reaction or green approval comment exists for the current head after any bot-comment repair
...
- in `PR Readiness`, require the copy-ready `## PR Creation Details` operator blocks when package-ready or green; require the standardized `## Next Branch` block only when selected-next truth is explicitly in scope or Branch Readiness is the next legal phase
