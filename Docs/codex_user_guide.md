...
Stage 1 must return `## PR Readiness Stage 1 Analysis Packet` with the planned PR title/base/head/summary, planned merge-target canon updates, planned next-branch block, planned watcher provisioning and reporting surface, planned validations, expected file changes, drift findings, blockers, waivers, release-window audit posture, rollback path, and the exact Stage 2 green-light decision needed.
