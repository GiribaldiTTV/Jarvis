...
Main is the first repo loader and source-truth router. For product/design/vision work, follow Main to `Docs/nexus_vision.md`, `Docs/family_visions/`, and the active branch plan instead of treating this guide or a prompt as the final vision owner.
For runtime Workstream Entry with multiple admitted slices or seams, expect whole-package analysis before first-seam implementation approval; a first-seam-only handoff is not enough.
...
Before Workstream implementation begins or resumes, Workstream Entry must return the `USER Branch Plan Review Gate` packet plus the stable Desktop review bundle. That packet is the readable USER branch plan: plain-language branch goal, planned user-facing outcome, visual/behavioral description when relevant, implementation breakdown, Element-to-Phase Proof Matrix summary, Hardening plan, Live Validation / UTS plan, open USER questions, USER design review questions, Codex recommendations, implementation options, alternatives/tradeoffs, USER Review Response, Codex Response Digest, accepted/deferred/rejected scope, exact USER decision needed, and `USER Review Packet Finding:` proving the Desktop packet was loaded, digested, and freshness-checked or explicitly waived. `USER_BRANCH_PLAN_REVIEW.md` is the design/vision review artifact where USER can accept, revise, reject, defer with waiver, answer generally, or request more analysis; Codex must attach or insert USER response and digest it before implementation, unless USER explicitly waives that step. Missing or first-seam-only review blocks on `USER Branch Plan Review Missing`; stale packet metadata blocks on `USER Review Packet Stale`; an omitted packet digest or missing USER response/digest/waiver blocks on `USER Review Packet Not Digested`. USER may accept, revise, defer with waiver, reject, or request more analysis before implementation.

The review file should read like a user-facing product/design planning document and decision ballot. It is the USER Branch Plan Contract. It must show `Contract Status`, `Contract Version / Revision`, `What Will I Actually See, And Where Will I See It?`, `End-State Vision`, `Visual / Functional Walkthrough`, surface map, implementation options with risks, Codex's recommended direction, `Why This Fits The Nexus Vision`, USER design direction decision, current scope, future-gated scope, Implementation Staging Notes, USER decisions, response area, Codex digest area, implementation constraints created by USER response, rejected/deferred ideas, Vision Delta / Source-Truth Impact, Contract Change Log, Contract Completion Checklist, and Workstream Entry result area, so USER can answer whether they actually like what Codex is about to build. `Contract Status` must be `Complete` or `Waived by USER` before implementation approval is legal; `Draft`, `Pending USER Response`, `Pending Codex Digest`, and `Pending USER Confirmation` block implementation. When USER feedback changes the plan, Codex updates source truth and the Desktop packet/ZIP and returns the contract to Pending USER Confirmation until USER confirms or explicitly waives it. The decision ballot is about accepting, changing, or adding to the end-state and recommendation; SLC/slice/seam details are background implementation staging, not the main product decision.
...
For Codex-created PRs, `PR Readiness GREEN` also requires the live PR to clear `Bot Review Signal Pending` through a thumbs-up reaction from the Codex GitHub bot on the current PR head. A bot comment keeps `PR Validation Pending` active until the branch fixes the comment on the same PR, pushes, replies to and resolves the review thread, asks Codex to re-analyze the updated head, and records a fresh current-head thumbs-up reaction; an eye/watch reaction or resolved comment alone does not clear the gate. This is the same-PR Codex bot-review repair loop. Stage 2 final handoff and merge execution cannot be green until current-head thumbs-up closeout is verified.
...
Standard operating procedure from now on is a watcher on an approved Codex reporting surface at minute cadence that reports only when a watched PR status changes. The current working thread is the default surface, but an explicitly recorded dedicated watcher-host thread is allowed when that is the validated user-visible route. That watcher contract must also say what happens next: thumbs-up reaction on the current head means report green for PR-entry validation; one or more actionable bot comments means trigger the bounded same-branch PR comment-repair worker, fix the issue, commit, push, reply, resolve the corresponding review thread, ask Codex to re-analyze, and then wait for a fresh thumbs-up reaction on the updated head.
