...
If USER approval for a new or successor backlog identity is absent, `Backlog Addition User Approval Missing` remains active and Codex must output the still-not-closed backlog list instead of adding selected-next truth.
...
`Backlog Addition User Approval Missing` also keeps PR Readiness non-green when Codex would need to add, split, promote, or select a backlog identity without explicit USER approval; its output is the still-not-closed backlog list.
