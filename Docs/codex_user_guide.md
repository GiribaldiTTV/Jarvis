...
Before Workstream implementation begins or resumes, Workstream Entry must return the `USER Branch Plan Review Gate` packet plus the stable Desktop review bundle. That packet is the readable USER branch plan: plain-language branch goal, planned user-facing outcome, visual/behavioral description when relevant, implementation breakdown, Element-to-Phase Proof Matrix summary, Hardening plan, Live Validation / UTS plan, open USER questions, Codex recommendations, alternatives/tradeoffs, accepted/deferred/rejected scope, and exact USER decision needed. Missing or first-seam-only review blocks on `USER Branch Plan Review Missing`; USER may accept, revise, defer with waiver, reject, or request more analysis before implementation.
