...
- `Current-Head Revalidation Pending` is an active PR2 continuation state, not a terminal `BLOCKED` state, while GitHub and the Codex Connector remain reachable and direct polling can continue. Codex must not end PR2 with a blocked handoff solely because the latest revalidation request has only an eyes reaction or no response; it must keep direct verification active until a terminal PR2 state above is proven.
- A `--require-current-green` helper result whose only failure is a missing current-head Codex Connector approval latch is merge-ready negative proof and `Current-Head Revalidation Pending`; it is not a terminal PR2 blocker by itself while direct verification remains possible. The helper may return nonzero to protect the merge gate, but Codex must interpret that state as continue-polling / continue-revalidation rather than final blocked handoff.
- Response-level stop is governed by the same rule. Codex must not send a final PR2 closeout or "still waiting" handoff while the only remaining condition is an eyes-only/no-response current-head Codex Connector wait and direct verification remains possible. It must either keep polling/repairing in the active turn, reach merge-ready/merged/closed state, hit a real blocker such as GitHub/connector unreachability or context/tool exhaustion, or receive an explicit USER stop/pause decision.
