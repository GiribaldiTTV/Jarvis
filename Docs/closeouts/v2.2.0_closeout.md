...
* the final `Shutting down.` line kept its dedicated shutdown-only branch inside `Audio/nexus_error_voice.py`
...
* the directly supportive normal-voice probe path in the voice regression harness now matches the current `Audio/orin_voice.py` repo layout
...
* the direct normal-voice probe passes against the current `Audio/orin_voice.py` path

---

## Current Guarantees

* `main.py` remains the dev-only Boot Nexus harness
* `launch_nexus_desktop.vbs` remains the normal manual user launch
* the controlled desktop path remains:
  * `desktop/nexus_desktop_launcher.pyw`
  * `desktop/nexus_desktop_main.py`
