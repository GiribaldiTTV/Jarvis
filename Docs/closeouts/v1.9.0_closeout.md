...
* `desktop/nexus_desktop_main.py` and `desktop/nexus_desktop_test.py` moved under `desktop/`
* `desktop/nexus_desktop_launcher.pyw` now targets `desktop/nexus_desktop_main.py` as the default renderer entrypoint
* `launch_nexus_desktop.vbs` remained unchanged
...
* accepted PySide dev toolkit:
  * `dev/launchers/nexus_dev_launcher.pyw`
  * `dev/launchers/launch_nexus_dev_toolkit.vbs`
* dedicated VBS validation and harness entry surfaces:
  * `dev/launchers/launch_orin_voice_regression_harness.vbs`
  * `dev/launchers/launch_nexus_desktop_entrypoint_validation.vbs`
  * `dev/launchers/launch_nexus_desktop_launcher_healthy_validation.vbs`
  * `dev/launchers/launch_nexus_desktop_launcher_regression_harness.vbs`
  * `dev/launchers/launch_nexus_support_bundle_triage_harness.vbs`
  * `dev/launchers/launch_nexus_support_bundle_triage_toolkit_validation.vbs`
  * `dev/launchers/launch_nexus_diagnostics_report_issue_validation.vbs`
...
* `FB-005` first workspace slice verification artifacts:
  ...
  * direct repo-truth surfaces: `desktop/nexus_desktop_main.py`, `desktop/nexus_desktop_test.py`, `desktop/nexus_desktop_launcher.pyw`
...
* the normal startup and desktop ORIN voice path remains intentionally separate from the diagnostics and error voice path
