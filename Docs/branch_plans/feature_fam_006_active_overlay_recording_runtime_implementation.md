...
Workstream / Seam Map: `SLC-051 through SLC-055 are admitted as the planned implementation package. BP3 is approved; SLC-051 Active Overlay recording target foundation is the default first bounded Workstream seam, pending separate USER implementation approval.`
...
Package Posture: `Stage 2 setup active / BP1 accepted / BP2 accepted / BP3 approved / implementation pending separate USER implementation approval`

| Slice | Admission State | Purpose | Current Status |
| --- | --- | --- | --- |
| `SLC-051` Active Overlay recording target foundation | First bounded Workstream seam / pending USER implementation approval | Define active Overlay Profile membership as the recording target while preserving Overlay Profile, Overlay Display, and Monitor Group separation. | Pending separate USER implementation approval on this carrier |
...
Contract Version / Revision: v4 accepted BP3 orchestration review and pending SLC-051 implementation approval.
...
Open USER Questions: USER must now approve, revise, hold, or reject bounded SLC-051 Workstream implementation. Later seams must still answer output file contract, Recording Control, HUD layout, snapshot-at-start execution behavior, and per-overlay effective polling policy questions when they become active.
USER Design Review Questions: Needs USER Implementation Decision - USER must decide whether to approve bounded SLC-051 Workstream implementation with the accepted BP3 guardrails: target/session truth only, actual owner verification, SLC-051 first seam by default, and SLC-052 minimal HUD preview only if inseparable and safe.
...
How Codex Would Build This After USER Accepts The Direction: `After USER separately approves bounded SLC-051 Workstream implementation, Codex should implement only target/session truth proof within the accepted guardrails before proposing later seams.`
...
Codex Response Digest: BP1 and BP2 acceptance digested into BP3 planning constraints, and BP3 approval digested into SLC-051 guardrails: active Overlay Profile membership is the recording target source; snapshot-at-start is the default target model for future execution; the HUD recording card remains small and quick-access; the standalone Recording Control window carries richer target/status/readiness/control detail later; hidden target state is rejected; separate Recording Profile remains outside this branch; Native Log Loader and per-overlay effective polling remain future-gated; SLC-051 must verify actual current owners before implementation. SLC-051 and all runtime mutation remain blocked until separate USER implementation approval.
Implementation Constraints Created By USER Response: This branch must not implement SLC-051 or any runtime/user-facing recording work until separate bounded implementation approval; active Overlay Profile membership remains the future recording target source; no separate Recording Profile system or recording-specific sensor chooser is admitted; recording execution, file writing, real Start/Stop behavior, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, and durable Native Log Loader mutation remain blocked; future recording execution should default to snapshot-at-recording-start for clean logs unless USER revises it; approved SLC-051 scope may prove the live current target because it records nothing; target proof must preserve null/empty/selected/switched/deleted-stale/duplicate-stale-ID/high-volume states and existing Overlay Profile, Overlay Display, Monitor Group, Dashboard, Manage Monitors, and Sensor Command Center behavior; per-overlay effective polling policy remains future-planning/source-truth constraint; `desktop/ui/dashboard_hud_panel.py` is not the current owner unless later repo truth creates it.
...
Workstream Entry Result: BP3 approved - BP3 packet was reviewable and USER Gate State is USER Approved; implementation remains blocked until separate bounded SLC-051 Workstream implementation approval.
Contract Completion Checklist: BP2 accepted and BP3 approved - accepted BP1, accepted BP2, and approved BP3 context are preserved; separate SLC-051 Workstream implementation approval is required before runtime mutation.
...
Deferred Scope: Deferred scope is runtime implementation until separate bounded SLC-051 approval, Workstream implementation execution, recording execution, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, PR creation, merge, release, issue mutation, artifacts/raw evidence handling beyond approved review packet, sibling-worktree mutation, old branch cleanup/deletion, and Governance mutation.
...
Exact USER Decision Needed: USER may approve bounded SLC-051 Workstream implementation, request revision/options, or hold; runtime implementation remains blocked until that separate USER decision.
