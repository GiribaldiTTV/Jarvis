...
Current Plan Phase: `Branch Planning BP3 approved / next legal action is separate bounded Workstream implementation approval after USER accepted the BP1 Dashboard Recording card amendment, accepted the BP2 Dashboard Recording card engineering plan, and approved BP3 Workstream Entry / Orchestration Validation. Prior SLC-051 through SLC-055, H1, and LV1 proof/handoff remain durable superseded-route evidence, and BP3 approval by itself does not authorize code repair.`
Runtime Implementation Approval: `No new runtime implementation approval is active. USER accepted the BP1 Dashboard Recording card amendment, accepted the BP2 Branch Plan revision, and approved BP3 Workstream Entry / Orchestration Validation on 2026-06-02; a separate bounded Workstream implementation approval is required before any code repair, renewed Hardening, renewed Live Validation, or PR Readiness. Recording execution, file writing, real Start/Stop controls, tray controls, export/share, and provider/model work remain future-gated unless source truth admits them.`
...
User-Facing Delta: `BP1 amendment and BP2 engineering plan accepted; BP3 orchestration approved. The revised user-facing direction is a separate Dashboard Recording card for recording target/status and future recording controls; the HUD Overlay card remains overlay-focused. Prior HUD Overlay launcher/preview and standalone Recording Control window foundation are superseded-route evidence until USER separately approves the bounded Workstream implementation repair for the revised engineering route.`
...
Workstream / Seam Map: `Prior SLC-051 through SLC-055 are implemented for the superseded accepted route. BP3 approved the revised Workstream seam map after USER accepted the BP1 Dashboard Recording card amendment and BP2 engineering plan. No direct runtime repair or LV continuation is authorized until USER grants the separate bounded Workstream implementation approval.`
...
Open Questions: `Active question: does USER approve the separate bounded Workstream implementation repair for the first Dashboard Recording card seam recommended by BP3? The implementation approval must preserve accepted BP1-to-BP2-to-BP3 trace, exact owner-file forecast, standalone Recording Control as future/secondary unless source truth revises it, revised proof paths, H1/LV/UTS scope, and implementation seam order. Snapshot-at-recording-start remains the default for future execution unless USER revises it.`
...
Runtime Implementation Approval: `Paused pending separate bounded Workstream implementation approval. SLC-051 target/session truth remains accepted evidence; SLC-052 HUD Overlay recording target preview and SLC-053 standalone Recording Control window foundation are superseded-route evidence after returned USER feedback moved recording placement to a dedicated Dashboard Recording card. BP3 approval is recorded after the accepted BP1 amendment and BP2 plan, but code repair, renewed H1, renewed LV, and PR Readiness require the separate Workstream implementation approval and later phase proof. Recording execution and file-writing authority remain future-gated unless admitted by source truth.`
...
Status: Blocking
Fold-Down Target: Docs/branch_plans/feature_fam_006_active_overlay_recording_runtime_implementation.md owns UFD-FAM006-20260602-001 until the BP3-approved Dashboard Recording card repair is implemented if approved, hardened, live-validated, and digested or explicitly waived.
...
Affected Workstream / H1 / LV: `Yes - previous implementation and proof remain durable evidence for the superseded route, but active repair must wait for separate bounded Workstream implementation approval.`
No-Action Reason: Not Applicable - bounded Workstream implementation approval is required after BP1 amendment acceptance, BP2 plan acceptance, and BP3 approval.
Exact Next USER Decision: `Approve, revise, reject, or hold bounded FAM-006 Workstream implementation repair for the first Dashboard Recording card seam recommended by BP3.`

## Package And Slice Admission

Family: `FAM-006 - Monitoring and HUD`
Package: `PKG-006 - Active Overlay Recording Runtime Foundation`
Package Posture: `BP3 approved / UFD-FAM006-20260602-001 blocking until approved repair, renewed H1, renewed LV, and UTS disposition or waiver / prior Workstream-H1-LV1 proof preserved as superseded-route evidence / BP1 amendment accepted / BP2 plan accepted / Workstream implementation approval pending`
...
| `SLC-052` Recording card target/status placement revision | Workstream implementation approval pending | Prior HUD Overlay card launcher/preview is superseded for active placement decisions; accepted BP2 and approved BP3 remap target/status and future recording controls to a dedicated Dashboard Recording card. | Pending bounded Workstream implementation approval |
| `SLC-053` Recording Control / secondary surface decision | Workstream implementation approval pending | Prior standalone Recording Control window foundation remains durable evidence only; accepted BP2 and approved BP3 keep any standalone or secondary recording detail surface future/secondary after the Dashboard Recording card becomes primary. | Pending bounded Workstream implementation approval |
...
Package Completion State: `Blocked for active advancement by UFD-FAM006-20260602-001 / prior Workstream-H1-LV1 evidence preserved / bounded Workstream implementation approval required before repair after BP1 amendment acceptance, BP2 plan acceptance, and BP3 approval`

## Element-to-Phase Proof Matrix

Matrix Status: `Required - revision needed for Dashboard Recording card direction`
USER Review Status: `Pending USER Decision - bounded Workstream implementation approval requires USER response`
...
| `AOR-004` | Recording Control or secondary recording detail surface | Affected | Prior SLC-053 standalone Recording Control window foundation remains durable evidence only. Accepted BP2 and approved BP3 keep any standalone Recording Control window as a later secondary surface unless later source truth admits it. | Revised Workstream proof must match the USER-accepted BP2 and approved BP3 route and prove no Dashboard child-window or standalone-window dependency is implied unless admitted. | H1 must stress the accepted surface only; standalone lifetime/minimize/taskbar proof applies only if the implementation approval keeps the standalone window. | LV1 proof applies to the accepted visible surface after approved implementation. | UTS required for any visible Recording card or secondary recording detail surface. | Real Start/Stop, file writing, and advanced bulky settings remain future-gated or move behind secondary surfaces only after USER/source-truth admission. | Needs USER Decision | This plan |
...
Review Status: Accepted by USER - BP2 Branch Plan revision is accepted by USER and retained as support context for the approved BP3 and pending Workstream implementation approval.
Contract Status: Complete - BP2 Branch Plan revision is accepted and BP3 is approved; runtime repair still requires separate bounded Workstream implementation approval.
...
USER Response Digested: BP1 acceptance, BP2 acceptance, and BP3 approval digested into the active branch record and branch plan; the next USER response is separate bounded Workstream implementation approval.
Acceptance / Waiver / Revision / Rejection Receipt: BP1 amendment accepted by USER; BP2 revision accepted by USER; BP3 Workstream Entry / Orchestration Validation approved by USER; bounded Workstream implementation approval remains pending USER approval, revision, rejection, or hold.
Contract Version / Revision: v17 records USER approval of BP3 Workstream Entry / Orchestration Validation and prepares the separate bounded Workstream implementation approval decision.
...
Branch Goal: `Revise the active-overlay-driven recording runtime foundation so recording-specific review and future controls live in a dedicated Dashboard Recording card after BP1/BP2 acceptance, BP3 approval, and separate bounded Workstream implementation approval, while preserving trustworthy target proof and future-gated execution boundaries.`
...
Open USER Questions: USER must decide whether to approve the separate bounded Workstream implementation repair for the first Dashboard Recording card seam recommended by BP3, while keeping standalone/secondary recording detail future-only, preserving proof expectations, and confirming the seam order remains right.
USER Design Review Questions: BP1 and BP2 are accepted and BP3 is approved; the active review question is whether USER approves the separate bounded Workstream implementation repair.
...
Implementation Options: Option A - approve the separate bounded Workstream repair for the first Dashboard Recording card seam; Option B - revise the first seam, file ownership, or proof expectations before implementation approval; Option C - hold implementation and keep BP3 approval as planning proof only; Option D - reject the implementation seam and request revised planning. Codex recommends Option A only if runtime execution, file writing, real Start/Stop, tray/export/provider/FAM-007/Governance/PR/merge/release/cleanup remain blocked.
...
Current Branch Scope: Current branch scope is the active-overlay-driven recording foundation plus a required design revision from returned USER Live Validation feedback. Runtime repair is not admitted until USER grants the separate bounded Workstream implementation approval after BP1/BP2 acceptance and BP3 approval.
...
How Codex Would Build This After USER Accepts The Direction: `BP3 validates the accepted implementation route to move recording target/status and future controls into a Dashboard Recording card. If USER approves the separate Workstream implementation request, Codex may implement the first bounded repair seam.`
Implementation Staging Notes: SLC-051 target model proof remains the base. The old HUD launcher/preview and standalone window proof cannot be treated as active placement proof after UFD-FAM006-20260602-001; repair staging must wait for separate bounded Workstream implementation approval.
...
USER Decisions Needed: Approve, revise, reject, or hold bounded Workstream implementation repair for the first Dashboard Recording card seam recommended by BP3. Runtime execution, recording execution, file writing, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, release execution, issue mutation, Governance worktree mutation, and durable Native Log Loader implementation remain pending decisions.
USER Review Response: Needs USER Response - bounded Workstream implementation approval is pending USER approval, revision, rejection, or hold.
Codex Response Digest: USER accepted the BP1 amendment, accepted the BP2 engineering plan, and approved BP3 orchestration; Codex digested those acceptances into source truth and is preparing the separate bounded Workstream implementation approval request.
...
Workstream Entry Result: BP3 is USER Approved for the revised Dashboard Recording card repair route; prior BP3 approval for the old HUD Overlay card route remains durable evidence for the superseded route only.
Contract Completion Checklist: BP1 amendment accepted; BP2 revision accepted; BP3 approved; runtime repair blocked until USER separately approves bounded Workstream implementation.
...
Exact USER Decision Needed: Approve, revise, reject, or hold bounded FAM-006 Workstream implementation repair for the first Dashboard Recording card seam recommended by BP3. Runtime repair remains blocked until that separate approval is granted.
Implementation Approval: No new implementation approval; later runtime mutation remains blocked until USER approves the repair seam.
