...
Current Plan Phase: `Branch Planning - BP3 Workstream Entry / Orchestration Validation after USER accepted the BP1 Dashboard Recording card amendment and BP2 Dashboard Recording card engineering plan. Prior SLC-051 through SLC-055, H1, and LV1 proof/handoff remain durable superseded-route evidence, but active advancement is blocked until BP3 is revised or explicitly waived.`
Runtime Implementation Approval: `No new runtime implementation approval is active. USER accepted the BP1 Dashboard Recording card amendment and BP2 Branch Plan revision on 2026-06-02; BP3 Workstream Entry / Orchestration Validation is required before any code repair, renewed Hardening, renewed Live Validation, or PR Readiness. Recording execution, file writing, real Start/Stop controls, tray controls, export/share, and provider/model work remain future-gated unless source truth admits them.`
Current-Main Reconciliation Status: `Reconciled - governed current-main reconciliation through origin/main@9b64ac1b4faf4d29033e3a8f299a1293eb26f2d7 completed on this branch using a non-rewrite merge path. Incoming Branch Planning BP1/BP2/BP3, C:\Nexus USER, external operational-state placement, helpers, validators, fixtures, and USER review artifact rules are current-main authority. FAM-006 did not mutate FAM-007 source truth. Pre-PR #248 FAM-006 BP1/BP2/BP3 packets and receipts are superseded legacy evidence for active decision purposes because active Branch Planning now requires distinct Packet Reviewability State and USER Gate State proof. Active FAM-006 BP1 Branch Vision and BP2 Branch Plan are accepted by USER; BP3 for the Dashboard Recording card revision is pending USER review; Workstream implementation authority is not active for the revised route.`
...
User-Facing Delta: `BP1 amendment and BP2 engineering plan accepted; BP3 revision pending. The revised user-facing direction is a separate Dashboard Recording card for recording target/status and future recording controls; the HUD Overlay card remains overlay-focused. Prior HUD Overlay launcher/preview and standalone Recording Control window foundation are superseded-route evidence until USER accepts or waives BP3 orchestration for the revised engineering route.`
...
Workstream / Seam Map: `Prior SLC-051 through SLC-055 are implemented for the superseded accepted route. A revised Workstream seam map is pending BP3 after USER accepted the BP1 Dashboard Recording card amendment and BP2 engineering plan. No direct runtime repair or LV continuation is authorized by this UFD entry.`
Per-Seam Implementation Checklist: `Revision required. BP2 must remap SLC-052 from HUD Overlay launcher/target preview to Dashboard Recording card target/status and control placement, decide whether any standalone Recording Control window remains future/secondary, and preserve SLC-051 target-source truth, SLC-054 output contract boundaries, and SLC-055 proof readiness as applicable.`
...
Open Questions: `Active question: does USER accept, revise, waive, reject, or hold BP3 Workstream Entry / Orchestration Validation for the accepted Dashboard Recording card engineering route? BP3 questions include accepted BP1-to-BP2 trace, exact owner-file forecast, whether standalone Recording Control remains future/secondary, revised proof paths, H1/LV/UTS scope, first bounded Workstream repair recommendation, and implementation seam order. Snapshot-at-recording-start remains the default for future execution unless USER revises it.`
USER Planning Decisions: `USER clarified that recording should be active-overlay-driven, avoid separate Recording Profile selection, and preserve per-overlay effective polling policy as future planning. USER later returned Live Validation feedback on 2026-06-02 that recording should live in its own Dashboard card, not inside the HUD Overlay card. USER then accepted the BP1 Branch Vision amendment and BP2 Branch Plan revision for that Dashboard Recording card direction. This supersedes prior HUD Overlay card placement authority for active decisions.`
Plan Revision History: `v1 - Created during Branch Readiness Stage 2 setup after rollback of the profile-loaded Recording Profile Workstream route. v2/v3 - hardened and revised USER Branch Plan Contract into accepted active-overlay product plan. v4 - USER accepted the plan and redirected the released foundation carrier to planning/governance PR Readiness without runtime Workstream execution. Implementation-carrier v1 imports that accepted contract onto this active runtime implementation carrier. v14 - Records UFD-FAM006-20260602-001 from returned Live Validation feedback and routes to BP1/BP2/BP3 revision for dedicated Dashboard Recording card placement. v15 - Records USER acceptance of the BP1 Dashboard Recording card amendment and prepares BP2 Branch Plan Review for the revised engineering route. v16 - Records USER acceptance of the revised BP2 Dashboard Recording card engineering plan and prepares BP3 Workstream Entry / Orchestration Validation.`
Plan-To-Implementation Traceability: `SLC-051 is implemented as target/session truth only in the current HUD / Overlay Profile owners and validators. SLC-052 is implemented in the HUD Overlay card HTML/CSS/JS and FAM-006 surface validator as target preview/launcher transparency. SLC-053 is implemented in the HUD launcher JavaScript, desktop renderer native Recording Control window foundation, and FAM-006 surface validator as compact standalone control-window proof with execution/file-writing blocked. SLC-054 is implemented in desktop/recording_output_contract.py and FAM-006 validators as durable output contract schema/readback proof with execution/file-writing blocked. SLC-055 is implemented in dev/orin_fam006_workstream_readiness.py and FAM-006 validators as Workstream readiness proof that routes H1/LV1/UTS without claiming those later phases complete. Hardening H1 is implemented in dev/orin_fam006_hardening_h1.py and the branch record as non-mutating proof that SLC-051 through SLC-055 match accepted BP1/BP2/BP3 and keep execution/file-writing/UTS boundaries blocked. Live Validation LV1 is implemented through dev/orin_monitoring_hud_human_client_validation.ps1 and dev/orin_monitoring_hud_live_validation.ps1 as proof/handoff only; returned USER UTS results remain pending.`
Plan-To-Implementation Traceability Table: `SLC-051 traces to desktop/monitoring_hud_state.py, nexus_visual/monitoring_hud.js, dev/orin_monitoring_hud_surface_validation.py, and dev/orin_monitoring_hud_internal_sandbox_validation.py. SLC-052 traces to nexus_visual/monitoring_hud.html, nexus_visual/monitoring_hud.css, nexus_visual/monitoring_hud.js, and dev/orin_monitoring_hud_surface_validation.py. SLC-053 traces to desktop/desktop_renderer.py, nexus_visual/monitoring_hud.html, nexus_visual/monitoring_hud.js, and dev/orin_monitoring_hud_surface_validation.py. SLC-054 traces to desktop/recording_output_contract.py, dev/orin_monitoring_hud_surface_validation.py, and dev/orin_monitoring_hud_internal_sandbox_validation.py. SLC-055 traces to dev/orin_fam006_workstream_readiness.py, dev/orin_monitoring_hud_surface_validation.py, and dev/orin_monitoring_hud_internal_sandbox_validation.py. H1 traces to dev/orin_fam006_hardening_h1.py and the Hardening H1 branch record section. Do not fill LV1 or UTS as complete until those phases run or are explicitly waived.`
...
Live Validation Proof Or Waiver Checklist: `Green for LV1 proof/handoff. Human-client shortcut validation passed, live-client self-QA passed, focused per-element screenshots and short video proof were generated, and C:\Nexus USER\User Test Summary.txt was refreshed after LV1 PASS. Returned UTS results remain pending before PR Readiness.`
...
USER Planning Review: `BP1 accepted and BP2 accepted under the post-PR #248 two-axis gate model. BP3 for the revised Dashboard Recording card route remains pending USER review. The imported active-overlay recording contract, revised BP1 packet, accepted BP2 packet, and prior approved BP3 packet are planning context; prior pre-PR #248 BP1/BP2/BP3 packet receipts remain superseded legacy evidence for active decision purposes.`
...
Runtime Implementation Approval: `Paused pending BP3 Branch Planning revision. SLC-051 target/session truth remains accepted evidence; SLC-052 HUD Overlay recording target preview and SLC-053 standalone Recording Control window foundation are superseded-route evidence after returned USER feedback moved recording placement to a dedicated Dashboard Recording card. BP3 revision is required after the accepted BP1 amendment and BP2 plan before any code repair, renewed H1, renewed LV, or PR Readiness. Recording execution and file-writing authority remain future-gated unless admitted by source truth.`
...
Status: Blocking
Fold-Down Target: Docs/branch_plans/feature_fam_006_active_overlay_recording_runtime_implementation.md owns UFD-FAM006-20260602-001 until BP3 revision closes or supersedes it after BP1 amendment and BP2 plan acceptance.
...
Affected BP3: `Yes - Workstream Entry / Orchestration Validation must re-check accepted BP1/BP2 alignment, package size, SLC order, direct proof, and future-gated boundaries before code repair.`
Affected Workstream / H1 / LV: `Yes - previous implementation and proof remain durable evidence for the superseded route, but active repair must wait for BP1/BP2/BP3.`
No-Action Reason: Not Applicable - BP3 revision is required after BP1 amendment and BP2 plan acceptance.
Exact Next USER Decision: `Accept, revise, waive, reject, or hold BP3 Workstream Entry / Orchestration Validation for the accepted Dashboard Recording card engineering route.`

## Package And Slice Admission

Family: `FAM-006 - Monitoring and HUD`
Package: `PKG-006 - Active Overlay Recording Runtime Foundation`
Package Posture: `Branch Planning revision required / UFD-FAM006-20260602-001 blocking / prior Workstream-H1-LV1 proof preserved as superseded-route evidence / BP1 amendment accepted / BP2 plan accepted / BP3 revision pending`
...
| `SLC-052` Recording card target/status placement revision | Revision required | Prior HUD Overlay card launcher/preview is superseded for active placement decisions; accepted BP2 remaps target/status and future recording controls to a dedicated Dashboard Recording card. | Pending BP3 revision |
| `SLC-053` Recording Control / secondary surface decision | Revision required | Prior standalone Recording Control window foundation remains durable evidence only; accepted BP2 keeps any standalone or secondary recording detail surface future/secondary after the Dashboard Recording card becomes primary. | Pending BP3 revision |
...
Package Completion State: `Blocked for active advancement by UFD-FAM006-20260602-001 / prior Workstream-H1-LV1 evidence preserved / revised BP2-BP3 required before repair after BP1 amendment acceptance`

## Element-to-Phase Proof Matrix

Matrix Status: `Required - revision needed for Dashboard Recording card direction`
USER Review Status: `Pending USER Decision - BP2 revision requires USER response`
...
| `AOR-002` | Dashboard Recording card target/status and future controls | Affected | Prior SLC-052 HUD Overlay card launcher/preview is superseded for active placement decisions. BP2 must remap recording target/status and future recording-specific controls to a dedicated Dashboard Recording card while real Start/Stop remains future-gated until recording execution and file writing are admitted. | Revised Workstream proof must include Dashboard Recording card target/status state, future-gated control state, active Overlay Profile target source, active monitor transparency, and no tray/export/provider coupling. | Revised H1 must stress compact/default Dashboard Recording card layout, dashboard regression, and preservation of Overlay card focus. | Revised LV must use real user-level mouse/keyboard input, before/after screenshots, compact/default photos, hover/focus proof, and pessimistic visual review if visible. | UTS must ask USER to verify the Dashboard Recording card placement, active monitor transparency, compact/default visual correctness, and absence of tray/export/provider scope. | Tray recording controls, real Start/Stop, file writing, and recording execution remain future-gated unless admitted later. | Needs USER Decision | This plan |
| `AOR-003` | Active monitored monitors transparency | Affected | SLC-052 must show recording-relevant active monitors in the Dashboard Recording card, while the HUD Overlay card may show overlay membership only for overlay clarity. | Revised Workstream proof includes null/stale filtering from SLC-051 plus visible count/name transparency in the Dashboard Recording card. | H1 must compare Dashboard, HUD Overlay, Manage Monitors, Overlay Profile, and Overlay Display behavior for regressions. | LV1 must capture focused Dashboard Recording card screenshots at default and compact sizes if visible. | UTS required if visible in Dashboard Recording card. | Monitor Group configuration remains owned by Manage Monitors / Sensor Command Center. | Needs USER Decision | This plan |
| `AOR-004` | Recording Control or secondary recording detail surface | Affected | Prior SLC-053 standalone Recording Control window foundation remains durable evidence only. Accepted BP2 keeps any standalone Recording Control window as a later secondary surface unless BP3 or later source truth admits it. | Revised Workstream proof must match the USER-accepted BP2 route and prove no Dashboard child-window or standalone-window dependency is implied unless admitted. | H1 must stress the accepted surface only; standalone lifetime/minimize/taskbar proof applies only if the revised plan keeps the standalone window. | LV1 proof applies to the accepted visible surface after BP3 revision. | UTS required for any visible Recording card or secondary recording detail surface. | Real Start/Stop, file writing, and advanced bulky settings remain future-gated or move behind secondary surfaces only after USER/source-truth admission. | Needs USER Decision | This plan |
...
USER Branch Plan Review: Accepted BP2 context - USER accepted the BP1 Dashboard Recording card amendment and the revised BP2 engineering plan; active USER review has moved to BP3 Workstream Entry / Orchestration Validation.
Review Status: Accepted by USER - BP2 Branch Plan revision is accepted by USER and retained as support context for BP3.
Contract Status: Complete - BP2 Branch Plan revision is accepted; BP3 must still be revised or explicitly waived before runtime repair.
...
USER Response Digested: BP1 and BP2 acceptance digested into the active branch record and branch plan; local USER packet refresh is required for BP3 Workstream Entry / Orchestration Validation.
Acceptance / Waiver / Revision / Rejection Receipt: BP1 amendment accepted by USER; BP2 revision accepted by USER; BP3 revision remains pending USER acceptance, revision, waiver, rejection, or hold.
Contract Version / Revision: v16 records USER acceptance of the revised BP2 Dashboard Recording card engineering plan and prepares BP3 Workstream Entry / Orchestration Validation.
...
USER Review Packet Finding: Required for BP3 review - START_HERE.md, USER_BRANCH_PLAN_REVIEW.md, WORKSTREAM_ENTRY_ANALYSIS_DIGEST.md, accepted BP1/BP2 Review Aid context, the exported zip, Source HEAD/current branch HEAD freshness, packet reviewability state, USER gate state, and loaded/digested blocker status must be proved by Codex digest/helper output while USER-facing files stay decision-focused.
...
Branch Goal: `Revise the active-overlay-driven recording runtime foundation so recording-specific review and future controls live in a dedicated Dashboard Recording card after BP1/BP2/BP3 approval, while preserving trustworthy target proof and future-gated execution boundaries.`
...
Planned User-Facing Outcome: SLC-051 target truth remains useful; revised future user-facing outcome is a Dashboard Recording card with transparent active target information and future-gated controls after BP3 revision and later implementation approval.
...
Live Validation / UTS Plan: Revised LV1 must use real user-level input, visible cursor movement/clicks, compact/default screenshots, focused per-element screenshots, output-file proof where applicable, and UTS handoff or explicit waiver after the repaired route is implemented.
Open USER Questions: USER must decide whether BP3 proves the accepted engineering plan correctly traces to the accepted Dashboard Recording card vision, whether the proposed first Workstream repair recommendation is bounded enough, whether standalone/secondary recording detail remains future-only, whether proof expectations are sufficient, and whether the seam order is right.
USER Design Review Questions: BP1 and BP2 are accepted; BP3 should not reopen the product direction or engineering plan unless USER requests revision. The active review question is whether orchestration is ready to request a later separate Workstream implementation approval.
...
Implementation Options: Option A - accept BP3 orchestration and prepare a separate bounded Workstream repair approval; Option B - revise BP3 file ownership, first seam, or proof expectations before implementation approval; Option C - waive BP3 only with explicit USER risk acceptance; Option D - hold or reject the revised branch direction. Codex recommends Option A only if BP3 keeps runtime implementation blocked until the separate implementation decision.
...
Current Branch Scope: Current branch scope is the active-overlay-driven recording foundation plus a required design revision from returned USER Live Validation feedback. Runtime repair is not admitted until BP1/BP2/BP3 are revised or waived.
...
How Codex Would Build This After USER Accepts The Direction: `BP3 now validates the accepted implementation route to move recording target/status and future controls into a Dashboard Recording card. If USER accepts or waives BP3, Codex may request a separate Workstream implementation approval for the first bounded repair seam.`
Implementation Staging Notes: SLC-051 target model proof remains the base. The old HUD launcher/preview and standalone window proof cannot be treated as active placement proof after UFD-FAM006-20260602-001; repair staging must wait for BP3 acceptance or waiver and separate implementation approval.
...
USER Decisions Needed: Accept, revise, waive, reject, or hold BP3 Workstream Entry / Orchestration Validation for the accepted Dashboard Recording card engineering route. Runtime execution, recording execution, file writing, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, release execution, issue mutation, Governance worktree mutation, and durable Native Log Loader implementation remain pending decisions.
USER Review Response: Needs USER Response - BP3 Workstream Entry / Orchestration Validation is pending USER acceptance, revision, waiver, rejection, or hold.
Codex Response Digest: USER accepted the BP1 amendment and BP2 engineering plan; Codex digested those acceptances into source truth and is refreshing the USER packet for BP3 Workstream Entry / Orchestration Validation.
...
Exact USER Decision Needed: Accept, revise, waive, reject, or hold BP3 Workstream Entry / Orchestration Validation for the accepted FAM-006 Dashboard Recording card engineering route. Runtime repair remains blocked.
Implementation Approval: No new implementation approval; later runtime mutation remains blocked until BP1/BP2/BP3 are revised or waived and USER approves the repair seam.
...
H1 Admission: `PASS - USER explicitly admitted Hardening H1 for the FAM-006 Active Overlay Recording Runtime Implementation Workstream package.`
H1 Result: `Green - SLC-051 through SLC-055 implementation matches accepted BP1/BP2/BP3.`
Accepted BP Trace: `PASS - BP1 Branch Vision, BP2 Branch Plan, and BP3 Workstream Entry / Orchestration Validation are accepted or approved.`
...
SLC-052 Pressure-Test: `PASS - HUD Overlay launcher and target preview markers are present and target/status preview stays separated from execution authority.`
SLC-053 Pressure-Test: `PASS - standalone Recording Control window foundation markers are present and Start/Stop remains future-gated.`
...
Next Legal Phase: `Branch Planning - BP3 Workstream Entry / Orchestration Validation`
Exact USER Decision Needed: `Accept, revise, waive, reject, or hold the revised FAM-006 BP3 Workstream Entry / Orchestration Validation that maps the accepted Dashboard Recording card engineering route into orchestration readiness. Workstream repair, Hardening, Live Validation, PR creation, merge, release, issue mutation, cleanup, FAM-007 mutation, Governance mutation, recording execution, file writing, real Start/Stop controls, tray controls, export/share, Native Log Loader implementation, and provider/model work remain excluded.`
