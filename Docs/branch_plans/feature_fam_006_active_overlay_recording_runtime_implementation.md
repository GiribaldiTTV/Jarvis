...
Current Plan Phase: `Branch Planning revision required - returned USER Live Validation feedback changes the accepted recording-card placement from HUD Overlay card to a dedicated Dashboard Recording card. Prior SLC-051 through SLC-055, H1, and LV1 proof/handoff remain durable superseded-route evidence, but active advancement is blocked until BP1/BP2/BP3 are revised or explicitly waived.`
Runtime Implementation Approval: `No new runtime implementation approval is active. USER feedback on 2026-06-02 requires BP1 Branch Vision amendment, BP2 Branch Plan revision, and BP3 Workstream Entry / Orchestration Validation before any code repair, renewed Hardening, renewed Live Validation, or PR Readiness. Recording execution, file writing, real Start/Stop controls, tray controls, export/share, and provider/model work remain future-gated unless source truth admits them.`
...
Engineering Plan Status: `Revision required`
...
User-Facing Delta: `Pending BP1/BP2/BP3 revision. The revised user-facing direction is a separate Dashboard Recording card for recording target/status and future recording controls; the HUD Overlay card remains overlay-focused. Prior HUD Overlay launcher/preview and standalone Recording Control window foundation are superseded-route evidence until USER accepts or waives a revised plan.`
...
Workstream / Seam Map: `Prior SLC-051 through SLC-055 are implemented for the superseded accepted route. A revised Workstream seam map is pending BP1/BP2/BP3 after USER feedback moves recording to a Dashboard Recording card. No direct runtime repair or LV continuation is authorized by this UFD entry.`
...
Open Questions: `Active question: does USER accept the BP1 Branch Vision amendment that moves recording into a dedicated Dashboard Recording card while keeping the HUD Overlay card overlay-focused? Follow-up BP2 questions include exact Recording card fields, whether standalone Recording Control remains future/secondary, revised proof paths, H1/LV/UTS scope, and implementation seam order. Snapshot-at-recording-start remains the default for future execution unless USER revises it.`
USER Planning Decisions: `USER clarified that recording should be active-overlay-driven, avoid separate Recording Profile selection, and preserve per-overlay effective polling policy as future planning. USER later returned Live Validation feedback on 2026-06-02 that recording should live in its own Dashboard card, not inside the HUD Overlay card. This supersedes prior HUD Overlay card placement authority for active decisions.`
Plan Revision History: `v1 - Created during Branch Readiness Stage 2 setup after rollback of the profile-loaded Recording Profile Workstream route. v2/v3 - hardened and revised USER Branch Plan Contract into accepted active-overlay product plan. v4 - USER accepted the plan and redirected the released foundation carrier to planning/governance PR Readiness without runtime Workstream execution. Implementation-carrier v1 imports that accepted contract onto this active runtime implementation carrier. v14 - Records UFD-FAM006-20260602-001 from returned Live Validation feedback and routes to BP1/BP2/BP3 revision for dedicated Dashboard Recording card placement.`
...
USER Planning Review: `BP1 accepted, BP2 accepted, and BP3 USER Approved under the post-PR #248 two-axis gate model. The imported active-overlay recording contract, revised BP1 packet, accepted BP2 packet, and approved BP3 packet are accepted planning context; prior pre-PR #248 BP1/BP2/BP3 packet receipts remain superseded legacy evidence for active decision purposes.`
...
Runtime Implementation Approval: `Paused pending Branch Planning revision. SLC-051 target/session truth remains accepted evidence; SLC-052 HUD Overlay recording target preview and SLC-053 standalone Recording Control window foundation are superseded-route evidence after returned USER feedback moved recording placement to a dedicated Dashboard Recording card. BP1/BP2/BP3 revision is required before any code repair, renewed H1, renewed LV, or PR Readiness. Recording execution and file-writing authority remain future-gated unless admitted by source truth.`
...
Status: Blocking
Fold-Down Target: Docs/branch_plans/feature_fam_006_active_overlay_recording_runtime_implementation.md owns UFD-FAM006-20260602-001 until BP1/BP2/BP3 revision closes or supersedes it.
...
Exact Next USER Decision: `Accept, revise, waive, reject, or hold the BP1 Branch Vision amendment for a dedicated Dashboard Recording card.`

## Package And Slice Admission

Family: `FAM-006 - Monitoring and HUD`
Package: `PKG-006 - Active Overlay Recording Runtime Foundation`
Package Posture: `Branch Planning revision required / UFD-FAM006-20260602-001 blocking / prior Workstream-H1-LV1 proof preserved as superseded-route evidence / BP1 amendment pending / BP2 revision pending / BP3 revision pending`
...
| `SLC-052` Recording card target/status placement revision | Revision required | Prior HUD Overlay card launcher/preview is superseded for active placement decisions; BP2 must remap target/status and future recording controls to a dedicated Dashboard Recording card. | Pending BP1/BP2/BP3 revision |
| `SLC-053` Recording Control / secondary surface decision | Revision required | Prior standalone Recording Control window foundation remains durable evidence only; BP2 must decide whether a standalone or secondary recording detail surface remains future/secondary after the Dashboard Recording card becomes primary. | Pending BP1/BP2/BP3 revision |
...
Package Completion State: `Blocked for active advancement by UFD-FAM006-20260602-001 / prior Workstream-H1-LV1 evidence preserved / revised BP1-BP2-BP3 required before repair`

## Element-to-Phase Proof Matrix

Matrix Status: `Required - revision needed for Dashboard Recording card direction`
USER Review Status: `Pending USER Decision - BP1 amendment requires USER response`
Open Element Questions: `Blocking - Dashboard Recording card placement and repair route queued`
...
| `AOR-004` | Recording Control or secondary recording detail surface | Affected | Prior SLC-053 standalone Recording Control window foundation remains durable evidence only. BP2 must decide whether a standalone Recording Control window remains a later secondary surface or whether the Dashboard Recording card becomes the only immediate recording-control surface. | Revised Workstream proof must match the USER-accepted BP2 route and prove no Dashboard child-window or standalone-window dependency is implied unless admitted. | H1 must stress the accepted surface only; standalone lifetime/minimize/taskbar proof applies only if the revised plan keeps the standalone window. | LV1 proof applies to the accepted visible surface after BP2/BP3 revision. | UTS required for any visible Recording card or secondary recording detail surface. | Real Start/Stop, file writing, and advanced bulky settings remain future-gated or move behind secondary surfaces only after USER/source-truth admission. | Needs USER Decision | This plan |
...
USER Branch Plan Review: Revision required - returned USER Live Validation feedback changes the branch vision and branch plan route.
Review Status: Needs USER Decision - BP1 amendment is pending before BP2/BP3 revision.
Contract Status: Pending USER Response - BP1 Branch Vision amendment is pending, and BP2/BP3 must be revised or explicitly waived before runtime repair.
...
USER Response Proof: Returned USER Live Validation feedback recorded as UFD-FAM006-20260602-001.
USER Response Digested: Feedback digested into family vision, active branch record, and active branch plan; local USER packet refresh is required for BP1 amendment review.
Acceptance / Waiver / Revision / Rejection Receipt: Prior BP1/BP2/BP3 route accepted/approved for old placement; active Dashboard Recording card placement is pending USER decision.
Contract Version / Revision: v14 records returned USER Live Validation design change and routes to BP1/BP2/BP3 revision before runtime repair.
...
USER Review Packet Finding: Required for BP1 amendment - START_HERE.md, USER_BRANCH_PLAN_REVIEW.md, the timestamped exported zip, Source HEAD/current branch HEAD freshness, and loaded/digested blocker status must be proved by Codex digest/helper output while USER-facing files stay decision-focused.
...
Planned User-Facing Outcome: SLC-051 target truth remains useful; revised future user-facing outcome is a Dashboard Recording card with transparent active target information and future-gated controls after BP1/BP2/BP3 revision.
...
Open USER Questions: USER must decide whether to accept the Dashboard Recording card BP1 amendment, revise it, waive it, reject it, or hold. BP2 must later answer exact Recording card fields, standalone/secondary surface posture, proof expectations, and seam order.
USER Design Review Questions: Does USER accept the revised branch vision where recording lives in its own Dashboard Recording card and the HUD Overlay card remains overlay-focused?
...
Implementation Options: Option A - accept the Dashboard Recording card as the primary recording surface; Option B - revise the card fields before BP2; Option C - keep standalone Recording Control as a future secondary surface only after BP2/BP3; Option D - hold or reject the revised branch direction. Codex recommends Option A with BP2 detail review before code repair.
...
Recommended Direction: Codex recommends accepting the BP1 amendment for a dedicated Dashboard Recording card, then revising BP2/BP3 before runtime repair. SLC-051 target model proof and SLC-054 output contract proof remain useful; SLC-052/SLC-053 placement must be repaired only after approval.
...
USER Design Direction Decision: Pending BP1 amendment decision after returned Live Validation feedback. Active Overlay Profile membership remains the recording target source; snapshot-at-start remains the default target model; Dashboard Recording card becomes the recommended primary recording surface; hidden target state and a separate Recording Profile system remain rejected unless USER later reopens them.
...
How Codex Would Build This After USER Accepts The Direction: `BP2 would revise the implementation route to move recording target/status and future controls into a Dashboard Recording card, then BP3 would validate the revised route before any code repair.`
Implementation Staging Notes: SLC-051 target model proof remains the base. The old HUD launcher/preview and standalone window proof cannot be treated as active placement proof after UFD-FAM006-20260602-001; repair staging must wait for BP2/BP3.
...
USER Decisions Needed: Accept, revise, waive, reject, or hold the BP1 Branch Vision amendment for a dedicated Dashboard Recording card. Runtime execution, recording execution, file writing, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, release execution, issue mutation, Governance worktree mutation, and durable Native Log Loader implementation remain pending decisions.
USER Review Response: Needs USER Response - BP1 amendment is pending USER acceptance, revision, waiver, rejection, or hold.
Codex Response Digest: USER response remains pending; Codex digested UFD-FAM006-20260602-001 into source truth and refreshed the USER packet for BP1 amendment review.
...
Vision Delta / Source-Truth Impact: Active branch plan, branch record, family vision, and local USER hub packet must preserve the active-overlay recording branch identity while recording the Dashboard Recording card placement revision as the active design direction pending USER decision. Family vision records per-overlay effective polling policy as a future FAM-006 planning constraint because it affects recording target model design. Native Log Loader remains future planning input only and is not admitted for durable implementation.
...
Contract Completion Checklist: BP1 amendment pending; BP2/BP3 revision pending; runtime repair blocked until USER accepts or waives the legal planning route.
...
Exact USER Decision Needed: Accept, revise, waive, reject, or hold the BP1 Branch Vision amendment for the FAM-006 Dashboard Recording card revision. Runtime repair remains blocked.
...
Next Legal Phase: `Branch Planning - BP1 Branch Vision Amendment`
Exact USER Decision Needed: `Accept, revise, waive, reject, or hold the FAM-006 BP1 Branch Vision amendment that moves recording target/status and future recording controls to a dedicated Dashboard Recording card while keeping the HUD Overlay card overlay-focused. BP2, BP3, Workstream repair, Hardening, Live Validation, PR creation, merge, release, issue mutation, cleanup, FAM-007 mutation, Governance mutation, recording execution, file writing, real Start/Stop controls, tray controls, export/share, Native Log Loader implementation, and provider/model work remain excluded.`
