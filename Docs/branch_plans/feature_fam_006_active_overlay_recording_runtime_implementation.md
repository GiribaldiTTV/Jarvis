...
Current Plan Phase: `Branch Planning - BP2 USER Branch Plan Review after USER accepted the BP1 Dashboard Recording card amendment. Prior SLC-051 through SLC-055, H1, and LV1 proof/handoff remain durable superseded-route evidence, but active advancement is blocked until BP2/BP3 are revised or explicitly waived.`
Runtime Implementation Approval: `No new runtime implementation approval is active. USER accepted the BP1 Dashboard Recording card amendment on 2026-06-02; BP2 Branch Plan revision and BP3 Workstream Entry / Orchestration Validation are required before any code repair, renewed Hardening, renewed Live Validation, or PR Readiness. Recording execution, file writing, real Start/Stop controls, tray controls, export/share, and provider/model work remain future-gated unless source truth admits them.`
...
Engineering Plan Status: `Revised`
...
User-Facing Delta: `BP1 amendment accepted; BP2/BP3 revision pending. The revised user-facing direction is a separate Dashboard Recording card for recording target/status and future recording controls; the HUD Overlay card remains overlay-focused. Prior HUD Overlay launcher/preview and standalone Recording Control window foundation are superseded-route evidence until USER accepts or waives the revised engineering plan.`
...
Workstream / Seam Map: `Prior SLC-051 through SLC-055 are implemented for the superseded accepted route. A revised Workstream seam map is pending BP2/BP3 after USER accepted the BP1 Dashboard Recording card amendment. No direct runtime repair or LV continuation is authorized by this UFD entry.`
...
Open Questions: `Active question: does USER accept, revise, waive, reject, or hold the BP2 Branch Plan revision that maps the accepted Dashboard Recording card vision to engineering work? BP2 questions include exact Recording card fields, whether standalone Recording Control remains future/secondary, revised proof paths, H1/LV/UTS scope, and implementation seam order. Snapshot-at-recording-start remains the default for future execution unless USER revises it.`
USER Planning Decisions: `USER clarified that recording should be active-overlay-driven, avoid separate Recording Profile selection, and preserve per-overlay effective polling policy as future planning. USER later returned Live Validation feedback on 2026-06-02 that recording should live in its own Dashboard card, not inside the HUD Overlay card. USER then accepted the BP1 Branch Vision amendment for that Dashboard Recording card direction. This supersedes prior HUD Overlay card placement authority for active decisions.`
Plan Revision History: `v1 - Created during Branch Readiness Stage 2 setup after rollback of the profile-loaded Recording Profile Workstream route. v2/v3 - hardened and revised USER Branch Plan Contract into accepted active-overlay product plan. v4 - USER accepted the plan and redirected the released foundation carrier to planning/governance PR Readiness without runtime Workstream execution. Implementation-carrier v1 imports that accepted contract onto this active runtime implementation carrier. v14 - Records UFD-FAM006-20260602-001 from returned Live Validation feedback and routes to BP1/BP2/BP3 revision for dedicated Dashboard Recording card placement. v15 - Records USER acceptance of the BP1 Dashboard Recording card amendment and prepares BP2 Branch Plan Review for the revised engineering route.`
...
Runtime Implementation Approval: `Paused pending Branch Planning revision. SLC-051 target/session truth remains accepted evidence; SLC-052 HUD Overlay recording target preview and SLC-053 standalone Recording Control window foundation are superseded-route evidence after returned USER feedback moved recording placement to a dedicated Dashboard Recording card. BP2/BP3 revision is required after the accepted BP1 amendment before any code repair, renewed H1, renewed LV, or PR Readiness. Recording execution and file-writing authority remain future-gated unless admitted by source truth.`
...
Status: Blocking
Fold-Down Target: Docs/branch_plans/feature_fam_006_active_overlay_recording_runtime_implementation.md owns UFD-FAM006-20260602-001 until BP2/BP3 revision closes or supersedes it after BP1 amendment acceptance.
...
Exact Next USER Decision: `Accept, revise, waive, reject, or hold the BP2 Branch Plan revision for the accepted Dashboard Recording card vision.`

## Package And Slice Admission

Family: `FAM-006 - Monitoring and HUD`
Package: `PKG-006 - Active Overlay Recording Runtime Foundation`
Package Posture: `Branch Planning revision required / UFD-FAM006-20260602-001 blocking / prior Workstream-H1-LV1 proof preserved as superseded-route evidence / BP1 amendment accepted / BP2 revision pending USER review / BP3 revision pending`
...
| `SLC-052` Recording card target/status placement revision | Revision required | Prior HUD Overlay card launcher/preview is superseded for active placement decisions; BP2 must remap target/status and future recording controls to a dedicated Dashboard Recording card. | Pending BP2/BP3 revision |
| `SLC-053` Recording Control / secondary surface decision | Revision required | Prior standalone Recording Control window foundation remains durable evidence only; BP2 must decide whether a standalone or secondary recording detail surface remains future/secondary after the Dashboard Recording card becomes primary. | Pending BP2/BP3 revision |
...
USER Branch Plan Review: Active BP2 review - USER accepted the BP1 Dashboard Recording card amendment, and BP2 must now map that vision into the revised engineering route.
Review Status: Needs USER Decision - BP2 Branch Plan revision is prepared for USER review.
Contract Status: Pending USER Response - BP2 Branch Plan revision is pending, and BP3 must be revised or explicitly waived before runtime repair.
...
USER Response Proof: USER accepted the BP1 Branch Vision amendment after returned Live Validation feedback recorded as UFD-FAM006-20260602-001.
USER Response Digested: BP1 acceptance digested into the active branch record and branch plan; local USER packet refresh is required for BP2 Branch Plan Review.
Acceptance / Waiver / Revision / Rejection Receipt: BP1 amendment accepted by USER; BP2 revision remains pending USER acceptance, revision, waiver, rejection, or hold.
Contract Version / Revision: v15 records USER acceptance of the BP1 Dashboard Recording card amendment and prepares BP2 Branch Plan Review for the revised engineering route.
...
USER Review Packet Finding: Required for BP2 review - START_HERE.md, USER_BRANCH_PLAN_REVIEW.md, the exported zip, Source HEAD/current branch HEAD freshness, packet reviewability state, USER gate state, and loaded/digested blocker status must be proved by Codex digest/helper output while USER-facing files stay decision-focused.
...
Planned User-Facing Outcome: SLC-051 target truth remains useful; revised future user-facing outcome is a Dashboard Recording card with transparent active target information and future-gated controls after BP2/BP3 revision.
...
Open USER Questions: USER must decide whether the BP2 engineering plan correctly builds the accepted Dashboard Recording card vision, whether the proposed card fields are enough, whether standalone/secondary recording detail remains future-only, whether proof expectations are sufficient, and whether the seam order is right.
USER Design Review Questions: BP1 is accepted; BP2 should not reopen the product direction unless USER requests revision. The active review question is whether the engineering plan correctly builds the dedicated Dashboard Recording card route.
...
How Codex Would Build This After USER Accepts The Direction: `BP2 now revises the implementation route to move recording target/status and future controls into a Dashboard Recording card. If USER accepts or waives BP2, BP3 validates the revised route before any code repair.`
Implementation Staging Notes: SLC-051 target model proof remains the base. The old HUD launcher/preview and standalone window proof cannot be treated as active placement proof after UFD-FAM006-20260602-001; repair staging must wait for BP2 acceptance or waiver and BP3 approval or waiver.
...
USER Decisions Needed: Accept, revise, waive, reject, or hold the BP2 Branch Plan revision for the accepted Dashboard Recording card vision. Runtime execution, recording execution, file writing, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, release execution, issue mutation, Governance worktree mutation, and durable Native Log Loader implementation remain pending decisions.
USER Review Response: Needs USER Response - BP2 Branch Plan revision is pending USER acceptance, revision, waiver, rejection, or hold.
Codex Response Digest: USER accepted the BP1 amendment; Codex digested that acceptance into source truth and refreshed the USER packet for BP2 Branch Plan Review.
...
Contract Completion Checklist: BP1 amendment accepted; BP2 revision pending USER review; BP3 revision pending; runtime repair blocked until USER accepts or waives the legal planning route.
...
Exact USER Decision Needed: Accept, revise, waive, reject, or hold the BP2 Branch Plan revision for the accepted FAM-006 Dashboard Recording card vision. Runtime repair remains blocked.
...
Next Legal Phase: `Branch Planning - BP2 USER Branch Plan Review`
Exact USER Decision Needed: `Accept, revise, waive, reject, or hold the revised FAM-006 BP2 Branch Plan that maps the accepted Dashboard Recording card vision into engineering work. BP3, Workstream repair, Hardening, Live Validation, PR creation, merge, release, issue mutation, cleanup, FAM-007 mutation, Governance mutation, recording execution, file writing, real Start/Stop controls, tray controls, export/share, Native Log Loader implementation, and provider/model work remain excluded.`
