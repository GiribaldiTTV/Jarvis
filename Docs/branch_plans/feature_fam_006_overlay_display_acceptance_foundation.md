...
Live Validation Proof Or Waiver Checklist: `LV1 uses real user-facing launcher proof where feasible, OneDrive focused screenshots, short video/frame-sequence proof, visual adjudication, and USER_TEST_REQUIRED until returned review. Current returned-UTS repair requires explicit live workflow proof for create -> dirty -> close guard -> save/discard, selector direct-load editing, empty/default-safe creation, compact/default action-row reachability, Compact Overlay Profiles delete confirmation reachability, child-window click isolation, null data, and high-volume dropdown/list states. Active LV1 route passes the no-synthetic-interaction preflight and real-input fallback policy gate, but USER witnessed a compact Overlay Profiles visual bug; validator PASS is not sufficient until Codex pessimistically inspects the focused screenshots/video, records any visual concern, repairs or obtains a waiver, and reruns proof.`
