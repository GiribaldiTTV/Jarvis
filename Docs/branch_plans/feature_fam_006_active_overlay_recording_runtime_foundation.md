...
Planned Runtime Delta: `None during Stage 2. Workstream Entry will analyze future runtime deltas for active-overlay recording target, HUD Overlay launcher/target preview, standalone Recording Control window, secondary settings surfaces, durable output contract, and validation/live proof.`
...
| `SLC-052` HUD Overlay recording launcher and target transparency | Admitted | Plan HUD Overlay card launcher, active target/status preview, and active monitored monitor transparency without real Start/Stop, tray/export/provider scope, or recording execution. | Pending Workstream Entry |
| `SLC-053` standalone Recording Control window foundation | Admitted | Plan compact normal OS-level Recording Control window behavior, independent lifetime, minimization/taskbar restore, target/status summary, and secondary settings-window routing without admitting recording execution. | Pending Workstream Entry |
...
Review Status: Revised by USER - USER accepted the active-overlay-driven recording product direction, then clarified that this contract must revise the end-state around HUD Overlay launcher/target preview, a standalone Recording Control window, Native Log Loader separation, and future per-overlay effective polling policy.
Contract Status: Pending USER Confirmation - Codex revised this contract to v3 after USER vision feedback; USER must confirm the revised contract or explicitly waive it before SLC-051 implementation approval is legal.
Contract Version / Revision: v3 - USER recording product-model revision: HUD Overlay card launcher/target preview, standalone Recording Control window, Native Log Loader separation, target-model-first SLC-051, and future per-overlay effective polling policy constraint.
...
USER Decisions Needed: USER should confirm or revise the v3 product contract: HUD Overlay card as launcher/target preview, standalone Recording Control window as compact control surface, Native Log Loader as future separate viewer, no separate Recording Profile system, SLC-051 target-model-first proof, and per-overlay effective polling policy recorded as future architecture rather than SLC-051 implementation.
USER Review Response: Revised by USER - USER accepted the active-overlay-driven direction and then refined the product model: recording should be intuitive and automatically connected to the loaded active Overlay Profile; no separate Recording Profile or recording-specific sensor chooser should exist; the HUD Overlay card should launch and preview recording target/status; the standalone Recording Control window should be small, normal OS-level, taskbar-restorable, minimizable, movable, and separate from Dashboard child panels; advanced settings should open secondary windows; Native Log Loader is a future separate graph/log viewer; same Monitor Groups should eventually support different effective polling intervals per Overlay Profile without duplicate FAST/SLOW groups; SLC-051 should prove target model first and avoid recording execution.
Codex Response Digest: Digested - the v3 contract changes the end-state from a generic Recording Settings window model to a sharper product model: HUD Overlay card is launcher/target preview, standalone Recording Control window is the compact control surface, Native Log Loader is separate future graph/log viewing, recording target derives only from active Overlay Profile membership, and per-overlay effective polling policy is a future FAM-006 architecture constraint. Current runtime surfaces expose Monitor Groups with pollingRateMs/source overrides and Overlay Profiles with active membership, but current source truth does not admit per-overlay effective polling policy as an implemented runtime feature. Therefore SLC-051 must not implement polling-policy customization; it must avoid blocking future effective policy metadata and remain state/proof-only.
Implementation Constraints Created By USER Response: SLC-051 selects Option A Target model proof first only after USER confirms v3; SLC-051 remains state/proof-only; active Overlay Profile membership is the only recording target source; no separate Recording Profile system or recording-specific sensor chooser is admitted; recording execution, file writing, real Start/Stop behavior, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, old branch cleanup/deletion, and durable Native Log Loader mutation remain blocked; future recording execution should default to snapshot-at-recording-start for clean logs unless USER revises it; SLC-051 may prove the live current target because it records nothing; target proof must preserve null/empty/selected/switched/deleted-stale/duplicate-stale-ID/high-volume states and existing Overlay Profile, Overlay Display, Monitor Group, Dashboard, Manage Monitors, and Sensor Command Center behavior; per-overlay effective polling policy must remain future-planning/source-truth constraint and SLC-051 must not design against it.
...
Vision Delta / Source-Truth Impact: Active branch plan, branch record, family vision, Desktop review packet, and ZIP must record this v3 contract and Pending USER Confirmation status. Family vision may record per-overlay effective polling policy as a future FAM-006 planning constraint because it affects recording target model design. Backlog/roadmap compact pointers do not need expanded live-state detail in this gate. Native Log Loader remains future planning input only and is not admitted for durable implementation. Workstream seam order remains target-model-first as implementation staging rather than the USER decision surface.
Contract Change Log: v1 introduced USER-facing Branch Plan Review packet with end-state/options sections. v2 hardened it into USER Branch Plan Contract with closed-loop USER response/digest, implementation constraints, source-truth impact, confirmation loop, stale-packet protection, and waiver semantics. v3 digests USER recording product-model feedback: HUD Overlay card launcher/target preview, standalone Recording Control window, Native Log Loader separation, future per-overlay effective polling policy, and target-model-first SLC-051.
Workstream Entry Result: Pending USER Confirmation - first seam remains expected to be SLC-051 Active Overlay recording target foundation, but implementation approval text is not legal until USER confirms this v3 USER Branch Plan Contract or explicitly waives it. Expected SLC-051 files remain limited to active-overlay target state/proof helpers, HUD Overlay card read-only target proof only if repo truth selects it, FAM-006 validators/fixtures, and directly supporting branch/source-truth records after confirmation; no recording execution, file writing, or real Start/Stop behavior is admitted.
Contract Completion Checklist: Pending USER Confirmation - contract is not Complete or Waived by USER yet; USER response is recorded; Codex digest is present; implementation constraints are present; source-truth impact is recorded; rejected/deferred ideas are recorded; contract change log is current; packet metadata must be refreshed to final HEAD and ZIP source HEAD; implementation approval text must cite Complete or Waived by USER status after USER confirms or waives v3.
...
Exact USER Decision Needed: USER may confirm the revised v3 USER Branch Plan Contract, request another revision, or explicitly waive the contract gate; the next runtime seam remains blocked while Contract Status is Pending USER Confirmation.
