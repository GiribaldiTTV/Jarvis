...
`Review Status:` must use `Accepted by USER`, `Revised by USER`, `Deferred With Waiver`, `Rejected by USER`, or `Needs USER Decision`. The packet must give USER answer paths to accept the recommendation, revise it with USER-written changes, defer/future-package/waive with reason, reject it, or request more analysis. `USER_BRANCH_PLAN_REVIEW.md` is a required user-facing product/design planning artifact, not a normal Codex status digest: it must present the branch summary, end-state vision, visual/functional walkthrough, surface map, implementation options with pros/cons/risk, Codex recommended direction, current branch scope, future-gated scope, plain-English SLC package plan, clear USER decisions, USER response area, Codex response digest, and Workstream Entry result area. Before bounded Workstream implementation can begin, USER's response must be attached or inserted into the review packet or review digest, and Codex must digest that response into the Workstream Entry packet, branch plan, UFD item, matrix row, or branch record as applicable. Proceeding without that completed response/digest requires an explicit USER waiver recorded in `USER Review Response:` and `Codex Response Digest:`. It is a review wrapper, not a duplicate permanent ledger; accepted answers fold into the Branch Vision Contract Snapshot, Branch Runtime Engineering Plan, UFD item, matrix row, or branch record only through a later USER-approved digest/source-truth pass.

Missing or shallow branch goal, user-facing outcome, end-state vision, visual/functional walkthrough, surface map, implementation options, recommended direction, SLC package plan, USER decisions, USER response/digest or waiver, Hardening plan, Live Validation / UTS plan, Desktop review bundle proof, `USER Review Packet Finding:`, or exact USER decision blocks Workstream implementation on `USER Branch Plan Review Missing`. A Desktop USER Review Packet that is stale against current `HEAD`, not loaded, not digested, missing `START_HERE.md`, missing `USER_BRANCH_PLAN_REVIEW.md`, missing the exported zip when required, or not explicitly waived blocks on `USER Review Packet Stale` or `USER Review Packet Not Digested`. A first-seam-only packet cannot satisfy this gate when multiple slices or seams are admitted.
