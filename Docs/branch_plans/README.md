...
RAR row statuses must use `CONFORMING`, `NONCONFORMING`, `UNPROVEN`, `PARTIAL`, `EXCEPTION NEEDED`, `ISSUE CANDIDATE`, `SOURCE-TRUTH GAP`, `REFERENCE GAP`, or `OUT OF CURRENT SCOPE`. Any unresolved status other than `CONFORMING` or justified `OUT OF CURRENT SCOPE` blocks normal BP, Workstream, Hardening, Live Validation, PR Readiness, or Release Readiness progression until repaired, explicitly waived, routed to an approved issue candidate, or dispositioned by the correct source-truth owner. RAR must include these tables when the trigger touches UI/UX, runtime status, or proof claims: `| Surface | Element Group | Source File / Code Region | Backend / State Owner | Rendered Evidence | Accepted Reference | Visual Match | Behavior Match | Status | Defect / Gap | Next Legal Action |`; `| Accepted Reference | Intended Reuse | Target Surface | Copy/Equivalent/Exception | Evidence | Issue |`; `| Issue Candidate | Owner FAM | Surface | Element Group | Defect Class | Evidence | Proposed Carrier | GitHub Issue Mutation Approved? |`; and `| RAR USER Decision | Meaning | What It Authorizes | What It Does Not Authorize |`.
...
- Accepted Reference / Template Comparator Matrix:
