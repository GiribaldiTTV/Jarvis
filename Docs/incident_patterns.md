...
- validation pattern:
  ...
  gate every changed `dev/orin_*.py` and `dev/fixtures/` path, bind every changed family to a successful command that emits its executable-family marker, and require the generated Stage 1 packet to carry a base/HEAD/diff-bound machine-readable firewall PASS. This prevents an implementation helper with a neutral filename, a fixture outside a hand-maintained allowlist, an unexecuted `covers_families` claim, or generic Stage 1 prose from producing a false green.
- source references:
  - `Docs/phase_governance.md`
...
- fix pattern:
  treat `C:\Nexus USER\<worktree-label>` as a clean-regenerated packet root, not an incremental folder. Before USER review or PR Readiness, prove root `START_HERE.md`, `USER Review` / `Review Aids` / `Source Truth Context` layout, exactly one current-gate primary file under `USER Review`, mandatory timestamped ZIP `C:\Nexus USER\<worktree-label>-YYYYMMDD-HHMMSS.zip`, no legacy stable `C:\Nexus USER\<worktree-label>.zip`, no previous same-label timestamped ZIPs, no matching `.zip`-named directory/symlink/non-file candidate, ZIP-beside-folder placement, duplicate ZIP entry rejection, folder/ZIP file-list plus content-hash parity, unresolved-placeholder absence, stale-stage scan, and final packet proof reporting. Same-label ZIP discovery must occur after canonical publication transaction ownership, and a fixture must inject a late competing ZIP at that boundary and prove it is removed. After both draft artifacts move to canonical paths, final validation must recompute whole-folder/ZIP file-list and byte-hash parity before commit; late binary/text mutation or file addition/removal blocks and rolls back. Every mapped copied source file must also match its expected Git object bytes exactly, including a binary/opaque file that happens to decode as UTF-8; normalized text equality is not source-identity proof. A non-file superseded ZIP candidate blocks before backup or cleanup and must be preserved for explicit USER disposition. If a packet was assembled outside the normal build path, run `dev/orin_user_review_bundle.py --validate-local-user-packet <folder> --review-export-zip <timestamped-zip> --worktree-label <label> --packet-validation-mode active-review --expected-branch <branch> --expected-head <HEAD> --expected-origin-main <origin-main>` before treating it as current.
- validation pattern:
