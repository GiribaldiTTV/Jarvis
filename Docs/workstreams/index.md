...
- operator-facing PR and release output contracts live in `Docs/phase_governance.md`; workstream docs may provide the underlying implemented work, validation, release scope, and post-merge truth, but final operator PR summaries and release notes must use inclusion-only copy blocks; PR Summary/GitHub PR body copy uses exactly `## Summary`, `## Branch Evidence`, and `## Validation`, and must not add exclusion lists, `Not Included` sections, or phase-digest handoff fields such as `Next Legal Phase`, `Next Safe Move`, `Continue Decision`, or `Stop Basis`
