...
- Current branch execution surface for new work is `feature/fb-042-step5-entrypoint-planning`.
...
- FB-042 `Branch Readiness` is now active on `feature/fb-042-step5-entrypoint-planning` as a planning-only / `Registry-only` branch-authority surface.
- No canonical FB-042 workstream doc or admitted Workstream slice exists yet.
...
Post-Release Truth: FB-005 is Released / Closed in v1.6.6-prebeta; release debt is clear; and FB-042 `Branch Readiness` is active on `feature/fb-042-step5-entrypoint-planning` as a planning-only / `Registry-only` branch-authority surface with no canonical workstream doc or admitted Workstream slice yet
