...
- FB-043 is now Released / Closed in `v1.6.8-prebeta`; Workstream, H-1, LV-1, PR Readiness, merge, and release publication are complete historical proof; and FB-044 remains selected-only / `Registry-only` on `feature/fb-044-boot-desktop-handoff-outcome-refinement` during Branch Readiness.
