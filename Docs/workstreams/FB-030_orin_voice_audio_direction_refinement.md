...
- FB-005 is now released and closed in `v1.6.6-prebeta`; release debt is clear; and FB-042 `Branch Readiness` is active on `feature/fb-042-step5-entrypoint-planning` as a planning-only / `Registry-only` branch-authority surface with no admitted Workstream slice.
...
- Branch Readiness Gate: Active on `feature/fb-042-step5-entrypoint-planning`; Workstream remains blocked until a bounded planning-first Step 5 / top-level entrypoint seam is explicitly admitted
