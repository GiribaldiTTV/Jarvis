...
| `PR #286 / resident access and quick actions` | `One-icon Nexus resident doorway, tray menu hierarchy, Global Settings / Tray / Quick Access configuration, quick-access slot ordering, dirty-state handling, resize/splitter behavior, and proof/validator support` | `Release-window target receipt` | `Indexed by RRI-20260629-001 release-target repair` | `Release Target: v1.7.37-prebeta. Release Floor: patch prerelease. Candidate anchor, target commit, current release-window assembly, and standing-governance PR posture live in external operational state and Git/GitHub live truth.` |
