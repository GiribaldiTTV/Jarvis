...
- the normal `Branch Readiness` sequence for a candidate branch is organized inside the same canonical phase:
  ...
  1. Stage 1 outputs `## Branch Readiness Stage 1 Analysis Packet` with the FAM/package candidate, package-size review, multiple admitted-slice plan, single-slice drift check, Element Coverage review, product vision, affected surfaces, acceptance criteria, USER vision questions, validation plan, expected docs sync, and explicit USER approval blocker.
