...
External operational state is a separate local coordination layer after USER-approved initialization. `Docs/governance_efficiency_operating_model.md` owns the External Operational State Store contract and transition drift gate: repo docs remain durable source truth, `C:\Nexus Governance State` is the planned accepted operational-state root, `<worktree>\.nexus_state_staging\` is proposed staging only, and external governance candidates do not become binding governance until a USER-approved repo source-truth update merges. `Docs/external_operational_state_store_reform_plan.md` owns the Docs Split Stage 0 implementation plan, target matrix, annotations, and future-work sequencing reference. Until USER approves external helper/bootstrap work and active-state migration, repo branch records and branch plans remain the legal active owners required by current governance; after migration, those repo surfaces must reduce to durable law, pointers, templates, and historical receipts. In clean clones and GitHub Actions, repo validators must validate durable repo truth without requiring the external state root.
...
- external-state Docs Split Stage 0 implementation plan, target matrix, annotations, and future-work sequencing: `Docs/external_operational_state_store_reform_plan.md`
