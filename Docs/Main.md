...
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is:
  ...
  15. only after the PR exists, has no conflicts, has no unresolved Codex comments/issues, matches merge-target canon, and clears any bot-review signal requirement for the current PR head may the branch report `PR Readiness GREEN`
...
- PR Readiness also owns `Bot Review Signal Pending` for Codex-created PRs: watch the current PR head until the Codex GitHub bot gives either a thumbs-up reaction or a bot comment; a thumbs-up reaction clears the gate, while a bot comment keeps `PR Validation Pending` active until the branch fixes the comment on the same PR, pushes, resolves the comment, and records that current-head comment-resolution closeout; no later thumbs-up is required
