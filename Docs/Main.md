...
- for relevant desktop user-facing Live Validation Stage 1 runs, export or refresh the worktree-specific local USER hub UTS file `C:\Nexus USER\UTS - <worktree-label>.txt` unless an explicit exception from `Docs/user_test_summary_guidance.md` applies; `C:\Nexus USER\User Test Summary.txt` is template-only, not an active returned-results file; Desktop / OneDrive copies are mirror-only unless USER grants a later exception
