...
External operational state is a separate local coordination layer after USER-approved initialization. `Docs/governance_efficiency_operating_model.md` owns the External Operational State Store contract and transition drift gate: repo docs remain durable source truth, `C:\Nexus Governance State` is the accepted operational-state root after local bootstrap approval, `<worktree>\.nexus_state_staging\` is proposed staging only, and external governance candidates do not become binding governance until a USER-approved repo source-truth update merges. `Docs/external_operational_state_store_reform_plan.md` owns the Docs Split implementation plan, target matrix, annotations, approved-stage boundaries, and future-work sequencing reference. External-state helpers may report, validate, preview, and produce dry-run packets unless a later USER-approved apply step explicitly authorizes mutation. Root initialization, Stage 4 active-state migration execution, Stage 5 validator transition, Stage 6 cleanup planning, Stage 6A compact pointer cleanup, Stage 6B branch-authority routing planning, and Stage 6C branch-authority routing cleanup have occurred after separate USER approvals; `Docs/branch_records/index.md` now keeps durable branch-record law, historical receipt routing, and the single standing Governance authority exception while non-standing active operational branch authority routes to `C:\Nexus Governance State\central\active_branch_authority_state.md`, branch-specific external records, and Git/GitHub/helper live checks. Stage 6D may plan branch-detail-record and branch-plan cleanup only; it must not edit those records or plans, move/delete/archive repo Docs, create worktree-local staging, or mutate FAM worktrees. Repo Docs movement, branch-record/branch-plan detail cleanup execution, worktree-local staging, and mandatory repo-validator enforcement remain separate stage decisions. Repo branch records and branch plans remain durable transition owners until a later USER-approved cleanup reduces them to durable law, pointers, templates, and historical receipts.
