...
- `PR Readiness Stage 1 - Analysis Gate` must include a user-facing `## Next Workstream` block with recommendation details, `Candidate Work To Be Done:`, and `Next Workstream User Waiver:`; it cannot continue to Stage 2 unless the packet analyzes a concrete candidate and the work planned for that candidate, or records `Next Workstream User Waiver: Granted`; otherwise `Next Workstream User Waiver Missing` blocks continuation. If no legal next workstream candidate is found, `Next Workstream Candidate Not Found` blocks Stage 1 and requires the still-not-closed FAM list plus every not-complete package and slice
...
- the normal `Branch Readiness` sequence for a candidate branch is organized inside the same canonical phase:
  ...
  3. after approval, run `Branch Readiness Stage 2 - Execution Gate` to perform approved branch/package admission work, docs sync, and branch creation only inside the USER-approved scope.
...
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is organized inside the same canonical phase:
  ...
  4. if Stage 1 records `Current-Branch Branch Readiness Re-entry Required`, the current branch is still the legal carrier but the fix is broader than PR-readiness sync and must re-enter Branch Readiness on the same branch.
  5. if Stage 1 records `New Carrier Branch Required`, the current branch is stale, merged, invalid, or cannot legally own the blocker, so a new real carrier branch is required.
  6. Branch Readiness fallback is not workstream selection by default; it is carrier/branch/package admission after the work direction has been identified in PR Readiness Stage 1.
  7. after Stage 1 is ready and USER approval exists, run `PR Readiness Stage 2 - Execution Gate`; Stage 2 owns final PR execution only: final PR package sync, commit/push if needed, PR creation, watcher provisioning, bot-review handling, mergeability validation, and merge-watch.
  8. validate current branch truth
  9. complete the merge-target canon updates on that same branch
  10. run the Governance Drift Audit
  11. record recurrence analysis for repeated or carried blocker classes before green
  12. clear the stale-canon blocker by proving current-state canon and merge-target canon reflect the branch's true state
  13. select the next workstream from current canon only when explicit USER approval for successor selection exists
  14. confirm the next workstream is recorded in backlog and roadmap
  15. confirm the next workstream has canon-valid record state and minimal scope
  16. confirm no branch exists yet for that next workstream while PR Readiness is encoding selected-next truth
  17. defer successor branch creation to `Branch Readiness` after merge and updated-`main` revalidation
  18. after explicit USER approval for successor selection exists, encode the machine-checkable selected-next markers: `Next Workstream: Selected` and runtime `Minimal Scope:` in backlog, plus `## Selected Next Workstream` with truthful branch status such as `Branch: Not created` before branch creation or the active Branch Readiness branch name after creation in roadmap; without approval, record `Backlog Addition User Approval Missing` instead
  19. If post-merge truth will resolve to `No Active Branch` because `Release Debt` or another repo-level admission blocker remains open, successor branch creation remains deferred; next-workstream selection requires explicit USER approval. If approval is absent, `Backlog Addition User Approval Missing` remains active and the branch stops with the still-not-closed FAM list plus every not-complete package and slice. If approval exists but no real runtime candidate can be selected, `Next Runtime Candidate Selection Pending` remains active and the branch stops in PR Readiness.
  20. commit all required docs, canon, validator, and branch-truth changes so the worktree is clean and truth is durable in commit history
  21. run the normal branch governance validator and the PR-readiness gate mode
  22. report `PR package ready`, create the PR, and validate the live PR state before reporting `PR READY: YES`
  23. only after the PR exists, has no conflicts, has no unresolved Codex comments/issues, matches merge-target canon, and clears any bot-review signal requirement for the live PR may the branch report `PR Readiness GREEN`
...
- release execution is not fully closed until post-release canon closure lands in remote source truth; a local-only post-release closure commit is a blocker, not completed source truth
- protected-main branch rejection must route to a real release-support closure branch/PR when release publication is complete but canon closure cannot land on `main`; do not mutate `main` directly, and do not create a standalone cleanup branch by default
- post-release validation must compare published GitHub release/tag truth against remote repo source truth before runtime Branch Readiness can resume
- runtime Branch Readiness remains blocked until release publication and canon closure are both complete; FAM-006 Monitoring and HUD selected-next truth requires explicit USER approval, and even after approval branch creation plus runtime package admission remain blocked until later Branch Readiness approval
- a post-release canon repair must not mutate `main`; if merged canon is stale, record the drift as a blocker for the next legitimate runtime-focused backlog branch's `Branch Readiness` or for a USER-approved real release-support closure branch when protected-main release closure is the blocker
...
- while a required User Test Summary handoff is outstanding in `Live Validation` or `PR Readiness`, the active branch must report `User Test Summary Results Pending`; automated validators and live helper evidence may be green, but final phase advancement is blocked until the filled User Test Summary is submitted or waived, digested into the authority record, and blockers are reevaluated
...
- Workstream may keep User Test Summary strategy current, but it must not use `User Test Summary Results Pending` as the Workstream completion blocker or stop condition.
...
- during bounded multi-seam Workstream execution, update the canonical User Test Summary strategy incrementally as user-visible seams land; refresh the formal desktop User Test Summary export during Live Validation after the user-facing shortcut or equivalent entrypoint gate is ready
...
- when Live Validation concerns a relevant desktop user-facing workstream, also require the `Codex Live Client Self-QA Gate`: the active authority record must declare `Codex Live Client Self-QA:`, `Visual Quality:`, `Usability Check:`, and `Platform Uniformity Check:` before User Test Summary handoff, and final green is blocked by `Codex Live Client Self-QA Pending` until Codex has inspected the live client like a user or an explicit waiver is recorded
