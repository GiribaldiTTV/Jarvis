...
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is organized inside the same canonical phase:
  0. run `PR Readiness Stage 1 - Analysis Gate` first. This is an analysis-first blocker repair gate: Stage 1 analyzes repo truth, fixes any current-branch PR-readiness drift/blocker it finds, validates and commits/pushes those repairs when files change, and remains blocked by `PR Readiness Stage 1 Repair Pending` until those repairs are durable.
     Stage 1 is the Stage 2 readiness-lock gate. It stays active until one explicit outcome is recorded: `Stage 1 Ready For Stage 2`, `PR Readiness Stage 1 Repair Required`, `Branch Readiness Fallback Required`, or `Stage 1 USER Waiver Required`.
  ...
  4. if Stage 1 records `Branch Readiness Fallback Required`, the blocker is broader than current-branch Stage 1 repair and must route to real carrier branch/package analysis; Branch Readiness fallback is not workstream selection by default.
  5. after Stage 1 is ready and USER approval exists, run `PR Readiness Stage 2 - Execution Gate`; Stage 2 owns final PR execution only: final PR package sync, commit/push if needed, PR creation, watcher provisioning, bot-review handling, mergeability validation, and merge-watch.
  6. validate current branch truth
  7. complete the merge-target canon updates on that same branch
  8. run the Governance Drift Audit
  9. record recurrence analysis for repeated or carried blocker classes before green
  10. clear the stale-canon blocker by proving current-state canon and merge-target canon reflect the branch's true state
  11. select the next workstream from current canon only when explicit USER approval for successor selection exists
  12. confirm the next workstream is recorded in backlog and roadmap
  13. confirm the next workstream has canon-valid record state and minimal scope
  14. confirm no branch exists yet for that next workstream while PR Readiness is encoding selected-next truth
  15. defer successor branch creation to `Branch Readiness` after merge and updated-`main` revalidation
  16. after explicit USER approval for successor selection exists, encode the machine-checkable selected-next markers: `Next Workstream: Selected` and runtime `Minimal Scope:` in backlog, plus `## Selected Next Workstream` with truthful branch status such as `Branch: Not created` before branch creation or the active Branch Readiness branch name after creation in roadmap; without approval, record `Backlog Addition User Approval Missing` instead
  17. If post-merge truth will resolve to `No Active Branch` because `Release Debt` or another repo-level admission blocker remains open, successor branch creation remains deferred; next-workstream selection requires explicit USER approval. If approval is absent, `Backlog Addition User Approval Missing` remains active and the branch stops with the still-not-closed FAM list plus every not-complete package and slice. If approval exists but no real runtime candidate can be selected, `Next Runtime Candidate Selection Pending` remains active and the branch stops in PR Readiness.
  18. commit all required docs, canon, validator, and branch-truth changes so the worktree is clean and truth is durable in commit history
  19. run the normal branch governance validator and the PR-readiness gate mode
  20. report `PR package ready`, create the PR, and validate the live PR state before reporting `PR READY: YES`
  21. only after the PR exists, has no conflicts, has no unresolved Codex comments/issues, matches merge-target canon, and clears any bot-review signal requirement for the live PR may the branch report `PR Readiness GREEN`
...
- PR Readiness Stage 1 also owns `Branch Readiness Fallback Required`; if the next-workstream or next-branch pre-plan cannot prove a broad family/package with concrete candidate work and multiple slices, and no USER waiver/approval clears the gate, PR Readiness Stage 1 must stop and route to Branch Readiness Stage 1 rather than continue to Stage 2
- `Branch Readiness Fallback Required` also applies to the full governance/source-of-truth ledger: identity/FAM/package drift, USER approval blockers, real-carrier routing, branch-authority lifecycle, watcher proof, release execution boundaries, Element Coverage misuse, ChatGPT loader drift, project-direction drift, or current/after-release workflow drift must fall back to Branch Readiness Stage 1 when the resolution is broader than current-branch PR Stage 1 repair
