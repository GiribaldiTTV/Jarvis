...
Merge-stable product/interface projection is owned by `Docs/phase_governance.md` and `Docs/branch_records/index.md`. Current task/branch authority and operational validation remain external; retained branch receipts and old pending-state wording are historical unless a material current claim depends on them.
...
Repo docs own durable product/architecture direction, product phases and acceptance contracts, stable interfaces and source routing, release interpretation, and historical receipts. Operational Governance policy and implementation are owned by the approved external Control Plane under `Docs/governance_efficiency_operating_model.md#source-truth-authority-hierarchy`.

Do not make backlog, roadmap, branch records, repo branch-plan files, worktree-slot records, or generated review surfaces manually own operational ledger material. They may point to the branch, PR, release, external-state owner, workstream, family vision, or historical receipt that proves context; they must not become the place where active status, current assignment, open PR state, release-window state, selected-next posture, or active branch-plan rows are maintained. Current operational truth and active branch planning must be derived from Git, GitHub, approved helpers, or `D:\Nexus Desktop AI\Governance State` before mutation.

`Docs/governance_efficiency_operating_model.md` owns the repository/external placement contract. Current operational Governance source is external; mutable task/worktree state belongs under `D:\Nexus Desktop AI\Governance State`. `Docs/external_operational_state_store_reform_plan.md` records historical migration sequencing and future design context, not additional current-execution prerequisites. Repository branch records and plans retain only the durable product/interface or historical roles assigned by their owner.
...
`automation/worktree governance intake`, `phase-gate governance intake`,
`Waiting For Governance Intake`, `Neutral Main Workspace Rebaseline`,
`Pre-Rebaseline Impact Audit`, `RRI-YYYYMMDD-NNN`, `One Active Cycle`,
`Return Digest`, and `Sync Rule`. The historical plan token
`C:\\Nexus Governance State\\branches\\<branch_slug>\\branch_plan.md` is retained
only as a compatibility reference; current active planning resolves through
the D-drive root map above.

Reference candidate and collision synchronization follows the same split: promoted Reference Standards remain in repo catalogs such as `Docs/ui_reference_catalog/`, while branch-owned candidate proposals and generated aggregate/collision reports may live under `D:\Nexus Desktop AI\Governance State` as external evidence only after the current workflow admits them. Candidate records, collision reports, and aggregate sync reports do not become source truth, USER acceptance, template/shared-primitive promotion, issue mutation authority, or FAM adoption proof until a USER-approved Governance source-truth update merges and consuming worktrees rebaseline through their legal gate.
...
`Docs/worktree_slots.md` owns the stable slot registry and intended lane assignment model. It defines reusable slots such as `neutral-main`, `runtime-active-1`, `runtime-active-2`, `runtime-active-3`, and `archived-historical`; `governance-standing` is retained only as a historical slot ID with no current assignment.

Slot assignment is not active branch authority. Repository records preserve durable routing and historical receipts; accepted live posture belongs to `D:\Nexus Desktop AI\Governance State` and current Git/GitHub facts. An old standing-intake record is not a current authority exception. Verify the assigned task/worktree before mutation.
...
- `D:\Nexus Desktop AI\Worktrees\Governance` and its standing-intake branch are retained migration/handoff context; current external source work uses its explicitly approved carrier and does not infer worktree retirement.
- A USER-approved bounded product-interface repair must use the exact approved branch and worktree, verified against live Git identity and the current external branch/worktree records. `Docs/branch_records/index.md` preserves durable receipts; it does not select an active owner or authorize mutation.
...
- Off-Worktree Work Routing: work outside the assigned worktree is reported to its recorded owner; operational Governance repair uses the explicitly approved external source carrier. Do not mutate a sibling worktree or infer a fixed standing-intake route.
...
`Docs/phase_governance.md` owns the `Stale-Snapshot Investigation Mutation Waiver`: a narrow USER-approved exception that can defer current-main reconciliation only for forensic investigation-support helpers, validators, proof/evidence tools, packet-generation tools, and USER review findings packets. It must label all outputs as stale-snapshot/pre-reconciliation evidence and does not authorize product/runtime fixes, phase advancement, PR/merge/release work, issue closeout, cross-worktree mutation, protected-main mutation, or claiming reconciled proof.
...
`Rebaseline Adoption & Reconciliation Phase` (`RAR`) is owned by `Docs/phase_governance.md` and routes active findings to the affected branch's external operational state plus USER packets. RAR applies after rebaseline/re-entry or returned-UTS standard drift when newly merged vision, UIREF, proof, packet, helper, validator, backend/runtime, or source-truth standards may affect existing branch work. It requires owned-surface inventory, code-to-visual trace, accepted-reference/template comparison, issue-candidate disposition, USER packet proof when decisions are needed, and no repo live-state tracking before normal phase progression resumes.

Automation Observability is multi-worktree aware. The applicable external operational owner evaluates directly verified Codex automation run/inbox and memory evidence; those reports remain evidence until classified as `BLOCKER_CANDIDATE` or `REVIEW_REQUIRED`. The repository automation report helper is historical forensic reference, not a current operational reader. Each standing automation must declare a configured cwd that resolves to a known worktree, and reports must include cwd/worktree/branch/`HEAD`/`origin/main` evidence before lane-sensitive findings affect source truth. `Automation CWD Worktree Mismatch` blocks any active-branch, PR Readiness, Release Readiness, post-merge, release-window, selected-next, toolchain, or branch-governance automation that runs from stale neutral main, a missing cwd, or the wrong assigned worktree.
...
`Docs/pr_watcher_mode_contract.md` owns the PR Watcher Mode Contract. Bounded PR Readiness Stage 2 denies recurring PR watcher automation by default and uses direct PR verification instead. Direct PR verification must inspect the live PR through GitHub connector, `gh`, GraphQL, status checks, or the active Codex turn for configured cwd, PR number, head SHA, mergeability, unresolved review-thread count, latest bot review, status checks, repair authority, approval latch posture, and next PR posture. PR watchers are exception-only or historical receipt surfaces; when explicitly approved, they must declare `Silent Monitor`, `Verify Once`, `Repair Mode`, or `Blocked Mode`, and include `Watcher Health Proof:` with configured cwd, PR number, head SHA, unresolved review-thread count, latest bot review, repair authority, delivery route proof, runtime proof, and next watcher posture.

PR Readiness Stage 2 approval no longer includes recurring watcher provisioning by default. Codex must not create or update heartbeat, cron, same-thread, or fallback PR watcher automations during bounded PR2 unless the USER explicitly approves a named watcher exception for that PR. Same-PR Codex review repair, concise revalidation comments, merge/close verification, and the Direct PR2 Continuation Rule remain governed through direct PR verification and Codex digest/helper output. No watcher does not mean no loop; bounded PR2 direct verification must continue in the active Codex turn until a new actionable bot comment is repaired or blocked, a current-head Codex approval latch plus green mergeability allows merge, the PR merges/closes, or a real blocker prevents further direct verification.
...
Prompt-ingress is part of that identity preflight. If a pasted prompt, attached prompt, referenced file, generated prompt, automation/heartbeat instruction, or ChatGPT review prompt names a different requested worktree, family, branch, repository path, packet path, or USER hub label than the assigned thread, report `Prompt-Ingress Lane Lock:` and stop on `Prompt-Ingress Lane Mismatch` before changing directories, loading sibling source truth for execution, running helpers, generating packets, mutating external state, or executing commands in the sibling lane unless USER grants an explicit lane-switch/worktree-escape decision.
...
- phase governance = product phases, product proof and acceptance, and release boundaries; operational Governance routes through the external ownership contract
- validation helper registry = product helper naming, ownership, reuse, and historical Governance-helper disposition
- branch records = durable product/interface identity and historical receipts; current phase, authority, and task/worktree state are external
...
- external reference candidate / collision synchronization schema and repo/external-state split: `Docs/governance_efficiency_operating_model.md`; branch-owned candidate evidence may be staged externally, but promoted references stay in repo source truth
- durable branch product/phase contracts and historical receipts, including the retired standing-intake record: `Docs/branch_records/`; current operational authority is external
- active branch vision, planning, UFD, Element-to-Phase Proof Matrix, and Branch Change Intent Ledger: `D:\Nexus Desktop AI\Governance State\branches\<branch_slug>\`
...
- historical external-state Docs Split plan, target matrix, stage receipts, annotations and future-design context: `Docs/external_operational_state_store_reform_plan.md`; not a current prerequisite or execution route
...
Vision routing follows this chain: `Docs/nexus_vision.md` for project-wide vision, `Docs/ai_runtime_and_trust_architecture.md` for cross-family AI-native runtime/trust architecture, `Docs/family_visions/` for reusable family-level vision, `Docs/family_feature_visions/` for USER-approved durable feature-category direction inside one family, and the active external branch planning owner for the Branch Vision Contract Snapshot and implementation proof. Family Feature Vision is the durable layer between Family Vision and Branch Vision Snapshot; it should use compact IDs such as `F7-FF01` plus element IDs such as `F7-FF01-E01` after USER-approved content/index creation, and it must not become backlog identity, branch route identity, Slice/SLC identity, seam identity, active branch state, selected-next truth, PR state, release-window state, implementation approval, or a live operational ledger. Selected feature-bearing branch routes block BP1 on `Family Feature Vision Required For Selected Feature` until the required Family Feature Vision exists and passes `Feature Vision Sufficiency Check`, unless the branch planning packet records `Family Feature Vision Not Applicable` for a governance-only, release-support, pure helper/validator, source-truth-only, or otherwise non-product route. Any FFV rename/reframe must update all in-scope repo, external-state, branch-plan, branch-record, and USER packet pointers or stop on `Family Feature Vision Pointer Migration Missing`. `Docs/ui_reference_catalog/` is a separate promoted-reference catalog for USER-accepted UI reference contracts only; it must not treat candidate screenshots, branch proof, helper output, or FAM-002 grammar as promoted golden references without an explicit promotion record. Codex must not promote proposed product/design ideas into durable vision owners by inference.
...
Establish the relevant system boundary from this router, then load only the owners needed to understand the operation and its material dependencies.
...
Repository product execution and phase contracts remain governed by the owners listed in this index, especially `Docs/development_rules.md`, `Docs/phase_governance.md` and `Docs/codex_modes.md`. The matching external operational record owns current task/phase state and permitted scope, reconciled with explicit USER authorization; repository workstream and branch records retain product contracts and durable receipts.
...
- `Docs/Main.md`
- `Docs/codex_modes.md`
- `Docs/orin_task_template.md` — retired compatibility/forwarding stub only; current loader and prompt-generation continuity belong to `Docs/nexus_startup_contract.md`
...
Use these for promoted work that needs a stable feature-state, branch-local validation/evidence record, executed seam trace, durable artifact/reuse history, and closure history:
...
- the relevant durable product/admission receipt under `Docs/branch_records/`; the former standing-intake record is historical only
...
- for product phases and acceptance, route through `Docs/phase_governance.md`; for operational Governance, use `Docs/governance_efficiency_operating_model.md#source-truth-authority-hierarchy`
- choose execution posture from the current explicit USER authorization and the material owner rules, not copied historical prompt markers
...
- keep product phase and acceptance rules with their repository owners, and operational policy and validation with the current external Control Plane
- preserve historical Nexus material as historical evidence, not current execution prerequisites
- Route operational Governance source repairs to the approved external carrier under `Docs/governance_efficiency_operating_model.md#source-truth-authority-hierarchy`; keep product changes and repository interface patches on their separately approved repository carrier.
- an escaped defect blocks only the operation whose material safety, correctness, authorization, or acceptance it affects; repair its owning source without importing unrelated Governance into product scope
- add or update a test only for a distinct material failure; a prior wording or historical-receipt miss does not require a new validator or helper
- when post-merge truth will remain `No Active Branch`, merge-stable pointer surfaces such as backlog and roadmap must not mirror transient repair-branch ownership; that transient execution truth belongs only in the matching external operational record and Git/GitHub-derived evidence; durable repository pointers do not become live authority
...
- during `Workstream`, run a `Post-Seam Continuation Self-Audit`; if `Completion Status: In Progress` and `Continue Decision: Continue`, report `Continue Same Workstream` and continue; it must not return a green seam closeout as terminal
...
- when a prompt names an active seam, treat it as the entry seam, not a terminal boundary; a slice is a bounded admitted backlog-completion unit, SLC is a branch-planning alias or historical ID for Slice-level line items, and a seam is the current execution checkpoint inside or between slices
...
- Branch Readiness Stage 1 owns the normal next runtime implementation pipeline selection. That choice is rooted in project vision, family vision, applicable Family Feature Vision, branch vision, current completed work, and the next implementation need. PR Readiness may recommend or validate selected-next context only when USER explicitly asks for PR-time successor selection or already-encoded selected-next truth would merge as durable source truth; PR Readiness does not require selected-next truth or a waiver by default.
...
- Route operational Governance source repairs to the approved external carrier under `Docs/governance_efficiency_operating_model.md#source-truth-authority-hierarchy`; keep product changes and repository interface patches on their separately approved repository carrier.
- historical repair-only branch records are traceability, not new carrier authority
- product PR-owned acceptance and release-contract defects remain the owning product carrier's responsibility; a completed merge is not authority to mutate main
- the normal `Branch Readiness` sequence for a candidate branch is organized inside the same canonical phase:
  ...
  1d. Before runtime/user-facing/source-truth Workstream implementation begins or resumes, Codex must complete Branch Planning: BP1 `USER Branch Vision Review`, BP2 `USER Branch Plan Review`, and BP3 `Workstream Entry / Orchestration Validation`. BP1 summarizes the applied Project Vision, Family Vision, applicable Family Feature Vision, branch vision, end-state, product shape, user-facing behavior, surfaces, options, recommendations, selected/deferred elements, and USER acceptance or waiver. BP2 summarizes the accepted Branch Vision, implementation package, branch scope size, Slice/SLC/seam plan, affected surfaces, likely files, validators/helpers, proof path, Hardening plan, Live Validation / UTS plan, risks, future-gated boundaries, USER plan review, exact BP3 approval text, `Contract Status:`, `Packet Reviewability State:`, `USER Gate State:`, `USER Review Response:`, `Codex Response Digest:`, and `USER Review Packet Finding:`. BP3 proves the plan implements the accepted Branch Vision and applied vision chain and can return first bounded implementation approval request only when BP1 and BP2 are Complete or Waived by USER and BP3 is approved or waived by USER. Missing review blocks on `BP1 Branch Vision Review Missing`, `BP2 Branch Plan Review Missing`, or `BP3 Orchestration Validation Missing`; stale packet metadata blocks on `USER Review Packet Stale`; an omitted packet digest or missing USER response/digest/waiver blocks on `USER Review Packet Not Digested`; treating packet validation as USER acceptance blocks on `Packet Validation Treated As USER Acceptance`; missing applied vision/evidence mapping blocks on `Vision Carrydown Chain Missing` or `USER Packet Vision Evidence Missing`.
...
- Follow `Docs/phase_governance.md#pre-pr-durability-rule`: local commit, push, PR, merge, release, publication, and live activation are separate explicitly authorized actions.
- PR Readiness still verifies the approved product branch's durable truth before PR creation
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is organized inside the same canonical phase:
  0. run `PR Readiness Stage 1 - Analysis Gate` first. This is an analysis-first blocker repair gate: Stage 1 analyzes repo truth, records `PR Readiness Stage 1 Repair Required` for bounded current-branch PR-readiness drift/blockers, validates and locally commits those repairs only under explicit approval on the legal carrier; upstream creation and push remain separately approved, and remains blocked by `PR Readiness Stage 1 Repair Pending` until those repairs are durable.
  ...
  3. if Stage 1 records `PR Readiness Stage 1 Repair Required`, bounded current-branch repair/sync remains in Stage 1 and must be validated and locally committed under explicit approval before the repaired Stage 1 packet is ready for USER review; push remains separately approved.
...
- PR creation details must use the operator copy-block contract from `Docs/phase_governance.md`: separate copy-ready blocks for `PR Title`, `Base Branch`, `Head Branch`, and `PR Summary`; the PR Summary/GitHub PR body uses exactly two top-level sections, `## Summary` and `## What Changed`; it must read as a human review summary grounded in what the branch changed, must not include `## Validation`, `## PR posture`, `## Branch Evidence`, defensive exclusion/deferred sections, or phase-digest handoff fields, and must route validation proof, command output, mergeability, bot-review state, watcher state, and PR Readiness posture to Codex digests, helper output, status checks, or external operational state instead of the GitHub body
...
- PR Readiness Stage 1 also owns `PR Readiness Stage 1 Repair Pending`; any repairable PR-readiness drift or blocker found during Stage 1 must be fixed, validated and locally committed before the repaired Stage 1 packet is ready for USER review; upstream creation, push and Stage 2 entry remain separately approved
...
- PR Readiness also owns `Bot Review Signal Pending` for Codex-created PRs: watch the live PR until the Codex Connector bot gives a thumbs-up reaction or green approval comment on the live PR. A bot comment is not approval; it keeps `PR Validation Pending` active until the branch fixes the comment on the same PR, pushes, replies to and resolves the review thread, requests Codex Connector bot revalidation, and then receives a later Codex Connector bot thumbs-up reaction or green approval comment for the repaired live PR head. That approval proof must be bound to the current live PR head by review commit SHA, PR timeline order, or equivalent GitHub live-head evidence, not by local commit time alone. This is the same-PR Codex bot-review repair loop. Stage 2 final handoff cannot be green until the post-repair bot thumbs-up/approval latch is verified.
- when a live Codex-created PR is waiting on `Bot Review Signal Pending`, PR Readiness Stage 2 uses direct PR verification by the GitHub connector, `gh`, GraphQL review-thread inspection, status checks, and the active Codex turn. Recurring PR watcher automation is denied by default for bounded PR2 because watcher delivery has proven inconsistent. Same-PR repair authority remains bounded to valid in-scope Codex bot comments: inspect the comment, repair only docs/governance/source-truth or other already-approved PR scope, rerun validation, commit and push to the same branch, reply/resolve only when the review thread contract requires it, request Codex Connector bot revalidation with a 3-5 word PR comment only, and continue direct PR verification until the later thumbs-up/approval latch clears. Every Codex Connector review-request or revalidation PR comment must be 3-5 words only, preferably `@codex review please`. If no Codex bot comment or thumbs-up/approval signal appears after the current PR head has been live for at least two minutes, Codex may post exactly one PR conversation nudge for that head SHA asking the Codex bot for the review signal, and must not repeat that nudge for the same head. After that nudge or any same-PR repair push, bounded PR2 must keep directly checking the live PR in the active Codex turn until a new actionable Codex comment is repaired or blocked, a current-head Codex approval latch plus green mergeability allows merge, the PR merges/closes, or a real blocker prevents further direct verification. Head SHAs, validation summaries, repair narratives, and governance proof belong in the Codex thread digest, helper output, validator output, or external operational state, not in the PR comment. If a bot comment asks for merge, release/tag/artifact work, runtime/provider/model/memory/shortcut/installer implementation, full AI Product Contract import, private Dev ORIN import, GitHub issue creation, unrelated cleanup, or another out-of-scope action, Codex must report `BLOCKED` instead of repairing.
- PR Readiness and any earlier planning phase that intends to request watcher-based PR monitoring also own `PR Watcher Provisioning Unproven`: this blocker applies only after USER explicitly approves a watcher exception for the exact PR. Otherwise direct PR verification owns live PR, bot-review, mergeability, and merge/close proof. For an approved watcher exception, the watcher target, approved reporting surface, routing proof, runtime path, run-proof method, fallback, teardown rule, replacement provisioning for the next live PR, and the live bot-review action contract must be explicit and proven before watcher evidence can support the gate. Manual rollout-file or transcript-file injection does not count as proof. Watcher configuration is not runtime proof.
- PR Readiness also owns `PR Watcher Routing Unverified` only for a USER-approved watcher exception: even after watcher provisioning exists and run proof is present, keep watcher-based proof blocked until the approved reporting surface is explicitly recorded and a validation pass confirms the configured thread/host target, state-file target, transcript target, and delivery proof all point to that recorded surface and at least one watcher emission has landed there
- Direct PR verification is the standard bounded PR2 operating procedure: inspect the live PR, comments, reviews, unresolved review threads, reactions where available, checks, mergeability, head SHA, and merge/close state in the active Codex turn. Recurring PR watcher automation may only be used after a separate USER-approved watcher exception names the exact PR, reporting surface, interval, delivery proof, teardown rule, and repair authority. The Direct PR2 Continuation Rule blocks quiet handoff while the PR is still waiting for Codex approval or merge verification.
- Watcher status-change output, delivery proof, and retry behavior are exception-only requirements for that USER-approved watcher path. Direct PR verification remains sufficient for normal live PR truth, bot-review, mergeability, and merge/close proof.
- Standard PR2 continuation rule from now on: after PR creation and live PR validation, PR Readiness remains active until direct GitHub/GitHub-connector verification proves the PR actually merged or closes with a blocker. A watcher may supplement that proof only when USER approved a watcher exception.
- PR Readiness also owns `PR Merge Verification Pending`: even after bot approval and green merge status, do not advance to `Release Readiness` until direct GitHub/GitHub-connector verification proves that the live PR is merged
...
- Automation Observability Review Pending is a governed source-of-truth review loop: standing automations may report into Codex automation run/inbox rows and `$CODEX_HOME/automations/*/memory.md`, but repo canon changes only when the applicable external operational owner classifies directly verified automation evidence as `BLOCKER_CANDIDATE` or `REVIEW_REQUIRED` and a bounded repair seam admits it. Informational green or waiting reports stay `REVIEW_INFO` unless they contradict repo truth. `Automation CWD Worktree Mismatch` blocks lane-sensitive automation output when the configured cwd is missing, stale neutral main, parked, or the wrong assigned worktree.
...
- Release Readiness remains analysis-only for repository files. Source changes return to the owning pre-merge product phase or a separately approved repository interface carrier; operational Governance changes follow the external owner.
...
- for relevant desktop user-facing Live Validation Stage 1 runs, export or refresh `User Test Summary.txt` within the approved collection and evidence set selected by the external USER lifecycle owner unless an explicit exception from `Docs/user_test_summary_guidance.md` applies; Desktop / OneDrive copies are mirror-only unless USER grants a later exception
...
- if the declared normal runtime worktree launcher is missing during Live Validation, the active branch may create or repair a worktree-local normal runtime launcher as validation setup before User Test Summary handoff; the launcher must target the branch's normal product runtime entrypoint, must not enable troubleshooting mode or launch a validation helper, and must not create installer, startup-registration, packaging, update, or persistent OS integration behavior
...
After D Main-first identity, load the owners materially needed for the requested operation. Expand reading only when a concrete dependency or source conflict requires it; a whole-governance scan is not a routine entry gate.
