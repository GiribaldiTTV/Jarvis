...
- `C:\Nexus Worktrees\Governance` is the only `Standing Governance Intake Branch` worktree; it uses `feature/release-readiness-source-truth-intake`, accepts a `Release Readiness digest` only, uses `RRI-YYYYMMDD-NNN`, enforces `One Active Cycle`, requires the clean pre-intake `Sync Rule` to `origin/main`, pauses originating lanes in `Waiting For Governance Intake` or `Waiting For Updated Main`, and returns a post-merge `Return Digest`
