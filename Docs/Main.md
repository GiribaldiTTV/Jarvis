...
- the normal `Branch Readiness` sequence for a candidate branch is organized inside the same canonical phase:
  ...
  1d. Before runtime/user-facing/source-truth Workstream implementation begins or resumes, Codex must provide the `USER Branch Plan Review Gate` packet through Workstream Entry. It summarizes the branch goal, planned user-facing outcome, visual/behavioral expectations, implementation breakdown, Element-to-Phase proof path, Hardening plan, Live Validation / UTS plan, Codex recommendations, implementation options, alternatives/tradeoffs, open questions, USER design review questions, accepted/deferred/rejected scope, exact USER decision needed, `USER Review Response:`, `Codex Response Digest:`, and `USER Review Packet Finding:`. Missing review blocks on `USER Branch Plan Review Missing`; stale packet metadata blocks on `USER Review Packet Stale`; an omitted packet digest or missing USER response/digest/waiver blocks on `USER Review Packet Not Digested`.
