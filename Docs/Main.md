...
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is:
  ...
  12. If post-merge truth will resolve to `No Active Branch` because `Release Debt` or another repo-level admission blocker remains open, successor branch creation remains deferred; next-workstream selection requires explicit USER approval. If approval is absent, `Backlog Addition User Approval Missing` remains active and the branch stops with the still-not-closed backlog list. If approval exists but no real runtime candidate can be selected, `Next Runtime Candidate Selection Pending` remains active and the branch stops in PR Readiness.
