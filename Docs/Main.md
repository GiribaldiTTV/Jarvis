...
Slot assignment is not active branch authority by itself. Repo branch records own durable authority law and historical receipts; accepted live posture belongs to `C:\Nexus Governance State` or Git/GitHub/helper-derived truth. The standing Governance intake record is the only repo routing exception, but its in-flight cycle, PR, return-digest, and release-window facts still belong external/derived. A slot says where work is intended to happen; Git/GitHub and identity preflight prove what is actually checked out and mutable.
...
- Computer Use is prohibited for Nexus repo validation and runtime-control work. Do not invoke it for inspection, capture, target discovery, launcher activation, UI interaction, evidence generation, or cleanup. Route desktop interaction through the Codex-operated governed human-client real OS cursor/keyboard path; if that path cannot safely acquire the visible control surface, stop on `Live Validation Control-Surface Acquisition Blocked` instead of switching to Computer Use or transferring operation of the validation step to the USER.
