...
- `PR Readiness Stage 1 - Analysis Gate` must include a user-facing `## Next Workstream` block with recommendation details; if no legal next workstream candidate is found, `Next Workstream Candidate Not Found` blocks Stage 1 and requires the still-not-closed FAM list plus every not-complete package and slice
