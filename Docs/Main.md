...
Slot assignment is not active branch authority by itself. Repo branch records own durable authority law and historical receipts; accepted live posture belongs to `D:\Nexus Desktop AI Data\Governance State` or Git/GitHub/helper-derived truth. The standing Governance intake record is the only repo routing exception, but its in-flight cycle, PR, return-digest, and release-window facts still belong external/derived. A slot says where work is intended to happen; Git/GitHub and identity preflight prove what is actually checked out and mutable.
...
- Off-Worktree Work Routing: work discovered outside the assigned worktree or unrelated to the active branch must route to `D:\Nexus Desktop AI Data\Worktrees\Governance`
...
- active branch vision, planning, UFD, Element-to-Phase Proof Matrix, and Branch Change Intent Ledger: `D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\`
...
- for relevant desktop user-facing Live Validation Stage 1 runs, export or refresh `D:\Nexus Desktop AI Data\USER\User Test Summary.txt` unless an explicit exception from `Docs/user_test_summary_guidance.md` applies; Desktop / OneDrive copies are mirror-only unless USER grants a later exception
