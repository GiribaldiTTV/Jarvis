...
- the normal `PR Readiness` sequence for a branch that changes release-facing canon is:
  0. clear the hard PR Readiness blockers: `stale-canon`, `post-merge`, `dirty`, `docs-sync`, `next-workstream`, `Next Runtime Candidate Selection Pending`, `Backlog Addition User Approval Missing`, `Backlog Exhaustion User Decision Pending`, and `uts-results`
  ...
  6. select the next workstream from current canon only when explicit USER approval for successor selection exists
