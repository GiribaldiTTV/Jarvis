...
- Live Validation helper PASS, real-input proof PASS, screenshot/video proof PASS, and desktop `User Test Summary.txt` export do not equal final Live Validation green while the UTS copy remains a draft/handoff/`NOT RETURNED RESULTS` artifact; keep `Next Legal Phase: Live Validation` and `User Test Summary Results Pending` until USER returns PASS/FAIL/WAIVED results or grants a waiver reason.
