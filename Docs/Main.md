...
- Computer Use is prohibited for Nexus repo validation and runtime-control work. Do not invoke it for inspection, capture, target discovery, launcher activation, UI interaction, evidence generation, or cleanup. Route desktop interaction through the governed human-client real OS cursor/keyboard path; if that path cannot safely acquire the visible control surface, stop on `Live Validation Control-Surface Acquisition Blocked` instead of switching to Computer Use.
