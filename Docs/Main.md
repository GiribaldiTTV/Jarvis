...
- PR Readiness and any earlier planning phase that intends to depend on watcher-based PR monitoring also own `PR Watcher Provisioning Unproven`: if the branch expects watcher-based PR monitoring, the watcher target, runtime path, run-proof method, fallback, teardown rule, replacement provisioning for the next live PR, and same-thread reporting contract must be explicit and proven before the gate clears
