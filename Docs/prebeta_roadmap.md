...
- current phase after `v1.7.0-prebeta` publication: FAM-006 Monitoring HUD Dashboard Product Surface is released historical traceability on main. LV2 digested the explicit USER waiver/passable Dashboard acceptance before PR #118 merged; Dashboard acceptance is USER WAIVED/PASSABLE for the current Dashboard-first surface, Overlay/display acceptance is deferred/non-gating, Core remains dependency-only, and the FAM-006 branch authority is historical rather than active. This Branch Readiness Stage 2 carrier closes the post-release source-truth drift and records the release-body SOP repair before any FAM-007 implementation or package admission may begin.
...
- current branch after `v1.7.0-prebeta` publication: `codex/fam-007-branch-readiness` carries the USER-approved Branch Readiness Stage 2 governance/canon repair only; no FAM-007 implementation package is admitted.
- next concern: complete and validate the combined FAM-007/governance Branch Readiness Stage 2 repair for post-release canon closure, release-body SOP enforcement, ChatGPT loader alignment, multi-worktree safety, and FAM-006 saved issue-thread planning. FAM-007 implementation, local AI/model/provider work, GitHub issue creation, tag creation, GitHub Release creation, and artifact publication remain outside this repair carrier.
- next legal analysis candidate after this Stage 2 repair: Branch Readiness Stage 1 revalidation for FAM-007 package admission and implementation planning. That revalidation must decide whether the legal next implementation carrier is FAM-007 AI foundation, governance/workspace-resilience, a combined FAM-007 foundation plus workspace-governance package, or another path required by updated main. No implementation package admission, GitHub issue creation, or parallel-work governance change is automatic before that revalidation result and later USER approval.
...
Current active workstream: None - PR #118 merged the FAM-006 Monitoring HUD Dashboard Product Surface package into main on 2026-05-12; Dashboard acceptance is USER WAIVED/PASSABLE for the current Dashboard-first surface; the FAM-006 branch authority is historical, Overlay/display acceptance remains deferred/non-gating, and Core remains dependency-only.
...
- Next Legal Seam: `Branch Readiness Stage 2 governance/canon repair on codex/fam-007-branch-readiness; FAM-007 implementation remains blocked until later revalidation and USER approval`
