...
| `FAM-007` Local AI and Capability Packs | local-only provider setup and consent readiness sequence is released through user-operated consent UX foundation in v1.7.18-prebeta via PR #206; setup completion foundation is Workstream Green on the active carrier with H1 pending; provider SDK/model execution and model work remain USER-gated | `Docs/family_visions/FAM-007_local_ai_and_capability_packs.md` | `Docs/branch_records/feature_fam_007_local_ai_provider_setup_completion_foundation.md` |
