...
- next concern: run Hardening H1 post-workstream governance validation now that the branch authority phase is aligned to `Hardening`, while preserving the converted roadmap family-anchor framing, the split workstream index, the aligned routing/loader surfaces, and the validated FB-049 selected-next lock.
