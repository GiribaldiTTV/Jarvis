...
| `FAM-007` Local AI and Capability Packs | local-only provider setup and consent readiness sequence is released through durable consent persistence foundation in v1.7.17-prebeta via PR #203; active Branch Readiness Stage 2 carrier admits user-operated consent UX foundation with Workstream Entry pending; provider execution and model work remain USER-gated | `Docs/family_visions/FAM-007_local_ai_and_capability_packs.md` | `Docs/branch_records/feature_fam_007_local_ai_provider_user_operated_consent_ux_foundation.md` |
