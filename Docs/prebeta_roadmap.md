...
Record State: Current-main reconciliation and refreshed LV1 repair setup complete on `feature/fam-006-monitor-groups-sensor-configuration`; PR #163 main truth remains context only.
Minimal Scope: Repair Workstream implementation for refreshed LV1 blockers only: close-while-dirty must queue pendingMonitorAction="close" through the real Manage Monitors close target, Save / Discard / Cancel must preserve and resolve visible draft values correctly for close, and refreshed LV1 must add Manage Monitors screenshot-sequence or video-style proof for dropdown, guard, delete, empty-state, and 20+ / 100+ scrollbar states.
...
Selection Scope: This branch carries FAM-006 Monitor Groups / Sensor Command Center close-guard and visual-proof repair setup and next Repair Workstream implementation. PR #162/FAM-007 source truth is preserved as current-main context only and remains routed to the FAM-007 lane.
