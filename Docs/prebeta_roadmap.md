...
- next concern: remove the temporary dual-shape tolerance in Phase 7 / Slice R7-S1 while preserving the converted roadmap family-anchor framing, the split workstream index, the aligned routing/loader surfaces, the validated FB-049 selected-next lock, and the now-sweep-clean current-state family model.
