...
- current branch after `v1.7.10-prebeta` tag context: `feature/fam-007-local-ai-provider-setup-contract-readiness` is the USER-approved FAM-007 runtime carrier for Stage 2 setup; `feature/repo-wide-source-owner-marker-adoption` remains historical PR #185 evidence, PR #186 is its fold-down repair evidence, PR #187 is release-window support publication evidence, and `feature/fam-006-monitor-groups-sensor-configuration` is historical released PR #180 evidence.
- next concern: USER decision for the PR execution gate on FAM-007 Local AI Provider Setup Contract Readiness. Runtime/provider SDK/model execution, shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, and `codex/ai-llm-lab` mutation remain outside this setup.
...
- next legal phase: USER decision for the PR execution gate on FAM-007 Local AI Provider Setup Contract Readiness. Issue work, artifacts beyond GitHub source archives, branch cleanup, provider SDK/model work, memory/indexing implementation, voice/Core runtime sync, shortcut/installer work, release, AI Product Contract import, Private Dev ORIN import, and future runtime branch creation require separate USER approval.
