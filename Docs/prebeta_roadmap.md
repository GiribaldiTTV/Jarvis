...
- unreleased non-doc implementation work: none after `v1.7.12-prebeta`; provider execution, SDK/model, memory, voice/Core, shortcut/installer, issue, artifact, and future release work remains USER-gated.
...
- current active workstream: `FAM-006 Overlay Profile Runtime Foundation` on `feature/fam-006-overlay-profile-runtime-foundation`; current main includes PR #192 FAM-007 setup implementation foundation as separate merged context.
- current branch after `v1.7.11-prebeta` tag context: USER approved Branch Readiness Stage 2 setup for `feature/fam-006-overlay-profile-runtime-foundation` in `C:\Nexus Worktrees\FAM-006`; `feature/fam-007-local-ai-provider-setup-implementation-foundation` is PR #192 current-main context, `feature/fam-007-local-ai-provider-setup-contract-readiness` is released PR #190 context, `feature/repo-wide-source-owner-marker-adoption` is historical PR #185 evidence, PR #186 is its fold-down repair evidence, PR #187 is release-window support publication evidence, PR #191 is released release-readiness source-truth context, and `feature/fam-006-monitor-groups-sensor-configuration` is historical released PR #180 evidence.
...
Merged-Main Selected-Next Context: PR #192 merged FAM-007 setup implementation foundation as current-main context; no FAM-006 successor is selected by main by inertia.
