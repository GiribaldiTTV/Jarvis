...
- unreleased non-doc implementation work exists: no current known release-window debt from PR #129, PR #132, PR #138, or PR #142 after `v1.7.1-prebeta`; those PRs are published in the latest prerelease. GitHub issue closeout/comments, branch/worktree cleanup, FAM-007 runtime/provider/model/shortcut/installer work beyond PR #138, Overlay acceptance, external telemetry parity, raw evidence handling, and AI Product work remain USER-gated.
...
- current branch after `v1.7.1-prebeta` publication: None after PR #142 merge; `feature/fam-006-dashboard-settings-panel` is released historical / merge-stable evidence.
