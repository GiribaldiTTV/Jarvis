...
- unreleased non-doc implementation work exists: none recorded after `v1.7.2-prebeta` in merged source truth. PR #152 FAM-007 local-only foundation continuation is published in the latest prerelease; PR #129, PR #132, PR #138, and PR #142 remain published in `v1.7.1-prebeta`; branch-local FAM-006 Monitor Groups sensor configuration remains unmerged active repair work on `feature/fam-006-monitor-groups-sensor-configuration`. GitHub issue closeout/comments, branch/worktree cleanup, provider/model/shortcut/installer work beyond the local-only scaffold, Overlay acceptance, external telemetry parity, raw evidence handling, and AI Product work remain USER-gated.
- the latest public released implementation milestone is the `v1.7.2-prebeta` FAM-007 local-only foundation continuation release window, with PR #152 released as FAM-007 local-only scaffold scope and PR #153/PR #154 preserving post-merge governance closeout; the prior `v1.7.1-prebeta` FAM-006 Dashboard issue-resolution/settings-panel release window remains released with PR #129, PR #132, and PR #142 as FAM-006 scope and PR #138 included as under-the-hood FAM-007 support; FAM-006 Monitoring HUD Dashboard Product Surface remains released historical traceability in `v1.7.0-prebeta`; earlier public released implementation milestones include `FAM-001` legacy `FB-049` Active-session pre-settled incoming-launch conflict truth and `FAM-004` legacy `FB-030` voice/audio runtime diagnostics proof in `v1.6.13-prebeta`; FB-048 Active-session relaunch signal-failure and wait-timeout truth in `v1.6.12-prebeta`; FB-047 Active-session relaunch decline session-preservation proof in `v1.6.11-prebeta`; FB-046 Active-session relaunch reacquisition and settled re-entry proof remains released in `v1.6.10-prebeta`; FB-044 Boot-to-desktop handoff outcome refinement and FB-045 Active-session relaunch outcome refinement remain released in `v1.6.9-prebeta`; FB-043 Top-level desktop entrypoint ownership and `main.py` handoff refinement remains released in `v1.6.8-prebeta`; FB-042 Desktop startup runtime family anchor remains released in `v1.6.7-prebeta`; FB-005 Workspace and folder organization remains released in `v1.6.6-prebeta`; FB-030 ORIN voice/audio direction refinement remains released in `v1.6.5-prebeta`; FB-015 Boot and desktop phase-boundary model plus FB-029 ORIN legal-safe rebrand, future ARIA persona option, and repo licensing hardening remain released in `v1.6.4-prebeta`
- current phase after `v1.7.2-prebeta` publication: merged main remains No Active Branch after PR #156 / RRI-20260515-001, while the branch-local FAM-006 lane is reconciling current main before returned UTS FAIL repair setup. PR #152 is historical released FAM-007 local-only scaffold scope. RRI-20260514-006 / PR #151 closed release-dependent canon drift before the FAM-007 continuation, RRI-20260514-007 / PR #153 repaired the post-PR #152 active-authority closeout, PR #154 returned the standing governance intake lane to idle before `v1.7.2-prebeta`, and PR #155/#156 hardened Branch Readiness planning. PR #129, PR #132, PR #138, and PR #142 are released in v1.7.1; PR #143 through PR #151 are governance/readiness support; PR #152, PR #153, and PR #154 are released in v1.7.2; PR #155 and PR #156 are governance/readiness support after the latest release. FAM-006 Monitor Groups returned UTS FAIL repair and FAM-007 runtime/provider/model/shortcut/installer work beyond the local-only scaffold remain pending USER approval in their owning lanes.
...
- current active workstream: Branch-local FAM-006 Monitor Groups returned UTS FAIL repair setup on `feature/fam-006-monitor-groups-sensor-configuration`; merged main has no active workstream after PR #152.
- current branch after PR #152: branch-local `feature/fam-006-monitor-groups-sensor-configuration`; merged main has no current branch. `feature/fam-007-local-ai-foundation-runtime-continuation` is a historical merged branch/ref until a later cleanup or rebaseline gate handles it.
- next concern: USER-approved FAM-006 returned UTS FAIL repair setup after current-main reconciliation, plus separately USER-approved release readiness for PR #152 or future Branch Readiness in the owning lane. Runtime/provider/model/shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, and `codex/ai-llm-lab` mutation remain outside this historical carrier.
...
- next legal phase after PR #152 merge: branch-local FAM-006 current-main reconciliation must validate before Branch Readiness Stage 2 returned UTS FAIL repair setup resumes; Release Readiness Stage 1 may separately evaluate PR #152 as merged-unreleased FAM-007 scope in its owning lane. PR creation, merge, release execution, provider SDK/model work, memory/indexing implementation, voice/Core runtime sync, shortcut/installer work, AI Product Contract import, Private Dev ORIN import, and GitHub issue creation require separate USER approval.
...
Next Legal Analysis Candidate: Branch Readiness Stage 2 repair setup for FAM-006 Monitor Groups returned UTS FAIL path; Release Readiness Stage 1 may separately evaluate PR #152 as merged-unreleased FAM-007 scope in its owning lane.
...
Backlog Addition User Approval Missing: Cleared for USER-approved FAM-006 Monitor Groups branch authority setup, profile planning admission, and returned UTS FAIL repair planning on `feature/fam-006-monitor-groups-sensor-configuration`; active for new backlog item, backlog split, full AI Product Contract import, GitHub issue work, release/tag/artifact work, PR creation, runtime recording, Overlay Profile runtime, Recording Profile runtime, export/share behavior, or any single-slice package waiver.
...
Selected Next Workstream: Branch-local FAM-006 Monitor Groups returned UTS FAIL repair setup after current-main reconciliation.
Record State: Active branch-local FAM-006 / PKG-006 runtime continuation; merged main remains No Active Branch after PR #152.
Minimal Scope: Dashboard resize/move live render smoothness repair, shrink and grow resize proof, during-drag visual/frame or pixel-signature proof before mouse release, scalable Manage Monitors split-layout repair, Nexus-styled scrollbars, Sensor Library large-inventory planning, and direct validator/source-truth support. Runtime recording, Overlay Profile runtime, tray recording controls, export/share behavior, provider expansion, Overlay acceptance, FAM-007 work, and AI Product work remain pending USER decisions.
Branch: `feature/fam-006-monitor-groups-sensor-configuration`.
Selection Scope: FAM-006 family only. PR #152 remains merged-unreleased FAM-007 historical evidence and does not own this branch-local FAM-006 repair path.
...
Runtime Implementation: Monitor Groups sensor/data-source configuration is implemented but returned USER UTS FAIL remains active for resize/render-freeze and Manage Monitors scalability/native scrollbar findings; PR Readiness remains blocked until repair/revalidation or explicit waiver with reason is digested.
