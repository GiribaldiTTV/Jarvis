...
| `FAM-006` Monitoring and HUD | Active Overlay Recording Runtime Foundation is a released planning/governance receipt from PR #222 in `v1.7.25-prebeta`, with PR #223 preserving fold-down evidence and PR #224 preserving PR-body drift guard evidence for the same release window. It is not an active runtime implementation branch. Workstream/runtime implementation was intentionally skipped for that branch by USER direction, and future FAM-006 recording runtime work requires a new USER-approved carrier. The prior Recording Profile Runtime Foundation Workstream was rolled back by USER request and preserved as historical receipt at `Docs/branch_records/feature_fam_006_recording_profile_runtime_foundation.md`. USER clarified future recording vision is active-overlay-driven rather than profile-loaded: active Overlay Profile membership defines the target, the HUD Overlay card is the launcher/target preview, the compact standalone Recording Control window is the future control surface, Native Log Loader is a separate future graph/log viewer, and per-overlay effective polling policy remains future FAM-006 planning. Overlay Display Acceptance Foundation is released in `v1.7.19-prebeta` via PR #207; H1, LV1 proof refresh, USER Test Summary PASS/closed, and SLC-042 through SLC-045 are historical green; Dashboard, Sensor Command Center, and Overlay Profile foundation evidence are released receipts; historical Overlay Display Acceptance detail remains at `Docs/branch_records/feature_fam_006_overlay_display_acceptance_foundation.md`; historical Overlay Profile Runtime Foundation detail remains at `Docs/branch_records/feature_fam_006_overlay_profile_runtime_foundation.md`; no runtime/user-facing recording behavior ships from the PR #222 planning receipt, so recording execution, output files, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, and future monitoring/HUD scope remains USER-gated with no release debt | `Docs/family_visions/FAM-006_monitoring_and_hud.md` | `Docs/branch_records/feature_fam_006_active_overlay_recording_runtime_foundation.md` |
| `FAM-007` Local AI and Capability Packs | local-only provider setup and consent readiness sequence is released through setup completion foundation in v1.7.20-prebeta via PR #210, with PR #211 release-readiness fold-down evidence; AI Edition Public Leak-Prevention Foundation is released in v1.7.22-prebeta via PR #215 with PR #216 release-readiness guard evidence; Dev/Owner Skeleton Readiness Foundation is released in v1.7.23-prebeta via PR #218 with PR #217 USER branch plan review gate governance and PR #219 fold-down prevention evidence; future private Dev/Owner skeleton implementation, off-boot AI data backup/recovery implementation, provider/model execution, and release execution remain USER-gated; Owner private repo and backup/recovery roots remain explicit USER Action Gates; provider SDK/model execution and model work remain USER-gated | `Docs/family_visions/FAM-007_local_ai_and_capability_packs.md` | `Docs/branch_records/feature_fam_007_ai_edition_dev_owner_skeleton_readiness_foundation.md` |
...
Branch Creation Gate: `After current origin/main is revalidated and USER approves a future Branch Readiness Stage 1 pass, Codex must analyze the exact FAM-007 successor carrier before any implementation branch is created.`
...
Selection Truth Status: `USER-approved during PR Readiness Stage 1 repair/sync for feature/fam-007-ai-edition-dev-owner-skeleton-readiness-foundation; PR #218 is released v1.7.23-prebeta evidence, PR #220 is historical post-release canon-closure evidence, and this roadmap records selection only while authorizing no branch creation, private repo creation, private remote setup, backup implementation, Public-to-Dev import, provider/model execution, memory, release, cleanup, AI Product Contract import, Private Dev ORIN import, or v1.8.0-prebeta execution.`
