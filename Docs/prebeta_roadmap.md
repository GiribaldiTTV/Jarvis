...
- unreleased non-doc implementation work: PR #162 and PR #163 are published in `v1.7.4-prebeta` but their release-canon closure is pending the next runtime Branch Readiness Stage 1 / possible Stage 2 setup path. PR #155 through PR #161 are released historical scope. The FAM-007 branch completed the local-only FAM-007 Local AI Runtime Contracts and Capability Foundation Workstream through SLC-036 before merge; remaining provider SDK/model/memory/voice/Core/shortcut/installer/release work remains USER-gated.
...
- current phase after `v1.7.4-prebeta` publication: no active runtime/product branch on main; v1.7.4-prebeta release-canon closure is recorded as carry-forward drift for the next runtime Branch Readiness path. FAM-007 provider SDK/model/shortcut/installer work remains pending USER approval.
...
- current active workstream: None - PR #162 merged the FAM-007 Local AI Runtime Contracts and Capability Foundation; Workstream Green through SLC-036, Hardening H1 Green, Live Validation LV1 Green, PR Readiness, and PR #162 merge are complete.
- current branch after `v1.7.4-prebeta` publication: no active runtime/product branch on main; `feature/fam-007-local-ai-runtime-expansion` is historical pending-closure scope; `feature/release-readiness-source-truth-intake` is the active standing governance intake carrier for RRI-20260518-002 with runtime branch engineering planning quality repair only.
- next concern: Next runtime Branch Readiness Stage 1 must carry forward v1.7.4-prebeta post-release canon closure drift, then Stage 2 may repair release-dependent fields if USER approves. Runtime/provider SDK/model execution, shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, and `codex/ai-llm-lab` mutation remain outside this Workstream.
...
- next legal phase: Next runtime Branch Readiness Stage 1 carry-forward for v1.7.4-prebeta post-release canon closure, with possible Stage 2 setup repair only after USER approval. Provider SDK/model work, memory/indexing implementation, voice/Core runtime sync, shortcut/installer work, release execution, AI Product Contract import, Private Dev ORIN import, and GitHub issue creation require separate USER approval.
...
Release-Debt Avoidance Status: v1.7.4-prebeta is published externally, but source-truth canon closure for PR #162 and PR #163 is carried to the next runtime Branch Readiness Stage 1 / possible Stage 2 setup path; release execution and future FAM-007 runtime expansion, provider/model/memory/shortcut/installer work, Overlay acceptance, and AI Product work remain separate USER-gated decisions.
Merged-main current active workstream: None after PR #162 merge and v1.7.4-prebeta publication
...
Next Legal Phase: Next runtime Branch Readiness Stage 1 carry-forward for v1.7.4-prebeta post-release canon closure, with possible Stage 2 setup repair only after USER approval; release work, issue work, provider SDK/model work, memory/indexing implementation, voice/Core runtime sync, shortcut/installer work, and AI Product Contract import remain separate USER decisions.

## Selected Next Workstream

Selected Next Workstream: FAM-006 Monitor Groups / Sensor Command Center refreshed LV1 interactive-control visual QA repair.
Backlog Record State: Registry-only.
Record State: Current-main reconciliation and refreshed LV1 close-guard / visual-proof repair implementation remain preserved on `feature/fam-006-monitor-groups-sensor-configuration`, but focused visual QA found a new empty-state interactive-control blocker; PR #164 main truth remains context only.
Minimal Scope: Repair Workstream implementation for refreshed LV1 interactive-control visual QA blockers only: no-monitor/no-selection empty state must not show invalid Save Monitor / Cancel actions, action buttons must not stretch into oversized pills, Create Monitor must be the primary recovery action, empty-state copy must be product-facing, and every user-facing interactable must pass code inspection plus focused visual inspection before Hardening and refreshed LV1 resume.
...
Selected Next Current-Carrier Note: `feature/fam-006-monitor-groups-sensor-configuration` is the current refreshed LV1 repair setup carrier after reconciliation; it is not authority for FAM-007 work.
Selection Scope: This branch carries FAM-006 Monitor Groups / Sensor Command Center close-guard and visual-proof repair implementation plus the new interactive-control visual QA repair setup. PR #162/FAM-007 source truth is preserved as current-main context only and remains routed to the FAM-007 lane.
...
Runtime Implementation: Prior returned-blocker implementation and H1 are complete; refreshed LV1 close-guard and visual-proof repair implementation is preserved, but the new interactive-control visual QA failure awaits USER-approved Repair Workstream implementation. Provider SDKs, model downloads/execution, external calls, memory/indexing implementation, voice/Core sync, shortcut/installer work, release work, AI Product Contract import, private Dev ORIN import, FAM-007 work, and GitHub issue mutation remain blocked.
