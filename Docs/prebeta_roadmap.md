...
- current phase after `v1.6.13-prebeta` release closure: FAM-006 is at a Hardening H1 phase-boundary stop on `feature/fam-006-monitoring-hud-product-surface` after returned LV1 real-client tray/shortcut failures blocked acceptance. Release debt is clear, USER-approved selected-next runtime truth remains active for `FAM-006 Monitoring and HUD`, product completion remains reopened, WS47 repaired the same-path active-client tray/shortcut precheck and proof-class separation as supporting Workstream proof, H1 pressure-tested that path, and LV2 remains blocked as a PASS path. The next legal seam is Live Validation LV1 after USER admission. Overlay/display acceptance is deferred/non-gating, and Core repair is dependency-only.
