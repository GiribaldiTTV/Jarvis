...
- next concern: run PR Readiness PR1 reform-branch PR validation now that the branch authority phase is aligned to `PR Readiness`, while preserving the converted roadmap family-anchor framing, the split workstream index, the aligned routing/loader surfaces, and the validated FB-049 selected-next lock.
