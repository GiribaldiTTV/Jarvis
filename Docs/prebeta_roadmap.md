...
- next concern: Hardening H1 for the bounded SLC-017/SLC-018 Assisted Desktop Mode No-Provider Interaction And Consent Surface seam, limited to proof review for the local-only disabled/no-provider interaction affordance, explicit consent posture, provider-visible-data disclosure, no-provider fallback behavior, and direct validation coverage. Real provider SDK integration, model downloads, memory/indexing, voice/Core sync, shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, PR #129/#132 release execution, and `codex/ai-llm-lab` mutation remain outside this carrier.
- next legal analysis candidate: none before Hardening H1 approval; later same-branch Workstream candidate is SLC-018 Local Provider Registry And Configuration State after the current phase boundary clears. Later provider/model/runtime implementation beyond a named bounded seam, full AI Product Contract import, additional issue-resolution branches, Workspace Runtime Isolation Stage 2, Repo-Wide High-Risk Source Owner Marker Adoption, or another carrier require separate USER approval.
...
Next Legal Analysis Candidate: none before Hardening H1 approval; later same-branch Workstream candidate is SLC-018 Local Provider Registry And Configuration State after the current phase boundary clears. Later provider/model/runtime implementation beyond a named bounded seam, Repo-Wide High-Risk Source Owner Marker Adoption, or another legal carrier requires separate USER approval.
