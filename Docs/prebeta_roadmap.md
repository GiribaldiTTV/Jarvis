...
Next Legal Runtime Step: Workstream WS41 - Dashboard Child-Window Scope Implementation If Admitted. USER Dashboard feedback blocks current acceptance, makes affected WS31-WS36/H1/LV1 proof supporting but stale, keeps LV2 blocked, and requires the remaining bounded Workstream repair/proof sequence before Hardening rerun and refreshed LV1 UTS handoff. WS37 runtime/window/tray safety and NCP regression isolation is green, WS38 Dashboard window ownership/focus/movement safety is green, WS39 Dashboard shell/layout/frame/resize/readability is green, and WS40 Dashboard IA/content/naming/control-hub is green. Dashboard/control panel remains the primary current-branch interface release gate, Overlay/display acceptance remains deferred/non-gating, Core repair remains dependency-only, WS41 child-window implementation requires USER admission if pursued, and package completion remains unclaimed.
...
Current Workstream State: WS40 green - Dashboard runtime/window/tray safety, NCP regression isolation, Dashboard window ownership/focus/movement safety, Dashboard shell/layout/frame/resize/readability, and Dashboard IA/content/naming/control-hub are repaired; affected Dashboard proof remains stale until optional child-window scope and proof repairs pass.
