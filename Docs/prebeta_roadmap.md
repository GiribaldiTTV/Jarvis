...
- next concern: request USER decision on Live Validation Stage 1 real USER-facing shortcut validation and formal UTS generation/handoff, or explicit USER waiver for either gate with a reason. Runtime/provider/model/shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, and `codex/ai-llm-lab` mutation remain outside this carrier.
- next legal analysis candidate after active-client precheck: Live Validation Stage 1 gate completion for `feature/fam-006-dashboard-settings-panel`; PR Readiness can follow only after shortcut validation and UTS are PASS or WAIVED and digested. FAM-007 Workstream implementation, full AI Product Contract import, additional issue-resolution branches, Workspace Runtime Isolation Stage 2, Repo-Wide High-Risk Source Owner Marker Adoption, or another carrier require separate USER approval.
...
Next Legal Analysis Candidate: Live Validation Stage 1 gate completion for `feature/fam-006-dashboard-settings-panel`; PR Readiness can follow only after shortcut validation and UTS are PASS or WAIVED and digested. FAM-007 runtime Workstream entry, Repo-Wide High-Risk Source Owner Marker Adoption, or another legal carrier requires separate USER approval.
