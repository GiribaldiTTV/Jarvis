...
- current branch after `v1.7.0-prebeta` publication: `feature/fam-007-provider-boundary-no-provider-shell`.
- next concern: decide PR Readiness Stage 2 / PR creation for `feature/fam-007-provider-boundary-no-provider-shell` or the next bounded same-branch FAM-007 provider-boundary seam after merge-target reconciliation validates. Real provider SDK integration, model downloads, memory/indexing, voice/Core sync, shortcut/installer work, AI Product Contract full import, GitHub issue creation or closeout, release/tag/artifact work, raw evidence upload/import/linking, PR #129/#132 release execution, and `codex/ai-llm-lab` mutation remain outside this carrier.
- next legal analysis candidate after this reconciliation validates: USER decision on PR Readiness Stage 2 / PR creation for `feature/fam-007-provider-boundary-no-provider-shell`, the next bounded provider-selection/consent seam, or another governed path; merge, later provider/model/runtime expansion, full AI Product Contract import, additional issue-resolution branches, Workspace Runtime Isolation Stage 2, Repo-Wide High-Risk Source Owner Marker Adoption, or another carrier require separate USER approval.
...
Latest Public Prerelease Publication: https://github.com/GiribaldiTTV/Nexus-Desktop-AI/releases/tag/v1.7.0-prebeta
Latest Public Prerelease Title: Pre-Beta v1.7.0
Release-Debt Avoidance Status: Existing exception - PR #129 FAM-006 Dashboard render/layout hardening and PR #132 FAM-006 Dashboard IA/control follow-through merged after v1.7.0-prebeta and remain unreleased implementation work; PR #133 merged the FAM-006 release-support source-truth carrier, while release execution remains pending USER approval.
...
Selected Next Status: USER approved FAM-007 Branch Readiness Stage 2 from current `origin/main` and the first bounded provider/no-provider shell implementation seam. PR #129/#132 release-support truth is merged by PR #133 and release execution remains a separate USER-gated FAM-006 lane.
...
Next Legal Analysis Candidate: USER decision for `feature/fam-007-provider-boundary-no-provider-shell` PR Readiness Stage 2 / PR creation, next same-branch seam, or another governed path; merge, later provider/model/runtime expansion, Repo-Wide High-Risk Source Owner Marker Adoption, or another legal carrier requires separate USER approval.
...
Next-Branch Creation Gate: Cleared only for `feature/fam-007-provider-boundary-no-provider-shell`. Release packaging, PR creation for this FAM-007 branch, merge, GitHub issue creation, model/provider expansion, and other successor branches remain blocked until later USER approval.
...
PR Creation: blocked until USER separately approves PR Readiness Stage 2 for `feature/fam-007-provider-boundary-no-provider-shell`.
