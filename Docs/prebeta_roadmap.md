...
Record State: Current-main reconciliation and refreshed LV1 close-guard / visual-proof repair implementation complete on `feature/fam-006-monitor-groups-sensor-configuration`; PR #164 main truth remains context only.
Minimal Scope: Hardening H1 for refreshed LV1 blockers only: close-while-dirty must continue to queue pendingMonitorAction="close" through the real Manage Monitors close target, Save / Discard / Cancel must preserve and resolve visible draft values correctly for close, and refreshed LV1 must retain Manage Monitors screenshot-sequence or video-style proof for dropdown, guard, delete, empty-state, and 20+ / 100+ scrollbar states.
...
Selection Scope: This branch carries FAM-006 Monitor Groups / Sensor Command Center close-guard and visual-proof repair implementation and next Hardening H1. PR #162/FAM-007 source truth is preserved as current-main context only and remains routed to the FAM-007 lane.
