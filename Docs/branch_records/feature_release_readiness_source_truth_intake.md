...
- Full Docs source-truth reform may be repaired on this standing lane when USER approves the single-merge reform path; `Docs/governance_docs_full_inventory_reform_audit.md` is the audit foundation, backlog remains product registry and pointer layer, roadmap remains stage-breakpoint schedule outline and broad milestone checkpoint reference, branch records remain authority and compact receipts, branch plans remain active runtime engineering plans, workstreams/family dossiers own durable package/slice/proof history, and Git/GitHub/helpers derive live operational truth.
