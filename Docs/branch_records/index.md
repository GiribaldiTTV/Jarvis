...
- family-package Branch Readiness must define product vision, user-facing goal, affected surfaces, data/control model, acceptance criteria, validation proof requirements, User Test Summary strategy, and USER vision questions before implementation; marker-only or scaffold-only proof must not be treated as user-facing product completion
