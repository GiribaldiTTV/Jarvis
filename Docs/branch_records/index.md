...
Active authority belongs to the USER-approved FAM-006 Monitoring and HUD Product Surface Package carrier, currently stopped at the Workstream-to-Hardening phase boundary after WS42 refreshed Dashboard-specific static/live proof and LV1 handoff readiness. WS18 through WS36 remain preserved as historical/supporting repair evidence; WS37, WS38, WS39, WS40, WS41 deferral, and WS42 proof readiness are current repair-chain evidence. Dashboard/control panel is the primary current-branch interface release surface, Overlay/display acceptance is deferred/non-gating, and Core repair is dependency-only. LV2 remains blocked until Hardening rerun, refreshed LV1 handoff, and returned User Test Summary acceptance or waiver pass. Merged `main` remains the validated source baseline before this branch merges; the active branch record must move to historical/no-active posture before PR green so merged-main truth does not retain stale active branch authority.
