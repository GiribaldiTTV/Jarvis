...
- Workstream Stage: `WS26 Green - standalone overlay/display native window, anchored uninteractable mode, and monitor position preservation are implemented and active-client proof backed; bounded Workstream continues through revised validation expansion and Workstream completion reconciliation before Hardening may be considered`
...
- Runtime Implementation State: `WS26 runtime repair complete for the standalone HUD overlay/display native window, anchored uninteractable desktop integration, and monitor position preservation. ORIN Core transparency/non-interference from WS18, dashboard/minimal-HUD renderer-visible split from WS19, dashboard configuration content from WS20, dashboard movement/scrollbar polish from WS21, native minimal-HUD click-through/no-focus proof from WS22, active-user-facing proof/USER overlay vision clarification from WS23, dashboard monitor-management controls from WS24, and edgeless overlay/display canvas from WS25 remain preserved. The dashboard remains a standalone control panel; the HUD overlay/display is now owned by the separate top-level native MonitoringHudOverlayDisplayWindow rather than a dashboard-locked DOM section, so dragging the dashboard does not drag the overlay. Unanchored mode is focusable/clickable with quick monitor edit controls visible; anchored mode is click-through/uninteractable, no-focus/no-activate, hides quick controls, preserves the unanchored monitor placement, and is proved through MONITORING_HUD_STANDALONE_OVERLAY_DISPLAY_WINDOW_READY, MONITORING_HUD_ANCHORED_OVERLAY_UNINTERACTABLE_READY, MONITORING_HUD_OVERLAY_POSITION_PRESERVED_READY, and MONITORING_HUD_UNANCHORED_OVERLAY_EDIT_WINDOW_READY without adding fake telemetry. Revised validation/live proof expansion and Workstream completion reconciliation still require WS27 and WS28 before Hardening may be considered. Future provider-platform parity, external/plugin telemetry, audio/spoken alerts, persona switching, Stream Deck, graphs/history/persistence dashboards, local AI/capability packs, installer work, and ultra-low polling remain deferred.`
...
Rollback Path: revert the WS26 standalone overlay/display native-window commit on `feature/fam-006-monitoring-hud-product-surface` before PR merge; no tags, releases, artifacts, PR, watcher, or `main` mutation are created by this Workstream repair pass.
...
Remaining Implementable Work: `WS27 Validation Live Proof Expansion For Revised Overlay Model; WS28 Workstream Completion Reconciliation And Hardening Handoff`
...
- PKG-006 currently has multiple visible same-branch steps after WS26 (`WS27` validation/live proof expansion; `WS28` Workstream completion reconciliation and Hardening handoff), so this blocker is not active; Workstream must complete those seams before any Hardening phase entry.
...
| `SLC-029` | `PKG-006` | `FAM-006` | Validation and live desktop proof | Admitted | Repair Required | WS18 adds Core transparency/non-interference proof, WS19 adds dashboard/minimal-HUD split proof, WS20 adds dashboard-content proof, WS21 adds movement/scrollbar proof, WS22 adds native minimal-overlay click-through/non-focus proof, WS23 refreshes active-user-facing proof/draft UTS handoff, WS24 adds dashboard monitor-management static/internal proof, WS25 adds edgeless overlay edit-canvas static/internal/live proof, and WS26 adds standalone native overlay/display, anchored-uninteractable, position-preservation, and dashboard-drag-isolation proof. Validators must still expand for final revised-overlay proof, Hardening rerun, Live Validation rerun, and UTS acceptance | `BR-S2-S6`; `WS6`; `WS8`; `WS17`; `H1`; `LV1-R1`; `WS18`; `WS19`; `WS20`; `WS21`; `WS22`; `WS23`; `WS24`; `WS25`; `WS26`; `WS27`; `dev/orin_monitoring_hud_internal_sandbox_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
## Remaining Bounded Workstream Repair Plan After WS26
