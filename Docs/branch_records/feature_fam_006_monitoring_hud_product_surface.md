...
- Workstream Stage: `WS24 Green - dashboard monitor management and overlay mode controls are implemented and marker-backed; bounded Workstream continues through edgeless overlay edit mode, anchored overlay desktop integration, validation expansion, and Workstream completion reconciliation before Hardening may be considered`
...
- Runtime Implementation State: `WS24 runtime repair complete for dashboard monitor management and overlay mode controls. ORIN Core transparency/non-interference from WS18, dashboard/minimal-HUD renderer-visible split from WS19, dashboard configuration content from WS20, dashboard movement/scrollbar polish from WS21, native minimal-HUD click-through/non-focus proof from WS22, and active-user-facing proof/USER overlay vision clarification from WS23 remain preserved. The dashboard now exposes HUD display enable/disable, anchor/unanchor posture, create-monitor affordance, selected-monitor edit state, monitor enable/disable, per-monitor polling selection, monitor quick actions, and runtime marker proof for MONITORING_HUD_MONITOR_MANAGEMENT_READY without adding fake telemetry. USER's edgeless overlay/display expectation still requires WS25 and WS26 before Workstream completion reconciliation. Future provider-platform parity, external/plugin telemetry, audio/spoken alerts, persona switching, Stream Deck, graphs/history/persistence dashboards, local AI/capability packs, installer work, and ultra-low polling remain deferred.`
...
Rollback Path: revert the WS24 dashboard monitor-management and overlay-mode-controls commit on `feature/fam-006-monitoring-hud-product-surface` before PR merge; no tags, releases, artifacts, PR, watcher, or `main` mutation are created by this Workstream repair pass.
...
Remaining Implementable Work: `WS25 Edgeless Overlay Edit Canvas And Monitor Layout; WS26 Anchored Overlay Uninteractable Desktop Integration And Position Preservation; WS27 Validation Live Proof Expansion For Revised Overlay Model; WS28 Workstream Completion Reconciliation And Hardening Handoff`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SLC-016` | `PKG-006` | `FAM-006` | HUD visual and user-facing monitoring surface | Admitted | Repair Required | WS19 separates the dashboard/configuration surface from the minimal anchored HUD overlay at DOM/runtime-marker/proof level, and WS22 adds the actual separate native minimal HUD overlay. WS23 records USER clarification that the final display is an edgeless overlay/canvas with monitor group cards, not a small fixed widget. WS24 implements dashboard monitor group-card management. User-facing HUD remains repair-required until WS25-WS27 implement/prove edgeless overlay edit mode, anchored uninteractable mode, refreshed live validation, and UTS acceptance | `BR-S2-S1`; `WS1`; `WS7`; `WS9`; `WS17`; `LV1-R1`; `WS19`; `WS22`; `WS23`; `WS24`; `WS25`; `WS26`; `WS27`; `dev/orin_monitoring_hud_surface_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
| `SLC-029` | `PKG-006` | `FAM-006` | Validation and live desktop proof | Admitted | Repair Required | WS18 adds Core transparency/non-interference proof, WS19 adds dashboard/minimal-HUD split proof, WS20 adds dashboard-content proof, WS21 adds movement/scrollbar proof, WS22 adds native minimal-overlay click-through/non-focus proof, WS23 refreshes active-user-facing proof/draft UTS handoff, and WS24 adds dashboard monitor-management static/internal proof. Validators must still expand for the edgeless overlay edit canvas, anchored uninteractable desktop mode, Hardening rerun, Live Validation rerun, and UTS acceptance | `BR-S2-S6`; `WS6`; `WS8`; `WS17`; `H1`; `LV1-R1`; `WS18`; `WS19`; `WS20`; `WS21`; `WS22`; `WS23`; `WS24`; `WS27`; `dev/orin_monitoring_hud_internal_sandbox_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
