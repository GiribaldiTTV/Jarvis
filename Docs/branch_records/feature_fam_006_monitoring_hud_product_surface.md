...
- Workstream Stage: `WS23 Green for active-user-facing proof and draft UTS handoff, with USER HUD overlay vision rebaseline recorded; bounded Workstream continues through dashboard monitor management, edgeless overlay edit mode, anchored overlay desktop integration, validation expansion, and Workstream completion reconciliation before Hardening may be considered`
...
- Runtime Implementation State: `WS23 proof/handoff repair complete and USER HUD overlay vision clarification recorded. ORIN Core transparency/non-interference from WS18, dashboard/minimal-HUD renderer-visible split from WS19, dashboard configuration content from WS20, dashboard movement/scrollbar polish from WS21, and native minimal-HUD click-through/non-focus proof from WS22 remain preserved. Active-user-facing live proof now uses a foreground/user-observable client run with full virtual-desktop screenshot evidence, copied USER-inspectable raw PNG evidence, interaction manifest proof, and a refreshed draft User Test Summary handoff. USER clarified that the dashboard is the HUD control panel; monitors are organizational group/title cards for underlying monitored things; and the HUD overlay/display should behave like an edgeless snipping-tool-style placement canvas. Unanchored overlay mode must be focusable, movable, resizable, scrollable, and show monitor move/resize/edit affordances. Anchored overlay mode must be uninteractable/click-through, non-focusable, non-interfering with desktop shortcuts, hide quick controls, preserve monitor positions, and use only unobtrusive framing/watermark identity. Remaining Workstream product work therefore continues through WS24, WS25, WS26, WS27, and WS28 before any Hardening handoff is legal. Future provider-platform parity, external/plugin telemetry, audio/spoken alerts, persona switching, Stream Deck, graphs/history/persistence dashboards, local AI/capability packs, installer work, and ultra-low polling remain deferred.`
...
Rollback Path: revert the WS23 validation/live proof expansion, draft UTS handoff, and HUD overlay vision rebaseline commit on `feature/fam-006-monitoring-hud-product-surface` before PR merge; no tags, releases, artifacts, PR, watcher, or `main` mutation are created by this Workstream repair pass.
...
Remaining Implementable Work: `WS24 HUD Dashboard Monitor Management And Overlay Mode Controls; WS25 Edgeless Overlay Edit Canvas And Monitor Layout; WS26 Anchored Overlay Uninteractable Desktop Integration And Position Preservation; WS27 Validation Live Proof Expansion For Revised Overlay Model; WS28 Workstream Completion Reconciliation And Hardening Handoff`
...
- PKG-006 currently has multiple visible same-branch steps after WS23 (`WS24` dashboard monitor management and overlay mode controls; `WS25` edgeless overlay edit canvas and monitor layout; `WS26` anchored overlay uninteractable desktop integration and position preservation; `WS27` validation/live proof expansion; `WS28` Workstream completion reconciliation and Hardening handoff), so this blocker is not active; Workstream must complete those seams before any Hardening phase entry.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SLC-016` | `PKG-006` | `FAM-006` | HUD visual and user-facing monitoring surface | Admitted | Repair Required | WS19 separates the dashboard/configuration surface from the minimal anchored HUD overlay at DOM/runtime-marker/proof level, and WS22 adds the actual separate native minimal HUD overlay. WS23 records USER clarification that the final display is an edgeless overlay/canvas with monitor group cards, not a small fixed widget. User-facing HUD remains repair-required until WS24-WS27 implement/prove dashboard monitor management, edgeless overlay edit mode, anchored uninteractable mode, refreshed live validation, and UTS acceptance | `BR-S2-S1`; `WS1`; `WS7`; `WS9`; `WS17`; `LV1-R1`; `WS19`; `WS22`; `WS23`; `WS24`; `WS25`; `WS26`; `WS27`; `dev/orin_monitoring_hud_surface_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
| `SLC-029` | `PKG-006` | `FAM-006` | Validation and live desktop proof | Admitted | Repair Required | WS18 adds Core transparency/non-interference proof, WS19 adds dashboard/minimal-HUD split proof, WS20 adds dashboard-content proof, WS21 adds movement/scrollbar proof, WS22 adds native minimal-overlay click-through/non-focus proof, and WS23 refreshes active-user-facing proof/draft UTS handoff. Validators must still expand for the revised dashboard monitor model, edgeless overlay edit canvas, anchored uninteractable desktop mode, Hardening rerun, Live Validation rerun, and UTS acceptance | `BR-S2-S6`; `WS6`; `WS8`; `WS17`; `H1`; `LV1-R1`; `WS18`; `WS19`; `WS20`; `WS21`; `WS22`; `WS23`; `WS27`; `dev/orin_monitoring_hud_internal_sandbox_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
- USER Overlay Vision Clarification: `Dashboard is the HUD control panel for enabling/disabling the overlay/display, anchoring/unanchoring, creating monitors, editing monitors, changing monitor polling rates, and enabling/disabling monitors. Monitors are organizational group/title cards for underlying monitored things. The HUD overlay/display should behave like an edgeless snipping-tool-style placement canvas: unanchored mode is a normal focusable, movable, resizable, scrollable edit window with monitor quick-edit buttons; anchored mode is uninteractable/click-through, non-focusable, non-interfering with desktop shortcuts, hides quick-edit affordances, preserves monitor positions, and uses only unobtrusive frame/watermark identity.`
...
## Remaining Bounded Workstream Repair Plan After WS23

- `WS24 - HUD Dashboard Monitor Management And Overlay Mode Controls`: implement and prove dashboard-level HUD display enable/disable, anchor/unanchor posture, monitor create/edit affordances, monitor polling/edit state, and monitor enabled/disabled state without adding fake telemetry.
- `WS25 - Edgeless Overlay Edit Canvas And Monitor Layout`: implement and prove the HUD overlay/display as an edgeless placement canvas with unobtrusive unanchored border, normal focus/click behavior, resizing/moving, scrollbar posture, monitor card movement/resize, quick edit affordances, snapping, and edge-to-edge monitor placement posture.
- `WS26 - Anchored Overlay Uninteractable Desktop Integration And Position Preservation`: implement and prove anchored overlay mode as uninteractable/click-through, non-focusable, non-interfering with desktop shortcuts, quick-controls hidden, monitor positions preserved from unanchored mode, and watermark-only program/persona identity.
- `WS27 - Validation Live Proof Expansion For Revised Overlay Model`: expand static/internal/live validators and full-desktop screenshot proof to verify dashboard control panel role, monitor model, edgeless overlay edit mode, anchored uninteractable mode, monitor position persistence, and no fake telemetry.
