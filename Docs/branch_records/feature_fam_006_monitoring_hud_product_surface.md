...
- Workstream Stage: `WS25 Green - edgeless overlay edit canvas and monitor layout are implemented and active-client proof backed; bounded Workstream continues through anchored overlay desktop integration, validation expansion, and Workstream completion reconciliation before Hardening may be considered`
...
- Runtime Implementation State: `WS25 runtime repair complete for edgeless overlay edit canvas and monitor layout. ORIN Core transparency/non-interference from WS18, dashboard/minimal-HUD renderer-visible split from WS19, dashboard configuration content from WS20, dashboard movement/scrollbar polish from WS21, native minimal-HUD click-through/non-focus proof from WS22, active-user-facing proof/USER overlay vision clarification from WS23, and dashboard monitor-management controls from WS24 remain preserved. The dashboard remains a control panel while the HUD overlay/display now has a separate edge-to-edge snipping-tool-style placement canvas with unobtrusive frame/watermark identity, monitor group cards, quick edit affordance posture, and static/internal/live proof through MONITORING_HUD_EDGELESS_OVERLAY_CANVAS_READY without adding fake telemetry. Anchored uninteractable desktop integration and position preservation still require WS26 before Workstream completion reconciliation. Future provider-platform parity, external/plugin telemetry, audio/spoken alerts, persona switching, Stream Deck, graphs/history/persistence dashboards, local AI/capability packs, installer work, and ultra-low polling remain deferred.`
...
Rollback Path: revert the WS25 edgeless overlay edit-canvas commit on `feature/fam-006-monitoring-hud-product-surface` before PR merge; no tags, releases, artifacts, PR, watcher, or `main` mutation are created by this Workstream repair pass.
...
Remaining Implementable Work: `WS26 Anchored Overlay Uninteractable Desktop Integration And Position Preservation; WS27 Validation Live Proof Expansion For Revised Overlay Model; WS28 Workstream Completion Reconciliation And Hardening Handoff`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `SLC-016` | `PKG-006` | `FAM-006` | HUD visual and user-facing monitoring surface | Admitted | Repair Required | WS19 separates the dashboard/configuration surface from the minimal anchored HUD overlay at DOM/runtime-marker/proof level, and WS22 adds the actual separate native minimal HUD overlay. WS23 records USER clarification that the final display is an edgeless overlay/canvas with monitor group cards, not a small fixed widget. WS24 implements dashboard monitor group-card management. WS25 implements/proves the edgeless overlay/display canvas and monitor group-card layout. User-facing HUD remains repair-required until WS26-WS27 implement/prove anchored uninteractable mode, refreshed live validation, and UTS acceptance | `BR-S2-S1`; `WS1`; `WS7`; `WS9`; `WS17`; `LV1-R1`; `WS19`; `WS22`; `WS23`; `WS24`; `WS25`; `WS26`; `WS27`; `dev/orin_monitoring_hud_surface_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
| `SLC-029` | `PKG-006` | `FAM-006` | Validation and live desktop proof | Admitted | Repair Required | WS18 adds Core transparency/non-interference proof, WS19 adds dashboard/minimal-HUD split proof, WS20 adds dashboard-content proof, WS21 adds movement/scrollbar proof, WS22 adds native minimal-overlay click-through/non-focus proof, WS23 refreshes active-user-facing proof/draft UTS handoff, WS24 adds dashboard monitor-management static/internal proof, and WS25 adds edgeless overlay edit-canvas static/internal/live proof. Validators must still expand for anchored uninteractable desktop mode, Hardening rerun, Live Validation rerun, and UTS acceptance | `BR-S2-S6`; `WS6`; `WS8`; `WS17`; `H1`; `LV1-R1`; `WS18`; `WS19`; `WS20`; `WS21`; `WS22`; `WS23`; `WS24`; `WS25`; `WS27`; `dev/orin_monitoring_hud_internal_sandbox_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
## Remaining Bounded Workstream Repair Plan After WS25
