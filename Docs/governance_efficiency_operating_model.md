...
The USER-facing planning files and `START_HERE.md` should avoid branch HEAD, SHA, origin/main, merge base, ZIP hash, and similar technical packet metadata. Technical metadata belongs in helper output, validator output, Codex chat digest, or external governance state.
...
When Codex asks USER to inspect repo files, review a generated dossier, approve a planning packet, or compare a source-truth reform surface, Codex may create or refresh the USER-facing stable local review hub for the active worktree.

The USER Review Hub bundle is a temporary USER/ChatGPT review aid, not durable repo evidence, not a posterity archive, and not a reporting surface for later phases. Its job is to provide a concrete, easy-to-upload copy of the current BP1 Branch Vision and BP2 Branch Plan so USER and ChatGPT can validate Codex's intended direction before implementation. After USER accepts, revises, rejects, defers, or waives the packet, Codex must fold the accepted outcome, constraints, and decisions into the proper source-truth owner. Governance must not treat the USER hub folder or zip as canon, must not retain it for posterity, and must not keep maintaining it after the review loop is complete.

The USER Review Hub bundle must:

- live under one stable local root, `C:\Nexus USER`
...
- refresh the same worktree-labeled child folder and sibling zip instead of creating a new top-level folder or uniquely named upload zip for each review packet
...
- include a `START_HERE.md` file as a plain-language index only, with `Review Purpose:`, `Temporary USER/ChatGPT Review Aid:`, `Active USER Hub:`, `Source-Truth Fold-In:`, `Technical Proof Location:`, `Review Order`, `Exact USER Decision This Bundle Supports:`, `Pending USER Decisions`, and copied source paths
...
For BP3 / Workstream Entry, the USER Review Hub bundle is required before USER green-lights implementation when the branch has runtime, user-facing, source-truth, helper/validator, or workflow impact and BP1/BP2 review has not been explicitly waived. The bundle must copy the accepted or waived branch vision, active external Branch Runtime Engineering Plan or approved transition Branch Engineering Plan, Element-to-Phase Proof Matrix owner, branch authority evidence pointer, relevant Nexus/family vision files, UFD/change-intent surfaces when applicable, and any other source-truth files the USER needs to inspect. The BP3 / Workstream Entry digest must report the folder path, copied files, `USER Branch Plan Review Gate` status, `USER Review Packet Finding:`, implementation options, whole-package analysis status when multiple slices or seams are admitted, technical proof metadata, and the durable source-truth owner that will receive the accepted packet outcome. `USER Review Packet Finding:` must name `START_HERE.md`, `USER_BRANCH_PLAN_REVIEW.md`, the review packet freshness result, digest status, and waiver/blocker status; technical branch/commit/zip proof stays in helper output or Codex digest. `USER_BRANCH_PLAN_REVIEW.md` is the BP2 USER-facing engineering plan derived from accepted or waived BP1; it must not replace the BP1 product/design vision review. Workstream implementation cannot begin until USER response is attached or inserted and Codex digests it into the Workstream Entry decision, or until USER explicitly waives that response/digest requirement. The bundle supports USER accepting, revising, deferring with waiver, rejecting, or requesting more analysis before implementation begins; it does not preserve the accepted decision after source truth is updated.

Branch Planning review reinforcement: BP1 / `USER_BRANCH_VISION_REVIEW.md` is the USER-facing product/design branch vision contract. BP2 / `USER_BRANCH_PLAN_REVIEW.md` is the USER Branch Plan Contract and engineering plan derived from accepted or waived BP1. BP1 helps USER decide whether the End-State Vision, Visual / Functional Walkthrough, planned feature look, feel, flow, and Nexus fit are right before bounded Workstream implementation. BP2 helps USER decide whether the engineering route correctly builds that accepted vision, with implementation package, scope size, SLC/seam route, affected surfaces/files, validators/helpers, proof requirements, H1/LV/UTS, rollback/safety, risks, future-gated boundaries, USER response, Codex digest, implementation constraints, Vision Delta / Source-Truth Impact, Contract Change Log, Contract Completion Checklist, waiver path, and Workstream Entry result area. `Contract Status` must be `Complete` or `Waived by USER` for required BP1/BP2 gates before implementation approval is legal; `Draft`, `Pending USER Response`, `Pending Codex Digest`, and `Pending USER Confirmation` block implementation. If USER feedback changes branch direction, feature shape, UI behavior, workflow, scope, boundaries, or seam order, Codex must update the durable source-truth owner, refresh the local USER hub packet/ZIP only for the active review loop, set Contract Status to Pending USER Confirmation, and wait for USER confirmation or explicit waiver.

For governance review or PR-readiness review, the USER Review Hub bundle should be self-checking through helper output, not through USER-facing technical fields. The helper must verify the actual file count present in the worktree review folder, copied repo-file count, intended copied repo-file count, stale extra-file count, stable zip replacement, final zip bytes, and stale-output guard before reporting PASS. Use `dev/orin_user_review_bundle.py` for repeatable local bundle creation; the helper defaults to `C:\Nexus USER\<worktree-label>` and should not require USER to name a new folder for active worktrees. The helper must also overwrite a stable sibling zip export at `C:\Nexus USER\<worktree-label>.zip` from the freshly refreshed worktree folder and print technical proof in helper output using `Review export zip:`, `Review export zip SHA256:`, `Upload this file:`, `Source Branch:`, `Source HEAD:`, and `origin/main:` fields; USER-uploadable review zips created outside this helper are stale-risk evidence and must be regenerated before review or PR Readiness. `--review-root-name`, `--worktree-label`, or legacy `--folder-name` customizations require `--allow-custom-review-path` plus a helper-output reason. If the local USER hub path cannot be created or the folder/zip cannot be created, stop with `USER Review Hub Bundle Missing` and return the exact blocker plus the copy command or helper command USER can run.
