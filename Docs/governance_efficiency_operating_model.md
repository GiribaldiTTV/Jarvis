...
- include a `START_HERE.md` file with `Review Purpose:`, source repo, `Review Export Zip:`, `Validation Summary:`, `Review Order`, `Exact USER Decision This Bundle Supports:`, `Pending USER Decisions`, copied source paths, explicit bundle/copy file counts, and an extra-file count for stale artifacts left in non-cleared folders
...
For governance review or PR-readiness review, the local USER hub packet should be self-checking: `Bundle File Count:` reports the actual file count present in the worktree review folder after copy plus `START_HERE.md`, `Copied File Count:` counts copied repo files only, `Expected File Count:` must match the intended copied repo-file count, and `Extra Bundle File Count:` reports stale or unrelated files that remain when a bundle is refreshed without clearing. Use `dev/orin_user_review_bundle.py` for repeatable local bundle creation; the helper defaults to `C:\Nexus USER\<worktree-label>` and should not require USER to name a new folder for active worktrees. The helper must also overwrite a stable zip export at `C:\Nexus USER\<worktree-label>.zip` from the freshly refreshed worktree folder and record `Review Export Zip:` in `START_HERE.md`; USER-uploadable review zips created outside this helper are stale-risk evidence and must be regenerated before review or PR Readiness. `--review-root`, `--worktree-label`, or legacy one-off folder customizations require `--allow-custom-review-path` plus a recorded reason. If the local USER hub folder/zip cannot be created, stop with `USER Review Hub Missing` and return the exact blocker plus the helper command USER can run.
