...
For governance review or PR-readiness review, the local USER hub helper should be self-checking while keeping USER-facing files readable. Use `dev/orin_user_review_bundle.py` for repeatable local bundle creation; the helper defaults to `C:\Nexus USER\<worktree-label>` and should not require USER to name a new folder for active worktrees. The helper must overwrite a stable zip export at `C:\Nexus USER\<worktree-label>.zip` from the freshly refreshed worktree folder and validate the folder/ZIP file list directly. The helper must also create the USER upload artifact as a timestamped byte-identical copy beside the stable zip, named `C:\Nexus USER\<worktree-label>__YYYYMMDD-HHMMSS.zip`, so repeated uploads do not collide on same-named artifacts in ChatGPT or other review surfaces. The timestamped copy is the upload instruction; the stable folder and stable zip remain the canonical local USER hub pair. The helper must report the exact timestamped path and SHA256 in helper output or Codex digest. USER-facing files must not center active branch status, current commit, current baseline, validation status, PR state, worktree status, ZIP SHA256, ZIP hash, packet hash, upload hash, or similar technical proof metadata. USER-uploadable review zips created outside this helper are stale-risk evidence and must be regenerated before review or PR Readiness. `--review-root`, `--worktree-label`, or legacy one-off folder customizations require `--allow-custom-review-path` plus a recorded reason. If the local USER hub folder/zip cannot be created, stop with `USER Review Hub Missing` and return the exact blocker plus the helper command USER can run.
