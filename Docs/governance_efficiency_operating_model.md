...
USER_BRANCH_PLAN_REVIEW.md product/design reinforcement: the review must also include `End-State Vision`, `Visual / Functional Walkthrough`, surface map, implementation options with pros/cons/risk, Codex recommended direction, current branch scope, future-gated scope, SLC package plan, USER decisions, Codex response digest, and Workstream Entry result area. The artifact is meant to help USER decide whether the planned feature looks and feels right before bounded Workstream implementation.
