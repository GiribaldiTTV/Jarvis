...
| --- | --- | --- | --- | --- | --- |
| `Docs/feature_backlog.md` | compact FAM registry, priority, and canonical pointers | detailed vision narrative, BP/LV/SLC/UTS state, branch lifecycle, PR/release-window state, active dependency queue | durable compact registry | Git/GitHub/helpers, active external branch plan, or `D:\Nexus Desktop AI\Governance State` | prove replacement registry owner and pointer preservation before any move/delete |
...
| USER evidence under `D:\Nexus Desktop AI\USER` | selected review content and materially needed retained evidence | repo source truth, active operational ledger, durable canon by themselves | continuing workstream evidence collection | current external USER lifecycle owner; product owners define review content | review transport and evidence retirement follow the external owner; no whole-collection refresh or purge |
...
- active Live Validation / UTS posture: active external branch plan or `D:\Nexus Desktop AI\Governance State`
...
- active adoption ledger: `D:\Nexus Desktop AI\Governance State\branches\<branch_slug>\adoption_reconciliation.md` or the active external branch plan
- USER-facing RAR packet: selected current evidence in the approved workstream collection, delivered through the external USER lifecycle owner; no collection is derived from the worktree basename
...
`RAR Live Adoption Ledger In Repo` blocks when Codex stores active RAR rows in backlog, roadmap, branch records, family visions, FFVs, UIREF files, or repo branch plans instead of external state or USER review evidence. `RAR Evidence Lost To Chat Digest` blocks when RAR findings exist but no active external ledger or USER packet is produced for USER-reviewable decisions. `RAR Packet Missing For USER Judgment` blocks when RAR needs USER visual/product judgment but provides only a chat digest, validator output, or helper log instead of selected current RAR review evidence and pending transport delivered through the external USER lifecycle owner.

RAR issue-candidate durability follows the same split. Repo docs own the durable issue-candidate schema, legal disposition vocabulary, consolidation schema, blocker names, helper contracts, and compact receipts. Active candidate rows, current GitHub mappings, live issue state, branch-specific blocking/non-blocking carry-forward, issue-group recommendations, USER create/defer/split/merge/reject/waive choices, and packet evidence belong in the active branch external state, USER packet, helper output, Codex digest, Git/GitHub evidence, or screenshot/video evidence roots. The current packet copy must be a USER-facing decision surface, not a repo live-state ledger: unresolved active candidates belong in the selected primary issue-candidate decision surface, with traceable supporting aids and source-truth context; the external USER lifecycle owner governs evidence organization and selection. `Packeted only` is not a durable disposition. Deterministic GitHub snapshot/live-read evidence may prove mapped issue state, but repo docs must not become the live issue ledger and GitHub mutation remains USER-gated. `RAR Issue Candidate Durability Missing`, `RAR Issue Candidate Disappeared From Active Packet`, `RAR Issue Candidate Packeted Only`, `RAR GitHub Issue State Unknown`, `RAR GitHub Issue Mapping Stale`, `Issue Candidate Table Only In Copied Context`, and `Issue Candidate Consolidation Missing` block when active issue-candidate facts cannot be traced from originating RAR evidence through current USER-facing disposition without turning repo docs into an active live ledger.
...
- Branch candidate proposals: `D:\Nexus Desktop AI\Governance State\branches\<branch_slug>\reference_candidates\...`
- Optional generated aggregate report: `D:\Nexus Desktop AI\Governance State\reference_standards\REFERENCE_CANDIDATE_SYNC_REPORT.md`
- Optional generated collision report: `D:\Nexus Desktop AI\Governance State\reference_standards\REFERENCE_COLLISION_REVIEW.md`
...
Repo validators running in GitHub Actions or on clean clones validate durable repo truth only. They must not require access to `D:\Nexus Desktop AI\Governance State`.
...
- `D:\Nexus Desktop AI\Governance State`
...
When Codex asks USER to inspect repo files, review a dossier, approve a planning packet, or compare source truth, deliver the review through the installed external `policy/POLICY.md` and `policy/USER_PUBLICATION_HYGIENE.json`. This applies to every phase, including RAR, Branch Planning, Workstream, Hardening, Live Validation, PR Readiness and release review.

The external owner selects one of the four approved continuing workstream evidence collections. Never derive a new collection from a worktree basename, caller label or review status. Evidence-set organization, current selection, ZIP creation/path/timestamp/safety/parity, one-current-transport enforcement, and transport/evidence retirement all belong to that owner. A review selects current evidence; it neither clears the collection nor packages unrelated retained evidence.

Repository product/phase owners define when review is required, the primary decision and its required content. The primary review or its entrypoint must identify `Review Purpose:`, `Local USER Hub Folder:` using the externally selected location, `Review Order`, `USER Decision This Packet Supports:`, `Pending USER Decisions`, and traceable source paths. Select the relevant context the USER needs; preserve source traceability and refresh stale decision content when its inputs change. A review packet never replaces source truth, validation proof, a commit or an authority record. Existing navigation/content filenames may organize an evidence set, but they do not impose a whole-collection layout or lifecycle.
