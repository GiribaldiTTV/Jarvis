...
RAR-triggered LV review must use non-circular visual proof. Helper green, validator green, marker files, manifest existence, attractive screenshots, or prior branch acceptance cannot by themselves prove adoption of a newly merged UIREF, Project Vision, FAM-002, proof, or packet standard. The RAR/LV handoff must compare rendered screenshots, video, or ordered frames against the accepted reference, code-to-visual trace, backend/state ownership, Vision-To-Proof Matrix, the NDAI Product Experience Contract, and USER visual judgment where required. If an owned surface is `UNPROVEN`, `PARTIAL`, `NONCONFORMING`, `REFERENCE GAP`, or `SOURCE-TRUTH GAP`, the UTS handoff remains blocked or routes to an explicit USER decision instead of returning green.
