...
Computer Use is prohibited for Nexus LV/UTS validation and runtime-control work. It must not perform or support inspection, capture, target discovery, control assistance, launcher activation, UI interaction, evidence generation, or cleanup. If Computer Use participates, the LV/UTS run is invalid, its gating artifacts must be excluded, `Computer Use Validation Path Prohibited` must be recorded, and Codex must restart through the governed visible real-cursor human-client path. Codex owns launcher and UI operation for validation; USER review, acceptance, or waiver does not make the USER responsible for performing blocked interaction steps.
