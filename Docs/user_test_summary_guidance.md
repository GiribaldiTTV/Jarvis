...
- A helper PASS, real-input proof PASS, screenshot/video proof PASS, or desktop `User Test Summary.txt` export is not final Live Validation green while the exported UTS remains a draft/handoff/`NOT RETURNED RESULTS` copy.
- While the exported UTS has not been returned, source truth must keep `Next Legal Phase: Live Validation`, list `User Test Summary Results Pending`, and mark PR Readiness blocked.
