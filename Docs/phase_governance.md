...
### Stale-Snapshot Investigation Mutation Waiver

The only exception to the mutation side of `Origin/Main Advanced Rebaseline Required` is a USER-approved `Stale-Snapshot Investigation Mutation Waiver`. This waiver exists for forensic investigation passes where current-main reconciliation would alter the as-found evidence that the USER wants Codex to inspect. It is not a general rebaseline bypass.

The waiver must explicitly name the worktree, branch, current `HEAD`, current `origin/main`, the fact that rebaseline/reconciliation is deferred, the investigation scope, allowed investigation-support file classes, required stale-snapshot labels, expiration or stop condition, required validation, explicit non-includes, and still-pending USER decisions.

Allowed under this waiver:

- read and preserve as-found branch evidence
- run existing read-only or evidence-producing helpers
- patch investigation-support helpers, validators, proof tools, stale-scan tools, screenshot/evidence tools, coverage tools, packet-generation tools, or USER review findings packets required to expose and document the failure
- commit and push those investigation-support changes on the active branch when validation is green and the changed files stay inside the waived scope

Forbidden under this waiver:

- product/runtime/UI behavior fixes
- Workstream, Hardening, Live Validation, UTS, PR Readiness, Release Readiness, merge, release, issue closeout, or phase-green advancement
- cross-worktree mutation
- Governance, sibling-family, neutral-main, provider/model/private, external-state, or protected-main mutation unless separately named by a legal carrier
- presenting stale-snapshot evidence as current-main-reconciled proof

Every packet, helper output, validator output, and Codex digest created under this waiver must state `Stale-Snapshot Investigation Waiver: Granted`, `Rebaseline/Reconciliation Deferred: YES`, `Baseline HEAD:`, `Observed origin/main:`, and `Findings May Be Affected By Deferred Reconciliation: YES`. Before product repair, phase advancement, PR Readiness, merge, release, or closeout can resume, the branch must clear normal current-main reconciliation rules unless a later source-truth owner creates a more specific legal path.
...
For desktop UI branches, this inspection must be human-client faithful when the USER will operate visible tray/menu/window behavior. Codex cannot mark Live Validation Stage 1 green from app-side callbacks, fake/offscreen models, marker-only proof, screenshot-only proof, hidden app activation calls, direct handler calls, internal signal dispatch, synthetic tray/menu activation, launcher foreground signals, shell-executed shortcuts, direct target launch, private/dev launcher paths, scripted helper clicks standing in for the Codex desktop-control session, or blind taskbar/tray coordinate probing. When Computer Use is available, the final LV1 handoff requires a manifest and photo/video evidence from that Computer Use pass: visible exact-shortcut click proof, visible tray/menu selection, mouse/cursor interaction evidence, window move/resize/open/close evidence where applicable, screenshot or frame-sequence artifacts, and Codex's own visual review of every issue-grounded UTS item. UIAutomation and scripted human-client helpers may support or preflight the pass, but they do not replace Computer Use as the primary proof path unless USER explicitly accepts a manual-validation blocker for that run. If a Computer Use action accidentally opens or changes an unrelated app/window, the pass is invalidated for that interaction, the accidental app/window must be cleaned up only with USER approval where needed, and the proof route must be repaired before continuing. Missing Computer Use or human-client evidence is a Live Validation failure unless USER explicitly waives it.
...
- Tray, taskbar, hidden-overflow, and notification-area targets require target-acquisition proof before PASS. A visible tooltip, the USER manually opening the menu, read-only accessibility output, runtime logs that say the icon exists, or a named icon rectangle can prove identity/supporting context, but they cannot prove Codex-operated control-surface success. The PASS condition is Codex-owned visible user-level input opening the intended menu/control and photo/video or ordered frame-sequence evidence showing the resulting NDAI menu or window. If the tray/taskbar target is visible but not exposed as a Computer Use targetable window, Codex must attempt the bounded visible desktop coordinate fallback before asking for USER waiver. If multi-monitor coordinate translation, screenshot origin drift, current-cursor mismatch, shell/taskbar target ambiguity, hidden-overflow uncertainty, or fallback movement prevents safe acquisition, stop on `Live Validation Control-Surface Acquisition Blocked`; do not keep probing nearby taskbar coordinates, do not mark product failure unless runtime evidence proves the app ignored the correctly targeted input, and do not use USER manual confirmation as green proof.
