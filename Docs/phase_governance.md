...
USER Branch Plan Review Gate: Workstream Entry must wrap the branch vision, Branch Runtime Engineering Plan, Element-to-Phase Proof Matrix, Hardening plan, Live Validation / UTS plan, open USER questions, USER design review questions, Codex recommendations, implementation options, alternatives/tradeoffs, accepted scope, deferred scope, rejected scope, and exact USER decision needed into a plain-language `USER Branch Plan Review` packet before runtime/user-facing/source-truth implementation begins or resumes. Required digest fields include `Plain-Language Branch Goal:`, `Planned User-Facing Outcome:`, `USER Design Review Questions:`, `USER Review Response:`, `Codex Response Digest:`, and `USER Review Packet Finding:` so USER can inspect what Codex intends to build before implementation. `USER Review Packet Finding:` must explicitly say whether the Desktop USER Review Packet was loaded and digested, waived by exact USER/source-truth text, missing, stale, shallow, or blocking; it must name `START_HERE.md`, `USER_BRANCH_PLAN_REVIEW.md`, and the exported zip when a Desktop bundle is required, and it must compare packet `Source HEAD` / `Review Export Zip Source HEAD` to the current branch `HEAD` before implementation approval. `USER_BRANCH_PLAN_REVIEW.md` is a user-facing design and vision review artifact: it must present Codex's proposed implementation approach, feature behavior, accessible surfaces, options/tradeoffs, and concrete questions the USER may answer in order or generally. USER's response must be attached or inserted into the packet or Workstream Entry digest, and Codex must digest that response before bounded Workstream implementation begins; the only bypass is an explicit USER waiver recorded in both `USER Review Response:` and `Codex Response Digest:`. The packet is the USER-facing checkpoint for "what Codex intends to build, how it should behave/look, where it will live, how it will be proven, and what questions remain"; it does not create a separate permanent planning ledger. Required disposition values are `Accepted by USER`, `Revised by USER`, `Deferred With Waiver`, `Rejected by USER`, and `Needs USER Decision`. Implementation remains blocked on `USER Branch Plan Review Missing`, `USER Review Packet Stale`, or `USER Review Packet Not Digested` until the review is accepted, revised by USER, deferred with waiver, refreshed, or until source truth records an explicit USER waiver. If later implementation proves the accepted plan too narrow, stale, or wrong, Codex must stop with a Vision Question Digest, Branch Plan Review addendum, or Branch Plan Revision Packet according to the Level 1 / Level 2 / Level 3 question severity model.

USER Branch Plan Review product/design format reinforcement: the packet must include `End-State Vision:`, `Visual / Functional Walkthrough:`, `Surface Map:`, `Implementation Options:`, `Recommended Direction:`, `Current Branch Scope:`, `Future-Gated Scope:`, `SLC Package Plan:`, `USER Decisions Needed:`, and `Workstream Entry Result:`. These fields are required so the review shows what the feature will look like, how it will function, which surfaces are touched, which options USER can choose, and why Codex recommends the selected path before Workstream implementation begins.
