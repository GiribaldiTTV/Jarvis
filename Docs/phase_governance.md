...
Formal Next Legal Phase Digest is required whenever a phase packet stops for USER approval. The response must include a `Next Legal Phase Digest` with `Current Phase:`, `Next Legal Phase:`, `Why This Phase Is Next:`, `Approval Required:`, `Exact USER Approval Text:`, `Allowed Scope:`, `Explicit Exclusions:`, `Validation Required:`, and `Stop Conditions:`. The digest must never be compacted, abbreviated, summarized away, replaced by a one-line next move, or omitted because the surrounding packet includes similar information. Each required field must be present in the USER-facing output every time, even when repetitive. When a phase creates or updates a plan that USER must inspect before implementation, the digest must also state `USER Plan Review Gate:`, `USER Inspection Files:`, and `Implementation Blocker:` in plain language, naming whether the USER may accept, change, waive, or reject the plan and confirming that implementation remains blocked until the next approved implementation phase. When review is not required, the digest must state `USER Plan Review Gate: Not required` and `Review Waiver Reason:`. Missing fields, compacted fields, generic approval language, missing inspection files, or a missing waiver reason block on `Next Legal Phase Digest Missing`; `Next Safe Move` or informal recommendations cannot replace the digest.
