...
Repair Path: During the next legal gate after rebaseline, inventory what the current branch already implemented or touched, compare branch output and plans against the new merged standard, name changed files/surfaces/claims and UI/UX violations if any, decide whether to repair now or route back, update only the owning branch-local source truth or external operational state allowed for that gate, and validate before advancing.
