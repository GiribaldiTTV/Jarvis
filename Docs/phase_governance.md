...
`BP1` creates or validates `USER_BRANCH_VISION_REVIEW.md` as a USER-facing product/vision document generated from project vision, family vision, feature vision, branch candidate, and accepted USER inputs. The file must avoid technical Git packet metadata such as branch HEAD, SHA, origin/main, merge base, and ZIP hash. Technical packet metadata belongs in helper output, validator output, Codex chat digest, or external governance state.
...
- `C:\Nexus USER\<worktree-label>.zip`

The USER workflow is: open `C:\Nexus USER`, read the label folder, and upload the matching sibling label zip. Before regeneration, Codex must quarantine or clear the old matched pair and prove `C:\Nexus USER\<worktree-label>` and `C:\Nexus USER\<worktree-label>.zip` are absent before writing the fresh pair. Cloud-backed Desktop / OneDrive copies are optional backup or convenience mirrors only and must not be the byte-authoritative upload source. Sidecar files, uniquely named upload ZIPs, separate top-level Review / Upload folder taxonomies, and alternate upload marker files are outside the stable model unless USER separately admits them. USER-facing review files (`START_HERE.md`, `USER_BRANCH_VISION_REVIEW.md`, and `USER_BRANCH_PLAN_REVIEW.md`) should stay focused on review context, product direction, and implementation planning and avoid active branch status or technical proof metadata. Technical proof metadata belongs in helper output, validator output, Codex chat digest, or external governance state.

If USER wants Desktop convenience access to the active USER hub, use a shortcut named `Nexus USER.lnk` that points to `C:\Nexus USER`; do not make the Desktop or OneDrive folder the active upload authority. Simple USER-facing handoff notes such as `User Test Summary.txt` may live at the `C:\Nexus USER` root when they are meant for USER convenience, while the durable UTS result/digest must still fold into the phase owner that requires it.

USER review packets are temporary USER/ChatGPT review aids for early-phase Branch Planning, not durable repo evidence, not a posterity archive, and not a reporting surface for later phases. Their purpose is to give USER and ChatGPT a concrete, inspectable BP1 Branch Vision and BP2 Branch Plan so they can hold Codex to a clear vision/plan before implementation. Once USER accepts, revises, rejects, defers, or waives the packet, Codex must fold the accepted outcome, constraints, and decisions into the proper durable repo source-truth owners. The local USER hub folder and zip may then be replaced or discarded; governance must not rely on them as canon or maintain them after their review purpose is complete.

USER review packets are required only while Branch Readiness / Branch Planning needs USER vision or plan review, through BP3 / Workstream Entry before implementation approval. After Workstream completes, the branch must not keep regenerating BP1/BP2 USER packets by inertia during Hardening, Live Validation, PR Readiness, or Release Readiness. Later phases validate against the accepted branch vision/plan in source truth, changed-file diff, proof artifacts, fold-down receipts, and phase-specific packets instead of maintaining the early USER review packet. If a later phase discovers that the accepted vision or plan is wrong, stale, or incomplete, it must route back to Branch Planning or Branch Readiness and create a new review packet only for that repair gate.
...
`Workstream Entry` is `BP3 - Workstream Entry / Orchestration Validation` inside the canonical `Branch Planning` phase. It is not a hidden planning gate inside `Workstream`, and it is not runtime/code implementation. Before USER can green-light Workstream implementation for a runtime/user-facing/source-truth branch, Codex must return a full, non-compacted BP3 / Workstream Entry Review Digest and create or refresh the active worktree's temporary local USER Review Hub bundle under `C:\Nexus USER\<worktree-label>` with sibling `C:\Nexus USER\<worktree-label>.zip` when BP1/BP2 review is required. The bundle contains flat copied files for the branch vision, branch plan, Branch Runtime Engineering Plan when applicable, Element-to-Phase Proof Matrix owner, branch authority record, relevant Nexus/family vision files, UFD/change-intent surfaces when applicable, and any other source-truth files USER and ChatGPT need to inspect before implementation. The digest/helper output, not the USER folder, must include technical proof such as branch identity, commit hashes, validation summary, and zip hash. The USER folder must include only the plain-language index, copied file list, exact USER decision text, pending USER decisions, review order, and copied review files. Missing review bundle proof blocks on `Workstream Entry Review Bundle Missing`.

USER Branch Plan Review Gate: Workstream Entry must wrap the branch vision, Branch Runtime Engineering Plan, Element-to-Phase Proof Matrix, Hardening plan, Live Validation / UTS plan, open USER questions, USER design review questions, Codex recommendations, implementation options, alternatives/tradeoffs, accepted scope, deferred scope, rejected scope, and exact USER decision needed into a plain-language `USER Branch Plan Review` packet before runtime/user-facing/source-truth implementation begins or resumes. Required digest fields include `Plain-Language Branch Goal:`, `Planned User-Facing Outcome:`, `End-State Vision:`, `Visual / Functional Walkthrough:`, `Surface Map:`, `Implementation Options:`, `Recommended Direction:`, `USER Design Review Questions:`, `USER Review Response:`, `Codex Response Digest:`, and `USER Review Packet Finding:` so USER can inspect what Codex intends to build before implementation. `USER_BRANCH_PLAN_REVIEW.md` is the USER Branch Plan Contract. It must include `Contract Status:`, `Contract Version / Revision:`, `What Will I Actually See, And Where Will I See It?:`, `Why This Fits The Nexus Vision:`, `USER Design Direction Decision:`, `Implementation Constraints Created By USER Response:`, `USER Rejected / Deferred Ideas:`, `Vision Delta / Source-Truth Impact:`, `Contract Change Log:`, and `Contract Completion Checklist:`. `Contract Status:` must be `Draft`, `Pending USER Response`, `Pending Codex Digest`, `Pending USER Confirmation`, `Complete`, or `Waived by USER`; implementation approval is illegal while Contract Status is Draft, Pending USER Response, Pending Codex Digest, or Pending USER Confirmation. `USER Review Packet Finding:` must explicitly say whether the local USER Review Hub packet was loaded and digested, waived by exact USER/source-truth text, missing, stale, shallow, or blocking, and must name the required packet files when a USER Review Hub bundle is required. USER's response must be attached or inserted into the packet or Workstream Entry digest, and Codex must digest that response into testable implementation constraints before bounded Workstream implementation begins; the only bypass is an explicit USER waiver recorded in both `USER Review Response:` and `Codex Response Digest:`. When Codex changes the contract after USER feedback, Contract Status becomes `Pending USER Confirmation`; Codex must update the durable source-truth owner, refresh the temporary local USER hub packet/ZIP only for the active review loop, and wait for USER confirmation or explicit waiver. The packet is the USER-facing checkpoint for "what Codex intends to build, how it should behave/look, where it will live, how it will be proven, and what questions remain"; it does not create a permanent planning ledger and must not be retained as canon after the accepted outcome is folded into source truth. Required disposition values are `Accepted by USER`, `Revised by USER`, `Deferred With Waiver`, `Rejected by USER`, and `Needs USER Decision`. Implementation remains blocked on `USER Branch Plan Review Missing`, `USER Review Packet Stale`, or `USER Review Packet Not Digested` until the contract is Complete or Waived by USER. If later implementation proves the accepted plan too narrow, stale, or wrong, Codex must stop with a Vision Question Digest, Branch Plan Review addendum, or Branch Plan Revision Packet according to the Level 1 / Level 2 / Level 3 question severity model.

Branch Planning review format reinforcement: BP1 / `USER_BRANCH_VISION_REVIEW.md` owns the product/design branch vision and must show what the feature will look like, how it will function, which surfaces are touched, which possibilities USER can accept/change/add to, and why Codex recommends the selected end-state before engineering planning or Workstream implementation begins. BP2 / `USER_BRANCH_PLAN_REVIEW.md` owns the USER Branch Plan Contract and engineering route derived from accepted or waived BP1; it must include accepted branch vision trace, `End-State Vision:`, `Visual / Functional Walkthrough:`, `Surface Map:`, `Implementation Options:`, `Recommended Direction:`, `Current Branch Scope:`, `Future-Gated Scope:`, `Implementation Staging Notes:`, `USER Decisions Needed:`, and `Workstream Entry Result:`. `Implementation Staging Notes:` may mention SLCs/slices/seams only as BP2 execution scaffolding after the BP1 product direction is understood; it is not the primary BP1 USER decision surface.

Workstream Entry USER hub packet acceptance requires more than file count, but technical proof must stay outside USER-facing files. Helper output, validator output, Codex chat digest, or external governance state must validate branch identity, current commit, baseline, required digest-file presence, zip freshness, exact USER decision text, and next-legal-phase consistency. The USER folder itself should expose only the plain-language index, copied source-review files, BP1/BP2 review content, exact USER decision text, and pending USER decisions. If those decision surfaces disagree about whether the next legal phase is repair/revalidation, Workstream Entry final decision review, or Workstream implementation approval, entry blocks on `Workstream Entry Packet Decision Path Conflict`. A packet that names Branch Readiness Stage 2 repair/revalidation, or otherwise states that Workstream implementation remains blocked, cannot be used as Workstream implementation approval until the USER hub packet is reissued or updated and the decision path validates through helper/Codex proof. Chat-only correction text cannot replace missing packet evidence.
...
- Workstream Entry must include a full non-compacted Workstream Entry Review Digest and, when BP1/BP2 review is required, the active worktree's temporary local USER Review Hub bundle under `C:\Nexus USER\<worktree-label>`, with sibling `C:\Nexus USER\<worktree-label>.zip`, flat copied branch vision, branch plan, branch authority, relevant Nexus/family vision, matrix, UFD/change-intent, and source-truth files needed only for USER/ChatGPT inspection before USER can green-light implementation
- Workstream Entry must include the named `USER Branch Plan Review Gate` for runtime/user-facing/source-truth work. The review packet summarizes the plain-language branch goal, planned user-facing outcome, visual/behavioral description when relevant, implementation breakdown, Element-to-Phase Proof Matrix summary, Hardening plan, Live Validation / UTS plan, open USER questions, USER design review questions, Codex recommendations, implementation options, alternatives/tradeoffs, accepted/deferred/rejected scope, exact USER decision needed, `USER Review Response:`, `Codex Response Digest:`, and `USER Review Packet Finding:`. The finding must prove the temporary USER Review Hub packet was loaded and digested or name the exact waiver/blocker. Workstream implementation stays blocked on `USER Branch Plan Review Missing`, `USER Review Packet Stale`, or `USER Review Packet Not Digested` until USER accepts, revises, rejects, asks for more analysis, defers with waiver, or approves packet refresh, and until USER response is attached/inserted, digested, and folded into the proper source-truth owner or explicitly waived.
- BP3 / Workstream Entry reads the accepted or waived BP1 vision and BP2 plan, then returns whole-package orchestration validation before first-seam implementation; each seam updates traceability, Hardening compares actual implementation against the accepted BP1/BP2 contracts, Live Validation records proof or waiver posture, PR Readiness produces the `PR Fold-Down Packet:`, and Release Readiness translates the plan into public scope without internal governance jargon
...
- `Bot Review Signal Pending`:
  for Codex-created PRs, PR Readiness cannot be green until the live PR has received a thumbs-up reaction from the Codex GitHub bot on the current PR head. A bot comment keeps `PR Validation Pending` active until the branch fixes the comment on the same PR, pushes, replies to and resolves the review thread, asks Codex to re-analyze the updated head, and records a fresh current-head thumbs-up reaction; an eye/watch reaction or resolved comment alone does not clear the gate. This is the same-PR Codex bot-review repair loop. Stage 2 final handoff and merge execution cannot be green until the current-head thumbs-up closeout is verified.
  When this blocker is active on a live Codex-created PR, Stage 2 must provision or update the PR watcher before handoff; a manual "check later" plan is not enough. The watcher must be authorized to inspect bot reactions, bot comments, review threads, PR comments, inline comments, PR state, and mergeability, and to perform bounded same-PR repairs for valid Codex bot comments that stay inside the approved PR scope. If no Codex bot comment or thumbs-up/approval signal appears after the current PR head has been live for at least two minutes, the watcher must post exactly one PR conversation nudge for that head SHA asking the Codex bot for the review signal, and must not repeat that nudge for the same head. The required repair loop is: verify identity, evaluate the bot comment against source truth, repair only approved same-PR scope, rerun required validation, commit and push to the same branch, reply/resolve only when the review-thread contract requires it, ask Codex to re-analyze the updated head, and report closeout only after the Codex GitHub bot gives a fresh current-head thumbs-up reaction. Out-of-scope bot requests must be reported as `BLOCKED`, not repaired.
- `PR Watcher Provisioning Unproven`:
  if the branch expects watcher-based PR monitoring or is waiting on `Bot Review Signal Pending`, the watcher target, approved reporting surface, routing proof, runtime path, run-proof method, fallback, teardown rule, replacement provisioning for the next live PR, and the live bot-review action contract must be explicit and proven before PR Readiness can turn green. Standard operating procedure from now on is a watcher on an approved Codex reporting surface at minute cadence that reports only when a watched PR status changes. The current working thread is the default surface, but an explicitly recorded dedicated watcher-host thread is allowed when that is the validated user-visible route. Accepted watcher proof may come from native Codex heartbeat run evidence or from a bounded local watcher that posts those status-change updates through the official Codex thread-resume path into the approved transcript and records delivery proof through assistant-message transcript presence, Codex thread-state refresh, and automation run/inbox visibility. Manual rollout-file or transcript-file injection does not count as proof.
  ...
  The action contract is part of provisioning proof: thumbs-up reaction on the current head means report green for PR-entry validation; one or more actionable bot comments means trigger the bounded same-branch PR comment-repair worker, fix the issue, commit, push, reply, resolve the corresponding review thread, ask Codex to re-analyze, and then wait for a fresh thumbs-up reaction on the updated head. If the repair worker cannot complete safely or the thumbs-up does not arrive, keep `PR Validation Pending` active and surface the exact blocking comment.
...
- the live PR has a thumbs-up reaction from the Codex GitHub bot on the current PR head; bot comment-resolution closeout without a fresh thumbs-up is not sufficient
...
Formal Next Legal Phase Digests must not be compacted, abbreviated, summarized away, replaced by one-line next-step wording, or omitted because similar information exists elsewhere in the packet. `USER Plan Review Gate:` must state whether USER may accept, revise, waive, or reject the plan. `USER Inspection Files:` must name the exact files or local USER Review Hub bundle when review is required. `Review Required Because:` must explain the reason for review. `Implementation Blocker:` must name the blocker when implementation remains unauthorized. `Review Waiver Reason:` must be `Not waived` when review is required, and must explain the waiver when USER plan review is not required.
...
- `Branch Readiness` -> `Workstream` only after branch base, branch class, authority record, branch objective, target end-state, complete family-package product planning packet when applicable, project-wide vision alignment, branch-specific vision alignment, product-system concept model, entity/profile model, user workflow model, scale/state planning, expected seam families and risk classes, validation contract, User Test Summary strategy, later-phase expectations, USER critique/decision loop, Codex additional recommendations, and first Workstream seam or initial seam sequence are explicit, and no `Branch Readiness Planning Incomplete` blocker remains active unless explicitly USER-waived
