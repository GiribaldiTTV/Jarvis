...
- local USER hub packet at `C:\Nexus USER\<worktree-label>` with matching timestamped upload ZIP at `C:\Nexus USER\<worktree-label>-YYYYMMDD-HHMMSS.zip` when USER review is required, including stale same-label ZIP cleanup, stable ZIP rejection, folder/ZIP parity, approved file-class layout, and exactly one primary current-gate USER review file
