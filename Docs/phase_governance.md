...
Reference Standard lifecycle: `Candidate -> USER Review -> Promoted Reference -> Consumed By Branch -> Effectiveness Reviewed -> Updated / Superseded / Deferred`. UIREF is the first domain implementation of this lifecycle. New non-UI reference families must not be created without repeated evidence, clear source-truth owner routing, USER promotion approval, and an adoption path. Vision Contracts remain broad product law; Reference Standards are detailed comparators; Templates are USER-approved scaffolds; Shared Primitives are reusable implementation sources. A screenshot, helper output, validator green, or branch-local packet is candidate evidence only.
