...
Repair Owner: the current branch/worktree repairs its own branch-local classification, active external branch plan, USER packet, and proof language. Standing Governance repairs reusable source-truth ambiguity. Helper/validator owners repair machine-checkable blind spots only after the source-truth rule is clear. USER owns foundation-only PR consideration approval, waivers, retargeting, or runtime-carrier selection.
...
Repair Owner: Standing Governance records durable dependency and classification rules. UIREF reference records live in `Docs/ui_reference_catalog/`. Future implementation-template or shared-primitive work belongs to a separately approved owner/carrier after USER admits the plan/promotion path. Consuming FAM worktrees own branch-local adoption proof and repairs at their next legal gate.
...
Repair Owner: consuming branch for branch-local candidate comparison and packet rows; standing Governance for durable source-truth ambiguity or promotion; USER for candidate promotion, waiver, exception, split, merge, rejection, or issue/PR decisions.
...
Repair Owner: The consuming FAM repairs its own window classification, behavior, fallback, and proof. FAM-002 repairs reusable presentation/geometry standards. FAM-003 repairs the global or user-accessible reset route when admitted. FAM-008 repairs setup/education wording when admitted. Standing Governance repairs reusable source-truth ambiguity only.
...
Repair Owner: The consuming FAM repairs its own product-window geometry, responsive layout, proof, and issue-candidate packet under its legal carrier. FAM-002 repairs reusable presentation/geometry ambiguity. Standing Governance repairs durable source-truth ambiguity only.
...
Repair Owner: The consuming FAM repairs its own window-control behavior, layout, exception, and proof. FAM-002 repairs reusable control grammar. FAM-003 repairs only the user-accessible reset/recovery route when admitted. Standing Governance repairs reusable source-truth ambiguity only.
...
Repair Owner: the current branch/worktree repairs its own LV blockers and reruns proof; standing Governance intake repairs reusable loop-rule ambiguity; USER owns manual validation, waivers, and out-of-scope repair decisions.
...
Validation Owner: Future helper/validator hardening should extend `dev/orin_branch_governance_validation.py`, `dev/orin_branch_readiness_planning_fixture_validation.py`, `dev/orin_worktree_rebaseline_audit.py`; USER review preparation and delivery must use the external USER lifecycle owner. Green validation is evidence only; Codex must still inspect dependency scope, overlap classification, active-state leakage, and USER decision state.
...
Codex may reconcile that external operational carry-forward in the same bounded RR2 post-release closeout without a separate USER decision when the reconciliation only updates `D:\Nexus Desktop AI\Governance State` to match live Git/GitHub/repo truth and does not mutate repo source files, create branches or PRs, merge, release again, clean branches or worktrees, or touch FAM/runtime/private/provider/cache/memory surfaces.
