...
- Workstream Entry must include the named `USER Branch Plan Review Gate` for runtime/user-facing/source-truth work. The review packet summarizes the plain-language branch goal, planned user-facing outcome, visual/behavioral description when relevant, implementation breakdown, Element-to-Phase Proof Matrix summary, Hardening plan, Live Validation / UTS plan, open USER questions, Codex recommendations, alternatives/tradeoffs, accepted/deferred/rejected scope, and exact USER decision needed. Workstream implementation stays blocked on `USER Branch Plan Review Missing` until USER accepts, revises, rejects, asks for more analysis, or defers with waiver.
