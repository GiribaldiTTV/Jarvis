...
Blocking Conditions: `Template Dependency Unresolved` when the FAM-007 prerequisite proof is missing, `Template / Reference Plan Blocked` when USER direction keeps the integrated Governance PR held pending the blocked template/reference plan, `Golden Reference Promotion Blocked` when promotion is attempted without USER approval, `Shared Primitive Promotion Blocked`, `Limited PR Path USER Approval Missing`, and `Template Treated As Existing Proof`.
...
Final Disposition: The gate may record prerequisite evidence and future promotion boundaries, but it must not close the no-PR dependency while USER direction keeps the integrated Governance PR held for the blocked template/reference plan. Actual template/reference promotion remains a separate USER-approved implementation path and must not create templates, golden references, shared primitives, design tokens, issue mutations, FAM mutations, or repo live-state ledgers in this repair.
