...
Validation Owner: future helpers may generate report-only candidate sync/collision digests and future validators may source-check packet fields when machine-checkable. Clean-clone repo validation must not require `C:\Nexus Governance State`.
...
GitHub connector PR facts remain Git/GitHub/helper-derived live truth. OpenAI Docs lookups are evidence for durable rule review. OpenAI Developers/API key/provider setup is sensitive setup state and remains blocked unless USER approves that exact setup path. Browser, Chrome, Documents, Spreadsheets, and Presentations outputs are review or validation evidence until digested into an owning source-truth file or historical receipt. Computer Use is an excluded tool class for Nexus repo validation and runtime-control work. It must not be invoked for inspection, capture, target discovery, control assistance, launcher activation, UI interaction, evidence generation, or cleanup, and no plugin plan, helper, waiver, or independent evidence may convert a Computer Use-involved run into valid proof. Record `Computer Use Validation Path Prohibited`, exclude the affected run's gating artifacts, and restart from a clean preflight through the governed human-client path.
...
Branch Planning Review Bundle: Before USER can green-light Workstream implementation for a runtime/user-facing/source-truth branch, Codex must complete BP1, BP2, and BP3 and create or refresh the active worktree's local USER review packet under `C:\Nexus USER\<worktree-label>` with a matching timestamped upload ZIP at `C:\Nexus USER\<worktree-label>-YYYYMMDD-HHMMSS.zip`. The packet root contains `START_HERE.md`; `USER Review` contains exactly one primary USER-facing decision file for the current BP gate; `Review Aids` contains generated supporting digests/checklists; and `Source Truth Context` contains copied context files for the Project Vision, Family Vision, Branch Vision, Branch Plan, Element-to-Phase Proof Matrix owner, branch authority record, relevant UFD/change-intent surfaces when applicable, and any other source-truth files needed for USER inspection. The digest must include the local USER hub path, copied file list, user-readable check/proof summary without live validation-status metadata, exact planning or implementation decision requested, pending USER decisions, and final packet proof that the folder was cleanly regenerated, the upload ZIP is timestamped, the upload ZIP lives beside the packet folder, duplicate ZIP entries are absent, stale same-label ZIPs and legacy stable ZIPs are absent, folder/ZIP file-list plus content-hash parity holds, the file-class layout is approved, exactly one primary USER review file exists, and stale-stage / unresolved-placeholder scans are green. Active branch status, current HEAD/origin/main, ahead/behind, validation log detail, current PR state, and ZIP hash belong in helper output, validator output, Codex chat digest, or external operational state rather than as the content focus of USER-facing review files. Missing review packet proof blocks on `Branch Planning Review Packet Missing`; stale folder/ZIP/layout proof blocks on `USER Review Packet Stale`.
...
- Branch Readiness Stage 1 proposes the contract and stops for USER planning review; Branch Readiness Stage 2 may admit or revise it, but `Runtime Implementation Approval:` remains pending until BP3 / Workstream Entry admits bounded Workstream phase conduct for the package and entry seam or initial seam sequence
...
- If launcher parity is missing, fails, or the validated claim could be affected by troubleshooting-mode differences, Live Validation must use the normal USER desktop runtime launcher or stop on a named blocker. Codex must not transfer launcher operation or interaction validation to the USER.

When the declared normal USER desktop runtime launcher is a Windows `.lnk`, shell-executing the `.lnk` path, launching the target script directly, or injecting private test environment variables is not enough for final proof. Codex must use visible user-level desktop control to click, double-click, or keyboard-open the exact USER desktop shortcut from a visible selected Desktop folder item, then exercise tray/menu/window interactions from that launched runtime. The governed human-client helper, old-style real-cursor movement, or another visible mouse/keyboard path may be the primary proof route when it preserves before/during/after photo/video or ordered-frame evidence and exercises the actual USER-facing control. Computer Use is prohibited and must not participate in inspection, capture, target discovery, control assistance, launcher activation, interaction, evidence generation, or cleanup. Every desktop-control method must be target-safe: Codex may move the mouse, click, right-click, double-click, or type only against a visible accessibility element, a visibly identified app/window control, or a bounded screenshot coordinate that has been visually tied to the intended target in the current screenshot. Codex owns a deterministic visible-target acquisition loop before USER handoff: capture the current visible screen/window, inventory accessible elements and screenshot-visible controls, derive candidate targets from label/icon/proximity/state evidence, exercise only user-equivalent input against the visually tied candidate, and verify the expected visible result before reusing that target model. Hidden app activation calls, direct handler calls, internal signal dispatch, synthetic tray/menu activation, launcher foreground signals, scripted shortcuts that bypass the visible control surface, or Computer Use activation are not user-simulation proof. A Computer Use-involved run is invalid even when independent screenshots or logs exist; record `Computer Use Validation Path Prohibited`, exclude its gating artifacts, and restart from clean preflight through the governed human-client path. Live Validation must exercise the actual USER-facing control surfaces the USER would use; Codex must not create, add, or depend on a new user-visible runtime control surface, dev-only control window, shortcut side channel, validation-only product window, desktop-wide minimize/show-desktop shortcut-discovery step, or broad window-state disruption merely to make existing tray/menu/window controls targetable.
Live Validation must preserve every window that was already open before the run: no setup, target-acquisition, backdrop, evidence, or cleanup step may use desktop-wide minimize/show-desktop, `Win+D`, per-window minimize, hide, restore, close, move, resize, rearrangement, or equivalent shell/Win32/UIAutomation window-state manipulation against those windows. An explicitly approved test may change the state of the in-scope product window only when that exact window-state behavior is under test; it never authorizes disruption of unrelated USER windows. Capture the initial visible state and verify preservation at closeout. If visible target acquisition or proof setup would require disturbing a pre-existing window, stop on `Live Validation USER Workspace Preservation Blocked`.
Activation-path proof is non-transferable. A visible mouse click proving a button path does not prove the matching keybind path, a keybind proof does not prove the click path, a tray/menu proof does not prove a dashboard/control-window path, and injected/internal activation never proves any USER-facing path. Each admitted activation path must be validated separately with photo/video or ordered frame evidence for the action plus the user-visible state/result it changes. If exhaustive Codex-owned visible attempts cannot safely prove exactly one activation path, USER may grant a named single-path waiver as an acceptance decision without operating the control; that waiver remains supporting evidence for all other Live Validation dimensions and cannot be generalized to other activation paths or product surfaces.
Visible desktop coordinate control is allowed and may be the primary route for OS-level surfaces such as taskbar tray icons, notification-area menus, window borders, resize handles, or desktop chrome. This route must use a visible desktop-control session or the governed human-client helper, slowly move the real Windows cursor to the screenshot-tied target, send real OS-level mouse down/up or drag input, preserve before/during/after photo/video or ordered-frame evidence, and verify the resulting visible menu/window/state. It is not an injection waiver: direct handler calls, shell commands, synthetic tray/menu activation, hidden app activation, blind coordinate probing, repeated nearby guessing, and desktop-wide minimize/show-desktop shortcut discovery remain invalid. If the actual tray/menu/window path cannot be safely targeted through visible evidence and bounded visible coordinate control is exhausted or unsafe, Codex must stop on `Live Validation Control-Surface Acquisition Blocked` and route the exact Codex-owned acquisition or tooling repair need instead of fishing for coordinates or transferring the interaction to the USER.
A blocked activation path requires a `Control-Surface Acquisition Blocked Digest`. The digest must name the exact control path, intended Codex-operated action, expected visible result/state change, screenshots or ordered frames already captured, acquisition methods attempted, why safe Codex control remains unavailable, the exact tooling or environment repair needed, pass/fail criteria, and the paths/surfaces that remain unproven. Codex must stop at this blocker; it must not continue as if the path passed and must not convert the blocked interaction into a USER-operated validation step.

Named blocker:

- `User-Facing Shortcut Validation Pending`
- `Normal Worktree Launcher Validation Pending`
- `Computer Use Validation Path Prohibited`
...
- helper-only, direct-Python, WebView-only, sandbox/offscreen, active-client direct-runtime, generated-shortcut, troubleshooting-launcher-without-parity, or harness-only evidence is not treated as a substitute for this final launcher gate when the normal USER desktop launcher path is feasible; Computer Use participation is prohibited and invalidates the affected validation run rather than creating a supporting evidence class
...
For desktop UI branches, this inspection must be human-client faithful when the USER will operate visible tray/menu/window behavior. Codex cannot mark Live Validation Stage 1 green from app-side callbacks, fake/offscreen models, marker-only proof, screenshot-only proof, direct handler calls, or UIAutomation-only interaction output. The final LV1 handoff requires a manifest that records visible desktop shortcut launch, visible tray/menu selection, governed visible mouse/cursor or keyboard interaction evidence, window move/resize/open/close evidence where applicable, screenshot or frame-sequence artifacts, and Codex's own visual review of every issue-grounded UTS item. Computer Use is prohibited for every part of this route; if it participates, invalidate the run, record `Computer Use Validation Path Prohibited`, and restart through the governed human-client path.
...
- for clickable desktop UI controls, `governed visible mouse/cursor interaction evidence` means the LV1 helper must visibly move the real Windows cursor to the control's screen coordinates, send OS-level mouse down/up input, and then inspect the resulting state. direct JavaScript `.click()`, synthetic DOM events, WebView callbacks that invoke handlers, direct handler calls, state mutation, marker toggles, QTest widget-only events, UIAutomation-only actions, or anything previously labeled `diagnostic-only` are banned from Live Validation phase interaction testing and may not be present as proof in the active LV1 interaction route. Computer Use is prohibited for operation, inspection, capture, target discovery, or evidence in the route. The Live Validation helper must run a no-synthetic-interaction and no-Computer-Use preflight before launch; if the active LV1 route contains synthetic/widget/direct-handler interaction code or any Computer Use participation, the helper must stop on Live Validation `REPAIR` before executing the test. Any Live Validation manifest that treats JS/DOM/synthetic/widget-only/UIAutomation-only actions as proof, or contains Computer Use participation, is invalid.
...
- prove the accepted plan is ready for Workstream phase entry and bounded conduct
...
- exact bounded Workstream phase conduct text, including the entry seam or initial seam sequence, when BP3 is green
...
- approving Workstream phase conduct while BP1 or BP2 is pending, stale, missing, rejected, or unwaived
...
- local USER hub packet at `C:\Nexus USER\<worktree-label>` with matching timestamped upload ZIP at `C:\Nexus USER\<worktree-label>-YYYYMMDD-HHMMSS.zip` when USER review is required, including ZIP-beside-folder placement, duplicate ZIP entry rejection, stale same-label ZIP cleanup, stable ZIP rejection, folder/ZIP file-list plus content-hash parity, approved file-class layout, and exactly one primary current-gate USER review file
...
- Workstream phase conduct text preserves all pending USER action gates
...
- for user-facing or visual work, the completed Workstream evidence is ready for a USER packet review for Visual Acceptance before downstream phase progression
