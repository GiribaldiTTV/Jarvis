...
Final Disposition: RAR exits only through `No Applicable Impact`, `Adoption Green With Evidence`, `Repair Completed And Revalidated`, `USER Waiver Recorded`, `Issue Candidate Packet USER-Reviewed`, `Route Back To Earlier Gate`, or `Blocked`. Normal BP, Workstream, Hardening, LV, PR Readiness, or Release Readiness progression remains blocked until the active RAR trigger is resolved and no unresolved blocking RAR row status remains.
