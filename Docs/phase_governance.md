...
- Trust-but-verify helper proof complete: when a helper or validator creates, refreshes, deletes, zips, uploads, copies, or claims freshness for USER-facing artifacts, Codex must independently inspect the relevant filesystem state before and after the helper action. For review bundles this means proving old folder contents and matching zip artifacts were moved to Recycle Bin or governed quarantine when practical, proving the stable folder path is absent or empty before generation, proving matching zip artifacts are absent from the stable upload location before generation, then proving the regenerated folder and zip contain only the expected current-HEAD packet contents. Codex must inspect the generated stable ZIP directly from the stable worktree-label path, report the final SHA256 from the final ZIP bytes, compare `START_HERE.md` Source Branch, Source HEAD, origin/main, and Merge Base with live Git, prove simplified-model exclusions such as no `UPLOAD_THIS_ZIP.md` when that file is not required, and compare the active branch record and branch plan inside the ZIP with the active carrier. Active implementation-carrier files must reject stale active-carrier wording such as "this branch is not the runtime implementation carrier", "future runtime implementation carrier", "later runtime carrier", "Workstream is skipped", or "planning/governance branch"; historical files may carry historical wording only when clearly included as released traceability. Helper PASS output alone is not sufficient closeout proof.
