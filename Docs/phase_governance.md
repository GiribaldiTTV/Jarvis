...
Validation Owner: future helpers may generate report-only candidate sync/collision digests and future validators may source-check packet fields when machine-checkable. Clean-clone repo validation must not require `D:\Nexus Desktop AI Data\Governance State`.
...
- new or re-entering runtime-focused branches must create or admit the detailed active-branch execution plan under `D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\branch_plan.md` during Branch Readiness Stage 2 when repo truth supports runtime work
...
- BP1/BP2/BP3 Branch Planning must include a full non-compacted Branch Planning / Workstream Entry Review Digest and the active worktree's local USER hub packet under `D:\Nexus Desktop AI Data\USER\<worktree-label>`, with a matching timestamped upload ZIP at `D:\Nexus Desktop AI Data\USER\<worktree-label>-YYYYMMDD-HHMMSS.zip`, root `START_HERE.md`, exactly one current-gate primary decision file under `USER Review`, generated supporting digests/checklists under `Review Aids`, and copied project vision, family vision, branch vision, branch plan, branch authority, matrix, UFD/change-intent, and source-truth context under `Source Truth Context` before USER can green-light implementation.
...
- `D:\Nexus Desktop AI Data\Governance State` for accepted local operational state
...
- local USER hub packet at `D:\Nexus Desktop AI Data\USER\<worktree-label>` with matching timestamped upload ZIP at `D:\Nexus Desktop AI Data\USER\<worktree-label>-YYYYMMDD-HHMMSS.zip` when USER review is required, including ZIP-beside-folder placement, duplicate ZIP entry rejection, stale same-label ZIP cleanup, stable ZIP rejection, folder/ZIP file-list plus content-hash parity, approved file-class layout, and exactly one primary current-gate USER review file
...
Codex may reconcile that external operational carry-forward in the same bounded RR2 post-release closeout without a separate USER decision when the reconciliation only updates `D:\Nexus Desktop AI Data\Governance State` to match live Git/GitHub/repo truth and does not mutate repo source files, create branches or PRs, merge, release again, clean branches or worktrees, or touch FAM/runtime/private/provider/cache/memory surfaces.
...
- one standing worktree: `D:\Nexus Desktop AI Data\Worktrees\Governance`
...
- the governance lane must not infer the originating workspace from `D:\Nexus Desktop AI Data\Worktrees\Governance`, `C:\Nexus Desktop AI`, GitHub Desktop's selected repository, or the current shell CWD
