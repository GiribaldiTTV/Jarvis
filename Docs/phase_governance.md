...
Required State: Hardening is the readiness inspection before Live Validation. Live Validation proves the required runtime or USER path. If Live Validation, USER-gated Live Validation, or returned UTS evidence finds defects, the branch must repair the Live Validation blockers first instead of rerunning full Hardening by default while the LV path is still known-failing. After LV blockers are repaired and the affected LV proof path is green, Hardening must rerun because the branch changed after the prior Hardening pass. After rerun Hardening is green, Live Validation must rerun or explicitly reconfirm the proof path affected by the repair. USER review may confirm only the final post-repair Hardening plus LV-green result when USER validation is required. PR Readiness may rely only on the final post-repair Hardening/LV/USER-review state, not on a superseded pre-repair Hardening pass.
Allowed Loop States: `Hardening Green Before LV`, `LV Blocked / Repair First`, `LV Repair Applied`, `Post-LV-Repair LV Green`, `Post-LV-Repair Hardening Rerun Required`, `Post-LV-Repair Hardening Green`, `LV Reconfirmation Required`, and `Final Hardening/LV/USER Review Green`.
Invalid Values: `Pre-Repair Hardening Treated As Final`, `Full Hardening Rerun While LV Known-Failing By Default`, `LV Repair Skips Hardening Rerun`, `Hardening Rerun Skips LV Reconfirmation`, `USER Review Based On Pre-Repair Proof`, and `PR Readiness Uses Superseded Hardening`.
Blocking Conditions: `Known-Failing LV Repair First`, `Post-LV-Repair Hardening Rerun Missing`, `Post-Hardening LV Reconfirmation Missing`, `Pre-Repair Hardening Treated As Final`, and `Final USER Validation Missing`.
...
Repair Path: classify the LV defect, repair in-scope LV blockers, rerun or reconfirm the affected LV proof path to green, rerun Hardening over changed files, surfaces, proof rows, and coverage rows, rerun or reconfirm LV after Hardening when Hardening changes anything or invalidates proof, digest USER validation when required, then allow PR Readiness to inspect only the final post-repair state.
