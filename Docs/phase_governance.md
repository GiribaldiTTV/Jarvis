...
- Tray, taskbar, hidden-overflow, and notification-area targets require target-acquisition proof before PASS. A visible tooltip, the USER manually opening the menu, read-only accessibility output, runtime logs that say the icon exists, or a named icon rectangle can prove identity/supporting context, but they cannot prove Codex-operated control-surface success. The PASS condition is Codex-owned visible user-level input opening the intended menu/control and photo/video or ordered frame-sequence evidence showing the resulting NDAI menu or window. If multi-monitor coordinate translation, screenshot origin drift, current-cursor mismatch, shell/taskbar target ambiguity, or hidden-overflow uncertainty prevents safe acquisition, stop on `Live Validation Control-Surface Acquisition Blocked`; do not keep probing nearby taskbar coordinates, do not mark product failure unless runtime evidence proves the app ignored the correctly targeted input, and do not use USER manual confirmation as green proof.
