...
Required State: Template, golden-reference, design-token, or shared UI primitive creation/promotion remains blocked until FAM-007 pushes its PR, the FAM-007 PR merges, post-merge rebaseline/reconciliation completes, durable FAM-007 lessons are digested, and USER explicitly approves template/reference promotion. Until then, branches may consume binding FAM-002 presentation grammar and current phase gates manually, but they must not claim a template/golden reference exists or treat future templates as current proof.
Allowed Values: `Template Not Yet Promoted`, `Future Template/Golden Reference Work`, `USER-Approved Template Promotion`, `Limited PR Path USER-Approved`, and `Blocked`.
...
Blocking Conditions: `Template Dependency Unresolved`, `Golden Reference Promotion Blocked`, `Shared Primitive Promotion Blocked`, `Limited PR Path USER Approval Missing`, and `Template Treated As Existing Proof`.
...
Repair Path: Keep current presentation rules enforceable manually, list the unresolved template/reference dependency in the reform plan, stop PR progression under the integrated PR path while the blocker is admitted, or request an explicit limited-PR USER decision if the USER wants to split the docs-only reform from later template/reference implementation.
...
Final Disposition: The blocker plan preserves traceability without creating templates, golden references, shared primitives, design tokens, issue mutations, FAM mutations, or repo live-state ledgers.
