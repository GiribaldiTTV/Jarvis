...
USER review packets are early-phase Branch Planning artifacts. They are required only while Branch Readiness / Branch Planning needs USER vision or plan review, through BP3 / Workstream Entry before implementation approval. After Workstream completes, the branch must not keep regenerating BP1/BP2 USER packets by inertia during Hardening, Live Validation, PR Readiness, or Release Readiness. Later phases validate against the accepted branch vision/plan, source truth, changed-file diff, proof artifacts, fold-down receipts, and phase-specific packets instead of maintaining the early USER review packet. If a later phase discovers that the accepted vision or plan is wrong, stale, or incomplete, it must route back to Branch Planning or Branch Readiness and create a new review packet only for that repair gate.
