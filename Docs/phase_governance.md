...
- `Pre-PR Adversarial Review Firewall Missing` / `Pre-PR Adversarial Review Firewall Failed`:
  ...
  Firewall discovery must gate every changed `dev/orin_*.py` implementation and every changed path under `dev/fixtures/`; a filename keyword such as `validator`, `helper`, or `parser` is not a legal ownership test. Every changed mapped family must be named by at least one successful validation command, and that command must emit the registered executable-family marker after its tests complete. Manifest prose, a `covers_families` declaration, source-path presence, or a zero exit code without the family marker is not executable family proof. A Stage 1 packet that claims `Stage 1 Ready For Stage 2` must carry machine-readable firewall evidence under `Source Truth Context/Proof Artifacts`, bound to the packet base, HEAD, complete diff hash, changed gated files, mapped families, command report, and PASS result. Missing, stale, failed, skipped, or unbound proof keeps `Pre-PR Adversarial Review Firewall Failed` active.
...
- `PR Readiness Stage 1 - Analysis Gate`: analysis-first readiness-lock gate. Stage 1 must analyze repo truth, identify PR-readiness drift/blockers, output the full `## PR Readiness Stage 1 Analysis Packet` for USER review, including Stage 2 execution plan, and remain active until one outcome is recorded: `Stage 1 Ready For Stage 2`, `PR Readiness Stage 1 Repair Required`, `Current-Branch Branch Readiness Re-entry Required`, `New Carrier Branch Required`, or `Stage 1 USER Waiver Required`. Bounded Stage 1 repair/sync is allowed only when the current branch is the legal carrier and the USER-approved current phase/seam authorizes that repair; repair truth must be validated, committed, and pushed before Stage 1 can be declared ready. Branch Readiness Stage 1 owns the normal next runtime implementation pipeline selection, rooted in Nexus Vision, family vision, branch vision, current completed work, and the next implementation need. PR Readiness does not require selected-next truth or a waiver by default; Stage 1 owns repair or validation of selected-next truth only when USER explicitly approves PR-time selected-next sync or selected-next truth already exists and would merge as durable repo truth. Stage 1 also owns merge-target `No Active Branch` projection, no-release-debt posture, release target/floor semantics and Release Window Audit when relevant, any unavoidable merged-unreleased release-debt owner contract, and active-branch-authority cleanup when Stage 1 finds them. Stage 1 still cannot create the PR, create recurring PR watcher automation, create the next branch, execute release work, create tags/artifacts/releases, admit packages, or grant waivers without explicit USER approval. Stage 1 may encode selected-next truth only when USER explicitly approves that selected-next sync, and Stage 2 must verify the synced truth before PR creation.
  When the branch changes helper, validator, parser, fixture, packet, RAR, UIREF, Product Experience, PR, release, issue-governance, or review-bundle code/data, Stage 1 must include `Pre-PR Adversarial Review Firewall:` with the command result from `dev/orin_pr_review_churn_validation.py --pre-pr-firewall`, the mapped changed-file families, local corpus replay result, mutation/sibling coverage summary, command-to-family executable markers, and any remaining unknown or untested connector-family risk. The generated Stage 1 packet must include the matching machine-readable proof artifact; copied generic adversarial prose is not proof that the firewall ran. A green normal validator suite cannot substitute for this firewall because the firewall exists to keep Codex Connector from becoming the first adversarial parser.
  This preserves the existing analysis-first blocker repair gate inside the readiness lock.
...
The stop boundary is immediate: the first threshold-crossing review wave ends ordinary PR2 comment repair. Codex must not push another literal-only fix and request another review while the gate is active. A later review request requires a fresh current-HEAD helper report proving the root-cause receipt, complete family inventory, executable family markers, remaining-risk disposition, and zero unresolved-current threads. Repeated review requests without that proof are a governance execution failure even when each individual patch is technically correct.
...
Merge-target post-merge-stable authority projection is mandatory before PR green and is a PR Readiness Stage 1 repair responsibility when Stage 1 finds it. A non-standing PR branch must not merge an active branch authority record into `main`; the active authority record must be moved to historical/no-active posture or otherwise made merge-stable during Stage 1 before Stage 2 can execute. The single `Standing Governance Intake Branch` is the only exception: its repo record remains the durable standing routing authority on merged `main`, while the completed `RRI-*` cycle, live PR, packet, lock, return-digest, release-window, and next-action state must fold down in external operational state or Git/GitHub/helper-derived truth before the next workload. Post-merge `No Active Branch` does not require selected-next waiver truth when no USER-approved selected-next truth exists; normal successor selection waits for Branch Readiness Stage 1. Historical branch records must not retain active PR Readiness phase, active seam ownership, live/open PR wording, merge-watch ownership, or `PR Merge Verification Pending`. Operational PR/watcher facts may live in operator output or explicit historical PR sections, but merged current-state owners and historical authority records must already describe the post-merge truth that will remain valid after merge.

A post-merge projection receipt is not enough by itself. If a non-standing branch creates a separate projection file, PR Readiness Stage 1 must still fold down the real active authority record or remove it from `Active Branch Authority Records` before it reports `Stage 1 Ready For Stage 2`, before Stage 2 PR creation, and before merge approval. Projection-beside-active-authority blocks on `Merge-Stable Projection Shadowed By Active Authority`. The standing Governance exception does not create a projection shadow when its durable repo record remains the indexed standing authority and its volatile cycle state is external.

Projection-only fold-down inside the active record is also not enough. If a non-standing active branch record's `Release Readiness Health Pass` or `Post-Merge State` says the branch must become historical/no-active, must not remain active branch authority, or must become historical merged-unreleased evidence, PR Readiness Stage 1 must perform that real fold-down before Stage 2, PR green, or merge approval. Leaving the actual record in `Active Branch Authority Records` blocks on `PR Readiness Stage 1 branch-authority fold-down required`. The standing Governance record must instead project continued durable standing authority plus external completion and zero-lock closeout for the completed cycle.
