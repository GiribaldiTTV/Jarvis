...
Required State: Template, golden-reference, design-token, or shared UI primitive creation/promotion remains blocked until FAM-007 pushes its PR, the FAM-007 PR merges, post-merge rebaseline/reconciliation completes, durable FAM-007 lessons are digested, and USER explicitly approves template/reference promotion. Once the FAM-007 dependency proof exists, Governance may record `Template Dependency Resolved / Promotion Deferred`: this clears the PR-hold dependency for the current Governance reform without creating or promoting templates, golden references, design tokens, or shared UI primitives. Until USER approves promotion, branches may consume binding FAM-002 presentation grammar and current phase gates manually, but they must not claim a template/golden reference exists or treat future templates as current proof.
Allowed Values: `Template Not Yet Promoted`, `Template Dependency Resolved / Promotion Deferred`, `Future Template/Golden Reference Work`, `USER-Approved Template Promotion`, `Limited PR Path USER-Approved`, and `Blocked`.
...
Blocking Conditions: `Template Dependency Unresolved` when the FAM-007 prerequisite proof is missing, `Golden Reference Promotion Blocked` when promotion is attempted without USER approval, `Shared Primitive Promotion Blocked`, `Limited PR Path USER Approval Missing`, and `Template Treated As Existing Proof`.
Repair Owner: Standing Governance records the dependency and PR-hold semantics. Future template/reference implementation belongs to a separately approved owner/carrier after the dependency clears.
Repair Path: Keep current presentation rules enforceable manually. If dependency proof is missing, list the unresolved template/reference dependency in the reform plan and stop PR progression under the integrated PR path while the blocker is admitted. If dependency proof exists, record `Template Dependency Resolved / Promotion Deferred` in the owner docs and planning status, keep actual promotion future-gated, and allow PR Readiness to evaluate the remaining branch scope without claiming template proof. USER may still request an explicit limited-PR path when a future blocker remains admitted.
...
Final Disposition: The gate may close the no-PR dependency without creating templates, golden references, shared primitives, design tokens, issue mutations, FAM mutations, or repo live-state ledgers. Actual template/reference promotion remains a separate USER-approved implementation path.
