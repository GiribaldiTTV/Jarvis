...
Accepted Reference / Template Comparator Requirement: For UI/UX work, RAR must compare each applicable element group against the highest available accepted reference. Valid references include promoted `Docs/ui_reference_catalog/UIREF-*` records, FAM-002 presentation grammar, AI Control Center where a UIREF record names it as the strongest seed, and any other accepted visual reference routed by current source truth. Target surfaces must either reuse the same visual/control grammar, implement an equivalent structure that is visually and behaviorally indistinguishable for the relevant state, or document an exception, issue candidate, source-truth gap, reference gap, future repair, or USER waiver. If the accepted reference or template is insufficient, ambiguous, missing, or unusable, RAR must classify that as `SOURCE-TRUTH GAP` or `REFERENCE GAP` and route a Governance/UIREF repair candidate; it must not force the target FAM to silently invent a new canonical standard.
Required Accepted-Reference Table: `| Accepted Reference | Intended Reuse | Target Surface | Copy/Equivalent/Exception | Evidence | Issue |`.
Visual Element / Element-Group Standard: For same-class Nexus-owned controls, including window-control clusters, buttons, dropdowns, filters, menus, lists, cards, rows, scrollbars, status chips, dialogs, tray menus, settings surfaces, and recovery panels, RAR must check anatomy, size family, spacing/density, border, radius, glow/shadow, color roles, typography, state behavior, hover, pressed, focus, disabled, selected, loading, error/blocked/empty states, accessibility labels, hitbox, visual hierarchy, content/action separation, and backend truth mapping where applicable. Text labels and backend ownership may differ by feature; the visual grammar and behavior must remain consistent enough that the USER perceives the same Nexus control family unless source truth grants a variant.
RAR Row Status Values: `CONFORMING`, `NONCONFORMING`, `UNPROVEN`, `PARTIAL`, `EXCEPTION NEEDED`, `ISSUE CANDIDATE`, `SOURCE-TRUTH GAP`, `REFERENCE GAP`, and `OUT OF CURRENT SCOPE`. `UNPROVEN`, `PARTIAL`, `NONCONFORMING`, `EXCEPTION NEEDED`, `ISSUE CANDIDATE`, `SOURCE-TRUTH GAP`, or `REFERENCE GAP` rows block normal progression until repaired, explicitly waived, routed to an approved issue candidate, or dispositioned by the correct owner.
...
### Template / Golden Reference Dependency Gate

Rule Name: `Template / Golden Reference Dependency Gate`
Owner: `Docs/phase_governance.md` for PR-hold and blocker routing; `Docs/governance_reliability_and_repo_split_reform_candidates.md` for non-binding planning status; `Docs/family_visions/FAM-002_desktop_interface.md` for presentation law; `Docs/validation_helper_registry.md` for future helper/validator guidance.
Applies To: Governance reform, FAM-002 presentation standardization, future UI template/golden-reference promotion, design-token/shared-primitive work, and any branch that claims a reference surface/template makes UI proof complete.
Required State: Template, golden-reference, design-token, shared UI primitive, or template/reference plan creation/promotion remains blocked until FAM-007 pushes its PR, the FAM-007 PR merges, post-merge rebaseline/reconciliation completes, durable FAM-007 lessons are digested, the template/reference plan itself is completed or explicitly reclassified by USER, and USER explicitly approves template/reference promotion. FAM-007 prerequisite evidence may be recorded as `Template Prerequisite Evidence Present / Plan Blocked`, but that evidence does not clear the current integrated Governance PR hold while USER direction says the template/reference work is still required before PR Readiness. After the 2026-06-16 planning completion, the current integrated Governance branch remains under `Template / Reference Plan Completed - Current Branch Completion Hold`: PR Readiness is blocked until the admitted current-branch template/reference work is completed and hardened, or USER explicitly removes/reclassifies that work from the current PR scope. Until USER approves promotion, branches may consume binding FAM-002 presentation grammar and current phase gates manually, but they must not claim a template/golden reference exists or treat future templates as current proof.
Allowed Values: `Template Not Yet Promoted`, `Template Prerequisite Evidence Present / Plan Blocked`, `Template / Reference Plan Completed - Current Branch Completion Hold`, `Future Template/Golden Reference Work`, `USER-Approved Template Promotion`, `Limited PR Path USER-Approved`, and `Blocked`.
Invalid Values: `Template Exists By Inference`, `ChatGPT/Codex Example Treated As Golden`, `FAM-007 Active Packet Treated As Template`, `Reference Promoted Without USER Review`, `Design Token Implemented In Docs-Only Pass`, and `PR Requested While Template Blocker Admitted`.
Blocking Conditions: `Template Dependency Unresolved` when the FAM-007 prerequisite proof is missing, `Template / Reference Plan Blocked` when USER direction keeps the integrated Governance PR held pending the blocked template/reference plan, `Current Branch Template Work Incomplete` when USER direction keeps the current Governance branch open until planned template/reference work is completed, `Golden Reference Promotion Blocked` when promotion is attempted without USER approval, `Shared Primitive Promotion Blocked`, `Limited PR Path USER Approval Missing`, and `Template Treated As Existing Proof`.
Repair Owner: Standing Governance records the dependency and PR-hold semantics. Future template/reference implementation belongs to a separately approved owner/carrier after the dependency proof exists and USER admits the plan/promotion path.
Repair Path: Keep current presentation rules enforceable manually. If dependency proof is missing, list the unresolved template/reference dependency in the reform plan and stop PR progression under the integrated PR path while the blocker is admitted. If prerequisite evidence exists but the template/reference plan remains USER-blocked, record `Template Prerequisite Evidence Present / Plan Blocked` in the owner docs and planning status. If the plan is completed but USER directs that all planned work belongs to the current branch/current PR, record `Template / Reference Plan Completed - Current Branch Completion Hold`, keep PR Readiness blocked, and route next to a bounded template/reference admission and implementation cycle on the current Governance branch. Only after the admitted current-branch template/reference work is complete and hardened, or USER explicitly removes/reclassifies that work from current scope, may PR Readiness evaluate the remaining branch scope without claiming unpromoted template proof.
USER Decision Required: Required for limited PR path, template/reference promotion, design-token/shared-primitive implementation, helper/validator code, fixture mutation, or any product worktree adoption.
Validation Owner: Future helper/validator coverage may check for template/golden-reference claims after USER-approved references exist; before that, validators and Codex digests must reject claims that a non-promoted reference has proof authority.
Final Disposition: The gate may record prerequisite evidence, planning completion, and future promotion boundaries, but it must not close the no-PR dependency while USER direction keeps the integrated Governance PR held for planned template/reference work. Actual template/reference promotion remains a separate USER-approved implementation path and must not create templates, golden references, shared primitives, design tokens, issue mutations, FAM mutations, or repo live-state ledgers until that path is explicitly admitted.
