...
Required RAR Packet Fields: `RAR Stage:`, `Merged Standard Source:`, `Rebaseline / Re-entry Event:`, `Current Branch Implementation Inventory:`, `Owned Surface Inventory:`, `Affected Files / Code Paths:`, `Code-To-Visual Trace Matrix:`, `Accepted Reference / Template Comparator Matrix:`, `Visual Element / Element-Group Inspection Ledger:`, `Runtime / Backend Truth Mapping:`, `Current Branch Repair Candidates:`, `Previous / Historical Branch Issue Candidates:`, `Issue Candidate Disposition:`, `USER Packet Path:`, `USER Packet ZIP Path:`, `No Repo Live-State Tracking:`, `Repair / Waiver / Blocker:`, and `Next Legal Phase:`.
Code-To-Visual Requirement: RAR must inspect every changed line that defines or affects an owned UI/runtime element and every line that defines an affected visible element group, state, styling rule, runtime state owner, proof marker, or user-visible text. Unaffected lines may be summarized only when the packet names why they are out of scope. Each row must map code path, line or selector/widget identifier, element group, expected behavior, reference owner, observed proof, and disposition.
Accepted Reference / Template Comparator Requirement: For UI/UX work, RAR must compare each applicable element group against the highest available accepted reference. When a promoted `Docs/ui_reference_catalog/UIREF-*` record applies, that record is the reference owner. Where the AI Control Center is recorded as the strongest accepted seed for UIREF-001 or UIREF-002, branches must compare their top-level window frame and compact window-control cluster against that seed unless a more specific promoted reference exists or a USER-approved exception is recorded. If a shared code primitive/template does not yet exist, RAR must record `Template / Shared Primitive Missing` and cannot claim code-level copy/paste parity; it may still require visual/reference parity against the promoted reference grammar.
Visual Element / Element-Group Standard: For same-class Nexus-owned controls, including window-control clusters, buttons, dropdowns, menus, lists, filters, cards, rows, scrollbars, status chips, dialogs, tray menus, and settings surfaces, RAR must check anatomy, size, border, radius, glow, color, font, state behavior, hover, pressed, focus, disabled, error/blocked/empty states, accessibility labels, hitbox, visual hierarchy, and backend truth mapping where applicable. The final disposition for each element group is `PASS`, `REPAIR`, `STOP`, `WAIVED_WITH_REASON`, `ISSUE_CANDIDATE_REQUIRED`, or `NOT_APPLICABLE_WITH_REASON`.
USER Packet Requirement: When RAR finds any UI/UX/runtime/backend nonconformance, issue candidate, waiver candidate, or route-back recommendation, Codex must generate a deterministic USER packet under `C:\Nexus USER\<worktree-label>\` plus timestamped upload ZIP `C:\Nexus USER\<worktree-label>-YYYYMMDD-HHMMSS.zip` using the approved clean-regeneration packet rules. The packet must include a primary USER review file, visual/code comparator summaries, issue-candidate table, repair-versus-issue boundary, and exact USER decision text. A Codex chat digest alone is not enough when USER review is needed.
External State Owner: Active RAR findings belong in `C:\Nexus Governance State\branches\<branch_slug>\adoption_reconciliation.md` or the active external branch plan for that branch. Repo docs define the rule, schema, and durable folded receipts only. RAR must not turn branch records, backlog, roadmap, family visions, FFVs, UIREF files, or repo branch plans into live adoption ledgers.
Blocking Conditions: `Rebaseline Adoption Review Missing`, `Merged Standard Impact Ledger Missing`, `Owned Surface Nonconformance Ledger Missing`, `Accepted Reference Comparator Missing`, `Template / Shared Primitive Missing`, `Code-To-Visual Trace Missing`, `Circular Validation Evidence`, `Visual Comparator Matrix Missing`, `Current Branch Repair Vs Issue Boundary Missing`, `Owned Surface Issue Candidate Missing`, `RAR USER Packet Missing`, `Reference Parity Claim Unsupported`, `Normal Phase Progression Blocked By RAR`, and `Repo File-State Tracking`.
Circular Validation Rule: RAR evidence must be non-circular. The same helper, marker, or plan that created or claimed a surface cannot be the sole proof that the surface satisfies the new standard. RAR must combine source-truth comparison, code inspection, independent visual evidence, accepted reference comparison, and USER review or waiver where human judgment is required.
Final Disposition: RAR exits only through `No Applicable Impact`, `Adoption Green With Evidence`, `Repair Completed And Revalidated`, `USER Waiver Recorded`, `Issue Candidate Packet USER-Reviewed`, `Route Back To Earlier Gate`, or `Blocked`. Normal BP, Workstream, Hardening, LV, PR Readiness, or Release Readiness progression remains blocked until the active RAR trigger is resolved.
