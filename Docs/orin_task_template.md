...
[D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\branch_plan.md / pending Stage 2 / not applicable]
...
UFD Ledger Owner:
[D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\branch_plan.md / not applicable]
...
[present at D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\branch_plan.md / missing / not applicable]
...
[Required when requested work is outside the assigned worktree or active branch scope: route packet to D:\Nexus Desktop AI Data\Worktrees\Governance / clear]
...
[active D:\Nexus Desktop AI Data\Governance State\branches\<branch_slug>\branch_plan.md before implementation / folded branch record, repo branch-plan historical receipt, or workstream owner after PR Readiness fold-down / not applicable with reason]
...
Branch Planning is the pre-implementation phase between Branch Readiness and Workstream. Before asking USER for Workstream green-light, Codex must complete BP1 USER Branch Vision Review, BP2 USER Branch Plan Review, and BP3 Workstream Entry / Orchestration Validation, and create or refresh the active worktree's local USER hub packet under `D:\Nexus Desktop AI Data\USER\<worktree-label>` with matching timestamped upload ZIP at `D:\Nexus Desktop AI Data\USER\<worktree-label>-YYYYMMDD-HHMMSS.zip`. The packet uses root `START_HERE.md`, exactly one primary current-gate decision file under `USER Review`, generated supporting digests/checklists under `Review Aids`, and copied branch vision, active external branch plan or repo historical receipt, branch authority record, relevant Nexus/family vision files, matrix/UFD/change-intent surfaces, and other source-truth context files under `Source Truth Context`; the full non-compacted digest must include the local USER hub folder path, copied files, validation summary, exact green-light decision, and pending USER decisions.
...
For relevant desktop UI Live Validation, use the exact normal USER desktop runtime launcher declared for the UTS path, or record an explicit USER waiver. Sandbox/offscreen/direct-runtime/WebView/helper launches are supporting evidence only and cannot replace that launcher gate.
...
- recognize that a slice is a bounded admitted backlog-completion unit, SLC is only a branch-planning alias or historical ID for Slice-level line items, and a seam is the current execution checkpoint inside or between slices
...
Bot review and direct PR verification remain required PR Stage 2 evidence: a Codex Connector bot `thumbs-up reaction` or green approval comment on the current head clears `Bot Review Signal Pending` only when approval proof must be bound to the current live PR head by review commit SHA, PR timeline order, or equivalent GitHub live-head evidence, not by local commit time alone; after a bot comment closeout, a later thumbs-up/approval signal is required before PR green or merge. Stage 2 final handoff cannot be green until the post-repair bot thumbs-up/approval latch is verified. The Direct PR2 Continuation Rule blocks quiet waiting handoff after revalidation requests or repair pushes. Codex Connector review-request or revalidation PR comments must be 3-5 words only, preferably `@codex review please`; proof detail belongs in the Codex digest/helper/validator/external-state surfaces. Watcher proof applies only to a USER-approved watcher exception or historical receipt; recurring PR watcher automation is denied by default for bounded PR2.
...
- The standing governance return digest must stop on `Return Digest Origin Identity Missing` if those concrete identity fields are absent, generic, contradictory, or inferred from `C:\Nexus Desktop AI`, `D:\Nexus Desktop AI Data\Worktrees\Governance`, GitHub Desktop, or the current shell CWD instead of the accepted intake.
- If no runtime-focused branch is legally admitted yet, record the drift as a blocker and wait instead of creating a repair branch by inertia.
- Historical repair-only branch records remain traceability only and do not authorize new repair-only branch creation.
