...
Bot review and direct PR verification remain required PR Stage 2 evidence: a Codex Connector bot `thumbs-up reaction` or green approval comment on the current head clears `Bot Review Signal Pending` only when approval proof must be bound to the current live PR head by review commit SHA, PR timeline order, or equivalent GitHub live-head evidence, not by local commit time alone; after a bot comment closeout, a later thumbs-up/approval signal is required before PR green or merge. Stage 2 final handoff cannot be green until the post-repair bot thumbs-up/approval latch is verified. Codex Connector review-request or revalidation PR comments must be 3-5 words only, preferably `@codex review please`; proof detail belongs in the Codex digest/helper/validator/external-state surfaces. Watcher proof applies only to a USER-approved watcher exception or historical receipt; recurring PR watcher automation is denied by default for bounded PR2.
