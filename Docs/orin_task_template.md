...
Bot review and watcher proof remain required PR Stage 2 evidence: a Codex Connector bot `thumbs-up reaction` or green approval comment on the current head clears `Bot Review Signal Pending`; after a bot comment closeout, a later thumbs-up/approval signal is required before PR green or merge. Stage 2 final handoff cannot be green until the post-repair bot thumbs-up/approval latch is verified. Watcher proof must name the approved reporting surface, minute cadence, reports only when a watched PR status changes, copy/paste Codex prompt basis after merge, delivery proof, `PR Watcher Routing Unverified`, `PR Merge Verification Pending`, and the merge-watch seam; watcher configuration is not runtime proof.
