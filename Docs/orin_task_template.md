...
Stage 2 owns final PR execution only after the readiness-lock outcome is green. Stage 2 final handoff and merge execution cannot be green until bot-review closeout is verified as a current-head thumbs-up reaction from the Codex GitHub bot. Actionable bot comments must use the same-PR Codex bot-review repair loop: fix the comment, validate, commit, push, reply/resolve when required; then the loop asks Codex to re-analyze and waits for the fresh thumbs-up before merge. Stage 2 final handoff cannot be green until watcher runtime proof is present or the runtime-proof blocker remains active. Remember that Watcher configuration is not runtime proof. Apply the PR Watcher Mode Contract from `Docs/pr_watcher_mode_contract.md`: `Silent Monitor`, `Verify Once`, `Repair Mode`, or `Blocked Mode`, with `Watcher Health Proof:` in Stage 2 final handoff.
...
[BP1 USER Branch Vision Review / BP2 USER Branch Plan Review / BP3 Workstream Entry Orchestration Validation / not applicable with reason]

USER Branch Vision Review:
[USER_BRANCH_VISION_REVIEW.md status, accepted/waived/revision-needed, branch goal, end-state, user-visible surfaces, flow, options, Codex recommendations, USER response/digest, and exact next decision]
...
Branch Planning is the canonical phase between Branch Readiness and Workstream. BP1 owns `USER_BRANCH_VISION_REVIEW.md`, BP2 owns `USER_BRANCH_PLAN_REVIEW.md`, and BP3 is Workstream Entry / Orchestration Validation. Workstream is runtime/code implementation and code-level validation only. Before asking USER for Workstream green-light, Codex must create or refresh the active worktree's local `USER Review Hub Bundle` under the stable `C:\Nexus USER\<worktree-label>` root with sibling upload zip `C:\Nexus USER\<worktree-label>.zip`, flat copied branch vision, active branch plan, branch authority record, relevant Nexus/family vision files, matrix/UFD/change-intent surfaces, and other source-truth owner files needed for inspection; the full non-compacted digest must include the review root, worktree folder path, upload zip path, copied files, validation summary, exact green-light decision, and pending USER decisions, while technical packet metadata stays in helper output, validator output, external governance state, or chat digest.
...
Bot review and watcher proof remain required PR Stage 2 evidence: only a `thumbs-up reaction` from the Codex GitHub bot on the current PR head clears `Bot Review Signal Pending`. Bot comment closeout without a fresh thumbs-up remains `PR Validation Pending`; after fixing and resolving a bot comment, the repair loop asks Codex to re-analyze the updated head and waits for the current-head thumbs-up before merge. Watcher proof must name the approved reporting surface, minute cadence, reports only when a watched PR status changes, copy/paste Codex prompt basis after merge, delivery proof, `PR Watcher Routing Unverified`, `PR Merge Verification Pending`, and the merge-watch seam; watcher configuration is not runtime proof.
