...
Current Active Workstream: FAM-006 Workstream WS30 - ORIN Core Preset Monitor And HUD Surface Isolation Repair; WS30 reconciles the bounded WS18-WS30 repair chain as green for Hardening rerun without claiming package completion.
...
Next Legal Runtime Step: Hardening H1 - Monitoring HUD Product Surface Hardening Rerun; WS18 repaired/proved Core desktop-layer/non-interference, WS19 split the dashboard/configuration surface from the minimal anchored HUD overlay, WS20 rerouted dashboard technical proof boxes into configuration-centered content, WS21 repaired dashboard movement/scrollbar polish, WS22 added/proved the separate native minimal HUD overlay, WS23 refreshed active-user-facing proof while recording the USER overlay vision clarification, WS24 implemented/proved dashboard monitor management plus overlay mode controls, WS25 implemented/proved the edgeless overlay/display canvas, WS26 implemented/proved the standalone native overlay/display window with anchored uninteractable/no-focus mode and position preservation, WS27 implemented/proved the revised-overlay validation/live-proof package, WS28 implemented/proved standalone dashboard/overlay/Core native-surface independence, WS29 reconciled the bounded Workstream chain, and WS30 repaired/proved the fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation regression as green for Hardening rerun.
...
Current Branch Readiness State: Complete - Stage 1-R8 revalidated the Stage 2-R11 dashboard/minimal-HUD and Core non-interference rebaseline.
Current Workstream State: Green for Hardening - WS30 reconciles the bounded WS18-WS30 Workstream repair chain after WS18 repaired Core desktop-layer/non-interference, WS19 split dashboard/minimal-HUD renderer ownership, WS20 rerouted dashboard content, WS21 repaired movement/scrollbar polish, WS22 added native minimal-HUD click-through/no-focus proof, WS23 refreshed active-user-facing proof / recorded USER overlay vision, WS24 implemented dashboard monitor-management controls, WS25 implemented/proved the edgeless overlay/display canvas, WS26 implemented/proved standalone native overlay/display anchored-uninteractable behavior, WS27 repaired/proved the revised-overlay validation/live-proof package, WS28 repaired/proved dashboard/overlay/Core native-surface independence, WS29 reconciled Workstream green, and WS30 repaired/proved fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation.
Current Hardening State: Prior H1 pressure-test evidence preserved, but not sufficient for phase advancement after returned UTS FAIL; repaired product behavior must harden again before Live Validation can continue.
Current Live Validation State: Red - returned User Test Summary Step 6 and Step 7 are historical FAIL evidence; WS18 through WS30 clear the bounded Workstream repair queue, but LV1 is not green until Hardening reruns, Live Validation reruns, and returned UTS acceptance passes.
...
| `FAM-006` | Monitoring and HUD | Workstream green for Hardening rerun / LV1 historical red until rerun | `PKG-006` admitted / In Progress; package completion unclaimed | `FB-040`, HUD surface gap |
...
Family Anchor: Self
Priority: Medium
Next Workstream: Workstream WS30 Core preset-monitor and HUD surface isolation repair is green for Hardening rerun after WS18 repaired/proved Core desktop-layer/non-interference, WS19 repaired/proved the renderer-visible dashboard/minimal-HUD split, WS20 rerouted dashboard technical boxes into configuration content, WS21 repaired dashboard movement/scrollbar polish, WS22 added/proved the separate native minimal HUD overlay, WS23 refreshed active-user-facing proof while recording the USER edgeless overlay/monitor vision clarification, WS24 implemented/proved dashboard monitor management plus overlay mode controls, WS25 implemented/proved the edgeless overlay/display canvas, WS26 implemented/proved the standalone native overlay/display window with anchored uninteractable/no-focus mode, dashboard-drag isolation, and position preservation, WS27 implemented/proved the revised-overlay validation/live-proof package, WS28 implemented/proved standalone dashboard/overlay/Core native-surface independence, WS29 reconciled Workstream green, and WS30 repaired/proved fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation; WS8 through WS30 and H1 are preserved as prior evidence but do not clear returned-UTS blockers or package completion.
...
Next Legal Runtime Step: Hardening H1 - Monitoring HUD Product Surface Hardening Rerun; returned User Test Summary results remain historical FAIL evidence and Live Validation remains red, but WS18 cleared the Core opaque foreground regression, WS19 cleared the renderer-visible dashboard/minimal-HUD split, WS20 cleared the dashboard content relevance gap, WS21 cleared dashboard movement/scrollbar polish, WS22 cleared native minimal-HUD click-through/non-focus proof, WS23 refreshed proof plus recorded USER clarification that the overlay/display must behave like an edgeless snipping-tool-style placement canvas, WS24 implemented/proved dashboard monitor management plus overlay mode controls, WS25 implemented/proved the edgeless overlay/display canvas, WS26 implemented/proved the standalone native overlay/display window with anchored uninteractable/no-focus mode, dashboard-drag isolation, and position preservation, WS27 implemented/proved the revised-overlay validation/live-proof package, WS28 implemented/proved dashboard/overlay/Core native-surface independence, WS29 reconciled Workstream green, and WS30 repaired/proved fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation for Hardening rerun.
Minimal Scope: admitted runtime package for optional Nexus/NDAI hardware-monitoring HUD visual/user-facing surface, HUD dashboard/control panel, monitor create/edit and enablement model, edgeless overlay/display placement canvas, unanchored focusable edit mode, anchored uninteractable/click-through desktop integration, provider-contract-first telemetry health/setup/unavailable states, desktop placement and renderer ownership, settings or user controls visibility, fail-safe/setup/reconnect/no-data/degraded-status behavior, visual/non-invasive warning posture, and validation/live desktop proof; audio/spoken warnings, Stream Deck/plugin telemetry implementation, full sensor-platform parity, broad hardware provider platform work, advanced graphs/history/persistence, local AI, installer/capability-pack work, and broad repo-wide legacy naming migration remain deferred unless later admitted.
...
Known Pending Gaps: returned User Test Summary FAIL is rebaselined in source truth by Stage 2-R11 and still requires Hardening rerun, Live Validation rerun, and returned UTS acceptance before package completion. WS18 repaired/proved Core desktop-layer/non-interference, WS19 repaired/proved renderer-visible dashboard/configuration surface versus minimal anchored HUD ownership, WS20 rerouted technical proof boxes into useful configuration content, WS21 repaired dashboard movement/scrollbar polish, WS22 repaired/proved native minimal-HUD click-through/non-focus behavior, WS23 refreshed active-user-facing proof plus draft UTS handoff, WS24 implemented/proved dashboard monitor management plus overlay mode controls, WS25 implemented/proved the edgeless overlay/display placement canvas, WS26 implemented/proved the standalone native overlay/display window, anchored uninteractable/click-through mode, dashboard-drag isolation, and position preservation, WS27 implemented/proved revised-overlay static/internal/live validation plus draft UTS handoff freshness, WS28 implemented/proved standalone dashboard/overlay/Core native-surface independence with overlay-owned monitor-card drag/resize, WS29 reconciled Workstream green, and WS30 repaired/proved fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation. USER clarified that the dashboard is the HUD control panel; monitors are organizational group/title cards; and the overlay/display should be an edgeless placement canvas with unanchored edit mode and anchored uninteractable/click-through mode. PR Readiness, release work, and final package closeout remain pending. Broad provider-platform parity, external/plugin telemetry, audio/spoken alerts, persona switching, Stream Deck, graphs/history/persistence dashboards, local AI/capability packs, installer work, and ultra-low polling remain future-package deferrals.
...
| Package ID | FAM ID | Package Name | Package Status | Package Completion State | Branch / Release Trace | Legacy FB / PR Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `PKG-006` | `FAM-006` | Monitoring and HUD product-surface package | WS18 repaired/proved Core desktop-layer/non-interference after Stage 1-R8 revalidated the LV1 FAIL repair truth; WS19 repaired/proved the renderer-visible dashboard/minimal-HUD split; WS20 rerouted dashboard technical boxes into configuration-centered content; WS21 repaired dashboard movement/scrollbar polish; WS22 added/proved the separate native minimal HUD overlay; WS23 refreshed active-user-facing proof and recorded the USER edgeless overlay/monitor model clarification; WS24 implemented/proved dashboard monitor management plus overlay mode controls; WS25 implemented/proved the edgeless overlay/display canvas; WS26 implemented/proved the standalone native overlay/display window, anchored uninteractable/no-focus mode, dashboard-drag isolation, and position preservation; WS27 implemented/proved the revised-overlay validation/live-proof package; WS28 implemented/proved dashboard/overlay/Core native-surface independence; WS29 reconciled Workstream green; WS30 repaired/proved fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation; final package completion unclaimed | In Progress | `feature/fam-006-monitoring-hud-product-surface`; historical baseline `feature/fb-040-monitoring-thermals-performance-hud-surface`; `v1.6.0-prebeta` | `FB-040`, HUD user-facing surface gap |
...
| `SLC-016` | `PKG-006` | `FAM-006` | HUD visual and user-facing monitoring surface | Admitted | Green for Hardening Rerun | WS19 separates the dashboard/configuration surface from the minimal anchored HUD overlay at DOM/runtime-marker/proof level, WS22 adds the actual separate native minimal HUD overlay, WS24 adds dashboard monitor-management product controls, WS25 implements/proves the edgeless overlay/display canvas, WS26 implements/proves the standalone native overlay/display window with anchored/unanchored visual-state separation, WS27 refreshes revised-overlay proof and UTS handoff evidence, WS28 proves dashboard selector/editor controls are not the overlay placement owner, and WS30 proves the Core is not rendered inside or overlapping the HUD overlay/display. Final package completion still requires Hardening, Live Validation rerun, and UTS acceptance | `BR-S2-S1`; `WS1`; `WS7`; `WS9`; `WS17`; `H1`; `LV1-R1`; `WS19`; `WS22`; `WS24`; `WS25`; `WS26`; `WS27`; `WS28`; `WS29`; `WS30`; `dev/orin_monitoring_hud_surface_validation.py`; `dev/orin_monitoring_hud_live_validation.ps1` |
...
| `SLC-026` | `PKG-006` | `FAM-006` | Desktop placement and renderer ownership | Admitted | Green for Hardening Rerun | WS18/WS30 repaired Core foreground opacity/non-interference with independent Core renderer ownership, desktop-specific Core render file, transparent host proof, fixed/non-movable preset-monitor placement, no topmost foreground ownership, and before/after active-client full-desktop proof; WS19 adds separate dashboard/minimal-HUD renderer-visible ownership markers and geometry proof; WS21 smooths dashboard/native movement; WS22 adds separate native minimal overlay ownership with transparent-input click-through and no-focus/no-activate proof; WS25 adds edgeless overlay/display canvas ownership proof; WS26 upgrades the overlay/display into a separate top-level native `MonitoringHudOverlayDisplayWindow`, proves dashboard-drag isolation, anchored uninteractable/no-focus/no-activate posture, hidden anchored quick controls, and position preservation; WS28 proves dashboard/control panel, overlay/display, and Core native-surface independence plus overlay-owned monitor-card movement/resizing across the virtual desktop; WS30 proves no dashboard/Core or overlay/Core overlap. | `BR-S2-S3`; `WS3`; `WS10`; `WS12`; `LV1-R1`; `WS18`; `WS19`; `WS21`; `WS22`; `WS25`; `WS26`; `WS28`; `WS29`; `WS30`; `desktop/monitoring_hud_placement.py`; `desktop/desktop_renderer.py`; `desktop/core_visualization_renderer.py` |
| `SLC-027` | `PKG-006` | `FAM-006` | Settings and user controls visibility | Admitted | Green for Hardening Rerun | WS19 clarifies the dashboard as the configuration/settings surface that configures the minimal HUD; WS20 replaces technical proof boxes with useful configuration content; WS21 adds movement and scrollbar polish; WS28 keeps dashboard content as monitor selector/editor controls instead of treating it as the overlay placement surface; WS29 reconciles Workstream green for Hardening rerun. | `BR-S2-S4`; `WS4`; `WS11`; `WS13`; `LV1-R1`; `WS19`; `WS20`; `WS21`; `WS28`; `WS29`; `desktop/monitoring_hud_controls.py`; `desktop/orin_desktop_main.py`; `nexus_visual/monitoring_hud.js` |
...
| `SLC-029` | `PKG-006` | `FAM-006` | Validation and live desktop proof | Admitted | Green for Hardening Rerun | WS18 adds Core non-interference proof, WS19 adds dashboard/minimal-HUD split proof, WS20 adds dashboard-content proof, WS21 adds movement/scrollbar proof, WS22 adds native minimal-overlay click-through/non-focus proof, WS23 refreshes active-user-facing proof, WS24 adds dashboard monitor-management proof, WS25 adds edgeless overlay edit-canvas proof, WS26 adds standalone native overlay/display, anchored-uninteractable, dashboard-drag isolation, and position-preservation proof, WS27 adds manifest-level revised-overlay proof checklist plus fresh screenshot/UTS handoff evidence, WS28 adds standalone dashboard/overlay/Core independence proof plus overlay-owned card drag/resize proof, WS29 adds Workstream reconciliation, and WS30 adds before/after active-client desktop proof plus fixed preset-monitor Core/HUD surface separation manifest fields. Final package completion remains pending Hardening rerun, Live Validation rerun, and UTS acceptance | `BR-S2-S6`; `WS6`; `WS8`; `WS17`; `H1`; `LV1-R1`; `WS18`; `WS19`; `WS20`; `WS21`; `WS22`; `WS23`; `WS24`; `WS25`; `WS26`; `WS27`; `WS28`; `WS29`; `WS30`; `dev/orin_monitoring_hud_live_validation.ps1`; `dev/orin_monitoring_hud_internal_sandbox_validation.py` |
...
Summary: Monitoring and HUD scaffold/boundary/runtime work remains credited as prior evidence, but returned LV1 UTS evidence is historical FAIL until rerun. WS18 repairs/proves Core desktop-layer/non-interference; WS19 repairs/proves the renderer-visible dashboard/minimal-HUD split; WS20 reroutes dashboard technical boxes into useful configuration content; WS21 repairs dashboard movement/scrollbar polish; WS22 repairs/proves native minimal-HUD click-through/non-focus behavior; WS23 refreshes active-user-facing proof and records the USER edgeless overlay/monitor model clarification; WS24 implements/proves dashboard monitor management plus overlay mode controls; WS25 implements/proves the edgeless overlay/display canvas; WS26 implements/proves the standalone native overlay/display window, anchored uninteractable/no-focus mode, dashboard-drag isolation, and position preservation; WS27 implements/proves revised-overlay proof expansion; WS28 implements/proves standalone dashboard/overlay/Core native-surface independence; WS29 reconciles Workstream green; WS30 repairs/proves fixed non-movable preset-monitor ORIN/Core plus HUD surface isolation. No PR has been created and no release work is authorized.
