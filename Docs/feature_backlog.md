...
| `FAM-006` | Monitoring and HUD | High | Open / Active Overlay Recording Runtime Foundation Branch Readiness Stage 2 setup; prior Recording Profile Runtime Foundation Workstream rolled back by USER request; Overlay Display Acceptance Foundation released in `v1.7.19-prebeta` via PR #207 remains historical green | `PKG-006` Active Overlay Recording Runtime Foundation is admitted for Workstream Entry analysis across SLC-051 through SLC-055; future recording must derive targets from active Overlay Profile membership, use the HUD Overlay card as launcher/target preview, use a compact standalone Recording Control window as the future control surface, keep Native Log Loader as a separate future graph/log viewer, and preserve per-overlay effective polling policy as future FAM-006 planning. Recording execution, output file writing, real Start/Stop controls, tray controls, export/share, provider/model work, broad theme/skin work, FAM-007 work, PR creation, merge, release, issue mutation, artifacts/raw evidence handling beyond approved review bundles, old branch cleanup/deletion, and sibling-worktree mutation remain USER-gated | `Docs/family_visions/FAM-006_monitoring_and_hud.md` | `Docs/branch_records/feature_fam_006_active_overlay_recording_runtime_foundation.md` |
...
Package Admission State: Active Branch Readiness setup / runtime implementation pending Workstream Entry
