...
Current Workstream State: Workstream completion remains green on `feature/backlog-family-governance-reform`; the active branch authority has now transitioned to `Hardening`, and Hardening H1 `Post-Workstream Governance Validation` is in progress; no backlog-family shape, selected-next truth, lifetime-dossier routing, family-index split, or alias/support non-selectability truth changed during the phase handoff; and FB-049 remains the only selected-next user-facing candidate.
