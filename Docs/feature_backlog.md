...
Current Active Workstream: None - FAM-006 is in Branch Readiness Stage 2 source-truth repair after USER Dashboard feedback blocked the refreshed LV1 handoff; next legal runtime seam is bounded Workstream WS37 after this source-truth repair, not LV2.
...
Next Legal Runtime Step: Workstream WS37 - Dashboard Runtime Window Tray Safety And NCP Regression Isolation. USER Dashboard feedback blocks current Dashboard acceptance, makes affected WS31-WS36/H1/LV1 proof supporting but stale, keeps LV2 blocked, and requires bounded Workstream repair before Hardening rerun and refreshed LV1 UTS handoff. Overlay/display acceptance remains deferred/dormant/non-gating supporting evidence, Core repair is dependency-only, and package completion remains unclaimed.
...
Current Branch Readiness State: Complete - Stage 2 records returned USER Dashboard feedback, repairs source-truth/ledger dispositions, keeps PKG-006 In Progress, and hands off to bounded Workstream WS37.
Current Workstream State: Reopened pending - WS31 through WS36 remain preserved as supporting history, but affected Dashboard proof is stale until runtime/window/tray, focus, layout, IA/content, and proof repairs pass.
...
| `FAM-006` | Monitoring and HUD | Branch Readiness Stage 2 USER feedback disposition repaired; next legal seam WS37 runtime/window/tray safety repair; LV2 blocked | `PKG-006` admitted / In Progress; package completion unclaimed | `FB-040`, HUD surface gap |
...
Family Anchor: Self
Priority: Medium
Next Workstream: Workstream WS37 - Dashboard Runtime Window Tray Safety And NCP Regression Isolation. WS31 through WS36, H1, and LV1 Stage 1 remain preserved as supporting history, but returned USER Dashboard feedback blocks acceptance and makes affected proof stale. The Dashboard/control panel remains the primary current-branch interface release surface, Overlay/display acceptance remains deferred/non-gating, ORIN/Core repair remains dependency-only, and no PR/package completion claim is allowed.
...
Next Legal Runtime Step: Workstream WS37 - Dashboard Runtime Window Tray Safety And NCP Regression Isolation. LV2 is blocked because USER Dashboard feedback rejected current acceptance before returned-result digestion; prior Workstream/H1/LV1 proof remains supporting/stale for affected rows until repair, Hardening rerun, refreshed LV1, and returned USER PASS/WAIVED results or waiver.
...
| Package ID | FAM ID | Package Name | Package Status | Package Completion State | Branch / Release Trace | Legacy FB / PR Evidence |
| --- | --- | --- | --- | --- | --- | --- |
| `PKG-006` | `FAM-006` | Monitoring and HUD product-surface package | Returned USER Dashboard feedback blocks current Dashboard acceptance; next legal seam is WS37 runtime/window/tray safety and NCP regression isolation; WS18 through WS36, H1, and LV1 are preserved as supporting/stale evidence for affected rows; Overlay/display acceptance deferred/non-gating; Core repair dependency-only; final package completion unclaimed | In Progress | `feature/fam-006-monitoring-hud-product-surface`; historical baseline `feature/fb-040-monitoring-thermals-performance-hud-surface`; `v1.6.0-prebeta` | `FB-040`, HUD user-facing surface gap |
