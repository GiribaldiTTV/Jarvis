...
def run_single_instance_wait_boundary_scenario():
    ...
    from desktop.single_instance import acquire_or_prompt_replace

    class FakeGuard:
        def __init__(self, release_after_seconds):
            self.release_after_seconds = max(0.0, float(release_after_seconds))
        ...
        def acquire(self):
            ...
            if not self.conflict_reported:
                ...
                return False
            return (time.monotonic() - self.started_at) >= self.release_after_seconds
    ...
    case_definitions = (
        ("early_release", 0.45, 0.5, True),
        ("exact_boundary_release", 0.5, 0.5, True),
        ("late_release", 0.65, 0.5, False),
    )
    case_results = []
    ...
    try:
        ...
        for case_name, release_after_seconds, wait_seconds, expected_success in case_definitions:
            guard = FakeGuard(release_after_seconds)
            signal = FakeSignal()
            events = []
            started_at = time.monotonic()
            acquired = acquire_or_prompt_replace(
                guard,
                signal,
            ...
            case_results.append(
                {
                    ...
                    "events": events,
                    "signal_calls": signal.signal_calls,
                    "clear_calls": signal.clear_calls,
                    "acquire_attempts": guard.acquire_attempts,
                }
            )
    finally:
    ...
    for case in case_results:
        ...
        detail = (
            f"release_after={case['release_after_seconds']:.2f}s, wait={case['wait_seconds']:.2f}s, "
            f"elapsed={case['elapsed_seconds']:.3f}s, signal_calls={case['signal_calls']}, "
            f"clear_calls={case['clear_calls']}, acquire_attempts={case['acquire_attempts']}, "
            f"events={case['events']}"
        )
        conflict_index = first_marker_index(case["events"], "SINGLE_INSTANCE_CONFLICT_DETECTED")
        prompt_accept_index = first_marker_index(case["events"], "REPLACE_PROMPT_AUTO_ACCEPTED")
        signal_sent_index = first_marker_index(case["events"], "RELAUNCH_SIGNAL_SENT")
        reacquire_index = first_marker_index(case["events"], "RELAUNCH_ACQUIRED_AFTER_WAIT")
        replacement_confirmed_index = first_marker_index(
            case["events"],
            "RELAUNCH_REPLACEMENT_SESSION_CONFIRMED",
        )
        wait_timeout_index = first_marker_index(case["events"], "RELAUNCH_WAIT_TIMEOUT")
        signal_failed_index = first_marker_index(case["events"], "RELAUNCH_SIGNAL_FAILED")

        checks[f"{prefix}_conflict_accept_and_signal_order"] = line_status(
            conflict_index >= 0
            and prompt_accept_index > conflict_index
            and signal_sent_index > prompt_accept_index,
            detail,
        )
        checks[f"{prefix}_classification_matches_expected_boundary"] = line_status(
            case["acquired"] is case["expected_success"],
            detail,
        )

        if case["expected_success"]:
            checks[f"{prefix}_reacquires_without_timeout"] = line_status(
                reacquire_index > signal_sent_index >= 0
                and replacement_confirmed_index >= reacquire_index
                and wait_timeout_index < 0
                and signal_failed_index < 0
                and case["clear_calls"] == 1,
                detail,
            )
        else:
            checks[f"{prefix}_times_out_without_replacement_markers"] = line_status(
                wait_timeout_index > signal_sent_index >= 0
                and reacquire_index < 0
                and replacement_confirmed_index < 0
                and signal_failed_index < 0
                and case["clear_calls"] == 0,
                detail,
            )
        ...
        class FakeWindow:
            def monitoring_hud_feature_state(self):
                return {
                    ...
                    "resident_route_reason": "HUD is off in settings",
                }
...
def validate_tray_monitoring_hud_route_state_actions():
    previous_qt_platform = os.environ.get("QT_QPA_PLATFORM")
    os.environ["QT_QPA_PLATFORM"] = "offscreen"
    ...
    try:
        ...
        def action_snapshot(tray_entry):
            ...
            hud_actions = []
            if tray_entry.hud_menu is not None and tray_entry.hud_menu.menuAction().isVisible():
                hud_actions = [
                    action
                    for action in tray_entry.hud_menu.actions()
                    if not action.isSeparator() and action.isVisible()
                ]
            all_actions = [*actions, *hud_actions]
            open_action_enabled = next(
                (action.isEnabled() for action in all_actions if action.text() == "Open HUD Dashboard"),
                False,
            )
            close_action_enabled = next(
                (action.isEnabled() for action in all_actions if action.text() == "Close HUD Dashboard"),
                False,
            )
            blocked_action_enabled = next(
                (
                    action.isEnabled()
                    for action in all_actions
                    if action.text() == "HUD Dashboard unavailable"
                ),
                None,
            )
            return {
                "texts": [action.text() for action in all_actions],
                ...
                "dashboard_blocked_enabled": blocked_action_enabled,
                "fallback_buttons": [],
            }
        ...
        try:
            ...
            initialized = tray_entry.initialize()
            user_disabled = action_snapshot(tray_entry)
            hidden_snapshots = {"disabled_by_user": user_disabled}
            for state_id in ("not_installed", "unsupported", "unknown"):
                fake_window.set_hud_state(
                    feature_enabled=False,
                    dashboard_visible=False,
                    resident_route_state=state_id,
                    resident_route_reason=f"HUD state {state_id}",
                )
                tray_entry.refresh_monitoring_hud_actions(f"validation_{state_id}")
                hidden_snapshots[state_id] = action_snapshot(tray_entry)
            fake_window.set_hud_state(
                feature_enabled=True,
                dashboard_visible=False,
                resident_route_state="enabled_available",
                resident_route_reason="HUD is available",
            )
            tray_entry.refresh_monitoring_hud_actions("validation_enabled_available")
            enabled_available = action_snapshot(tray_entry)
            ...
            fake_window.set_hud_state(
                feature_enabled=True,
                dashboard_visible=False,
                resident_route_state="enabled_temporarily_blocked",
                resident_route_reason="Dashboard is warming up",
            )
            tray_entry.refresh_monitoring_hud_actions("validation_temporarily_blocked")
            temporarily_blocked = action_snapshot(tray_entry)
            requests_before_blocked_click = list(fake_window.dashboard_requests)
            tray_entry.request_monitoring_hud_dashboard_from_tray("validation_blocked")
            requests_after_blocked_click = list(fake_window.dashboard_requests)
            blocked_after_click = action_snapshot(tray_entry)
            tray_entry.close()
        finally:
            tray_mod.QSystemTrayIcon = original_tray_icon
        ...
        return {
            ...
            "retry_request_result": retry_request_result,
            "retry_request_snapshot": retry_request_snapshot,
            "temporarily_blocked": temporarily_blocked,
            "blocked_after_click": blocked_after_click,
            "requests_before_blocked_click": requests_before_blocked_click,
            "requests_after_blocked_click": requests_after_blocked_click,
        ...
        return {
            ...
            "temporarily_blocked": {},
            "blocked_after_click": {},
            "requests_before_blocked_click": [],
            "requests_after_blocked_click": [],
            "dashboard_requests": [],
            "final_state": {},
            "error": f"{type(exc).__name__}: {exc}",
        }
...
def run_repeated_signal_failure_relaunch_scenario():
    scenario_name = "vbs_consecutive_relaunch_signal_failures"
    scenario_root = os.path.join(BASE_LOG_ROOT, scenario_name)
    preexisting_processes_before, preexisting_processes_killed, preexisting_processes_after = (
        cleanup_launch_chain_processes_for_log_root(BASE_LOG_ROOT)
    )
...
'''
    ...
    try:
        if scenario_root_entries_after_reset or not launch_command:
            ...
            for index in range(len(incoming_procs)):
                ...
                failure_deadline = time.time() + 15.0
                while time.time() < failure_deadline:
                    ...
                    if incoming_runtime_logs[index]:
                        ...
                        if any(
                            RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line
        ...
        incoming_indexes.append(
            {
                ...
                "signal_failed": first_marker_index(lines, "RELAUNCH_SIGNAL_FAILED"),
                "failure_outcome": first_marker_index(
                    lines,
                    RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER,
                ),
                ...
                "wait_timeout": first_marker_index(lines, "RELAUNCH_WAIT_TIMEOUT"),
    ...
    checks = {
        ...
        "active_session_reached_settled_before_failures": line_status(
        ...
        "active_session_unchanged_during_signal_failures": line_status(
        ...
        "active_session_lifecycle_completed_after_failures": line_status(
            (
        ...
        "incoming_launches_conflict_and_accept": line_status(
            all(
                info["conflict"] >= 0 and info["prompt_accept"] > info["conflict"] >= 0
                for info in incoming_indexes
            ),
            ordering_detail,
        ),
        "incoming_launches_record_signal_failure": line_status(
            all(
                info["signal_failed"] > info["prompt_accept"] >= 0
                and info["failure_outcome"] > info["signal_failed"]
                for info in incoming_indexes
            ),
            ordering_detail,
        ),
...
def run_accepted_relaunch_wait_timeout_scenario():
    scenario_name = "vbs_accepted_relaunch_wait_timeout"
    scenario_root = os.path.join(BASE_LOG_ROOT, scenario_name)
    preexisting_processes_before, preexisting_processes_killed, preexisting_processes_after = (
        cleanup_launch_chain_processes_for_log_root(BASE_LOG_ROOT)
    )
...
'''
    ...
    incoming_stderr = ""
    first_runtime_log = ""
    incoming_runtime_log = ""
    first_runtime_lines = []
    incoming_runtime_lines = []
    first_settled_seen = False
    active_session_lost_ownership_before_manual_shutdown = False
    shutdown_hotkey_sent = False
    shutdown_hotkey_detail = "hotkey not sent"
    shutdown_hotkey_attempts = 0
    ...
    try:
        if scenario_root_entries_after_reset or not launch_command:
            ...
            timeout_deadline = time.time() + 20.0
            while time.time() < timeout_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                new_logs = [path for path in runtime_logs if path != first_runtime_log]
                if new_logs and not incoming_runtime_log:
                    incoming_runtime_log = new_logs[0]

                if first_runtime_log:
                    first_runtime_lines = read_lines(first_runtime_log)
                    if any("RENDERER_MAIN|SHUTDOWN_REQUESTED" in line for line in first_runtime_lines):
                        active_session_lost_ownership_before_manual_shutdown = True
                    if any(SINGLE_INSTANCE_RELEASED_MARKER in line for line in first_runtime_lines):
                        active_session_lost_ownership_before_manual_shutdown = True
                    if any(
                        RELAUNCH_REPLACEMENT_SESSION_ACTIVE_MARKER in line
                ...
                if incoming_runtime_log:
                    ...
                    if any(
                        ...
                        break
                if incoming_proc.poll() is not None and incoming_runtime_log:
                    break
                time.sleep(0.2)
        ...
        for proc in (first_proc, incoming_proc):
            if proc is None:
                continue
            try:
                if proc.poll() is None:
            ...
            if proc is first_proc:
                ...
                incoming_stdout = (stdout_text or "").strip()
                incoming_stderr = (stderr_text or "").strip()
    ...
    first_settled_index = first_marker_index(first_runtime_lines, AUTHORITATIVE_DESKTOP_SETTLED_MARKER)
    first_relaunch_request_index = first_marker_index(first_runtime_lines, "RENDERER_MAIN|RELAUNCH_REQUEST_RECEIVED")
    first_ignore_index = first_marker_index(first_runtime_lines, "RENDERER_MAIN|HARNESS_RELAUNCH_REQUEST_IGNORED")
    first_shutdown_index = first_marker_index(first_runtime_lines, "RENDERER_MAIN|SHUTDOWN_REQUESTED")
    first_exit_index = first_marker_index(first_runtime_lines, "RENDERER_MAIN|EVENT_LOOP_EXIT|code=0")
    first_recoverable_complete_index = first_marker_index(
        first_runtime_lines,
    ...
    incoming_conflict_index = first_marker_index(incoming_runtime_lines, "SINGLE_INSTANCE_CONFLICT_DETECTED")
    incoming_prompt_accept_index = max(
        first_marker_index(incoming_runtime_lines, "REPLACE_PROMPT_ACCEPTED"),
        first_marker_index(incoming_runtime_lines, "REPLACE_PROMPT_AUTO_ACCEPTED"),
    )
    incoming_signal_sent_index = first_marker_index(incoming_runtime_lines, "RELAUNCH_SIGNAL_SENT")
    incoming_signal_failed_index = first_marker_index(incoming_runtime_lines, "RELAUNCH_SIGNAL_FAILED")
    ...
    incoming_renderer_spawn_index = first_marker_index(
        incoming_runtime_lines,
        "STATUS|SUCCESS|RENDERER_PROCESS_SPAWN",
    )
    incoming_reacquire_index = first_marker_index(incoming_runtime_lines, "RELAUNCH_ACQUIRED_AFTER_WAIT")
    incoming_replacement_confirmed_index = first_marker_index(
        incoming_runtime_lines,
        "RELAUNCH_REPLACEMENT_SESSION_CONFIRMED",
    )
    incoming_replacement_active_index = first_marker_index(
        incoming_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_ACTIVE_MARKER,
    )
    incoming_settled_index = first_marker_index(incoming_runtime_lines, AUTHORITATIVE_DESKTOP_SETTLED_MARKER)
    incoming_replacement_settled_index = first_marker_index(
        incoming_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER,
    )
    incoming_release_index = first_marker_index(incoming_runtime_lines, SINGLE_INSTANCE_RELEASED_MARKER)
    incoming_already_running_index = first_marker_index(
        incoming_runtime_lines,
        "STATUS|SKIP|LAUNCHER_RUNTIME|ALREADY_RUNNING",
    )
    ...
    ordering_detail = (
        f"first_settled={first_settled_index}, first_relaunch_request={first_relaunch_request_index}, "
        f"first_ignore={first_ignore_index}, first_shutdown={first_shutdown_index}, first_exit={first_exit_index}, "
        f"first_recoverable_complete={first_recoverable_complete_index}, first_release={first_release_index}, "
        f"first_replacement_active={first_replacement_active_index}, "
        f"first_replacement_settled={first_replacement_settled_index}, "
        f"incoming_conflict={incoming_conflict_index}, incoming_prompt_accept={incoming_prompt_accept_index}, "
        f"incoming_signal_sent={incoming_signal_sent_index}, incoming_signal_failed={incoming_signal_failed_index}, "
        f"incoming_wait_timeout={incoming_wait_timeout_index}, "
        f"incoming_wait_timeout_outcome={incoming_wait_timeout_outcome_index}, "
        f"incoming_runtime_start={incoming_runtime_start_index}, incoming_renderer_spawn={incoming_renderer_spawn_index}, "
        f"incoming_reacquire={incoming_reacquire_index}, "
        f"incoming_replacement_confirmed={incoming_replacement_confirmed_index}, "
        f"incoming_replacement_active={incoming_replacement_active_index}, incoming_settled={incoming_settled_index}, "
        f"incoming_replacement_settled={incoming_replacement_settled_index}, incoming_release={incoming_release_index}"
    )
    ...
    checks = {
        ...
        "two_runtime_logs_created": line_status(
            bool(first_runtime_log)
            and bool(incoming_runtime_log)
            and first_runtime_log != incoming_runtime_log,
            ", ".join(path or "missing" for path in [first_runtime_log, incoming_runtime_log]),
        ),
        "active_session_reached_settled_before_timeout": line_status(
            first_settled_index >= 0,
            AUTHORITATIVE_DESKTOP_SETTLED_MARKER if first_settled_index >= 0 else "missing authoritative settled marker",
        ),
        "active_session_received_but_ignored_relaunch_request": line_status(
            first_relaunch_request_index > first_settled_index >= 0
            and first_ignore_index > first_relaunch_request_index,
            ordering_detail,
        ),
        "active_session_remained_owner_during_wait_timeout": line_status(
            not active_session_lost_ownership_before_manual_shutdown
            and (
                first_shutdown_index < 0
                or first_shutdown_index > first_ignore_index >= 0
            ),
            ordering_detail,
        ),
        "active_session_replacement_markers_absent": line_status(
        ...
        "incoming_launch_conflict_and_accept": line_status(
            incoming_conflict_index >= 0 and incoming_prompt_accept_index > incoming_conflict_index,
            ordering_detail,
        ),
        "incoming_launch_records_wait_timeout": line_status(
            incoming_signal_sent_index > incoming_prompt_accept_index >= 0
            and incoming_wait_timeout_index > incoming_signal_sent_index
            and incoming_wait_timeout_outcome_index > incoming_wait_timeout_index,
            ordering_detail,
        ),
        "incoming_launch_never_starts_runtime_ownership": line_status(
            incoming_runtime_start_index < 0
            and incoming_renderer_spawn_index < 0
            and incoming_settled_index < 0,
            ordering_detail,
        ),
        "incoming_launch_emits_no_replacement_markers": line_status(
            incoming_signal_failed_index < 0
            and incoming_reacquire_index < 0
            and incoming_replacement_confirmed_index < 0
            and incoming_replacement_active_index < 0
            and incoming_replacement_settled_index < 0
            and incoming_release_index < 0,
            ordering_detail,
        ),
        "incoming_launch_generic_already_running_marker_absent": line_status(
            incoming_already_running_index < 0,
            ordering_detail,
        ),
        "incoming_launch_no_failure_flow": line_status(
            not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in incoming_runtime_lines),
            "STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE absent",
        ),
        "single_instance_guard_remained_with_active_session": line_status(
            first_relaunch_request_index >= 0
        ...
        "active_session_lifecycle_completed_after_timeout": line_status(
            (
                first_shutdown_index > first_ignore_index >= 0
                and first_exit_index > first_shutdown_index
                and first_release_index > first_exit_index
            )
            or (
                first_recoverable_complete_index > first_ignore_index >= 0
                and first_release_index > first_recoverable_complete_index
            ),
            ordering_detail,
        ),
        ...
        "traceback_absent": line_status(
            "Traceback" not in first_stderr and "Traceback" not in incoming_stderr,
            "\n".join(
                part for part in [first_stderr, incoming_stderr, first_stdout, incoming_stdout] if part
            ) or "no traceback in launcher output",
        ),
        "scenario_preflight_cleanup_optional": line_status(
            not preexisting_processes_after,
    ...
    return {
        "scenario_name": scenario_name,
        "log_root": scenario_root,
        "runtime_log": incoming_runtime_log or first_runtime_log,
        "stdout": "\n".join(part for part in [first_stdout, incoming_stdout] if part),
        "stderr": "\n".join(part for part in [first_stderr, incoming_stderr] if part),
        "checks": checks,
    }
...
def run_declined_relaunch_cycle_scenario():
    ...
    ordering_detail = (
        ...
        f"first_replacement_settled={first_replacement_settled_index}, "
        + ", ".join(
            f"incoming{index + 1}={info}"
            for index, info in enumerate(incoming_indexes)
        )
    )
...
def run_mixed_signal_failure_then_accept_relaunch_scenario():
    scenario_name = "vbs_signal_failure_then_accept_relaunch_cycle"
    scenario_root = os.path.join(BASE_LOG_ROOT, scenario_name)
    preexisting_processes_before, preexisting_processes_killed, preexisting_processes_after = (
        cleanup_launch_chain_processes_for_log_root(BASE_LOG_ROOT)
    )
...
'''
    ...
    try:
        if scenario_root_entries_after_reset or not launch_command:
            ...
            while time.time() < failure_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                new_failure_logs = [path for path in runtime_logs if path != first_runtime_log]
                if new_failure_logs and not failure_runtime_log:
                    failure_runtime_log = new_failure_logs[0]
                ...
                if failure_runtime_log:
                    failure_runtime_lines = read_lines(failure_runtime_log)
                    if any(
                        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line
                        for line in failure_runtime_lines
                    ):
                        break
                if failure_proc.poll() is not None and failure_runtime_log:
                    break
                time.sleep(0.2)
            ...
            while time.time() < accept_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                known_logs = {first_runtime_log, failure_runtime_log}
        ...
        for proc in (first_proc, failure_proc, accept_proc):
            if proc is None:
                continue
            try:
                if proc.poll() is None:
            ...
            if proc is first_proc:
                first_stdout = (stdout_text or "").strip()
                first_stderr = (stderr_text or "").strip()
            elif proc is failure_proc:
                failure_stdout = (stdout_text or "").strip()
                failure_stderr = (stderr_text or "").strip()
            else:
    ...
    classified_failure_runtime_log = ""
    classified_accept_runtime_log = ""
    for runtime_log in all_runtime_logs:
        lines = read_lines(runtime_log)
        if any(RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line for line in lines):
            classified_failure_runtime_log = runtime_log
            continue
    ...
    if classified_first_runtime_log and classified_failure_runtime_log and classified_accept_runtime_log:
        first_runtime_log = classified_first_runtime_log
        failure_runtime_log = classified_failure_runtime_log
    ...
    failure_prompt_accept_index = max(
        first_marker_index(failure_runtime_lines, "REPLACE_PROMPT_ACCEPTED"),
        first_marker_index(failure_runtime_lines, "REPLACE_PROMPT_AUTO_ACCEPTED"),
    )
    failure_signal_failed_index = first_marker_index(failure_runtime_lines, "RELAUNCH_SIGNAL_FAILED")
    failure_outcome_index = first_marker_index(
        failure_runtime_lines,
        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER,
    )
    failure_runtime_start_index = first_marker_index(
        failure_runtime_lines,
        "STATUS|START|LAUNCHER_RUNTIME",
    )
    failure_renderer_spawn_index = first_marker_index(
        failure_runtime_lines,
        "STATUS|SUCCESS|RENDERER_PROCESS_SPAWN",
    )
    failure_signal_sent_index = first_marker_index(failure_runtime_lines, "RELAUNCH_SIGNAL_SENT")
    failure_reacquire_index = first_marker_index(failure_runtime_lines, "RELAUNCH_ACQUIRED_AFTER_WAIT")
    failure_replacement_active_index = first_marker_index(
        failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_ACTIVE_MARKER,
    )
    failure_replacement_settled_index = first_marker_index(
        failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER,
    )
    failure_release_index = first_marker_index(failure_runtime_lines, SINGLE_INSTANCE_RELEASED_MARKER)
    failure_settled_index = first_marker_index(failure_runtime_lines, AUTHORITATIVE_DESKTOP_SETTLED_MARKER)
    ...
    ordering_detail = (
        ...
        f"failure_reacquire={failure_reacquire_index}, failure_replacement_active={failure_replacement_active_index}, "
        f"failure_settled={failure_settled_index}, accept_conflict={accept_conflict_index}, "
        f"accept_prompt_accept={accept_prompt_accept_index}, accept_signal_sent={accept_signal_sent_index}, "
        f"accept_reacquire={accept_reacquire_index}, accept_replacement_confirmed={accept_replacement_confirmed_index}, "
        f"accept_replacement_active={accept_replacement_active_index}, accept_settled={accept_settled_index}, "
        f"accept_replacement_settled={accept_replacement_settled_index}, accept_shutdown={accept_shutdown_index}, "
        f"accept_exit={accept_exit_index}, accept_recoverable_complete={accept_recoverable_complete_index}, "
        f"accept_release={accept_release_index}"
    )
    ...
    checks = {
        ...
        "three_runtime_logs_created": line_status(
            bool(first_runtime_log)
            and bool(failure_runtime_log)
            and bool(accept_runtime_log)
            and len({first_runtime_log, failure_runtime_log, accept_runtime_log}) == 3,
            ", ".join(path or "missing" for path in [first_runtime_log, failure_runtime_log, accept_runtime_log]),
        ),
        "first_session_reached_settled_before_failure_attempt": line_status(
            first_settled_index >= 0,
            AUTHORITATIVE_DESKTOP_SETTLED_MARKER if first_settled_index >= 0 else "missing authoritative settled marker",
        ),
        "first_session_unchanged_during_failure_attempt": line_status(
            not active_session_changed_before_accept and first_relaunch_request_index > first_settled_index >= 0,
            ordering_detail,
        ),
        "failure_launch_conflict_accept_and_signal_failure": line_status(
            failure_conflict_index >= 0
            and failure_prompt_accept_index > failure_conflict_index
            and failure_signal_failed_index > failure_prompt_accept_index
            and failure_outcome_index > failure_signal_failed_index,
            ordering_detail,
        ),
        "failure_launch_never_starts_runtime_ownership": line_status(
            failure_runtime_start_index < 0
            and failure_renderer_spawn_index < 0
            and failure_signal_sent_index < 0
            and failure_reacquire_index < 0
            and failure_settled_index < 0
            and failure_release_index < 0,
            ordering_detail,
        ),
        "failure_launch_emits_no_replacement_markers": line_status(
            failure_replacement_active_index < 0 and failure_replacement_settled_index < 0,
            ordering_detail,
        ),
        "accept_launch_transfers_ownership_cleanly": line_status(
            accept_conflict_index >= 0
            and accept_prompt_accept_index > accept_conflict_index
            and accept_signal_sent_index > accept_prompt_accept_index
            and accept_reacquire_index > accept_signal_sent_index
            and accept_replacement_confirmed_index >= accept_reacquire_index
            and accept_replacement_active_index > accept_reacquire_index
        ...
        "accept_launch_shutdown_hotkey_optional": line_status(
            True,
            accept_shutdown_hotkey_detail if accept_shutdown_hotkey_sent else "not sent",
        ),
        "failure_and_success_outcomes_stay_distinct": line_status(
            failure_outcome_index >= 0
            and accept_replacement_settled_index >= 0
            and not any(RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line for line in accept_runtime_lines)
            and not any(RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER in line for line in failure_runtime_lines),
            ordering_detail,
        ),
        "no_failure_flow_in_failure_or_accept_launch": line_status(
            not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in failure_runtime_lines)
            and not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in accept_runtime_lines),
            "STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE absent",
        ),
        "traceback_absent": line_status(
            "Traceback" not in first_stderr
            and "Traceback" not in failure_stderr
            and "Traceback" not in accept_stderr,
            "\n".join(
                part
                for part in [
                    first_stderr,
    ...
    return {
        "scenario_name": scenario_name,
        "log_root": scenario_root,
        "runtime_log": accept_runtime_log or failure_runtime_log or first_runtime_log,
        "stdout": "\n".join(part for part in [first_stdout, failure_stdout, accept_stdout] if part),
        "stderr": "\n".join(part for part in [first_stderr, failure_stderr, accept_stderr] if part),
        "checks": checks,
    }
...
def run_mixed_failure_decline_accept_failure_relaunch_scenario():
    scenario_name = "vbs_failure_decline_accept_failure_relaunch_cycle"
    scenario_root = os.path.join(BASE_LOG_ROOT, scenario_name)
    preexisting_processes_before, preexisting_processes_killed, preexisting_processes_after = (
        cleanup_launch_chain_processes_for_log_root(BASE_LOG_ROOT)
    )
...
'''
    ...
    first_proc = None
    first_failure_proc = None
    decline_proc = None
    accept_proc = None
    final_failure_proc = None
    ...
    accept_stdout = ""
    accept_stderr = ""
    final_failure_stdout = ""
    final_failure_stderr = ""

    first_runtime_log = ""
    first_failure_runtime_log = ""
    decline_runtime_log = ""
    accept_runtime_log = ""
    final_failure_runtime_log = ""

    first_runtime_lines = []
    first_failure_runtime_lines = []
    decline_runtime_lines = []
    accept_runtime_lines = []
    final_failure_runtime_lines = []

    first_settled_seen = False
    accept_settled_seen = False
    active_session_changed_before_accept_phase = False
    accepted_session_changed_during_final_failure = False
    accept_shutdown_hotkey_sent = False
    accept_shutdown_hotkey_detail = "hotkey not sent"
    accept_shutdown_hotkey_attempts = 0
    ...
    try:
        if scenario_root_entries_after_reset or not launch_command:
            ...
            while time.time() < failure_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                new_failure_logs = [path for path in runtime_logs if path != first_runtime_log]
                if new_failure_logs and not first_failure_runtime_log:
                    first_failure_runtime_log = new_failure_logs[0]

                if first_runtime_log:
                    first_runtime_lines = read_lines(first_runtime_log)
                    if any("RENDERER_MAIN|RELAUNCH_REQUEST_RECEIVED" in line for line in first_runtime_lines):
                        active_session_changed_before_accept_phase = True
                    if any("RENDERER_MAIN|SHUTDOWN_REQUESTED" in line for line in first_runtime_lines):
                        active_session_changed_before_accept_phase = True
                    if any(SINGLE_INSTANCE_RELEASED_MARKER in line for line in first_runtime_lines):
                        active_session_changed_before_accept_phase = True
                ...
                if first_failure_runtime_log:
                    first_failure_runtime_lines = read_lines(first_failure_runtime_log)
                    if any(
                        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line
                        for line in first_failure_runtime_lines
                    ):
                        break
                if first_failure_proc.poll() is not None and first_failure_runtime_log:
                    break
                time.sleep(0.2)
            ...
            while time.time() < decline_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                known_logs = {first_runtime_log, first_failure_runtime_log}
                new_decline_logs = [path for path in runtime_logs if path not in known_logs]
                if new_decline_logs and not decline_runtime_log:
                    decline_runtime_log = new_decline_logs[0]
            ...
            final_failure_proc = subprocess.Popen(
                launch_command,
                cwd=ROOT_DIR,
                env=failure_env,
                **hidden_subprocess_kwargs(),
            ...
            final_failure_deadline = time.time() + 15.0
            while time.time() < final_failure_deadline:
                runtime_logs = files_matching_sorted(scenario_root, "Runtime_")
                known_logs = {
                    first_runtime_log,
                    first_failure_runtime_log,
                ...
                new_final_failure_logs = [path for path in runtime_logs if path not in known_logs]
                if new_final_failure_logs and not final_failure_runtime_log:
                    final_failure_runtime_log = new_final_failure_logs[0]

                if accept_runtime_log:
                    accept_runtime_lines = read_lines(accept_runtime_log)
                    if any("RENDERER_MAIN|RELAUNCH_REQUEST_RECEIVED" in line for line in accept_runtime_lines):
                        accepted_session_changed_during_final_failure = True
                    if any("RENDERER_MAIN|SHUTDOWN_REQUESTED" in line for line in accept_runtime_lines):
                        accepted_session_changed_during_final_failure = True
                    if any(SINGLE_INSTANCE_RELEASED_MARKER in line for line in accept_runtime_lines):
                        accepted_session_changed_during_final_failure = True

                if final_failure_runtime_log:
                    final_failure_runtime_lines = read_lines(final_failure_runtime_log)
                    if any(
                        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line
                        for line in final_failure_runtime_lines
                    ):
                        break
                if final_failure_proc.poll() is not None and final_failure_runtime_log:
                    break
                time.sleep(0.2)
        ...
        proc_map = (
            (first_proc, "first"),
            (first_failure_proc, "first_failure"),
            (decline_proc, "decline"),
            (accept_proc, "accept"),
            (final_failure_proc, "final_failure"),
        )
        for proc, label in proc_map:
            if proc is None:
                continue
            try:
                if proc.poll() is None:
            ...
            if label == "first":
                first_stdout = (stdout_text or "").strip()
                first_stderr = (stderr_text or "").strip()
            elif label == "first_failure":
                first_failure_stdout = (stdout_text or "").strip()
                first_failure_stderr = (stderr_text or "").strip()
            elif label == "decline":
                decline_stdout = (stdout_text or "").strip()
                decline_stderr = (stderr_text or "").strip()
            elif label == "accept":
                ...
                final_failure_stdout = (stdout_text or "").strip()
                final_failure_stderr = (stderr_text or "").strip()
    ...
    first_failure_conflict_index = first_marker_index(first_failure_runtime_lines, "SINGLE_INSTANCE_CONFLICT_DETECTED")
    first_failure_prompt_accept_index = max(
        first_marker_index(first_failure_runtime_lines, "REPLACE_PROMPT_ACCEPTED"),
        first_marker_index(first_failure_runtime_lines, "REPLACE_PROMPT_AUTO_ACCEPTED"),
    )
    first_failure_signal_failed_index = first_marker_index(first_failure_runtime_lines, "RELAUNCH_SIGNAL_FAILED")
    first_failure_outcome_index = first_marker_index(
        first_failure_runtime_lines,
        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER,
    )
    first_failure_runtime_start_index = first_marker_index(
        first_failure_runtime_lines,
        "STATUS|START|LAUNCHER_RUNTIME",
    )
    first_failure_renderer_spawn_index = first_marker_index(
        first_failure_runtime_lines,
        "STATUS|SUCCESS|RENDERER_PROCESS_SPAWN",
    )
    first_failure_signal_sent_index = first_marker_index(first_failure_runtime_lines, "RELAUNCH_SIGNAL_SENT")
    first_failure_reacquire_index = first_marker_index(first_failure_runtime_lines, "RELAUNCH_ACQUIRED_AFTER_WAIT")
    first_failure_replacement_active_index = first_marker_index(
        first_failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_ACTIVE_MARKER,
    )
    first_failure_replacement_settled_index = first_marker_index(
        first_failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER,
    )
    first_failure_release_index = first_marker_index(first_failure_runtime_lines, SINGLE_INSTANCE_RELEASED_MARKER)
    ...
    decline_outcome_index = first_marker_index(
        decline_runtime_lines,
        RELAUNCH_DECLINED_SESSION_PRESERVED_MARKER,
    )
    decline_runtime_start_index = first_marker_index(decline_runtime_lines, "STATUS|START|LAUNCHER_RUNTIME")
    decline_renderer_spawn_index = first_marker_index(
        decline_runtime_lines,
        "STATUS|SUCCESS|RENDERER_PROCESS_SPAWN",
    )
    ...
    accept_relaunch_request_index = first_marker_index(
        accept_runtime_lines,
        "RENDERER_MAIN|RELAUNCH_REQUEST_RECEIVED",
    )
    accept_settled_index = first_marker_index(accept_runtime_lines, AUTHORITATIVE_DESKTOP_SETTLED_MARKER)
    ...
    accept_failure_outcome_index = first_marker_index(
        accept_runtime_lines,
        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER,
    )

    final_failure_conflict_index = first_marker_index(final_failure_runtime_lines, "SINGLE_INSTANCE_CONFLICT_DETECTED")
    final_failure_prompt_accept_index = max(
        first_marker_index(final_failure_runtime_lines, "REPLACE_PROMPT_ACCEPTED"),
        first_marker_index(final_failure_runtime_lines, "REPLACE_PROMPT_AUTO_ACCEPTED"),
    )
    final_failure_signal_failed_index = first_marker_index(final_failure_runtime_lines, "RELAUNCH_SIGNAL_FAILED")
    final_failure_outcome_index = first_marker_index(
        final_failure_runtime_lines,
        RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER,
    )
    final_failure_runtime_start_index = first_marker_index(
        final_failure_runtime_lines,
        "STATUS|START|LAUNCHER_RUNTIME",
    )
    final_failure_renderer_spawn_index = first_marker_index(
        final_failure_runtime_lines,
        "STATUS|SUCCESS|RENDERER_PROCESS_SPAWN",
    )
    final_failure_signal_sent_index = first_marker_index(final_failure_runtime_lines, "RELAUNCH_SIGNAL_SENT")
    final_failure_reacquire_index = first_marker_index(final_failure_runtime_lines, "RELAUNCH_ACQUIRED_AFTER_WAIT")
    final_failure_replacement_active_index = first_marker_index(
        final_failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_ACTIVE_MARKER,
    )
    final_failure_replacement_settled_index = first_marker_index(
        final_failure_runtime_lines,
        RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER,
    )
    final_failure_release_index = first_marker_index(final_failure_runtime_lines, SINGLE_INSTANCE_RELEASED_MARKER)

    ordering_detail = (
        f"first_settled={first_settled_index}, first_relaunch_request={first_relaunch_request_index}, "
        f"first_shutdown={first_shutdown_index}, first_exit={first_exit_index}, first_release={first_release_index}, "
        f"failure1_conflict={first_failure_conflict_index}, failure1_prompt_accept={first_failure_prompt_accept_index}, "
        f"failure1_signal_failed={first_failure_signal_failed_index}, failure1_outcome={first_failure_outcome_index}, "
        f"failure1_runtime_start={first_failure_runtime_start_index}, failure1_signal_sent={first_failure_signal_sent_index}, "
        f"failure1_reacquire={first_failure_reacquire_index}, failure1_replacement_active={first_failure_replacement_active_index}, "
        f"decline_conflict={decline_conflict_index}, decline_prompt_decline={decline_prompt_decline_index}, "
        f"decline_outcome={decline_outcome_index}, decline_runtime_start={decline_runtime_start_index}, "
        f"decline_signal_sent={decline_signal_sent_index}, decline_reacquire={decline_reacquire_index}, "
        f"accept_conflict={accept_conflict_index}, accept_prompt_accept={accept_prompt_accept_index}, "
        f"accept_signal_sent={accept_signal_sent_index}, accept_reacquire={accept_reacquire_index}, "
        f"accept_replacement_active={accept_replacement_active_index}, accept_settled={accept_settled_index}, "
        f"accept_replacement_settled={accept_replacement_settled_index}, accept_shutdown={accept_shutdown_index}, "
        f"accept_exit={accept_exit_index}, accept_release={accept_release_index}, "
        f"failure2_conflict={final_failure_conflict_index}, failure2_prompt_accept={final_failure_prompt_accept_index}, "
        f"failure2_signal_failed={final_failure_signal_failed_index}, failure2_outcome={final_failure_outcome_index}, "
        f"failure2_runtime_start={final_failure_runtime_start_index}, failure2_signal_sent={final_failure_signal_sent_index}, "
        f"failure2_reacquire={final_failure_reacquire_index}, failure2_replacement_active={final_failure_replacement_active_index}"
    )
    ...
    checks = {
        ...
        "five_runtime_logs_created": line_status(
            all(
                [
                    first_runtime_log,
                    first_failure_runtime_log,
        ...
        "first_session_settled_before_mixed_sequence": line_status(
            first_settled_index >= 0,
            AUTHORITATIVE_DESKTOP_SETTLED_MARKER if first_settled_index >= 0 else "missing authoritative settled marker",
        ),
        "first_session_preserved_during_failure_and_decline": line_status(
            not active_session_changed_before_accept_phase,
            ordering_detail,
        ),
        "first_failure_launch_conflict_accept_and_signal_failure": line_status(
            first_failure_conflict_index >= 0
            and first_failure_prompt_accept_index > first_failure_conflict_index
            and first_failure_signal_failed_index > first_failure_prompt_accept_index
            and first_failure_outcome_index > first_failure_signal_failed_index,
            ordering_detail,
        ),
        "first_failure_launch_never_starts_runtime_ownership": line_status(
            first_failure_runtime_start_index < 0
            and first_failure_renderer_spawn_index < 0
            and first_failure_signal_sent_index < 0
            and first_failure_reacquire_index < 0
            and first_failure_replacement_active_index < 0
            and first_failure_replacement_settled_index < 0
            and first_failure_release_index < 0,
            ordering_detail,
        ),
        "decline_launch_conflict_and_decline": line_status(
            decline_conflict_index >= 0 and decline_prompt_decline_index > decline_conflict_index >= 0,
            ordering_detail,
        ),
        "decline_launch_never_starts_runtime_ownership": line_status(
            decline_runtime_start_index < 0
            and decline_renderer_spawn_index < 0
            and decline_prompt_accept_index < 0
            and decline_signal_sent_index < 0
            and decline_reacquire_index < 0
            and decline_replacement_active_index < 0
            and decline_replacement_settled_index < 0
        ...
        "accept_launch_transfers_ownership_cleanly": line_status(
            first_relaunch_request_index >= 0
            and first_shutdown_index > first_relaunch_request_index
            and first_exit_index > first_shutdown_index
            and first_release_index > first_exit_index
            and accept_conflict_index >= 0
            and accept_prompt_accept_index > accept_conflict_index
            and accept_signal_sent_index > accept_prompt_accept_index
            and accept_reacquire_index > accept_signal_sent_index
            and accept_replacement_confirmed_index >= accept_reacquire_index
            ...
            and accept_launcher_settled_observed_index >= accept_settled_index
            and accept_replacement_settled_index > accept_settled_index
            and accept_decline_index < 0
            and accept_failure_outcome_index < 0,
            ordering_detail,
        ),
        "accepted_session_preserved_during_final_failure": line_status(
            not accepted_session_changed_during_final_failure and accept_relaunch_request_index < 0,
            ordering_detail,
        ),
        "final_failure_launch_conflict_accept_and_signal_failure": line_status(
            final_failure_conflict_index >= 0
            and final_failure_prompt_accept_index > final_failure_conflict_index
            and final_failure_signal_failed_index > final_failure_prompt_accept_index
            and final_failure_outcome_index > final_failure_signal_failed_index,
            ordering_detail,
        ),
        "final_failure_launch_never_starts_runtime_ownership": line_status(
            final_failure_runtime_start_index < 0
            and final_failure_renderer_spawn_index < 0
            and final_failure_signal_sent_index < 0
            and final_failure_reacquire_index < 0
            and final_failure_replacement_active_index < 0
            and final_failure_replacement_settled_index < 0
            and final_failure_release_index < 0,
            ordering_detail,
        ),
        "accepted_session_lifecycle_completed_after_final_failure": line_status(
            (
                accept_shutdown_index > accept_settled_index >= 0
                and accept_exit_index > accept_shutdown_index
                and accept_release_index > accept_exit_index
        ...
        "guard_transfers_only_in_accept_phase": line_status(
            first_failure_reacquire_index < 0
            and decline_reacquire_index < 0
            and accept_reacquire_index >= 0
            and final_failure_reacquire_index < 0,
            ordering_detail,
        ),
        "classification_stays_distinct_across_failure_decline_accept_failure": line_status(
            first_failure_outcome_index >= 0
            and decline_outcome_index >= 0
            and accept_replacement_settled_index >= 0
            and final_failure_outcome_index >= 0
            and not any(RELAUNCH_DECLINED_SESSION_PRESERVED_MARKER in line for line in first_failure_runtime_lines)
            and not any(RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line for line in decline_runtime_lines)
            and not any(RELAUNCH_DECLINED_SESSION_PRESERVED_MARKER in line for line in accept_runtime_lines)
            and not any(RELAUNCH_SIGNAL_FAILED_SESSION_PRESERVED_MARKER in line for line in accept_runtime_lines)
            and not any(RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER in line for line in first_failure_runtime_lines)
            and not any(RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER in line for line in decline_runtime_lines)
            and not any(RELAUNCH_REPLACEMENT_SESSION_SETTLED_MARKER in line for line in final_failure_runtime_lines),
            ordering_detail,
        ),
        "no_failure_flow_in_incoming_mixed_sequence": line_status(
            not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in first_failure_runtime_lines)
            and not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in decline_runtime_lines)
            and not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in accept_runtime_lines)
            and not any("STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE" in line for line in final_failure_runtime_lines),
            "STATUS|SUCCESS|LAUNCHER_RUNTIME|FAILURE_FLOW_COMPLETE absent",
        ),
        "traceback_absent": line_status(
            all(
    ...
    return {
        "scenario_name": scenario_name,
        "log_root": scenario_root,
        "runtime_log": accept_runtime_log or final_failure_runtime_log or first_runtime_log,
        "stdout": "\n".join(
            part
            for part in (
                first_stdout,
...
def run_validation():
    ...
    tray_hud_result = validate_tray_monitoring_hud_route_state_actions()
    tray_hud_events = tray_hud_result["events"]
    checks["tray_hud_route_state_validation_imported"] = line_status(
        tray_hud_result["ok"],
        tray_hud_result["error"] or "DesktopTrayEntry HUD route-state path exercised",
    )
    hidden_snapshots = tray_hud_result.get("hidden_snapshots") or {}
    checks["tray_hud_hidden_optional_states"] = line_status(
        bool(hidden_snapshots)
        and all(
            "Open HUD Dashboard" not in snapshot.get("texts", ())
            and "HUD Overlay Deferred" not in snapshot.get("texts", ())
            and "Enable HUD Feature" not in snapshot.get("texts", ())
            and all(
                button.get("text") not in {"Open HUD Dashboard", "HUD Overlay Deferred", "Enable HUD Feature"}
                for button in snapshot.get("fallback_buttons", ())
            )
            for snapshot in hidden_snapshots.values()
        ),
        f"hidden_snapshots={hidden_snapshots}",
    )
    checks["tray_hud_enabled_available_dashboard_row"] = line_status(
    ...
    checks["tray_hud_target_failure_is_bounded_and_retryable"] = line_status(
        ...
        and any(
            "TRAY_MONITORING_HUD_DASHBOARD_ROUTED|source=validation_retry|visible=true|owner=FAM-006"
            in event
            for event in tray_hud_events
        ),
        (
            f"failed_result={tray_hud_result['failed_request_result']}; "
            f"failed_snapshot={tray_hud_result['failed_request_snapshot']}; "
            f"retry_result={tray_hud_result['retry_request_result']}; "
            f"retry_snapshot={tray_hud_result['retry_request_snapshot']}"
        ),
    )
    checks["tray_hud_temporarily_blocked_disabled_with_reason"] = line_status(
        any(
            text == "HUD Dashboard unavailable"
            for text in tray_hud_result["temporarily_blocked"].get("texts", ())
        )
        and tray_hud_result["temporarily_blocked"].get("dashboard_blocked_enabled") is False,
        f"temporarily_blocked={tray_hud_result['temporarily_blocked']}",
    )
    checks["fallback_hud_temporarily_blocked_disabled_with_reason"] = line_status(
        not tray_hud_result["temporarily_blocked"].get("fallback_buttons"),
        f"temporarily_blocked={tray_hud_result['temporarily_blocked']}",
    )
    checks["tray_hud_temporarily_blocked_does_not_route_dashboard"] = line_status(
        tray_hud_result["requests_before_blocked_click"]
        == tray_hud_result["requests_after_blocked_click"],
        (
            f"before={tray_hud_result['requests_before_blocked_click']}; "
            f"after={tray_hud_result['requests_after_blocked_click']}"
        ),
    )
    checks["tray_hud_labels_not_conflated"] = line_status(
        all("Enable HUD Feature" not in snapshot.get("texts", ()) for snapshot in hidden_snapshots.values())
        and "HUD Overlay Deferred" not in tray_hud_result["temporarily_blocked"].get("texts", ())
        and any(
            text in {"Open HUD Dashboard", "HUD Dashboard unavailable"}
            for text in tray_hud_result["temporarily_blocked"].get("texts", ())
        ),
        (
            f"hidden_snapshots={hidden_snapshots}; "
    ...
    checks["tray_hud_route_state_markers"] = line_status(
        ...
        and any(
            "TRAY_MONITORING_HUD_DASHBOARD_ABORTED|source=validation_blocked|reason=route_unavailable|route_state=enabled_temporarily_blocked"
            in event
            for event in tray_hud_events
        ),
    ...
    mixed_signal_failure_accept_result = run_mixed_signal_failure_then_accept_relaunch_scenario()
    ...
    for check_name, check_result in repeated_signal_failure_result["checks"].items():
        checks[f"{repeated_signal_failure_result['scenario_name']}::{check_name}"] = check_result
    for check_name, check_result in accepted_relaunch_wait_timeout_result["checks"].items():
        checks[f"{accepted_relaunch_wait_timeout_result['scenario_name']}::{check_name}"] = check_result
    ...
    for check_name, check_result in mixed_signal_failure_accept_result["checks"].items():
        checks[f"{mixed_signal_failure_accept_result['scenario_name']}::{check_name}"] = check_result
    ...
    return {
        ...
        "nonlaunch_scenarios": [
            ...
            accepted_relaunch_wait_timeout_result,
