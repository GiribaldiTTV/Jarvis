...
HISTORICAL_WORDS = re.compile(
    ...
    r"prior|pre[- ]migration|stale|forbidden|cleanup|migration|retired|receipt)\b",
    re.IGNORECASE,
)
TEXT_EXTENSIONS = {".md", ".py", ".ps1", ".json", ".txt", ".yaml", ".yml"}
...
def classify_line(path: Path, line: str, *, fixture_mode: bool = False) -> str | None:
    if not any(root in line for root in LEGACY_ROOTS):
        return None
    if not fixture_mode and path.name in {
        ...
        return "policy"
    if not fixture_mode and path.name == "orin_branch_readiness_planning_fixture_validation.py":
        return "test-only"
    if not fixture_mode and path.name in HISTORICAL_DOCUMENTS:
        return "historical"
    if not fixture_mode and (
        "fixtures" in path.parts
        or "branch_records" in path.parts
        or "branch_plans" in path.parts
    ):
        return "historical"
...
def scan(scan_root: Path, *, fixture_mode: bool = False) -> list[dict[str, object]]:
    ...
    for path in _iter_files(scan_root):
        for line_number, line in enumerate(path.read_text(encoding="utf-8", errors="ignore").splitlines(), 1):
            classification = classify_line(path, line, fixture_mode=fixture_mode)
            if classification:
                findings.append(
                    {
