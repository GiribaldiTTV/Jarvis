...
PACKET_CONTAINED_ZIP_SHA_PLACEHOLDER = (
    ...
    "Codex return packet, helper output, and live external operational state."
)
...
PR_READINESS_STAGE1_PACKET_DECISION_FILES: tuple[str, ...] = (
    "START_HERE.md",
    "PR_READINESS_STAGE1_COVERAGE_DIGEST.md",
    "PR_READINESS_STAGE1_CONTRADICTION_CHECKLIST.md",
)
...
DECISION_STATUS_WORKSTREAM_CONDUCT_READY = "workstream-conduct-ready"
...
BRANCH_PLANNING_IMPLEMENTATION_REQUEST_MARKERS = (
    "approve bounded workstream phase conduct",
    "approve bounded workstream conduct",
    "bounded workstream phase conduct",
    "workstream phase conduct approval",
    "workstream conduct approval",
    "workstream conduct is approved",
...
BRANCH_PLANNING_IMPLEMENTATION_BLOCKING_MARKERS = (
    "workstream phase conduct remains pending",
    "workstream conduct remains pending",
    "does not authorize workstream conduct",
    "before workstream phase conduct",
...
REQUIRED_FAM007_LIVE_PROOF_CHECKS: tuple[str, ...] = (
    ...
    "controlCenterLocalBoundaryControlProven",
    "capabilitiesMaintenanceDisplayWorkflowProven",
...
FAM007_REQUIRED_LIVE_PROOF_SCREENSHOT_CLASSES = {
    ...
    "control_before_boundary_review",
    "control_after_boundary_review",
    "control_after_recovery_route",
    "control_after_state_taxonomy",
    "readiness_before_run",
    "readiness_after_local_check",
    "readiness_after_report_generation",
    "readiness_after_copy_action",
    "capabilities_before_lifecycle_review",
    "capabilities_after_lifecycle_review",
    "capabilities_after_edition_gates",
    "capabilities_after_maintenance_hold",
}
FAM007_LIVE_PROOF_MANIFEST_NAME = "live_resize_manifest.json"
FAM007_UDL_IMAGE_PROOF_IDS = ("F7-UDL-006", "F7-UDL-007", "F7-UDL-016")
FAM007_CHILD_DOMAIN_LABELS = {
    "control-center": "AI Control Center",
    "readiness-diagnostics": "AI Readiness & Diagnostics",
    "capabilities-maintenance": "Capabilities & Maintenance",
}
FAM007_VISUAL_ADJUDICATION_FILES = (
    "CHILD_WINDOW_VISUAL_PROOF_INDEX.md",
    "PER_WINDOW_VISUAL_ADJUDICATION.md",
    "PER_ELEMENT_VISUAL_ADJUDICATION.md",
    "FAM_007_UNIFIED_DEFECT_LEDGER.md",
...
class WorkstreamEntryPacketDecisionPathResult:
    ...
    def implementation_ready(self) -> bool:
        return self.workstream_conduct_ready and not self.failures

    @property
    def workstream_conduct_ready(self) -> bool:
        return self.status in {
            DECISION_STATUS_IMPLEMENTATION_READY,
            DECISION_STATUS_WORKSTREAM_CONDUCT_READY,
        } and not self.failures

    @property
    def blocks_implementation(self) -> bool:
        return self.blocks_workstream_conduct

    @property
    def blocks_workstream_conduct(self) -> bool:
        return self.status in {
...
def _same_label_root_artifact_paths(review_root: Path, label: str) -> list[Path]:
    safe_label = _sanitize_folder_name(label)
    prefix = f"{safe_label}-"
    ...
    return sorted(
        ...
        if path.is_file() and path.name.startswith(prefix) and path.suffix.lower() != ".zip"
    )


def _remove_stale_same_label_root_artifacts(review_root: Path, label: str) -> None:
    for artifact in _same_label_root_artifact_paths(review_root, label):
        artifact.unlink()


def _same_label_root_artifact_failures(
    review_root: Path,
    label: str,
    *,
    validation_mode: str,
...
def _validate_export_zip(
    ...
    *,
    ...
    artifact_failures = [
        ...
        *_fam007_slc001_support_bp1_context_failures(generated_packet_files),
...
def _pr_stage1_packet_coherence_failures(packet_files: Mapping[str, str]) -> list[str]:
    """Reject mixed current-gate language in a PR Stage 1 packet."""
    ...
    if "stage 1 ready for stage 2" in normalized_primary:
        if "stage 1 ready for stage 2" not in normalized_summary:
            failures.append(
                ...
                "Stage 1 Ready For Stage 2 outcome"
            )
    if re.search(r"workstream\s+entry|workstream\s+implementation", normalized_summary):
        failures.append(
            "START_HERE.md: Workstream Entry or implementation is presented as the "
            "current Stage 1 decision path"
        )
    if re.search(
        r"(?:primary|active|current\s+(?:decision|gate)).{0,100}"
        r"(?:user_branch_plan_review\.md|user_branch_vision_review\.md|"
        r"workstream_entry_analysis_digest\.md)",
        normalized_summary + " " + normalized_primary,
    ):
        failures.append(
            "PR Stage 1 packet: a BP or Workstream artifact is named as the "
            "primary/current decision surface"
        )
    ...
    for file_name, text in sorted(support_files.items()):
        ...
        if re.search(r"decision path\s*:\s*workstream\s+entry", normalized):
            failures.append(
                f"{file_name}: active Workstream Entry decision path conflicts with PR Stage 1"
            )
        if re.search(
            r"(?:primary|active|current\s+(?:decision|gate)).{0,100}"
            r"(?:user_branch_plan_review\.md|user_branch_vision_review\.md|"
            ...
            failures.append(
                f"{file_name}: names a BP or Workstream artifact as the current decision file"
            )
        if re.search(
            r"(?:bp1|bp2|bp3).{0,80}(?:remains?\s+pending|pending\s+user|"
            r"pending\s+acceptance|pending\s+review)",
            normalized,
        ):
            failures.append(
...
def _pr_stage1_source_coverage_failures(
    ...
    *,
    ...
    coverage = packet_files.get(coverage_name, "")
    if not coverage:
    ...
    listed = set(
        re.findall(
            rf"`({re.escape(SOURCE_TRUTH_CONTEXT_DIR_NAME)}/[^`\n]+)`",
            coverage,
        )
    )
    ...
    if missing:
        failures.append(
            f"{coverage_name}: copied source files missing from coverage list: "
            + ", ".join(missing)
        )
    if extra:
        failures.append(
            f"{coverage_name}: coverage list claims absent files: " + ", ".join(extra)
        )
    count_match = re.search(r"Copied Source Count:\s*`?(\d+)`?", coverage)
    if count_match and int(count_match.group(1)) != len(actual):
        failures.append(
            f"{coverage_name}: Copied Source Count does not match the packet "
            f"({count_match.group(1)} vs {len(actual)})"
        )
    return failures
...
def _active_review_aid_false_green_failures(packet_files: Mapping[str, str]) -> list[str]:
    ...
    stage1_current_artifacts = {
        "START_HERE.md",
        f"{USER_REVIEW_DIR_NAME}/{PR_READINESS_STAGE1_REVIEW_FILE}",
    ...
    for file_name, text in sorted(packet_files.items()):
        ...
        for reason, pattern in FALSE_GREEN_STALE_ACTIVE_PATTERNS:
            if stage1_pending_posture and reason in {
                "stale-governance-pr-readiness-stage-1",
                "stale-approve-pr-readiness",
            }:
                continue
            if reason == "stale-governance-pr-readiness-stage-1" and stage1_current_posture:
                normalized_file = file_name.replace("\\", "/")
                if normalized_file in stage1_current_artifacts:
                    continue
                if re.search(
                    r"\b(?:historical|accepted\s+context|not\s+applicable|"
                    r"receipt[- ]only)\b",
                    text,
                    re.IGNORECASE,
                ) and not re.search(
                    r"\b(?:approve|begin|start|request|remains?\s+pending)\b",
                    text,
                    re.IGNORECASE,
                ):
                    continue
...
FAM007_DECOMPOSITION_ROUTE_CODES: tuple[str, ...] = (
    "ROUTE_A_SEPARATE",
    "ROUTE_B_SHARED_SHELL_LOCAL_LIFECYCLE",
    "ROUTE_C_INDIVISIBLE",
)
...
def _fam007_field(text: str, field_name: str) -> str:
    return _field_value(text, field_name).strip().strip("`").strip()


def _fam007_field_code(text: str, field_name: str) -> str:
    return _fam007_field(text, field_name).upper()


def _fam007_receipt_is_recorded(value: str) -> bool:
...
def _fam007_text_has_all(text: str, markers: tuple[str, ...]) -> bool:
...
def _fam007_decomposition_packet_failures(
    ...
    state_name = "Review Aids/CURRENT_DECOMPOSITION_STATE.md"
    state_text = packet_files.get(state_name, "")
    state = _fam007_field_code(state_text, "Declared Decomposition State")
    candidate_code = _fam007_field_code(state_text, "Named Candidate Code")
    candidate_name = _fam007_field(state_text, "Named Candidate")
    selected_next_posture = _fam007_field_code(
        state_text, "Selected-next Posture"
    )
    route_code = _fam007_field_code(
        state_text, "Shell / Lifecycle Route Code"
    )
    ...
    for marker in (
        ...
        if marker.casefold() not in state_model.casefold():
            failures.append(
                "FAM-007 decomposition: transition-state model is missing "
                f"{marker}"
            )
    ...
    elif state not in FAM007_DECOMPOSITION_STATES:
        failures.append(
    ...
    if expectation:
        state_fields = {
            ...
            "branch_exists": _fam007_field_code(
                state_text, "Successor Branch / Worktree Exists"
            ),
            "branch_mutation": _fam007_field_code(
                state_text, "Branch / Worktree Mutation"
            ),
            "stage1": _fam007_field_code(
                state_text, "Stage 1 Analysis Status"
            ),
            "stage2": _fam007_field_code(
                state_text, "Stage 2 Creation Approval"
            ),
            "identity": _fam007_field_code(
                state_text, "Successor Identity Verification"
            ),
            "bp_entry": _fam007_field_code(
                state_text, "Branch Planning Entry Approval"
            ),
            "current_gate": _fam007_field_code(
                state_text, "Current Gate Code"
            ),
            "next_gate": _fam007_field_code(
                state_text, "Next Legal Gate Code"
            ),
        }
        for field_key, expected_value in expectation.items():
            ...
            if state_fields.get(field_key) != expected_value:
                failures.append(
                    ...
                    f"{state} requires {field_key}={expected_value}, got "
                    f"{state_fields.get(field_key) or 'MISSING'}"
                )
        if expectation["candidate"] == "NONE":
            ...
            not candidate_code
            or candidate_code == "NONE"
            or not re.fullmatch(r"[A-Z0-9_]+", candidate_code)
            or not candidate_name
            or candidate_name.upper() == "NONE"
        ):
            failures.append(
                "FAM-007 decomposition: selected transition state requires one named candidate"
            )

    if _fam007_field_code(state_text, "Implementation Approval") != "NOT_APPROVED":
        failures.append(
            "FAM-007 decomposition: transition model must not imply implementation approval"
        )
    if _fam007_field_code(state_text, "Forbidden Phase Collapse") != "CONFIRMED":
        failures.append(
            "FAM-007 decomposition: forbidden phase-collapse boundary is not confirmed"
        )
    ...
    for field_name, expected_value in (
        ("Declared Decomposition State", state),
        ("Named Candidate Code", candidate_code),
        ("Named Candidate", candidate_name),
        ("Selected-next Posture", selected_next_posture),
    ):
        if _fam007_field_code(selected_next_receipt, field_name) != expected_value.upper():
            failures.append(
                "FAM-007 decomposition: selected-next receipt disagrees on "
                f"{field_name}"
            )
    if (
        _fam007_field_code(selected_next_receipt, "External-State Consistency")
        != "CONFIRMED"
    ):
        failures.append(
            "FAM-007 decomposition: selected-next external-state consistency is not confirmed"
        )

    ai_receipt = packet_files.get(
        "Review Aids/AI_READINESS_RECOMMENDATION_SELECTION_RECEIPT.md", ""
    )
    expected_ai_posture = (
    ...
    if _fam007_field_code(
        ai_receipt, "Declared Decomposition State"
    ) != state:
        failures.append(
            "FAM-007 decomposition: AI Readiness receipt state disagrees with the declaration"
        )
    if _fam007_field_code(
        ai_receipt, "Named Candidate Code"
    ) != candidate_code:
        failures.append(
            "FAM-007 decomposition: AI Readiness receipt candidate disagrees with the declaration"
        )
    if _fam007_field_code(
        ai_receipt, "AI Readiness Posture"
    ) != expected_ai_posture:
        failures.append(
            "FAM-007 decomposition: AI Readiness recommendation/selection posture "
            "does not match the approved candidate state"
        )
    ai_stage1_receipt = _fam007_field(
    ...
    elif _fam007_receipt_is_recorded(ai_stage1_receipt):
        failures.append(
            "FAM-007 decomposition: AI Readiness carries a selection receipt while "
            "another candidate or no candidate is selected"
        )
    ...
    route_rules = packet_files.get(
        "Review Aids/TRANSITION_SAFE_ROUTE_RULES.md", ""
    )
    route_fields = {
        "separability": _fam007_field_code(separability, "Declared Route"),
        "rules": _fam007_field_code(route_rules, "Declared Route"),
    }
    if route_code not in FAM007_DECOMPOSITION_ROUTE_CODES:
        failures.append(
            f"FAM-007 decomposition: missing or unknown route code {route_code or 'MISSING'}"
        )
    for source_name, declared_route in route_fields.items():
        if declared_route != route_code:
            failures.append(
                "FAM-007 decomposition: route declaration mismatch between current state "
                f"and {source_name}"
            )
    ...
    if route_code != "ROUTE_C_INDIVISIBLE" and combined_carrier_claimed:
        failures.append(
    ...
    for field_name, expected_value in route_requirements.get(route_code, ()):
        actual_value = _fam007_field_code(separability, field_name)
        if actual_value != expected_value:
            failures.append(
                "FAM-007 decomposition: "
                f"{route_code} requires {field_name}={expected_value}, got "
                f"{actual_value or 'MISSING'}"
            )
    if route_code == "ROUTE_C_INDIVISIBLE" and not combined_carrier_claimed:
        failures.append(
            "FAM-007 decomposition: Route C must explicitly declare one combined "
            "shell/lifecycle carrier"
        )
    ...
    issue307_state = _fam007_field_code(
        issue307_receipt, "Issue #307 Resolution State"
    )
    issue307_route = _fam007_field_code(
        issue307_receipt, "Issue #307 Resolution Route"
    )
    issue307_approval_receipt = _fam007_field(
        issue307_receipt, "Issue #307 USER Approval Receipt"
    )
    issue307_approval_text = _fam007_field(
        issue307_receipt, "Issue #307 Exact USER Decision"
    )
    ...
    if issue307_state not in {"UNRESOLVED", "RESOLVED_ATOMIC"}:
    ...
    elif issue307_state == "UNRESOLVED":
        if issue307_route != "PENDING_USER_APPROVED_RESOLUTION":
            failures.append(
                "FAM-007 decomposition: unresolved Issue #307 must preserve the "
                "future USER-approved split, replacement, or equivalent clarification route"
            )
        if (
        ...
        if _fam007_field_code(
            issue307_receipt, "Implementation Or Closeout Allowed"
        ) != "NO":
            failures.append(
                "FAM-007 decomposition: unresolved Issue #307 must block implementation and closeout"
            )
    else:
        if issue307_route not in {
            "USER_APPROVED_SPLIT",
            "USER_APPROVED_REPLACEMENT",
            "USER_APPROVED_LIVE_ISSUE_CLARIFICATION",
        }:
            failures.append(
                "FAM-007 decomposition: resolved Issue #307 uses an unsupported resolution route"
            )
        if not _fam007_receipt_is_recorded(issue307_approval_receipt):
            failures.append(
        ...
        for field_name in (
            ...
            "Implementation Obligations Atomic",
            "Proof Obligations Atomic",
            "Historical Traceability Preserved",
        ):
            if _fam007_field_code(issue307_receipt, field_name) not in {
                "CONFIRMED",
                "YES",
            }:
                failures.append(
        ...
        if _fam007_field_code(
            ...
            failures.append(
                "FAM-007 decomposition: Issue #307 resolution must not imply "
                "implementation or closeout approval"
            )
    ...
    if missing_columns:
        ...
        issue307_matrix_owner = ""
        issue307_matrix_split_state = ""
        ...
        for issue_number in range(292, 309):
            ...
            if issue_number == 307:
                issue307_matrix_owner = final_owner
                issue307_matrix_split_state = split_state
        if issue307_state == "UNRESOLVED" and (
            "SPLIT_REQUIRED" not in issue307_matrix_split_state
            and "RESOLUTION_REQUIRED" not in issue307_matrix_split_state
        ):
            failures.append(
                f"{ownership_name}: Issue #307 must expose the future USER-approved "
                "split, replacement, or equivalent clarification requirement"
            )
        if issue307_state == "RESOLVED_ATOMIC" and (
            issue307_matrix_split_state != "ATOMIC_RESOLVED"
        ):
            failures.append(
                f"{ownership_name}: resolved Issue #307 must use ATOMIC_RESOLVED ownership"
            )
        if (
            issue307_matrix_owner
            and issue307_receipt_owner
            and issue307_matrix_owner != issue307_receipt_owner
        ):
            failures.append(
                f"{ownership_name}: Issue #307 final closure owner disagrees with "
    ...
    for surface_name, surface_text in (
        ...
        if _fam007_field_code(
            surface_text, "Declared Decomposition State"
        ) != state:
            failures.append(
                f"FAM-007 decomposition: {surface_name} disagrees on the current "
                "declared decomposition state"
            )
        if _fam007_field_code(
            surface_text, "Selected-next Posture"
        ) != selected_next_posture:
            failures.append(
    ...
    if expectation and expectation["candidate"] == "NONE":
        if re.search(
            ...
            failures.append(
                "FAM-007 decomposition: a successor is implied as selected-next "
                "without USER approval"
            )
        if "recommendation_only_not_selected" not in ai_receipt.casefold():
            failures.append(
        ...
        if re.search(
            r"\bsuccessor\s+selected\s*:\s*`?no\b",
            current_decision_text,
            re.IGNORECASE,
        ):
            failures.append(
                "FAM-007 decomposition: selected state still claims no successor is selected"
            )
        if candidate_name.casefold() not in current_decision_text.casefold():
            failures.append(
                "FAM-007 decomposition: named candidate is absent from the current "
                "decision surfaces"
            )
    ...
    identity_markers = (
        ...
        "Declared Decomposition State:",
        "Selected-next State:",
        "Named Candidate Code:",
        "Successor Branch / Worktree Exists:",
        "Successor Identity Verification:",
        "Current Branch Role:",
    ...
    for field_name, expected_value in (
        ...
        (
            "Successor Branch / Worktree Exists",
            expectation["branch_exists"] if expectation else "",
        ),
        (
            "Successor Identity Verification",
            expectation["identity"] if expectation else "",
        ),
    ):
        if expected_value and _fam007_field_code(identity, field_name) != expected_value:
            failures.append(
                "FAM-007 decomposition: identity receipt disagrees on "
                f"{field_name}"
            )
    ...
    for field_name, expected_value in (
        ...
        ("Named Candidate", candidate_name),
        ("Selected-next Posture", selected_next_posture),
        ("Shell / Lifecycle Route Code", route_code),
        ("Issue #307 Resolution State", issue307_state),
    ):
        if _fam007_field_code(external_receipt, field_name) != expected_value.upper():
            failures.append(
...
def _fam007_packet_image_entry(folder: str, source_value: object) -> str:
    basename = _proof_image_basename(source_value)
    if not basename:
        return "MISSING"
    return f"{REVIEW_AIDS_DIR_NAME}/Inspectable Evidence/{folder}/{basename}"
...
def _fam007_child_probe(manifest: Mapping[str, object], domain_id: str) -> Mapping[str, object]:
    probes = manifest.get("childChromeProbe")
    if isinstance(probes, Mapping):
        probe = probes.get(domain_id)
        if isinstance(probe, Mapping):
            return probe
    return {}


def _fam007_child_dom(probe: Mapping[str, object]) -> Mapping[str, object]:
    dom = probe.get("dom")
    return dom if isinstance(dom, Mapping) else {}


def _fam007_child_rows(probe: Mapping[str, object]) -> list[Mapping[str, object]]:
    dom = _fam007_child_dom(probe)
    inventory = dom.get("elementInventory")
...
def _fam007_child_buttons(probe: Mapping[str, object]) -> list[Mapping[str, object]]:
    dom = _fam007_child_dom(probe)
    inventory = dom.get("elementInventory")
    if not isinstance(inventory, Mapping):
        return []
    buttons = inventory.get("buttons")
    return [button for button in buttons if isinstance(button, Mapping)] if isinstance(buttons, list) else []


def _fam007_child_material_group_status(probe: Mapping[str, object], group_name: str) -> str:
    dom = _fam007_child_dom(probe)
    groups = dom.get("materialGroups")
    group = groups.get(group_name) if isinstance(groups, Mapping) else None
    return _fam007_verdict(isinstance(group, Mapping) and group.get("present") is True)


def _write_fam007_child_visual_adjudication_artifacts(
    target: Path,
    manifest: Mapping[str, object],
) -> set[Path]:
    review_aids = target / REVIEW_AIDS_DIR_NAME
    review_aids.mkdir(parents=True, exist_ok=True)
    written: set[Path] = set()
    screenshots = manifest.get("screenshots")
    screenshots = screenshots if isinstance(screenshots, Mapping) else {}
    contact_sheets = manifest.get("childPurposeMatchedContactSheets")
    ...
    control_result = manifest.get("controlCenterResult")
    control_result = control_result if isinstance(control_result, Mapping) else {}
    capabilities_result = manifest.get("capabilitiesResult")
    capabilities_result = capabilities_result if isinstance(capabilities_result, Mapping) else {}
    ...
    control_after_action_ok = checks.get("controlCenterLocalBoundaryControlProven") is True
    capabilities_after_action_ok = checks.get("capabilitiesMaintenanceDisplayWorkflowProven") is True

    parent_focused = _fam007_packet_image_entry(
        "focused_window_screenshots",
        (screenshots.get("dashboard_initial") or {}).get("focusedWindow") if isinstance(screenshots.get("dashboard_initial"), Mapping) else "",
    )
    parent_full = _fam007_packet_image_entry(
        "full_desktop_screenshots",
        (screenshots.get("dashboard_initial") or {}).get("fullDesktop") if isinstance(screenshots.get("dashboard_initial"), Mapping) else "",
    )
    ...
    child_role_evidence = {
        "control-center": "boundary/recovery/taxonomy mode probes plus before/after focused and full-desktop screenshots; provider/model execution remains blocked.",
        ...
        "capabilities-maintenance": "lifecycle, edition-gate, and maintenance-hold mode probes plus before/after focused and full-desktop screenshots; all download/install/update/fetch/package execution remains blocked.",
    }
    child_functional_verdict = {
        "control-center": "PASS" if control_after_action_ok else "UNPROVEN",
        "readiness-diagnostics": "PASS" if readiness_after_action_ok else "UNPROVEN",
        "capabilities-maintenance": "PASS" if capabilities_after_action_ok else "UNPROVEN",
    }
    child_functional_limit = {
        "control-center": (
            "Current runtime actions are photographed before boundary review, after boundary review, after recovery route, and after state taxonomy while provider execution remains blocked."
            if control_after_action_ok
            else "Control Center after-action visual proof is missing or incomplete."
        ),
        ...
        "capabilities-maintenance": (
            "Current display-only workflow modes are photographed before lifecycle review, after lifecycle review, after edition gates, and after maintenance hold while execution remains blocked."
            if capabilities_after_action_ok
            else "Capabilities & Maintenance after-action visual proof is missing or incomplete."
        ),
    }
    ...
    def _row_contract_ok(probe: Mapping[str, object]) -> bool:
        ...
        return (
            bool(rows)
            and dom.get("rowLabelColumnSource") == "measured-max-visible-label-content-px"
            and dom.get("rowValueColumnContract") == "shared-max-label-content-plus-fixed-gutter"
            and str(dom.get("rowValueGutter") or "") in {"8", "8px"}
            and str(dom.get("rowVerticalGutter") or "") in {"6", "6px"}
        )
    ...
    child_action_proof = {
        "control-center": control_after_action_ok,
        "readiness-diagnostics": readiness_after_action_ok,
        "capabilities-maintenance": capabilities_after_action_ok,
    }
    ...
    def _result_visual_image(result: Mapping[str, object], domain_state: str, key: str) -> str:
        visual = result.get("visualProofScreenshots")
        visual = visual if isinstance(visual, Mapping) else {}
        shot = visual.get(domain_state)
        shot = shot if isinstance(shot, Mapping) else {}
        folder = "focused_window_screenshots" if key == "focusedWindow" else "full_desktop_screenshots"
        return _fam007_packet_image_entry(folder, shot.get(key))
    ...
    index_lines = [
        ...
        "",
        "Packet Status: `Bounded runtime repair completed / pending USER Visual Acceptance`",
    ...
    window_lines = [
        ...
        "",
        "Review State: `Bounded runtime repair completed / pending USER Visual Acceptance`",
    ...
    trace_lines = [
        ...
        "",
        "Trace Boundary: implementation proof only; this does not promote a shared primitive/template or approve downstream gates.",
        "",
        ...
        "| Live visual/function proof | `dev/orin_ai_control_center_live_resize_validation.py` | `childChromeProbe`, `childPurposeMatchedContactSheets`, `controlCenterLocalBoundaryControlProven`, `readinessAfterActionVisualProofProven`, `capabilitiesMaintenanceDisplayWorkflowProven`, `checks` | `live_resize_manifest.json`, contact sheets, and child after-action screenshots. | `PASS` |",
    ...
    implementation_lines = [
        ...
        "",
        "Implementation Match State: `Bounded runtime repair completed / pending USER Visual Acceptance`",
    ...
    nonconformance_rows = [
        ...
        "| --- | --- | --- | --- | --- | --- |",
        "| F7-UDL-006 | Child/domain visual-family proof | Child/domain shell, title, geometry, row grammar, 720 text, contact sheets, and domain-local after-action screenshots are now proven; same-class comparator adequacy is tracked separately in F7-PQ-001. | `PASS` | `COMPARATOR_ADEQUACY_ANALYSIS.md`; `PER_WINDOW_VISUAL_ADJUDICATION.md`; contact sheets; live child after-action screenshots. | Pending USER Visual Acceptance; not a final acceptance claim. |",
        "| F7-UDL-007 | Functional-role conformity | AI Control Center boundary/recovery/taxonomy modes, Readiness local check/report/copy, and Capabilities lifecycle/edition/maintenance modes are proven while execution remains blocked. | `PASS` | `FUNCTIONAL_ROLE_CONFORMITY_MATRIX.md`; `STATE_COVERAGE_AND_READINESS_AFTER_ACTION_PROOF.md`; `live_resize_manifest.json`. | Pending USER Visual Acceptance; not a downstream gate approval. |",
        ...
        "| F7-PQ-002 | Child/domain visual-family feel | Runtime repair adds purpose-matched local workflow modes, deterministic rows, 720 text parity, contact sheets, and focused/full-desktop after-action photos for every child. | `PASS` | focused screenshots, full-desktop screenshots, purpose contact sheets, per-element proof. | USER review required for Visual Acceptance; proof does not self-accept. |",
        "| F7-PQ-003 | AI Control Center role | Current child now proves boundary review, recovery route, and state-taxonomy local modes without provider/model execution. | `PASS` | functional-role matrix, child rows, and Control Center after-action screenshots. | Pending USER Visual Acceptance. |",
        "| F7-PQ-004 | Capabilities & Maintenance role | Current child now proves lifecycle review, edition gates, and maintenance-hold display workflow without download/install/update/fetch/package execution. | `PASS` | functional-role matrix, child rows, and Capabilities after-action screenshots. | Pending USER Visual Acceptance. |",
    ]
    ...
    for domain_id, title in FAM007_CHILD_DOMAIN_LABELS.items():
        ...
        row_contract_ok = _row_contract_ok(probe)
        text_contract_ok = _row_text_ok(probe, "rowLabel") and _row_text_ok(probe, "rowValue")
        action_proof_ok = child_action_proof[domain_id]
        base_visual_evidence_ok = title_ok and shell_ok and geometry_ok and len(rows) > 0 and contact_path != "MISSING"
        visual_verdict = "PASS" if base_visual_evidence_ok and row_contract_ok and text_contract_ok and action_proof_ok else "UNPROVEN"
        index_lines.append(
        ...
        for group_name, expected in (
            ...
            ("controls", "compact minimize/close control cluster"),
            ("header", "title-first domain header"),
            ("description", "compact explanatory description"),
            ("card", "single focused domain card"),
            ("rows", "deterministic label/value row stack"),
            ("rowLabel", "720-weight label grammar"),
            ("rowValue", "720-weight value grammar"),
            ...
            if group_name in {"rows"}:
                status = "PASS" if row_contract_ok else "UNPROVEN"
                evidence = (
            ...
            if group_name in {"actions", "button"}:
                status = "PASS" if action_proof_ok and _group_present(probe, group_name) else "UNPROVEN"
                evidence = (
                    f"{title} child has after-action visual proof for its domain-local modes; "
                    "provider/model, prompt, download, install, update, fetch, memory, private setup, and packaging execution remain blocked where applicable."
                )
    ...
    state_lines = [
        "# State Coverage And Child After-Action Proof",
        "",
        f"AI Control Center After-Action Visual Proof Verdict: `{'PASS' if control_after_action_ok else 'UNPROVEN'}`",
        f"Readiness After-Action Visual Proof Verdict: `{'PASS' if readiness_after_action_ok else 'UNPROVEN'}`",
        f"Capabilities & Maintenance After-Action Visual Proof Verdict: `{'PASS' if capabilities_after_action_ok else 'UNPROVEN'}`",
        "",
        "| State | Focused screenshot | Full-desktop screenshot | Expected visible state | Verdict |",
        "| --- | --- | --- | --- | --- |",
        f"| Control before boundary review | `{_result_visual_image(control_result, 'beforeBoundary', 'focusedWindow')}` | `{_result_visual_image(control_result, 'beforeBoundary', 'fullDesktop')}` | Control Center boundary mode visible, provider/model blocked, no prompt/provider path | `{'PASS' if control_after_action_ok else 'UNPROVEN'}` |",
        f"| Control after boundary review | `{_result_visual_image(control_result, 'afterBoundaryReview', 'focusedWindow')}` | `{_result_visual_image(control_result, 'afterBoundaryReview', 'fullDesktop')}` | boundary review selected, guard closed, provider action not executed | `{'PASS' if control_after_action_ok else 'UNPROVEN'}` |",
        f"| Control after recovery route | `{_result_visual_image(control_result, 'afterRecoveryRoute', 'focusedWindow')}` | `{_result_visual_image(control_result, 'afterRecoveryRoute', 'fullDesktop')}` | recovery route selected, local-only guidance visible | `{'PASS' if control_after_action_ok else 'UNPROVEN'}` |",
        f"| Control after state taxonomy | `{_result_visual_image(control_result, 'afterStateTaxonomy', 'focusedWindow')}` | `{_result_visual_image(control_result, 'afterStateTaxonomy', 'fullDesktop')}` | state taxonomy selected, fail-closed states visible | `{'PASS' if control_after_action_ok else 'UNPROVEN'}` |",
        ...
        f"| Capabilities before lifecycle review | `{_result_visual_image(capabilities_result, 'beforeLifecycle', 'focusedWindow')}` | `{_result_visual_image(capabilities_result, 'beforeLifecycle', 'fullDesktop')}` | lifecycle display workflow visible, download/install/update blocked | `{'PASS' if capabilities_after_action_ok else 'UNPROVEN'}` |",
        f"| Capabilities after lifecycle review | `{_result_visual_image(capabilities_result, 'afterLifecycleReview', 'focusedWindow')}` | `{_result_visual_image(capabilities_result, 'afterLifecycleReview', 'fullDesktop')}` | lifecycle selected, source-trust prerequisites visible | `{'PASS' if capabilities_after_action_ok else 'UNPROVEN'}` |",
        f"| Capabilities after edition gates | `{_result_visual_image(capabilities_result, 'afterEditionGates', 'focusedWindow')}` | `{_result_visual_image(capabilities_result, 'afterEditionGates', 'fullDesktop')}` | edition gates selected, Developer/Owner private setup blocked | `{'PASS' if capabilities_after_action_ok else 'UNPROVEN'}` |",
        f"| Capabilities after maintenance hold | `{_result_visual_image(capabilities_result, 'afterMaintenanceHold', 'focusedWindow')}` | `{_result_visual_image(capabilities_result, 'afterMaintenanceHold', 'fullDesktop')}` | maintenance hold selected, update/install/fetch/package execution blocked | `{'PASS' if capabilities_after_action_ok else 'UNPROVEN'}` |",
        "",
    ...
    ledger_header = [
        ...
        "",
        "OPEN_NONCONFORMANCE_COUNT: 1",
        "Ledger State: `Bounded runtime repair completed / pending USER Visual Acceptance`",
        "Boundary: `PASS` rows mean this bounded runtime repair is reviewable; `PARTIAL` rows remain USER visual-acceptance decision items or source-truth/template blockers. This ledger does not accept Workstream-exit Visual Acceptance.",
        "",
        *nonconformance_rows,
        "",
        ...
        "| --- | --- | --- |",
        *[
            f"| {title} | purpose contact sheet, per-window adjudication, per-element adjudication, functional-role matrix, code-to-visual trace, implementation-match proof | `REPAIRED_FOR_USER_VISUAL_ACCEPTANCE_REVIEW` |"
    ...
    for check_name in checks_required:
        ledger_header.append(f"| {check_name} | `{checks.get(check_name)}` |")

    files = {
        "CHILD_WINDOW_VISUAL_PROOF_INDEX.md": index_lines,
        "PER_WINDOW_VISUAL_ADJUDICATION.md": window_lines,
        "PER_ELEMENT_VISUAL_ADJUDICATION.md": element_lines,
        "FAM_007_UNIFIED_DEFECT_LEDGER.md": ledger_header,
...
def _copy_fam007_visual_proof_artifacts(target: Path, proof_root: Path | None) -> set[Path]:
    ...
    supplemental_patterns = (
        ...
        "18_child_*_purpose_contact_sheet.png",
    )
    ...
    copied.update(_write_fam007_child_visual_adjudication_artifacts(target, manifest))
    return copied
...
def _normalized_external_state_context_text(text: str, *, live_text: bool = False) -> str:
    ...
    post_zip_receipt_normalizers = (
        ...
        (
            r"^External State Current Acceptance Receipt:\s*`?(?:RETURNED_WORKSTREAM_VISUAL_PROOF_QUALITY_ADJUDICATION_REPAIR_PACKET(?:_PREZIP)?_CURRENT|PROOF_QUALITY_PACKET_REVIEWABILITY_ACCEPTED_BOUNDED_RUNTIME_REPAIR_APPROVED_CURRENT|BOUNDED_RUNTIME_REPAIR_PACKET(?:_PREZIP)?_CURRENT)\b.*$",
            "External State Current Acceptance Receipt: <post-zip-reconciled-current-receipt>",
        ),
        (
            r"^External State Item Status:\s*`?(?:Returned-packet proof-quality/adjudication repair is source/helper/live-proof(?:/packet-validation)? PASS|Proof-quality packet accepted for reviewability; bounded returned-Workstream runtime repair approved|Bounded returned-Workstream runtime repair is source/helper/live-proof(?:/packet-validation)? PASS)\b.*$",
        ...
        (
            r"^Receipt Status:\s*`?(?:RETURNED_WORKSTREAM_VISUAL_PROOF_QUALITY_ADJUDICATION_REPAIR_PACKET(?:_PREZIP)?_CURRENT|BOUNDED_RUNTIME_REPAIR_PACKET(?:_PREZIP)?_CURRENT|PROOF_QUALITY_PACKET_REVIEWABILITY_ACCEPTED_BOUNDED_RUNTIME_REPAIR_APPROVED_CURRENT)`?\s*$",
        ...
        (
            r"^Validation:\s*`?(?:PRE-ZIP )?PASS - .*?(?:pending post-ZIP receipt\.|final ZIP SHA256 (?:sidecar|external-state) proof\.)`?\s*$",
            "Validation: <post-zip-reconciled-validation>",
        ),
        (
            r"^Validation:\s*`?PRE-ZIP PASS - .*?Active-review packet validation and final ZIP SHA256 (?:sidecar|external-state) proof remain the next packet-generation step\.`?\s*$",
            "Validation: <post-zip-reconciled-validation>",
        ),
        (
...
def _fam007_workstream_exit_visual_packet_detected(packet_files: Mapping[str, str]) -> bool:
    combined = "\n".join(
        packet_files.get(name, "")
        for name in (
            "START_HERE.md",
    ...
    return (
        "fam-007" in combined
        and "ai dashboard" in combined
        and "workstream-exit visual acceptance" in combined
    )


def _fam007_visual_adjudication_artifact_failures(packet_files: Mapping[str, str]) -> list[str]:
    if not _fam007_workstream_exit_visual_packet_detected(packet_files):
        return []
    failures: list[str] = []
    review_names = [
        name
        ...
        or name.startswith(f"{USER_REVIEW_DIR_NAME}/")
        or (
            name.startswith(f"{REVIEW_AIDS_DIR_NAME}/")
            and not name.startswith(f"{REVIEW_AIDS_DIR_NAME}/{VALIDATION_OUTPUTS_DIR_NAME}/")
        )
    ]
    for name in sorted(review_names):
        text = packet_files.get(name, "")
        if "Workstream Green" in text:
            failures.append(
    ...
    if "Bounded runtime repair completed / pending USER Visual Acceptance" not in primary:
        failures.append(
            "WORKSTREAM_ENTRY_ANALYSIS_DIGEST.md: missing returned-packet-safe status "
            "`Bounded runtime repair completed / pending USER Visual Acceptance`"
        )
    for artifact in FAM007_VISUAL_ADJUDICATION_FILES:
        text = _packet_file_text(packet_files, artifact)
        display = _packet_file_path(packet_files, artifact)
        if not text:
        ...
        if artifact == "FUNCTIONAL_ROLE_CONFORMITY_MATRIX.md":
            for required in (
                "Parent/global hub",
                "focused AI operational control/status domain",
                "local-only diagnostic/readiness/report surface",
                "display-only capability/maintenance/boundary surface",
            ):
                if required not in text:
                    failures.append(f"{display}: missing functional-role marker {required}")
        if artifact == "IMPLEMENTATION_MATCH_PROOF.md":
        ...
        if artifact == "STATE_COVERAGE_AND_READINESS_AFTER_ACTION_PROOF.md":
            for required in (
                "AI Control Center After-Action Visual Proof Verdict",
                "Readiness After-Action Visual Proof Verdict",
                "Capabilities & Maintenance After-Action Visual Proof Verdict",
                "Control after state taxonomy",
                "Before run",
                "After local check",
                "After report generation",
                "After copy action",
                "Capabilities after maintenance hold",
                "controlCenterLocalBoundaryControlProven",
                "readinessAfterActionVisualProofProven",
                "capabilitiesMaintenanceDisplayWorkflowProven",
            ):
...
def validate_local_user_packet(
    ...
    *,
    ...
    failures.extend(
        _same_label_root_artifact_failures(
            review_root,
            label,
            validation_mode=validation_mode,
        )
    ...
    failures.extend(_fam007_visual_adjudication_artifact_failures(packet_files))
    ...
    *,
    ...
    if fam007_ai_dashboard_workstream_exit_context_packet:
        lines = [
            ...
            "## Codex Recommendations",
            "",
            "- Recommendation 1: Accept the Workstream-exit Visual Acceptance packet only if the packet-contained focused screenshots, full-desktop screenshots, live proof manifest, and supplemental visual audit prove the AI Dashboard parent and all three child/domain windows match the accepted visual grammar. Rationale: this keeps visual acceptance tied to inspectable image proof rather than helper green alone. Tradeoff: any unproven element group should route to a named repair instead of being silently accepted.",
            "- Recommendation 2: Request targeted repair instead of broad rework when a specific element group, state, or proof artifact fails review, such as title/status wrapping, row spacing, typography, gutters, button placement, scrollbar behavior, or child-domain lifecycle. Rationale: SLC-001 through SLC-006 are otherwise complete and should not be reopened wholesale. Tradeoff: targeted repair may leave unrelated future enhancements deferred.",
            "- Recommendation 3: Preserve the downstream gate boundary even if Visual Acceptance is accepted. Rationale: H1, Live Validation, UTS, PR Readiness, PR creation, merge, release, cleanup, issue mutation, provider/model execution, prompt send, downloads, cache, memory, private setup, packaging, Settings implementation, persistence expansion, drag/drop, imports, v1.8.0, sibling worktree mutation, and Governance repo mutation require separate approvals. Tradeoff: acceptance advances review posture but does not make the branch PR-ready by itself.",
            "- Recommendation 4: Keep no-provider and provider-visible-data-none truth as a required acceptance condition. Rationale: the user-facing AI Dashboard must explain blocked AI behavior without implying ORIN persona, provider/model execution, downloads, memory, or private capability setup exists. Tradeoff: richer AI behavior remains future-gated until a separate source-truth and USER approval path admits it.",
            "",
            "## Why This Fits The Nexus Vision",
            "",
    ...
    if fam007_slc001_workstream_entry_context_packet:
        lines = [
            ...
            "## Contract Revision",
            "",
            "v1 - accepted BP1 context aid for the active FAM-007 SLC-001 / Seam 1 Workstream Entry review packet.",
            "",
            "## Project Vision Context",
            "",
            "Nexus stays Windows-first, local-first, inspectable, privacy-aware, and USER-controlled. The accepted BP1 direction supports that project vision by keeping AI Dashboard doorway work visible, understandable, and gated before any provider, prompt, cache, memory, private setup, packaging, PR, merge, or release action exists.",
            "",
            "## Family Vision Context",
            "",
            "FAM-007 owns local AI, capability packs, provider readiness, consent posture, provider-visible data boundaries, execution gates, memory and cache boundaries, and User/Public, Developer, and Owner lane separation. This SLC-001 support aid preserves that family context without activating provider behavior or private capability surfaces.",
            "",
            "## Feature Vision Context",
            "",
            "The accepted feature route is the AI Dashboard parent hub plus the first child-domain doorway restoration toward AI Control Center Card 1. SLC-001 may prepare the doorway contract only after USER accepts the Workstream Entry packet; this aid records accepted planning context, not implementation permission.",
            "",
            "## Codex Understanding",
            "",
            "Codex understands this file as supporting BP1 context for the active FAM-007 BP3-to-Workstream phase packet. BP1, BP2, and BP3 are already accepted planning context; the active USER decision authorizes bounded SLC-001 / Seam 1 Workstream phase conduct for the admitted entry seam, not a later duplicate implementation approval packet.",
            "",
            "## Branch Goal",
            "",
            ...
            "2. Codex preserves BP1, BP2, and BP3 as accepted context while treating the active packet as bounded SLC-001 / Seam 1 Workstream phase conduct authority.",
            ...
            "## Surface Map",
            "",
            "- Review surface: `USER Review/WORKSTREAM_ENTRY_ANALYSIS_DIGEST.md` is the primary active decision file for SLC-001 / Seam 1.",
            "- Support surface: `Review Aids/USER_BRANCH_VISION_REVIEW.md` records accepted BP1 context and must not ask USER to re-accept BP1.",
            "- Product surface: AI Dashboard is the parent hub; AI Control Center Card 1 / first child-domain doorway is the future restoration target.",
            "- Proof surface: later evidence must map visible rows, status language, disabled controls, and no-provider diagnostics back to accepted source truth without provider/runtime activation.",
            "",
            "## Product Options / Design Paths",
            "",
            "- Option A - accept this SLC-001 / Seam 1 Workstream phase conduct packet as the bounded doorway-restoration route. Tradeoff: it authorizes only the admitted entry seam and keeps later seams gated by Workstream continuation proof.",
            ...
            "## Why This Fits The Nexus Vision",
            "",
            "This route fits Nexus because it makes AI capability boundaries legible before capability exists. The USER can review the doorway contract, blocked diagnostics state, and future proof path without accidentally authorizing provider behavior, private setup, persistence, packaging, or release mechanics.",
            "",
            "## USER Design Questions",
            "",
            "- Does USER accept SLC-001 / Seam 1 as the bounded Workstream phase conduct target?",
            ...
            "## USER Response",
            "",
            "BP1 Branch Vision is accepted context; BP2 and BP3 are also accepted planning context. The active USER response still belongs to the SLC-001 / Seam 1 Workstream Entry packet.",
            "",
            "## Codex Digest",
            "",
            "Codex records this support aid as accepted BP1 context for the SLC-001 / Seam 1 packet. It carries forward the AI Dashboard parent hub and first child-domain doorway direction while keeping every runtime/provider/private/package/release action outside the admitted seam blocked.",
            "",
            "## USER Response Proof",
            "",
            "Accepted by USER - BP1 Branch Vision acceptance is recorded in external branch planning state and carried forward by this SLC-001 / Seam 1 packet as supporting context.",
            "",
            "## USER Response Digested",
            "",
            "Digested - accepted BP1, BP2, and BP3 context is preserved; the active SLC-001 / Seam 1 packet remains pending USER review.",
            "",
            "## Accepted Branch Vision",
            "",
            ...
            "- The active packet is bounded to SLC-001 / Seam 1 Workstream phase conduct after rebaseline/RAR.",
            ...
            "## Family-Vision Versus Branch-Only Vision Impact",
            "",
            "Branch-only by default: this support aid carries accepted FAM-007 and AI runtime/trust architecture context into SLC-001. If later review changes provider behavior, memory/cache policy, edition boundaries, UIREF standards, or AI Control Center doorway ownership, Codex must route that change to the durable source-truth owner before implementation relies on it.",
            "",
            "## Must-Have Behavior",
            "",
            "- BP1, BP2, and BP3 acceptance must be treated as accepted context; this Workstream phase conduct packet is the bounded SLC-001 / Seam 1 implementation authority.",
            ...
            "## Future-Gated Decisions And Regression-Risk Controls",
            "",
            "- Regression-risk control: this accepted BP1 context aid cannot independently approve Workstream/runtime implementation.",
            "- Regression-risk control: a generated review aid must not re-open BP1 or report BP1 acceptance as pending when external branch planning state records BP1 accepted.",
            "- Future-gated decision: any implementation of taskbar, tray, menu, Settings, persistence, provider/model execution, prompt send, downloads, cache, memory, private setup, packaging, PR, merge, release, imports, cleanup beyond routed packet/state currentness, v1.8.0, or sibling worktree mutation.",
            "",
            "## Deferred And Future-Gated Ideas",
            "",
            *_markdown_lines(pending_user_decisions),
            ...
            "## Acceptance / Revision / Rejection / Waiver Decision",
            "",
            "- Accept: USER accepts this SLC-001 / Seam 1 Workstream phase conduct packet as the bounded implementation authority for the admitted entry seam.",
            ...
            "- Reject: USER rejects this SLC-001 Workstream phase conduct packet.",
            "- Waive: USER explicitly waives this review step while naming the remaining Workstream conduct authority or blocker.",
            "",
            "## Current Packet Gate",
            "",
            "SLC-001 / Seam 1 is the active Workstream phase conduct target. This support aid cannot expand scope beyond the primary packet.",
            "",
            "## Exact USER Decision Supported",
            "",
            exact_user_decision,
...
def _write_pr_readiness_stage1_review(
    *,
    ...
    copied_sources = "\n".join(
        f"- `{copied_rel}`" for _source_rel, copied_rel in copied
    ) or "- No copied context files were supplied."
    pending = "\n".join(f"- {decision}" for decision in pending_user_decisions) or "- None recorded."
    lines = [
        f"# {title} - PR Readiness Stage 1 Review",
        "",
        "This is the primary current-gate review artifact for PR Readiness Stage 1. "
        "BP1, BP2, and BP3 files remain supporting planning context; they are not this "
        "gate's decision surface.",
        "",
        "## Review Status",
        "",
        "Reviewable - this packet presents the current PR Readiness Stage 1 analysis "
        "and its repair posture for USER inspection.",
        "",
        "## Contract Status",
        "",
        "Complete - the current-gate artifact class is distinct from the BP2 Branch Plan "
        "Contract and does not imply Stage 2 approval, PR creation, merge, or release.",
        "",
        "## Packet Reviewability State",
        "",
        "Reviewable - structural parity and current-gate content must still be validated "
        "against the generated folder and timestamped upload artifact.",
        "",
        "## USER Gate State",
        "",
        "Pending USER Review - USER may accept, revise, waive, reject, or hold the Stage 1 "
        "analysis. This state is not a USER acceptance receipt.",
        "",
        "## Current-Gate Purpose",
        "",
        review_purpose,
        ...
        "## Scope And Authority",
        "",
        "This review covers only the approved Governance repair. Durable phase law, "
        "source ownership, transition rules, and validation contracts remain governed by "
        "the routed repository owners. Live branch, packet, and external projection facts "
        "remain evidence owned by the helper and external operational state.",
        "",
        "## Transition-Safety Review",
        "",
        "The Stage 1 decision must account for target-currentness, alias disagreement, "
        "pre-write snapshots, no-loss recovery, atomic replacement, complete audit fields, "
        "and the distinction between a target-scoped PASS and a root-wide PASS.",
        "",
        "## Adversarial And False-Green Review",
        "",
        "A green packet requires adversarial coverage for intervening target edits, stale or "
    ...
    review_path = target / PR_READINESS_STAGE1_REVIEW_FILE
    review_path.write_text("\n".join(lines), encoding="utf-8")
    return review_path.resolve()


def _write_pr_readiness_stage1_packet_digests(
    *,
    target: Path,
    stage1_outcome: str,
    exact_user_decision: str,
    ...
    """Write only current-gate support aids for a PR Stage 1 packet.

    BP and Workstream artifacts are useful in their own gates, but retaining
    their active pending language in a completed PR Stage 1 packet creates a
    false current-gate signal. This profile is intentionally source-list
    driven and phase-specific.
    """

    normalized_decision = re.sub(r"\s+", " ", exact_user_decision).strip()
    copied_paths = [copied_rel for _source_rel, copied_rel in copied]
    copied_lines = "\n".join(f"- `{path}`" for path in copied_paths) or "- None."
    pending_lines = "\n".join(
        f"- {decision}" for decision in pending_user_decisions
    ) or "- None recorded."
    outcome_line = (
        "Stage 1 is complete and the packet supports the separate Stage 2 USER decision."
        if stage1_outcome == PR_STAGE1_OUTCOME_READY
        else "Stage 1 remains in repair-required posture; Stage 2 is not supported."
    )
    common = (
        "Current Gate: PR Readiness Stage 1\n"
        f"Stage 1 Outcome: {stage1_outcome}\n"
        "Primary Current-Gate File: `USER Review/PR_READINESS_STAGE1_REVIEW.md`\n"
        "Next Decision: PR Readiness Stage 2 only when Stage 1 is ready.\n"
    )
    files: dict[str, str] = {
        "PR_READINESS_STAGE1_COVERAGE_DIGEST.md": (
            "# PR Readiness Stage 1 Coverage Digest\n\n"
            f"{common}\n"
            f"{outcome_line}\n\n"
            "This aid is the current-gate digest for the Governance PR Readiness "
            "Stage 1 result. It does not reopen BP1, BP2, or BP3, and it does not "
            "authorize Workstream implementation, PR creation, merge, release, "
            "issue mutation, or sibling-worktree mutation.\n\n"
            "## Review Order\n\n"
            "Open the primary Stage 1 review first. Use this digest for the bounded "
            "repair outcome, then use the source coverage and contradiction checklist.\n\n"
            "## Stage 1 Result\n\n"
            f"{stage1_outcome}. The exact next USER decision is:\n\n"
            f"{normalized_decision}\n\n"
            "## Current-Gate Boundary\n\n"
            "Stage 1 is the current review surface. Stage 2 remains a separate USER "
            "decision. BP1, BP2, BP3, Workstream, Hardening, Live Validation, "
            "Release Readiness, merge, release, and sibling adoption are not current "
            "decision paths in this packet.\n\n"
            "## Pending USER Decisions\n\n"
            f"{pending_lines}\n"
        ),
        "PR_READINESS_STAGE1_SOURCE_COVERAGE.md": (
            "# PR Readiness Stage 1 Source Coverage\n\n"
            f"{common}\n"
            "The following list is generated directly from the files copied into "
            "this packet. No additional backlog, roadmap, family vision, edition, "
            "runtime, or worktree-slot coverage is implied unless it appears below.\n\n"
            "## Copied Source-Truth Files\n\n"
            f"{copied_lines}\n\n"
            f"Copied Source Count: `{len(copied_paths)}`\n"
            "Coverage Rule: every source-coverage claim in this packet must resolve "
            "to one of these copied paths.\n"
        ),
        "PR_READINESS_STAGE1_CONTRADICTION_CHECKLIST.md": (
            "# PR Readiness Stage 1 Contradiction Checklist\n\n"
            f"{common}\n"
            "## Required Current-Gate Conditions\n\n"
            "- PASS: the Stage 1 review is the only primary USER decision artifact.\n"
            "- PASS: START_HERE and all current-gate aids route to Stage 1 / Stage 2.\n"
            "- PASS: no active Workstream Entry decision path is emitted.\n"
            "- PASS: no BP1, BP2, or BP3 gate is presented as the current pending gate.\n"
            "- PASS: copied-source coverage is limited to the actual packet file set.\n"
            "- PASS: Stage 2 remains pending and is not treated as accepted.\n\n"
    ...
    *,
    ...
    if fam007_slc001_workstream_entry_review_packet:
        ...
        lines = [
            ...
            "## Contract Status",
            "",
            "Complete - BP2 engineering plan context is accepted by USER and carried forward only as support for the active SLC-001 / Seam 1 Workstream phase conduct packet.",
            "",
            "## Packet Reviewability State",
            "",
            "Reviewable - supporting accepted BP2 context for the active SLC-001 / Seam 1 Workstream phase conduct packet.",
            "",
            "## USER Gate State",
            "",
            ...
            "## USER Response Proof",
            "",
            "Accepted by USER - BP2 acceptance is recorded in external branch planning state and carried forward by this packet as support context.",
            "",
            "## USER Response Digested",
            "",
            "Digested - accepted BP2 context is preserved; the active SLC-001 / Seam 1 packet carries Workstream phase conduct authority.",
            "",
            "## Acceptance / Waiver / Revision / Rejection Receipt",
            "",
            ...
            "## Contract Version / Revision",
            "",
            "v2 - accepted BP2 support context for FAM-007 SLC-001 / Seam 1 Workstream phase conduct.",
            "",
            "## Plain-English Branch Summary",
            "",
            ...
            "- Confirm SLC-001 is bounded to Workstream phase conduct and that provider/runtime-private/package/release behavior remains blocked.",
            ...
            "## Implementation Options",
            "",
            "- Option A - approve the active SLC-001 / Seam 1 Workstream phase conduct packet and preserve all provider/private/package gates. Pros: starts bounded doorway repair; Cons: later seams remain gated by Workstream continuation proof; Risk: low if proof remains strict.",
            ...
            "## Recommended Direction",
            "",
            "Codex recommends approving only the bounded SLC-001 / Seam 1 Workstream phase conduct packet if USER agrees the doorway route, blocked boundaries, and proof expectations are clear. The recommendation does not authorize provider behavior, private setup, packaging, merge, release, imports, or cleanup beyond routed packet/state currentness.",
            "",
            "## Why This Fits The Nexus Vision",
            "",
            "This plan context fits Nexus because it keeps AI capability boundaries visible and USER-controlled before capability exists. It preserves local-first behavior, no-provider diagnostics, deterministic review gates, and clear separation between accepted planning context and future implementation authority.",
            "",
            "## USER Plan Review Decision",
            "",
            "Choose one path for the active SLC-001 / Seam 1 packet: approve Workstream phase conduct, revise, hold for more options, reject, or waive this review step. No duplicate later implementation approval is required for the admitted entry seam.",
            "",
            "## USER Decisions Needed",
            "",
            "- Does USER approve SLC-001 / Seam 1 as the bounded Workstream phase conduct target?",
            ...
            "## USER Response",
            "",
            "Status: USER Workstream phase conduct correction pending in the active SLC-001 / Seam 1 packet. BP2 is accepted context only.",
            "",
            "## Codex Response Digest",
            "",
            "Status: Workstream phase conduct correction is active. Codex must use the primary packet, not this support aid alone, before conducting SLC-001 / Seam 1.",
            "",
            "## Implementation Constraints Created By USER Response",
            "",
            ...
            "## Vision Delta / Source-Truth Impact",
            "",
            "No branch-vision or reusable source-truth delta is created by this support aid. If USER changes AI Dashboard doorway ownership, UIREF expectations, provider-state meaning, or durable FAM-007 boundaries, that change must route to the proper source-truth owner before implementation relies on it.",
            "",
            "## Contract Change Log",
            "",
            "- v2 - Generated as accepted BP2 support context for the active FAM-007 SLC-001 / Seam 1 Workstream phase conduct packet.",
            "",
            "## Current Branch Scope",
            "",
            "- Packet artifact repair and reviewability for FAM-007 SLC-001 / Seam 1 Workstream phase conduct.",
            ...
            "## Workstream Entry Result",
            "",
            "Workstream phase conduct - SLC-001 / Seam 1 is the bounded entry seam; implementation is not complete or green until proof passes.",
            "",
            "## Contract Completion Checklist",
            "",
            "- BP2 support context records USER Accepted.",
            "- Active SLC-001 decision records Workstream phase conduct authority through the primary packet.",
    ...
    if fam007_ai_dashboard_workstream_exit_packet:
        ...
        lines = [
            ...
            "## Workstream Entry Result",
            "",
            "Bounded runtime repair completed / pending USER Visual Acceptance - SLC-001 through SLC-006 remain complete and the returned child/domain runtime repair is routed to USER Visual Acceptance review without pre-accepting the packet.",
            "",
            "## Contract Completion Checklist",
            "",
    ...
    if (
        ...
        user_response_text = (
            ...
            "engineering-plan context for the Workstream-conduct-ready packet."
            if workstream_package_approval_packet
            else
            "Status: Accepted by USER - this BP2 support file is closed as the "
            "accepted engineering-plan context for the active BP3 Workstream Entry / "
        ...
        codex_response_digest = (
            ...
            "phase conduct context. The accepted BP2 plan remains the engineering "
            "basis for the bounded same-branch Workstream phase, with Seam 1 as "
        ...
        workstream_entry_result = (
            "Workstream-conduct-ready - BP1, BP2, and BP3 are accepted; bounded "
            "Workstream phase conduct is approved by this packet with "
        ...
        contract_status = (
            ...
            "and this packet records bounded Workstream phase conduct approval."
            if workstream_package_approval_packet
            else
            "Complete - USER accepted the BP2 Branch Plan Contract; BP3 is the "
            "active Workstream Entry / Orchestration Validation gate."
        ...
        contract_version = (
            ...
            "Workstream-conduct-ready support context."
            if workstream_package_approval_packet
            else
        ...
        plain_english_summary = (
            ...
            "Workstream-conduct-ready: BP3 has been accepted and USER is approving "
            "bounded same-branch Workstream phase conduct with Seam 1 as the entry checkpoint."
            if workstream_package_approval_packet
            else
            "This support file records the accepted BP2 engineering plan for the "
        ...
        current_scope = [
            (
                "Bounded same-branch Workstream phase conduct approval with "
        ...
        user_decisions = [
            (
                "Does USER approve bounded Workstream phase conduct with Seam 1 "
        ...
        completion_checklist = [
            ...
            (
                "BP3 is accepted or waived and Workstream phase conduct approval is recorded "
            ...
            (
                "Bounded Workstream phase conduct is approved with Seam 1 as "
    ...
    lines = [
        ...
        *_markdown_lines(slc_package_plan),
        ...
        "",
        workstream_entry_result
        or "Status: Pending USER Response - first seam, affected files, validators, proof requirements, USER-facing proof, and exact Workstream phase conduct text must be returned only after USER response/digest or explicit waiver.",
        "",
        "## Contract Completion Checklist",
        "",
        *_markdown_lines(completion_checklist),
...
def _write_workstream_entry_packet_digests(
    *,
    ...
    packet_status = (
        ...
        "workstream-conduct-ready - BP1, BP2, and BP3 are accepted; bounded "
        "Workstream phase conduct is approved by this packet with Seam 1 as the "
        ...
        "revision, waiver, rejection, or hold remains pending; Workstream phase "
        "conduct remains pending USER approval."
        if bp3_packet
        else
    ...
    if bp1_packet:
        ...
        analysis_status = (
            "Analysis Summary: Bounded runtime repair completed / pending "
        ...
        digest_status = (
            ...
            "adequacy analysis, child after-action screenshots, and nonconformance "
            "ledger are loaded and digestible for USER review; the contract records "
            "bounded runtime repair completed / pending USER Visual "
        ...
        analysis_status = (
            ...
            "this packet carries the branch from BP3 Workstream Entry into bounded "
            "SLC-001 / Seam 1 Workstream phase conduct for the FAM-007 AI Dashboard "
            "child/domain doorway restoration package."
        )
        implementation_posture = (
            "Workstream Phase Posture: bounded Workstream phase conduct is approved "
            "by this packet decision path for SLC-001 / Seam 1 only. Continuation must proceed "
        ...
        checklist_status = (
            "Checklist Focus: SLC-001 / Seam 1 Workstream phase conduct - accepted "
        ...
        digest_status = (
            ...
            "packet wording treats SLC-001 / Seam 1 as the admitted entry seam, not a "
...
def _write_packet_validation_output_artifacts(
    *,
    ...
    command_output_path.write_text(
        ...
        "Final post-ZIP byte proof is recorded in helper output, Codex return, and live external operational state.\n\n"
    ...
    strategy_path.write_text(
        ...
        "Final Byte Proof Surface: `Helper output, Codex return, and routed external operational state; loose root-level USER hub sidecars are not part of the normal active packet shape.`\n\n"
        ...
        "byte proof is reported by the helper/Codex return and recorded in live external operational state after export.\n",
        encoding="utf-8",
    )
...
def _packet_text_status(text: str) -> str:
    ...
    if any(marker in normalized for marker in workstream_conduct_markers) and not any(
        marker in normalized for marker in blocking_markers
    ):
        return DECISION_STATUS_WORKSTREAM_CONDUCT_READY
    ...
    *,
    ...
    for file_name in WORKSTREAM_ENTRY_PACKET_DECISION_FILES:
        ...
        if status == DECISION_STATUS_UNKNOWN:
            failures.append(f"{display_name}: next legal phase / Workstream conduct posture is not machine-readable")
    ...
    if require_implementation_ready and status not in {
        DECISION_STATUS_IMPLEMENTATION_READY,
        DECISION_STATUS_WORKSTREAM_CONDUCT_READY,
    }:
        joined = ", ".join(f"{file_name}={status_value}" for file_name, status_value in sorted(file_statuses.items()))
        failures.append(
            "Workstream Entry packet is not Workstream-conduct-ready: "
...
def build_bundle(
    *,
    ...
    machine_readable_packet_status = (
        ...
        "pr readiness stage1 repair review - PR Readiness Stage 1 remains in "
        "repair-required posture; PR Readiness Stage 2 is not supported."
        if pr_stage1_packet
        else
        "bp1 branch vision review - BP1 Branch Vision Review remains pending "
        "USER acceptance, revision, waiver, rejection, or hold; BP2 remains pending."
    ...
    digest_required_files = (
        PR_READINESS_STAGE1_PACKET_REQUIRED_FILES
        if pr_stage1_packet
        else WORKSTREAM_ENTRY_PACKET_REQUIRED_FILES
    )
    digest_paths = {
        (review_aids_dir / name).resolve()
        for name in digest_required_files
        if name != "START_HERE.md"
    }
    ...
    effective_review_order = list(review_order)
    if primary_user_review_file_name == PR_READINESS_STAGE1_REVIEW_FILE:
        effective_review_order = [
            f"Open `{USER_REVIEW_DIR_NAME}/{PR_READINESS_STAGE1_REVIEW_FILE}` first; it is the current PR Readiness Stage 1 decision surface.",
            f"Then inspect `{REVIEW_AIDS_DIR_NAME}/PR_READINESS_STAGE1_COVERAGE_DIGEST.md` for the current-gate digest.",
            f"Then inspect `{REVIEW_AIDS_DIR_NAME}/PR_READINESS_STAGE1_SOURCE_COVERAGE.md` for the exact copied-source set.",
            f"Then inspect `{REVIEW_AIDS_DIR_NAME}/PR_READINESS_STAGE1_CONTRADICTION_CHECKLIST.md` for coherence proof.",
            "Use Source Truth Context files only as supporting evidence; BP1, BP2, BP3, and Workstream aids are not current-gate artifacts in this packet.",
        ]
    readme_lines: list[str] = [
        ...
        *_markdown_lines(pending_user_decisions),
        ...
        "## Review Order",
        "",
        *_markdown_lines(effective_review_order),
        "",
        "## Extra Bundle Files",
        "",
    ...
    artifact_failures = [
        ...
        *_pr_stage1_source_coverage_failures(
            packet_files,
            packet_entries={path.relative_to(target).as_posix() for path in bundle_paths},
        ),
    ]
    if artifact_failures:
        raise ValueError(
    ...
    _remove_stale_same_label_root_artifacts(review_root, label)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Compatibility gate: fail if the packet is branch-correct but still blocks Workstream phase conduct.",
    )
    parser.add_argument(
        "--stage1-outcome",
    ...
    if args.validate_workstream_entry_packet:
        ...
        if result.failures:
            ...
            for failure in result.failures:
                print(f"- {failure}")
            return 1
        if result.blocks_workstream_conduct:
            print("PASS: Workstream Entry packet is self-consistent and keeps Workstream phase conduct pending.")
            print(f"Packet status: {result.status}")
        else:
            print("PASS: Workstream Entry packet is self-consistent and Workstream-conduct-ready.")
            print(f"Packet status: {result.status}")
        return 0
    ...
    print(f"Review export zip sha256: {_file_sha256(export_zip)}")
    print("Post-ZIP receipt: not generated; final byte proof is reported here and in routed external state.")
