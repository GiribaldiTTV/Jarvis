...
def _write_upload_instruction(
    *,
    ...
    upload_path = target / UPLOAD_THIS_ZIP_FILE
    lines = [
        "# Upload This ZIP",
        "",
        "This file is generated by `dev/orin_user_review_bundle.py` as a stale-upload guard.",
        "",
        "Upload the sibling exported ZIP named below. Do not upload an older ZIP with the same label from another location.",
        "",
        ...
        "Freshness Rules:",
        "",
        "- `START_HERE.md` inside the ZIP must show the same Source Branch, Source HEAD, and origin/main as this file.",
        "- The ZIP must contain this `UPLOAD_THIS_ZIP.md` file.",
        "- If the uploaded ZIP shows a different branch, HEAD, or origin/main, it is stale even if the filename is `FAM-006.zip`.",
        "",
    ]
...
def _validate_export_zip(
    ...
    *,
    ...
    with zipfile.ZipFile(export_zip, "r") as archive:
        ...
        try:
            upload_instruction = archive.read(UPLOAD_THIS_ZIP_FILE).decode("utf-8")
        except KeyError as exc:
            raise ValueError(
                f"Review export zip is missing {UPLOAD_THIS_ZIP_FILE}: {export_zip}"
            ) from exc
    ...
    for expected_upload_marker in (
        ...
        if expected_upload_marker not in upload_instruction:
            raise ValueError(
                "Review export zip upload-instruction guard failed: "
                f"{UPLOAD_THIS_ZIP_FILE} missing {expected_upload_marker}"
            )
...
def build_bundle(
    *,
    ...
    upload_instruction_file = _write_upload_instruction(
        target=target,
    ...
    readme_lines: list[str] = [
        ...
        (
            ...
            "origin/main, and exact stable upload path."
        ),
        (
            "USER Review Packet Finding: PASS - helper generated and validated "
            f"`START_HERE.md`, `{USER_BRANCH_PLAN_REVIEW_FILE}`, "
            f"`{UPLOAD_THIS_ZIP_FILE}`, and exported zip "
