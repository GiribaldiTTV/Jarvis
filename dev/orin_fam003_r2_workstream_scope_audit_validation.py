...
FILE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "desktop/orin_desktop_launcher.pyw": _meta(
        ...
        "USER-approved bounded mixed decline-then-accept relaunch lifecycle repair",
        "Uses the stable software-compositor path for Windows Qt WebEngine shutdown, propagates launcher failure codes, and classifies every post-settled nonzero first-session exit as abnormal.",
        "R2 Workstream completion lifecycle repair / R2-EVIDENCE-004.",
        "Normal launcher child environment, first-session/replacement-session outcome separation, and fail-closed process result propagation.",
        "Low: the launcher contract is FAM-003-owned and does not change installer, shortcut, packaging, or sibling runtime behavior.",
        "Negative abnormal-exit fixture, desktop-entrypoint validation, repeated relaunch lifecycle stress, and Option C aggregate.",
        "Revert only with the coordinated runtime teardown and validator contract; never restore recoverable-success classification for a native crash.",
    ),
