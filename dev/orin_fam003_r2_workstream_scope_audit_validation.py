...
FILE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "dev/fixtures/fam003_option_d_nonintrusive_performance_negative_cases.json": _meta(
        ...
        "Adds 21 fail-capable mutations for nested event loops, GUI-thread sampling, observer attribution and material overhead, PID/role CPU, CPU denominators, private/shared memory, idle and post-use state, meaningful workload, timing, baseline, external-state, release-health, and raw-summary parity.",
    ...
    "dev/fixtures/fam003_performance_decision_packet_negative_cases.json": _meta(
        "fixture",
        "FAM-003 post-R2 performance and migration decision packet",
        "USER-approved post-Workstream decision-packet validation on the current FAM-003 carrier",
        "Added fail-capable performance-decision and external-state migration specification cases.",
        "Post-R2 Decision 1 / Decision 2 packet preparation; outside R2-WS01 through R2-WS10.",
        "Decision-packet and migration-sequencing false-green prevention only.",
        "None; fixture mutations are temporary and do not change runtime or live external state.",
        "Performance-decision packet self-test and final packet semantic validation.",
        "Remove only with the FAM-003 performance-decision packet helper or an equivalent replacement.",
        disposition="ADJACENT_REQUIRED",
    ),
    ...
    "dev/orin_external_state_target_currentness_fixture_validation.py": _meta(
        "shared validator",
        "Reusable external-state currentness fixture validator",
        "USER-approved FAM-003 post-R2 external-state migration and semantic-closeout repair",
        "Extended fail-capable currentness, projection-set coherence, writer, lock, audit, and historical-boundary fixtures.",
        "Post-R2 external-state migration execution and closeout; outside R2-WS01 through R2-WS10.",
        "Reusable external-state validation behavior exercised only in temporary fixture roots.",
        "Low: shared governance helper changed on the legal FAM-003 carrier without mutating another worktree or live state.",
        "Target-currentness fixture validation, projection-set semantic coherence, and root external-state validation.",
        "Revert only the bounded fixture cases together with the validator behavior they protect.",
        disposition="ADJACENT_REQUIRED",
        shared=_shared_audit(
            carryforward="NONE - the reusable fixture contract is branch-independent after merge.",
            detail="YES - changes are bounded to generic external-state failure classes exposed by the FAM-003 migration.",
        ),
    ),
    "dev/orin_external_state_target_reconcile.py": _meta(
        "shared helper",
        "Reusable target-scoped external-state projection writer",
        "USER-approved FAM-003 post-R2 external-state migration execution",
        "Added legacy-schema transition and BOM-preserving projection updates under the existing lock/snapshot/atomic-write contract.",
        "Post-R2 external-state migration execution; outside R2-WS01 through R2-WS10.",
        "Reusable projection-transition semantics; no product/runtime behavior.",
        "Medium: shared external-state writer changed, but only through registered reusable behavior and current-carrier validation.",
        "Target-currentness fixtures, external-state validation, lock/snapshot proof, and semantic-coherence validation.",
        "Revert only with the matching migration/currentness validator behavior and without rewriting historical receipts.",
        disposition="ADJACENT_REQUIRED",
        shared=_shared_audit(
            carryforward="NONE - later carriers consume the reusable writer through current main.",
            detail="YES - changes preserve generic lock, snapshot, atomic-write, audit, and no-loss requirements.",
        ),
    ),
    "dev/orin_external_state_validation.py": _meta(
        "shared validator",
        "Reusable external operational state validator",
        "USER-approved FAM-003 post-R2 migration and projection semantic-closeout repair",
        "Added legacy-schema transition acceptance, BOM-safe reads, and fail-closed projection-set semantic coherence checks.",
        "Post-R2 external-state migration execution and closeout; outside R2-WS01 through R2-WS10.",
        "Reusable external-state currentness and semantic-coherence validation only.",
        "Medium: shared validator changed, but no live record or sibling worktree is owned by this source file.",
        "Target-currentness fixtures, scoped/root external-state validation, and projection-set semantic coherence.",
        "Revert only with the matching reusable fixtures and transition helper behavior.",
        disposition="ADJACENT_REQUIRED",
        shared=_shared_audit(
            carryforward="NONE - later carriers consume the reusable validator through current main.",
            detail="YES - changes enforce generic external-state currentness and coherence exposed by the FAM-003 migration.",
        ),
    ),
    ...
    "dev/orin_fam003_option_d_nonintrusive_performance_validation.py": _meta(
        ...
        "Sixty combined fixtures, Python compile, three exact-launcher sessions, Option C aggregate, scope audit, governance, and packet parity.",
        "Retire only after an approved replacement supplies equivalent nonintrusive evidence and currentness controls.",
    ),
    "dev/orin_fam003_performance_decision_packet_validation.py": _meta(
        "packet helper",
        "FAM-003 post-R2 Option D performance and migration decision review",
        "USER-approved post-Workstream decision-packet preparation on the current FAM-003 carrier",
        "Built and validated the performance decision, migration specification, exact packet semantics, and current external-state review context.",
        "Post-R2 Decision 1 / Decision 2 packet preparation; outside R2-WS01 through R2-WS10.",
        "Decision packet generation, semantic review, byte-preserving copies, and false-green prevention only.",
        "None; helper is FAM-003-specific and cannot mutate product runtime or perform the migration.",
        "Twenty-five negative fixtures, active-review packet validation, folder/ZIP parity, and Git blob-byte checks.",
        "Retire only after the decision sequence is folded down or an equivalent packet helper replaces it.",
        disposition="ADJACENT_REQUIRED",
    ),
    "dev/orin_fam003_renderer_backend_workstream_validation.py": _meta(
        ...
        "Compatibility entrypoint routes active execution to the external-observer adjudicator, preserves the historical visual/lifecycle support functions, refuses unsupported equivalence without a safe baseline, and enforces 60 combined negative fixtures.",
        "Slice E / SLC-R2-009,010 / R2-WS09,10 renderer-backend completion proof.",
    ...
    "dev/orin_user_review_bundle_false_green_fixture_validation.py": _meta(
        "shared validator",
        "Reusable USER packet false-green fixture validator with FAM-003 Option G cases",
        "USER-approved post-R2 Option G planning and BP3 packet validation repair",
        "Added fail-capable BP2/BP3 gate-separation, live-header, UFD, launcher, observability, ordered-proof, raw-evidence, and Element-to-Phase packet cases.",
        "Post-R2 Option G BP2/BP3 packet preparation; outside R2-WS01 through R2-WS10.",
        "Reusable packet false-green prevention with branch-specific temporary fixtures.",
        "Low: the helper is shared, but the added checks activate only for the FAM-003 Option G packet contract.",
        "Twenty-two Option G negative fixtures plus reusable local USER packet regression validation.",
        "Remove only the FAM-003 Option G fixture cases if the packet contract is retired or replaced equivalently.",
        disposition="ADJACENT_REQUIRED",
        shared=_shared_audit(
            carryforward="NONE - FAM-003-specific cases do not change another family packet contract.",
            detail="YES - additions are branch-specific fixtures inside the registered reusable validator.",
        ),
    ),
}
...
WORKSTREAM_COMMIT_AUDIT_BY_SUBJECT = {
    ...
    "Repair FAM-003 Option D nonintrusive performance proof": {
        ...
        "unrelatedChanges": "NO.",
        "validation": "60 negative fixtures, Python compile, three exact Desktop-launcher sessions, Option C aggregate, scope audit, governance, external-state validation, release-health supporting gate, and packet parity.",
    ...
    "Repair FAM-003 Option D shortcut resolution": {
        ...
        "unrelatedChanges": "NO.",
        "validation": "Python compile, 60 negative fixtures, exact Desktop-shortcut resolution, three normal-launcher sessions, Option C aggregate, and scope audit.",
        "supersession": "Supersedes the failed-before-launch attempt; no runtime measurement was emitted by that attempt.",
    },
    "Reduce FAM-003 Option D observer overhead": {
        "reason": "The first complete external-observer run measured a median 32.285 percent of one core in the observer, so it was rejected as non-negligible. Cached immutable process metadata, retained 250 ms CPU/private-commit sampling, and reduced expensive USS collection to a documented 2-second cadence.",
        "traceability": "R2-WS10 / SLC-R2-010 sampler-overhead proof repair.",
        "coherent": "YES - observer sampling cost, methodology version, and exact commit-audit metadata only.",
        "unrelatedChanges": "NO.",
        "validation": "Python compile, 60 negative fixtures, observer-overhead proof, three exact Desktop-launcher sessions, raw-summary parity, Option C aggregate, and scope audit.",
        "supersession": "Supersedes the v3 performance run at dev/logs/fam003_renderer_backend_workstream/20260721-175652; its product numbers remain non-authoritative because observer overhead was not negligible.",
    },
...
WORKSTREAM_COMMIT_EXCLUDED_SUBJECTS = {
    ...
    "Validate FAM-003 performance decision packet",
    "Repair FAM-003 packet seed retry path",
    "Align FAM-003 packet with active review rules",
    "Preserve Git blob bytes in FAM-003 packet",
    "Repair FAM-003 migration decision specification",
    "Normalize FAM-003 packet semantic checks",
    "Separate FAM-003 packet review and audit metadata",
    "Allow legacy external state schema migration",
    "Preserve BOM-prefixed external state receipts",
    "Harden external projection semantic closeout",
    "Harden Option G planning packet gates",
    "Harden FAM-003 Option G BP3 packet validation",
    "Align BP3 validation with external live headers",
