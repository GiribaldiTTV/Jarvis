...
FILE_METADATA: dict[str, dict[str, Any]] = {
    ...
    "dev/fixtures/fam003_renderer_backend_negative_cases.json": _meta(
        ...
        "Adds 15 fail-capable mutations for missing/duplicate flags, stale or private launcher evidence, incomplete surface/HUD/lifecycle/performance proof, hidden regressions, scope misclassification, sibling mutation, and approval overreach.",
        "Slice E / SLC-R2-009,010 / R2-WS09,10 renderer-backend completion proof.",
    ...
    "dev/orin_fam003_option_c_workstream_proof_validation.py": _meta(
        ...
        "Makes HUD access, HUD visual, Settings, resident-adjacent, NCP, desktop-entrypoint, and Option D renderer-backend children fail closed; selects exact current child roots; copies complete 26-state and renderer evidence; records active child roles; and packages current pushed-HEAD ordered visible-cursor frames.",
    ...
    "dev/fam003_renderer_backend_runtime_probe.py": _meta(
        "proof helper",
        "FAM-003 R2 Option D in-process runtime probe",
        "Explicit USER-approved Option D normal-launcher all-surface proof",
        "Environment-gated probe exercises and captures ORIN Core, HUD Dashboard, Recording Suite, Log Viewer, and AI Status / Command Center, verifies deferred AI domains remain disabled, records lifecycle/performance/process evidence, and requests normal shutdown.",
        "Slice E / SLC-R2-009,010 / R2-WS09,10 renderer-backend completion proof.",
        ...
        "Three normal-launcher sessions, PNG/nonblank checks, DOM/action checks, lifecycle report, and aggregate consumption.",
    ...
    "dev/orin_fam003_renderer_backend_workstream_validation.py": _meta(
        ...
        "Inventories eight WebEngine rows, blocks foreign runtimes, resolves the exact Desktop shortcut, runs three normal-launcher sessions, consumes current lifecycle proof, summarizes performance, records rollback/scope/defect receipts, and enforces 15 negative fixtures.",
        "Slice E / SLC-R2-009,010 / R2-WS09,10 renderer-backend completion proof.",
